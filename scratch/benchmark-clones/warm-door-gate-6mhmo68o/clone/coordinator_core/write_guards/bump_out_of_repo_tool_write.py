...
(ruled 2026-08-11, DoE, PM-delegated). DR-277
...
SETTINGS HOME IS NOT A FOREIGN REPO (DoE finding #2, parity). A target that
...
def _target_is_under_settings_home(
    ...
    REPO" (DoE finding #2, parity with the Bash-surface AC9 exemption).
...
def _resolve_target_gitdir(
    ...
    EXISTING directory ancestor first (DoE finding #1). `resolve_gitdir`
    ...
    (DoE finding #3): `nearest_existing_ancestor` calls `os.path.isdir()`,
