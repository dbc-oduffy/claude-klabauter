...
Negative-spec). Seeded into `docs/decisions/DR-210-claude-klabauter-native-tooling-ownership-strangler.md`
...
Negative-spec:
    ...
    - Manifest home is `docs/decisions/` ONLY — there is deliberately no `docs/plans/`
      manifest path. Live-verified root cause (2026-07-21, the Staff Engineer + code-reviewer
      ...
      (`*.the Staff Engineer-review.md`, `*.sonnet-review.md`, `*.prior-art-check.md`,
...
def _derive_slug(path: str, strangler_id: Optional[str] = None) -> str:
    ...
    that prefix too (so "DR-210-claude-klabauter-native-tooling-ownership-strangler.md" with
    strangler_id "DR-210" yields "claude-klabauter-native-tooling-ownership-strangler", not a
    redundant "DR-210-claude-klabauter-native-...").
    """
    stem = Path(path).stem
