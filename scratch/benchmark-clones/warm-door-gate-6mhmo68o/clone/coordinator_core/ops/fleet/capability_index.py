...
``fleet-capability-index.schema.json``) — the engine-tier read-aggregation claude-klabauter
was asked to build per DR-047/DR-059 (engine-tier read-aggregation is claude-klabauter-owned).
...
manifest exemplar at example-retrieval-repo's own state/capabilities/manifest.json).
...
Behaviour (see ``build_fleet_index`` for the full algorithm):
  ...
  8. Registered through claude-klabauter's existing op-invocation seam (``register_op`` +
