...
def test_tracked_file_count_is_none_when_claude_base_is_not_a_repo(tmp_path):
    """No repo, no spawn, no finding — the ordinary box. `None` (cannot
    establish) is distinct from `0` (a repo that tracks nothing there), and
    only a positive count blocks."""
    claude_base = tmp_path / ".claude"
    (claude_base / "machine-local").mkdir(parents=True)

    assert sm._tracked_file_count(claude_base, "machine-local") is None


def test_tracked_file_count_degrades_to_none_when_git_is_unavailable(tmp_path, monkeypatch):
    """git absent, wedged, or refusing must degrade to the pre-existing
    per-file behaviour, never to an installer that cannot finish."""
    import subprocess as _sp

    claude_base = tmp_path / ".claude"
    (claude_base / ".git").mkdir(parents=True)
