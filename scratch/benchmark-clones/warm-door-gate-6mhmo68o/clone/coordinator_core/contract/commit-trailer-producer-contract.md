...
> → DoE/central; item 3 (claude-klabauter `commit.anchors` op + invocation shim) shipped. Sibling EMs
...
> - DoE ratification memo: `cross-repo/inbox/2026-07-04-claude-klabauter-stamper-ratify-and-wedge-closed.md`
...
| 3 | **claude-klabauter ships `commit.anchors` op + invocation shim + finalizes this contract** (plan AC1/AC2/AC7) → DoE wires the `prepare-commit-msg` hook (gated on this delivery). | claude-klabauter (op + shim) → DoE (hook wiring) | **SHIPPED** — `043b214` C1–C4, `9f3ad55` shim, `da13148` workstream-complete |
...
- **The DoE append-hook wiring** — editing `coordinator-prepare-commit-msg` to call `commit.anchors`
  ...
  `DoE-claude/coordinator/bin/`); claude-klabauter proposes via memo + PM-relay per
...
- **The claude-klabauter invocation shim** (`bin/claude-klabauter-commit-anchors.py`) — the fail-open shim the DoE hook
