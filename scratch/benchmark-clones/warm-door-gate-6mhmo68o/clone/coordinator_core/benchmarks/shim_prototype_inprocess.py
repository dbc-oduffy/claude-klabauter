...
chained. But `coordinator/lib/resolve-claude-klabauter/_resolve_claude_klabauter.py::exec_cli`
...
def run_in_process() -> int:
    ...
    (`_run_target_in_process` in `coordinator/lib/resolve-claude-klabauter/
    _resolve_claude_klabauter.py`) -- no child process, no second interpreter, the
