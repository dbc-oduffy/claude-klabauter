...
sovereign-tracker plan's own, separately-tracked open item (cockpit's
...
(cockpit's own DR states the fix in terms, § 4.3 PARTICIPATION FILTER (A5):
...
This rule set narrows the concurrency window; it does not close it. Cockpit
...
Negative-spec:
  ...
  - Do NOT resolve a same-axis concurrent-apply merge question here — that
    remains cockpit's deferred merge policy (DR-241 negative-spec); this
