...
# ---------------------------------------------------------------------------
# Per-fact op_latency instrumentation (C1, plan
# 2026-08-27-the-fact-layer-is-measured-on-the-one-hot-path) — AC4: a
# "fact_span" row is emitted per served fact call, distinguishing computed
# from degraded outcomes, without changing any fact's return value.
# ---------------------------------------------------------------------------


def _fact_span_rows(repo_root: Path) -> list[dict]:
    sink = repo_root / ".git" / "coordinator-sessions" / "logs" / "op-latency.jsonl"
    if not sink.exists():
        return []
    rows = []
...
class TestFactSpanInstrumentation:
    """AC4: each of the six served facts emits one `op_latency` `"fact_span"`
    row per call, carrying an outcome that distinguishes a computed record
    from a degraded one — and never changes what the fact itself returns."""

    def test_computed_call_emits_a_fact_span_row(self, tmp_path, monkeypatch):
        (tmp_path / ".git").mkdir()
        monkeypatch.setattr(
            branch_resolution, "_cached_session_commits", lambda root, sid: _fake_commits(3)
        )
    ...
    def test_degraded_call_emits_a_distinguishable_fact_span_row(self, tmp_path, monkeypatch):
        """A degraded fact record still emits a row — the row's own outcome
        distinguishes it from a computed one; there is no `raise` anywhere in
        this module (module docstring), so this is not a raise-emits-a-row
        assertion."""
        (tmp_path / ".git").mkdir()
        monkeypatch.setattr(
            branch_resolution, "_cached_session_commits", lambda root, sid: None
        ...
        assert rows[0]["fact"] == "session_facts.session_magnitude_attributed"

    def test_no_sid_fact_still_emits_a_row_with_null_sid(self, tmp_path):
        """`session_terminal_sizings`/`session_fold_sidecars` take no `sid` —
        their row's `sid` field is `None`, not omitted or fabricated."""
        (tmp_path / ".git").mkdir()
        ...
        rows = _fact_span_rows(tmp_path)
        assert len(rows) == 1
        assert rows[0]["fact"] == "session_facts.session_terminal_sizings"
        assert rows[0]["sid"] is None

    def test_instrumentation_never_changes_the_returned_value(self, tmp_path, monkeypatch):
        """The timing wrapper must be transparent: the record returned with
        instrumentation live is identical to the record the untimed impl
        produces on its own."""
        (tmp_path / ".git").mkdir()
        monkeypatch.setattr(
            branch_resolution, "_cached_session_commits", lambda root, sid: _fake_commits(5)
        )

        wrapped = session_facts.session_magnitude_attributed(tmp_path, _SID)
        direct = session_facts._session_magnitude_attributed_impl(tmp_path, _SID)
