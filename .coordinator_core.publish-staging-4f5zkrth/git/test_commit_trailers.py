...
# project-rag-em-wave-commit-deliverable-id-is-per-session.md. Deliberately
...
def test_commit_scoped_refuses_a_branch_name_in_the_deliverable_id_trailer(tmp_path):
    # The exact malformed value observed on project-rag's e78b55610.
    from coordinator_core.ops.ceremony import git_native

    (tmp_path / "f.txt").write_text("x\n", encoding="utf-8")
    f = _msg(
        tmp_path,
        "C1: subject\n\nbody\n\nDeliverable-Id: work/striker/2026-08-16to18\n",
    )
    result = git_native.commit_scoped(["f.txt"], f, tmp_path)
    assert result.returncode == -1
    assert "work/striker/2026-08-16to18" in result.stderr
