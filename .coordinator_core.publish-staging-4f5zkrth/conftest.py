...
- ``cross-repo/inbox/2026-07-20-project-opticon-em-doe-root-clobbered-by-windows-test-home-leak.md``
...
def _build_stub_doe_root(base_dir: str) -> str:
    ...
    # makima-side per `_doe_bin_dir()`'s own docstring), but a real DoE-claude
...
def _fail_on_environ_leak(request):
    ...
    ``MAKIMA_ROOT`` resolution error three files away) rather than the resolver that
