...
`bin/makima-commit-anchors`-era `bin/makima-doctor-probe.cmd`). Outside the
...
LAUNCHER_PARITY_EXEMPTIONS: dict[str, str] = {
    ...
    "coordinator/bin/install-makima-precommit-hook.cmd": (
        "LF in the index under an eol=crlf attr; awaiting a renormalize pass"
    ),
    # -- Orphaned launcher --------------------------------------------------
    ...
    # but never held for makima's. Settled on disk, not by convenience: see
    ...
    # holodeck forwarders are EXEMPT, not deleted, for exactly this reason).
...
def test_declared_spec_backlink_is_part_of_the_byte_contract(tmp_path, monkeypatch):
    ...
    which is the failure mode that made `bin/makima-doctor-probe.cmd`'s
