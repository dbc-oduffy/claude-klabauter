...
# repo-wide (migrated to makima's coordinator/lib/coordinator_session.py,
...
# therefore no 15s hang path if MAKIMA_ROOT can't be resolved).
# ---------------------------------------------------------------------------
...
def _lib_path() -> Optional[str]:
    ...
    (<makima_root>/coordinator/lib/coordinator_session.py), or None if
    MAKIMA_ROOT can't be resolved or the file isn't there."""
    global _CACHED_LIB
    ...
    try:
        ...
        logger.debug("coordinator_core.liveness: _lib_path: MAKIMA_ROOT unresolvable: %s", exc)
        return None
