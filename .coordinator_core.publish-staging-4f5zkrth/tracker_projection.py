...
membership set, and materialize opticon's pinned wire shape on top of it.
...
The property under test CHANGED at opticon's ruling (DEC-21): mutual
...
Negative-spec:
  ...
  - Do NOT move the fold across the wire. `fold_membership_wire` stops at
    ...
    opticon's, and folding again on their side would turn one invariant
...
def fold_membership(*, repo_root: Path) -> dict[str, set[str]]:
    ...
    order, never wall-clock) and project-rag's `current_claim_status`
    latest-wins pattern.
...
def fold_membership_wire(*, repo_root: Path) -> dict[str, list[str]]:
    """Materialize `fold_membership`'s output as opticon's pinned wire shape
