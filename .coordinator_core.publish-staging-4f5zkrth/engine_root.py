"""
coordinator_core.engine_root — ported from coordinator/lib/coordinator-makima-root.sh
(DoE clean-slate migration, sourced-lib variant — DoE .sh is left untouched; its ~60
`source coordinator-makima-root.sh` callers switch to `import coordinator_core.engine_root`
in a later gated wave, per port-template variant "SOURCED LIB").

Purpose: resolves the makima sibling-repo root, analogous to how CLAUDE_HOME->~/.claude
...
MAKIMA_ROOT for callers already running inside the makima engine.

Spec backlink: pln-stop-the-rot-makima-state-home-placement-4cc787 § C1 / AC1
Windows portability rung: docs/plans/2026-07-14-makima-windows-portability.md § C1
...
Resolution chain (rung 1 renamed by C14; the rest unchanged from the bash oracle):
  1. COORDINATOR_ENGINE_ROOT env var — if already set, return it unchanged. The
     retired MAKIMA_ROOT is read at this rung only to report itself as retired
     (see `coordinator_engine_root_env`); it never supplies a value.
  1.5. <settings-home>/machine-local/.makima-root pointer file — a cheap direct-file-read,
       ...
       absent/empty.
  2. `machine-local get repos.project_makima` (CLI, PATH-resolved) — delegates to the
...
Public API:
    def coordinator_engine_root() -> str   — mirrors the shell function of the same name.
        ...
        does NOT export MAKIMA_ROOT into os.environ on success as a side effect free
        of caller intent — callers that want the §4b idempotency-gate behavior set
        os.environ["MAKIMA_ROOT"] themselves after a successful call.
...
Negative-spec:
    ...
    - Does NOT export MAKIMA_ROOT to os.environ as a side effect (the bash oracle does,
...
_REMEDIATION = (
    "coordinator_engine_root: cannot resolve MAKIMA_ROOT — repos.project_makima is not set.\n"
    "  The machine-local registry has no 'repos.project_makima' entry on this machine.\n"
    "  Remediate (choose one):\n"
    "    machine-local set repos.project_makima /path/to/project-makima\n"
...
_TIMEOUT_REMEDIATION = (
    "coordinator_engine_root: cannot resolve MAKIMA_ROOT — "
    f"{_REGISTRY_READ_TIMEOUT_TOKEN}.\n"
    "  The `machine-local get repos.project_makima` subprocess did not return within "
...
def coordinator_engine_root() -> str:
    """Resolve the makima sibling-repo root via the documented chain.
    ...
    # "MAKIMA_ROOT" here is rewritten by the publish transform, which splits
    # env-var names as readily as module names — so the mirror's copy of this
    # rung looks for CLAUDE_KLABAUTER_ROOT and can never see the MAKIMA_ROOT a
    ...
    # intact where the raw name cannot. Surfaced by project-makima-ff,
    ...
    pointer_path = ml_dir / ".makima-root"
    try:
    ...
    if ml_bin is not None:
        try:
            ...
            result = subprocess.run(
                [ml_bin, "get", "repos.project_makima"],
                capture_output=True,
                text=True,
                check=False,
...
# (`coordinator/lib/resolve-makima/_resolve_makima.py`) rather than
# reimplementing its published-engine-vs-live-working-tree gate — the
# single-implementation property (C3<->C4b<->makima_root all defer to one
...
# ``resolved-engine`` (see the shim's own `resolve_makima_root_with_class`
...
# instance's `resolve_makima_root_with_class()` — so the shim's own
...
#: `coordinator_core/engine_root.py`'s parent-of-parent is the makima repo
...
#: any already-resolved MAKIMA_ROOT value.
_SHIM_PATH = Path(__file__).resolve().parent.parent / "coordinator" / "lib" / "resolve-makima" / "_resolve_makima.py"
...
#: (`_ml_dir`, `_registry_value`, `_resolve_makima_root`) rather than going
#: exclusively through the shim's public `resolve_makima_root_with_class()`.
...
#: plus `resolve_makima_root_with_class` and the `RESOLUTION_*` constants —
#: see the matching declaration in the shim's own module docstring
#: (`coordinator/lib/resolve-makima/_resolve_makima.py`). Changing
#: `resolve_makima_root_with_class()`'s step-1 precondition (the
...
def _load_shim() -> types.ModuleType:
    ...
    if not _SHIM_PATH.is_file():
        raise RuntimeError(
            ...
            "broken or partial project-makima checkout."
        )
    spec = importlib.util.spec_from_file_location("_makima_root_gate_shim", _SHIM_PATH)
    if spec is None or spec.loader is None:
        ...
        raise RuntimeError(
            ...
            f"for shim at '{_SHIM_PATH}' — broken or partial project-makima checkout."
        )
...
def _registry_mtime_pair(ml_dir: Path) -> Tuple[float, float, float]:
    """`(registry.toml mtime, registry.local.toml mtime, .makima-root mtime)`,
    `-1.0` for a missing file — cheap staleness key for the gate memo below.
    Never raises.

    Review: code-reviewer — the `.makima-root` sentinel mtime is included
    because the full-gate branch's `_resolve_makima_root` falls back to
...
#: `repos.project_makima` becomes `repos.claude_klabauter` in the published
...
def coordinator_engine_root_with_class() -> Tuple[str, str]:
    """Resolve the makima root AND the DR-132 resolution class alongside it.

    Wraps C3's `resolve_makima_root_with_class()` shim — does NOT
    ...
    HOT-PATH SHAPE (do not "simplify" away — see plan § C4 wrapper half):
      1. Rung 1 (`MAKIMA_ROOT` env var) — `coordinator_engine_root()`'s
      ...
      2. Cheap short-circuit: if `repos.claude_klabauter` (the published
         ...
         shim's own live-tree resolution (`_resolve_makima_root`, which
         itself reads the `.makima-root` pointer as ITS OWN rung 2) rather
         than paying for the full `_is_makima_source_tree` session-root
         ...
         THIS branch is where Rung 1.5's `.makima-root` pointer fast path
         ...
         on the dual-boot path: the shim's `_resolve_makima_root` already
         reads `.makima-root` as its own rung inside the gate, so a working
         repo still resolves via the pointer from inside step 3.
      3. Otherwise, run the full gate (`resolve_makima_root_with_class()`),
    ...
    (`MakimaResolutionError`) — this function does not translate that into
    ...
    # a literal "MAKIMA_ROOT" and the mirror's copy of this rung then cannot
    ...
    if not published_key:
        # Rung 1.5 (`.makima-root` pointer) fast path — see this function's
        ...
        root = shim._resolve_makima_root(ml_dir)
    ...
    result = shim.resolve_makima_root_with_class()
...
#   - The old name (`MAKIMA_ROOT`) is never republished as new API — it is
...
_ENGINE_ROOT_OLD_VAR = "MAKIMA_ROOT"
...
def coordinator_engine_root_env(site: str) -> Optional[str]:
    ...
    Answers from `COORDINATOR_ENGINE_ROOT` only. `MAKIMA_ROOT` is still READ
    ...
    disagreement advisory fires. A stale `MAKIMA_ROOT` inherited from an
    ...
    still read `MAKIMA_ROOT` directly through a module-local `_MAKIMA_ROOT_ENV`
...
def _maybe_emit_engine_root_retired(site: str, root_value: str = "") -> None:
    ...
    fired when `MAKIMA_ROOT` ANSWERED, and the census existed to evidence that
...
def _maybe_emit_engine_root_conflict(new_val: str, old_val: str) -> None:
    ...
    fires whenever `COORDINATOR_ENGINE_ROOT` and `MAKIMA_ROOT` are both set
    ...
    print(
        ...
        f"MAKIMA_ROOT={old_val!r} disagree — COORDINATOR_ENGINE_ROOT wins.",
        file=sys.stderr,
    )
...
def coordinator_engine_root_env_exports(value: str) -> dict:
    ...
    `CLAUDE_KLABAUTER_ROOT`, never `MAKIMA_ROOT`), the deployed settings-home
...
# MAKIMA_ROOT SURVIVES on the locator axis. It does not. DR-326's 2026-08-20
...
#   operation rather than the thing); `MAKIMA_ROOT` retained as the locator name
...
def published_engine_mirror_path() -> Optional[str]:
    ...
    question (which root the CURRENT process should treat as ITS OWN makima
