...
Coverage:
  ...
  History-shaped git fixture (op-level, Patrik Finding 2 — REQUIRED shape):
...
§ chunk C3 (cartography.churn); AC6 fixture-shape note (patrik Finding 2).
"""
...
def _make_git_repo(tmp_path: Path) -> Path:
    ...
    _git(repo, "config", "user.email", "churn-test@makima.test")
...
# ---------------------------------------------------------------------------
# History-shaped git fixture — Patrik Finding 2 (op-level, REQUIRED shape)
# ---------------------------------------------------------------------------

def test_op_deleted_at_head_requires_history_shaped_fixture(tmp_path):
    """CRITICAL (patrik Finding 2): the deleted-at-HEAD mitigation can only be
...
# supplies the set (docs/plans/2026-08-06-makima-ize-the-survey-census.md,
# AC10).
# ---------------------------------------------------------------------------
