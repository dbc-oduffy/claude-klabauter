...
def test_registry_repos_project_makima_is_trusted_anchor(tmp_path):
    """The makima repo's own root — where the trust-check call sites
    themselves now live — must be trusted via the registry-resolved
    `repos.project_makima` key, mirroring the `repos.doe_claude` anchor."""
    settings_home_dir = tmp_path / "settings-home"
    (settings_home_dir / "machine-local").mkdir(parents=True)
    makima_root = tmp_path / "project-makima"
    (settings_home_dir / "machine-local" / "registry.local.toml").write_text(
        f'"repos.project_makima" = "{makima_root}"\n'
    )

    home = tmp_path / "home"
    env = {"HOME": str(home), "COORDINATOR_SETTINGS_HOME": str(settings_home_dir)}

    assert is_trusted(str(makima_root / "coordinator"), env=env)
    assert not is_trusted(str(tmp_path / "project-makima-evil" / "coordinator"), env=env)


def test_registry_repos_project_makima_durable_pointer_file_fallback(tmp_path):
    """Absent the registry key, the durable `.makima-root` pointer file
    under machine-local/ still resolves the anchor."""
    settings_home_dir = tmp_path / "settings-home"
    (settings_home_dir / "machine-local").mkdir(parents=True)
    makima_root = tmp_path / "project-makima"
    (settings_home_dir / "machine-local" / ".makima-root").write_text(str(makima_root) + "\n")
...
def test_absent_repos_project_makima_key_degrades_cleanly(tmp_path):
    """No registry key and no durable pointer file — the makima anchor
...
#
# F2 (striker install dogfood): the guard's home chain was CLAUDE_HOME ->
...
def test_fail_loud_diagnostics_show_empty_anchor_and_caveat_override(capsys, tmp_path):
    ...
    assert "makima_root resolved to:" in err
