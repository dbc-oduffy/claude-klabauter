...
2026-07-22 class defect: the advisory existed but had zero callers, so makima's
...
Cadence seam: the `makima.schema.vendor_drift` doctor probe
(`bin/makima-doctor-probe.py`, manifest entry in `bin/doctor-probes.toml`). That probe
runs in the `--triage` set, whose envelope writes `state/doctor-last-run.json`, which
DoE's `/workday-start` already reads via
`coordinator_core.ops.check_makima_doctor_sentinel`. Drift therefore surfaces daily
with NO change to any DoE-owned surface — makima owns its probe manifest
...
Why TWO surfaces exist (this function AND the `makima.schema.vendor_drift` sentinel
key below) rather than just one: the sentinel is written on a DAILY cadence
(`--triage`/full doctor runs only — see `bin/makima-doctor-probe.py`'s
...
Negative-spec:
  ...
  - Does NOT re-vendor, write, or mutate anything — read-only probe. Re-vendoring is
    the operator's call (see `bin/makima-revendor-handoff-schema.py` for that half).
...
Spec backlink: CLAUDE.md § Architecture (vendored-schema/DoE boundary);
               docs/wiki/makima-install-doctor-system.md § Probe clusters.
"""
...
# Directory holding makima's vendored copies of DoE's canonical schemas.
...
# sense for a file makima VENDORS FROM DoE. Those two memo schemas are now
# the reverse: makima generates them from its own SSOT
...
# 2026-08-19 — the six pinned test vectors in person_resolver keep makima's
# derivation in sync with makima's OWN prior run, never with opticon's; this is
...
OPTICON_SOURCE_GLOB = "*.vendor.ts"

# Machine-readable header line each `*.vendor.ts` file carries (see
# `contributor-id.vendor.ts`'s header comment) — the opticon-repo-relative
...
_OPTICON_SOURCE_PATH_PREFIX = "// opticon-source-path:"


def vendored_source_paths(schemas_dir: Optional[Path] = None) -> list[Path]:
    """Every vendored opticon-source file, sorted — glob-derived like `vendored_schema_paths`.
    ...
    try:
        return sorted(
            p for p in directory.glob(OPTICON_SOURCE_GLOB) if p.is_file()
        )
    except OSError:
        return []


def _read_opticon_source_path(vendor_path: Path) -> Optional[str]:
    """Parse the `// opticon-source-path: <repo-relative path>` header line.
    ...
    for line in text.splitlines()[:10]:
        stripped = line.strip()
        if stripped.startswith(_OPTICON_SOURCE_PATH_PREFIX):
            value = stripped[len(_OPTICON_SOURCE_PATH_PREFIX):].strip()
...
def _vendor_body_without_header(text: str) -> str:
    ...
    The header (opticon-source-path, opticon-source-pin, and any accompanying
    `//` prose) is a makima-local annotation absent from opticon's own file, so
    it must not itself register as drift. Strips every contiguous leading line
    that starts with `//`; the first line that does not (opticon's real content
...
def resolve_opticon_repo_path() -> Optional[Path]:
    """Best-effort resolution of the project-opticon sibling clone root.

    Ladder (first rung that yields a directory wins), mirroring
    `resolve_doe_repo_path`'s shape:
      1. REPO_PROJECT_OPTICON env var — operator/caller override.
      2. registry `repos.project_opticon` (`coordinator_core.machine_resolver.registry_get`).

    Returns None when neither rung resolves to an existing directory — the
    honest "opticon clone not present on this machine" answer. Never raises.
    """
    candidates: list[str] = []

    env_root = os.environ.get("REPO_PROJECT_OPTICON", "").strip()
    if env_root:
        candidates.append(env_root)

    try:
        registry_root = registry_get("repos.project_opticon") or ""
    except Exception:
        registry_root = ""
    if registry_root:
...
def _cat_file_batch(opticon_repo_path: Path, tokens: list[str]) -> Optional[dict[str, Optional[str]]]:
    ...
    """
    return scoped_cat_file_batch(opticon_repo_path, tokens)


def check_source_drift_advisory_batch(
    vendor_paths: list[Path], opticon_repo_path: Path
) -> list[dict[str, Any]]:
    """Non-gating drift check for N `*.vendor.ts` files against opticon HEAD, in TWO spawns.
    ...
    version/bump-class fields are always None here (opticon source files carry no such
    headers), never fabricated.

    Negative-spec (mirrors check_schema_drift_advisory exactly):
      - NEVER raises. An unreadable/foreign opticon clone, an unresolvable header, or a failed
    ...
    source_paths: dict[Path, Optional[str]] = {
        vendor_path: _read_opticon_source_path(vendor_path) for vendor_path in vendor_paths
    }

    # Loop-invariant by inspection: the probe reads the OPTICON clone, which does not vary
    # across `vendor_paths`. One call, not one per file.
    unusable = foreign_repo_unusable_reason(opticon_repo_path)
    ...
    if unusable is None and tokens:
        blobs = _cat_file_batch(opticon_repo_path, tokens)
    ...
    for vendor_path in vendor_paths:
        ...
        if source_path is None:
            results.append(_source_drift_indeterminate(
                ...
                f'"{_OPTICON_SOURCE_PATH_PREFIX}" header — cannot resolve which opticon '
        ...
        if unusable is not None:
            results.append(_source_drift_indeterminate(
                schema_filename,
                f"opticon repo ({opticon_repo_path}) could not be read as a git "
        ...
        if blobs is None:
            results.append(_source_drift_indeterminate(
                schema_filename,
                f"Could not run git against opticon repo ({opticon_repo_path}): "
                "`git cat-file --batch` did not complete.",
            ))
            continue

        opticon_content = blobs.get(f"HEAD:{source_path}")
        if opticon_content is None:
            results.append(_source_drift_indeterminate(
                schema_filename,
                f'Cannot read opticon HEAD source "{source_path}". '
                f"opticon repo path ({opticon_repo_path}) unreadable or missing this file "
                "at HEAD.",
            ))
            continue
        ...
        if local_body.rstrip("\n") != opticon_content.rstrip("\n"):
            results.append({
                ...
                "doe_bump_note": None,
                "detail": (
                    f'Vendored copy "{schema_filename}" diverges from opticon HEAD '
                    f"({opticon_repo_path}:{source_path}) — opticon has moved since the "
        ...
        results.append({
            ...
            "doe_bump_note": None,
            "detail": (
                f'Vendored copy "{schema_filename}" matches opticon HEAD '
                f"({opticon_repo_path}:{source_path})."
            ),
        })
...
def check_source_drift_advisory(vendor_path: Path, opticon_repo_path: Path) -> dict[str, Any]:
    """Non-gating drift check for ONE `*.vendor.ts` file against opticon HEAD.
    ...
    """
    return check_source_drift_advisory_batch([vendor_path], opticon_repo_path)[0]
...
def resolve_doe_repo_path() -> Optional[Path]:
    ...
    Negative-spec:
      ...
      - Does NOT walk `Path(__file__).resolve().parents[N]` to guess a flat-sibling
        `<makima repo root>/../DoE-claude` layout. A prior rung 3 did exactly that
        and was retired 2026-07-22: it hardcoded both makima's checkout depth from
        ...
        to makima — the antipattern `coordinator_core/tests/test_no_hardcoded_paths.py`
...
def scan_vendored_schema_drift(
    ...
    opticon_repo_path: Optional[Path | str] = None,
) -> dict[str, Any]:
    ...
    Args:
        ...
        opticon_repo_path: project-opticon clone root, for the `*.vendor.ts`
            comparison. None → resolve_opticon_repo_path(). Additive parameter
    ...
    Returns a dict with keys:
        ...
        opticon_repo_path (str | None): the project-opticon clone root actually
            used, if resolved — via resolve_opticon_repo_path() (REPO_PROJECT_OPTICON
            env var, then registry `repos.project_opticon`). Additive key (2026-08-19,
            C7 — the `*.vendor.ts`-vs-opticon-HEAD parallel comparison described
            below). None whenever no opticon clone resolved on this machine; the two
        ...
        checked (int): number of vendored files compared — JSON schemas (against
            DoE HEAD) plus `*.vendor.ts` files (against opticon HEAD), summed.
        matched (list[str]): filenames confirmed to match their upstream HEAD
            (DoE for `*.schema.json`, opticon for `*.vendor.ts`).
    ...
    """
    try:
        return _scan(doe_repo_path, schemas_dir, opticon_repo_path)
        ...
        return {
            ...
            "opticon_repo_path": str(opticon_repo_path) if opticon_repo_path else None,
            "checked": 0,
            "matched": [],
            "drifted": [],
            "indeterminate": [],
...
def _scan(
    ...
    parallel `*.vendor.ts`-vs-opticon-HEAD comparison (2026-08-19, the C7
    durability-gap mitigation for C1's opticon `contributor_slug` derivation —
    ...
    resolved_opticon = (
        Path(opticon_repo_path) if opticon_repo_path is not None else resolve_opticon_repo_path()
    )
    ...
    if resolved_doe is None and resolved_opticon is None:
        return {
            ...
            "summary": (
                ...
                "REPO_DOE_CLAUDE, the .doe-root pointer, REPO_PROJECT_OPTICON, and the "
                "registry repos.project_opticon key) — vendored drift is not determinable "
                "here."
            ),
        }
    ...
    if resolved_opticon is None and source_paths:
        for source_path in source_paths:
            indeterminate.append({
                "schema": source_path.name,
                "detail": (
                    "No sibling project-opticon clone resolved on this machine (checked "
                    "REPO_PROJECT_OPTICON and the registry repos.project_opticon key) — "
                    "drift could not be determined."
                ),
            })
    elif resolved_opticon is not None:
        ...
        source_results = check_source_drift_advisory_batch(source_paths, resolved_opticon)
    ...
    if drifted:
        ...
        # verdict is derived here; see the docstring's negative-spec). opticon-source
    ...
    return {
        "status": status,
        "doe_repo_path": str(resolved_doe) if resolved_doe else None,
        "opticon_repo_path": str(resolved_opticon) if resolved_opticon else None,
        "checked": checked,
        "matched": matched,
        "drifted": drifted,
