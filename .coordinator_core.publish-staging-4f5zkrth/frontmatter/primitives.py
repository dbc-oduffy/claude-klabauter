...
helpers above (`replace_fm_field`/`remove_fm_field`) carry a matching guard (Patrik
...
def serialize_yaml_scalar(v: object, *, numeric_quoting: bool = False) -> str:
    ...
    - ``numeric_quoting=True``: additionally quotes all-digit values (SHA-as-int
      defence) and YAML-1.1 scientific-notation floats (stamp-shipped-in Patrik F0).
...
def _is_nested_block_key(fm: str, key: str) -> bool:
    ...
    (Patrik F4; Review: code-reviewer — Finding 4, unindented-sequence gap).
...
def _raise_nested_block_guard(fn_name: str, key: str) -> None:
    ...
    (Patrik F4): a mechanical refusal, not a comment a future caller has to
    read."""
    raise ValueError(
...
def read_fm_block_scalar(fm: str, key: str) -> Optional[BlockScalar]:
    ...
    (cross-repo memo, project-rag-em, 2026-08-20).
...
def replace_fm_field(fm: str, key: str, v: object, *, numeric_quoting: bool = False) -> str:
    ...
    Block-scalar guard (Patrik F1): raises ``ValueError`` when the current value
    ...
    Nested-block guard (Patrik F4, AC11): raises the same ``ValueError`` when the
...
def replace_fm_field_raw(fm: str, key: str, raw_value: str) -> str:
    ...
    Negative-spec (break-class fix, 2026-08-01 — trailing-inline-comment
    preservation, project-opticon repro): this function used to substitute
...
def insert_fm_field(
    ...
    **Append-only** (``after_key=None``): trim trailing whitespace and append
    ...
    ``numeric_quoting`` is forwarded to ``serialize_yaml_scalar`` — set True when
    writing commit SHAs that may be all-digit (stamp-shipped-in Patrik F0).
