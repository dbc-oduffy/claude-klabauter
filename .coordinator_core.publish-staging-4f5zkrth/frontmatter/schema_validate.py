...
def _plan_tasks_schema_without_pm_approved_required(schema: dict | None = None) -> dict:
    """`schema` (default `_PLAN_TASKS_SCHEMA_DICT`, makima's own vendored copy)
    ...
    inverts the makima layering), and the mutate op and both write guards
    ...
    have drifted from makima's own — see the "config-edit re-vendor gap"
    ...
    own resolved schema in rather than silently substituting makima's
...
def _validate_json_schema_node(
    ...
    if/then (object-level conditional; makima-local addition —
...
def _cf_origin_handoff_self_reference(fm: dict) -> ErrorDict | None:
    ...
    walker's job (walk-handoff-dag.js visited-set) + opticon depth-cap; this rule
...
def _cf_initiative_non_empty(fm: dict) -> ErrorDict | None:
    ...
    if str(initiative).strip() == '':
        return {
            ...
            'hint': (
                'Set initiative to a valid initiative id (e.g. "delphi-pro-launch") from '
...
def is_unowned(v: object) -> bool:
    """Canonical is_unowned(v) predicate (Camelia F1, P2): absent | null | whitespace-only.
        ...
        if kind not in _GATE_EVIDENCE_REPO_EXEMPT_KINDS and not (isinstance(repo, str) and repo):
            return {
                ...
                'hint': (
                    ...
                    'project_makima), never inferred from context (ruling 6).'
                ),
            }
...
# (2026-07-19-pcli-phase2-stubs-makima-contracts.md C2). Enforcement below is
...
# cross-repo/inbox/2026-07-29-project-makima-em-grouping-approval-contract-confirmed.md).
#
...
def _memo_cf_disposition_superseded_requires_companions(fm: dict) -> ErrorDict | None:
    ...
    Native-only rule, no JS oracle (post-DR-210 makima owns cross-repo-memo tooling
    ...
    Spec: cross-repo/inbox/2026-08-12-project-rag-em-git-index-lock-reaper.md —
...
def _memo_cf_distill_fate(fm: dict) -> ErrorDict | None:
    ...
    optional/back-compat (absent is valid on legacy memos, backfilled by the makima
    memo.triage op); ephemeral and commitment fates impose no in_repo_capture
    requirement here.
    Spec backlink: docs/plans/2026-07-12-distill-rebuild-makima-reliant.md § C2
    """
    ...
    if str(distill_fate) not in valid_fates:
        return {
            ...
            'hint': (
                ...
                'Absent is also valid (legacy memos backfilled by the makima memo.triage op).'
            ),
        }
...
# (Review: zoli-cutover-review F1, F3, F7).
# ---------------------------------------------------------------------------
...
# DR-210 §2(a) (docs/decisions/DR-210-makima-native-tooling-ownership-strangler.md:89)
...
# the vendored copy still equal what makima PINNED at ref?), and
...
def check_schema_drift(
    ...
    """Tamper-check: prove the vendored schema still equals what makima PINNED at ref.
    ...
    if local_content != doe_content:
        ...
        # (a) cross-repo/inbox 2026-07-30 (market-intelligence-em) -- a vendored
        ...
        # clobbered ~31 lines of makima-owned validator rules, because
        ...
        remedy = {
            ...
            DIRECTION_WE_BEHIND: (
                ...
                'bin/makima-revendor-schema.py (which also moves any pin) -- '
                'not a bare cp.'
            ),
            DIRECTION_BOTH: (
...
# leaf-retention check is told NOT to require makima's copy to retain
...
#     DoE's text is expected to stay a literal substring of makima's -- the
...
# DoE's value being a substring of makima's is an honest append, not a
...
def check_schema_ahead_of_doe(
    ...
    *,
    ...
    """Gating check for a pin declared "makima leads DoE, awaiting upward vendor".
    ...
    asserts the DECLARED-AHEAD state is still honest, by checking four things:
      ...
      1. DoE's copy at `doe_ref` is byte-identical to DoE's copy at the most
         ...
         "unmoved").
      2. makima's vendored copy RETAINS every leaf DoE's copy (at `doe_ref`)
         ...
         DoE's value being a substring of makima's is treated as an honest
         ...
         that makima's schema *validates* a superset of what DoE's does (leaf
      ...
      3. makima's top-level `x-schema-version` parses as strictly greater
         than DoE's `x-schema-version` at `doe_ref`.
      4. If `local_shape_hash` is supplied, makima's CURRENT vendored bytes
    ...
    When DoE later vendors makima's version (or later), remove this schema's
    ...
    if doe_pinned_content != doe_latest_content:
        raise SchemaDriftError(
            ...
            'the new DoE state, or -- if DoE has vendored makima\'s version -- '
    ...
    if not_retained:
        raise SchemaDriftError(
            ...
            f'check -- makima\'s copy no longer retains DoE\'s ref {doe_ref} '
    ...
    if not (local_semver > doe_semver):
        raise SchemaDriftError(
            f'Ahead-pin for "{schema_filename}" requires makima\'s '
...
def check_bump_adds_self_consistency(schema: dict) -> list[ErrorDict]:
    ...
    Three-way key state, load-bearing (do not collapse to a boolean):
      - key ABSENT: the schema hasn't adopted the convention. Inert — `[]`
        errors, same as every makima schema today. This is deliberate, not
...
def check_schema_drift_advisory(schema_path: str | Path, doe_repo_path: str | Path) -> dict:
    ...
    whole vendored set for the makima.schema.vendor_drift doctor probe — through
...
def check_schema_drift_advisory_batch(
    ...
    uses against the opticon clone.
    ...
    if bytes_differ:
        ...
        if divergence_kind == 'prose-only':
            kind_prose = (
                ...
                'makima\'s prose (regressed once, commit 1825e7771) — reconcile by hand'
            )
        else:
...
def _validate_legacy_yaml_frontmatter(frontmatter: dict | None, schema: dict) -> dict:
    ...
    # JSON-Schema-backed in makima's vendored set); a YAML-dialect schema name
...
#
# Reads from makima's vendored schemas directory (coordinator_core/frontmatter/
...
# makima vendors a single fixed schema set, no consumer-test fixture redirection).
# ---------------------------------------------------------------------------
...
def describe(schema_name: str) -> dict:
    ...
    Loads makima's vendored schema set (coordinator_core/frontmatter/schemas/)
...
def validate_frontmatter_obj(fm_dict: dict, schema_obj: dict) -> dict:
    ...
    DoE's ~40-schema corpus, NOT makima's ~10-schema vendored copy — and
    ...
    1. Takes schema_obj as a fully-formed dict. It never resolves a schema
       ...
       corpus can be validated without makima's vendored set shadowing it.
...
# (deleted at makima commit c79e66cd; retrieved for this port via
...
#        This is a makima-local addition beyond the oracle's original scope
...
def _build_handoff_id_index(repo_root: str) -> dict[str, str]:
    ...
    coordinator/bin/lint-frontmatter.js, deleted at makima commit c79e66cd;
    ...
    central/makima state plane; a local scan would false-positive every ref
...
def _check_referential_integrity_id_refs(
    ...
    coordinator/bin/lib/schema.js, deleted at makima commit c79e66cd;
    ...
    (central/makima state plane, out of scope — mirrors the oracle). Does NOT
    ...
    def norm_path(v: Any) -> str:
        ...
        # 2026-07-29 auditing makima's own corpus (3 of 5 flagged pairs were this
...
def _check_handoff_refs(
    ...
    docstring above for the full reconciliation rationale:
      1. PATH-field reachability (predecessor / forked_from /
         ...
         coordinator_core.dag.check_lineage_reachability — a makima-local
...
def _lint_parse_args(argv: list[str]) -> dict | None:
    ...
    while i < n:
        ...
        if tok == '--root':
            ...
            print(
                f'lint-frontmatter: {tok}: not ported — makima BIG_PORT. '
...
def _run_tree_walk(repo_root: str, as_json: bool, strict_refs: bool) -> int:
    ...
    for name in schema_names:
        ...
        # (single star — DoE-owned SSOT, makima only vendors it, see
