...
makima's own vendored _SCHEMAS_DIR — these tests construct schema objects
inline (never touching disk) to prove the function validates a foreign
caller-supplied schema corpus without shadowing through makima's vendored set,
...
class TestValidateFrontmatterObjTypeStringDateAcceptanceRegression:
    """Standing-ruling pin (DoE PM + makima PM, cross-repo memo 2026-08-06):
...
class TestValidateFrontmatterObjNoDiskAccess:
    def test_never_touches_makima_vendored_schemas_dir(self, monkeypatch):
