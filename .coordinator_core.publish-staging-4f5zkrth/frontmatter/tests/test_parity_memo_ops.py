...
makima-native Python op handler for all three memo lifecycle verbs:
...
def _write_memo(path: Path, content: str) -> None:
    ...
    tracked here immediately, mirroring `coordinator/bin/cross-repo-memo.py`'s
...
# dropped them on write, even though the DoE-side strangler facade
# (coordinator/lib/memo-transition-facade.sh) already forwarded them.
# Spec backlink: docs/plans/2026-07-12-distill-rebuild-makima-reliant.md § C3
# ---------------------------------------------------------------------------
...
class TestValidationRejectionParity:
    ...
    def test_receive_overcap_summary_truncated_and_warned(self, tmp_path, capsys):
        """Claim on memo with >120-char summary → makima ABSORBS it, JS golden still rejects.
        ...
        1. 2026-07-13 (`2914a382`, "makima-canonical-memo-summary-reject"): makima ratified
           canonical REJECT for the DR-210 x-repo memo cap — "fail-loud over silent truncation
           of a human-authored capped field; makima is the DR-210 owner of the x-repo memo
        ...
        2. 2026-07-22 (`ba858ae6`, actioning claude-central-em's cross-repo Ask 2,
           ...
           normalization.md`): makima deliberately SUPERSEDED rule (1) on the receive path only.
        ...
        never authoritative over this receive-path ruling; makima owns this surface per (1).
    ...
    def test_claim_tolerates_invalid_kind_js_still_rejects(self, tmp_path):
        ...
        (`_memo_cf_kind_enum` in `schema_validate.py`, both write guards, DoE's
    ...
    @pytest.mark.xfail(
        ...
        reason=(
            ...
            "DoE's normalizeMemoSummary() re-serializes the grandfathered summary without YAML "
            "quotes, diverging from makima's Python truncated-and-quoted serialization. Both "
            ...
            "live-JS-oracle capture) until DoE reconciles its quoting. XPASS means DoE reconciled "
            "— remove this marker. See memo 2026-07-13 makima-canonical-memo-summary-reject."
        ),
    )
