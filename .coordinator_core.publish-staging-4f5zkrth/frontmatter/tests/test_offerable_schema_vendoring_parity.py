...
`offerable` flag from makima's side, which is the surface
...
_MAKIMA_SCHEMAS_DIR = Path(__file__).resolve().parents[2] / "frontmatter" / "schemas"
...
def test_every_offerable_doc_type_has_vendored_schema(schema_name: Optional[str]) -> None:
    ...
    candidates = [
        _MAKIMA_SCHEMAS_DIR / f"{schema_name}.schema.json",
        _MAKIMA_SCHEMAS_DIR / f"{schema_name}.yaml",
    ]
    assert any(c.is_file() for c in candidates), (
        ...
        f"vendor the schema into {_MAKIMA_SCHEMAS_DIR}, or mark the manifest entry "
