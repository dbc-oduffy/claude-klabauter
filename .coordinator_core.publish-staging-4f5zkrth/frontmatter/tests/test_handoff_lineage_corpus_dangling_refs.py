"""
Corpus lint: makima's own state/handoffs/ + archive/handoffs/ must contain zero
...
def test_no_dangling_handoff_lineage_references_in_corpus():
    ...
    reference in makima's own handoff corpus must resolve to a real artifact —
    ...
    assert not failures, (
        f'{len(failures)} dangling handoff lineage reference(s) found in makima\'s '
...
def test_no_dangling_handoff_id_references_in_corpus():
    ...
    assert not dangling, (
        f'{len(dangling)} dangling handoff_id reference(s) found in makima\'s own '
