...
Parity harness — CLI byte-parity between makima's native handoff op handlers and
...
from DoE's CROSS_FIELD_RULES['handoff']) so makima's validate_frontmatter() can be
...
# makima's own vendored copy of handoff.schema.json — the drift-checked schema
...
# LIVE DoE JS rule engine vs makima's Python re-implementation, both consulted
# fresh per assertion — not a frozen transcription of either side's output.
_MAKIMA_HANDOFF_SCHEMA = Path(__file__).parent.parent / "schemas" / "handoff.schema.json"
...
def _js_normalize(repo_root: Path) -> Tuple[int, str, str]:
    ...
    MAKIMA_ROOT is set to a non-existent directory so resolveMakimaRoot() returns
    ...
    env = {
        **os.environ,
        "MAKIMA_ROOT": str(repo_root / "__fake_makima_nonexistent"),
...
def _cross_field_golden(case: str, capture_fn) -> dict:
    ...
    against makima's validate_frontmatter() output instead. No tmp_path
...
class TestConsumeParity:
    ...
    def test_consume_preamble_preserved(self, tmp_path):
        ...
        assert "<!-- project_rag_setup baton v2 -->" in py_content
...
class TestNormalizeParity:
    ...
    MAKIMA_ROOT env override forces JS CLI onto the legacy_normalize() path so we
...
# ---------------------------------------------------------------------------
# Tests — empty session-id FAIL-LOUD parity (Patrik P2)
# ---------------------------------------------------------------------------


class TestEmptySessionIdParity:
    """Both JS and Python must reject empty session-id (Patrik P2 fail-loud).
...
# H-CROSS-EXEC-1/2) on every test run and asserts makima's validate_frontmatter()
...
# makima's _cf_execution_stamp_required / _cf_handoff_phase_kind_gate
# ports are freshly landed in lockstep with no pre-existing divergence.
# ---------------------------------------------------------------------------


class TestHandoffPhaseCrossFieldParity:
    """makima validate_frontmatter() vs a frozen golden of lint-frontmatter.js's
    live H-CROSS-EXEC-1/2 verdict — handoff_phase."""

    def _assert_parity(
        self, tmp_path: Path, fm: dict, *, heading: str, filename: str, case: str,
    ) -> None:
        """Write fm to a real repo, diff makima's validator against the golden verdict
        ...
        py_errors = validate_frontmatter(fm, _MAKIMA_HANDOFF_SCHEMA)
        py_valid = len(py_errors) == 0

        assert py_valid == js_valid, (
            f"parity mismatch for {filename}: golden (DoE lint-frontmatter.js) "
            f"valid={js_valid} vs makima valid={py_valid} (errors={py_errors!r})"
        )
