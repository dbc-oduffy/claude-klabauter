...
def _make_git_repo(root: Path) -> Path:
    ...
    subprocess.run(
        ["git", "config", "user.email", "roadmap-schema-glob-test@makima.test"],
        cwd=str(root),
        capture_output=True,
        check=True,
    )
...
def test_sprint_path_overview_matches_roadmap_schema_via_match_schema():
    ...
    resolved = match_schema(
        "state/roadmap/makima-strangler/sprint-1/OVERVIEW.md", fm, schemas
    )
...
def test_sprint_path_overview_is_returned_by_a_real_query_records_call(tmp_path: Path):
    ...
    overview_dir = worktree / "state" / "roadmap" / "makima-strangler" / "sprint-1"
    ...
    assert any(
        Path(p).as_posix().endswith("makima-strangler/sprint-1/OVERVIEW.md")
        for p in paths
    )
