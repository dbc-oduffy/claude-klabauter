...
from coordinator_core.frontmatter.schema_validate import (
    ...
    _SCHEMAS_DIR as _MAKIMA_SCHEMAS_DIR,
...
# docs/plans/2026-07-24-python-ize-makima-bin-oracles-doe-forwards-to.md) alongside
...
class TestReadyToFireIfThenSchemaLevel:
    ...
    (docs/plans/2026-07-13-makima-auto-reconcile-open-handoffs.md § C1) originally added
    this if/then construct as a makima-local addition, later dropped and re-added
...
class TestLastGateRecheckSchemaDeclaration:
    ...
    DoE rather than makima-local.

    History: C1's schema-hardening sub-step
    (docs/plans/2026-07-13-makima-auto-reconcile-open-handoffs.md § C1) originally added
    a JSON-Schema-level (type: string, format: date) declaration as a makima-local
    ...
    re-vendor, not a merge. makima memo'd DoE asking for the property to be re-added
    upstream; DoE landed it at bfbaac70 ("schema: re-add last_gate_recheck property +
    ready_to_fire if/then dropped by DR-084 P0 widen (makima memo)"). This re-vendor
    (pulling in bfbaac70) restores the declaration, now as an upstream-owned DoE
    property rather than makima-local drift. last_gate_recheck remains a real,
...
class TestIsUnowned:
    """Camelia F1: is_unowned(v) treats absent/None/whitespace-only as equivalent."""
    ...
    def test_real_slug_is_not_unowned(self):
        assert is_unowned('em-striker') is False
...
class TestOwnerAxisScalar:
    ...
    def test_explicit_owner_slug_passes(self):
        ...
        fm = _valid_handoff(**{field: 'em-striker' for field in _OWNERSHIP_AXIS_FIELDS})
        errors = validate_frontmatter(fm, _HANDOFF_SCHEMA)
        assert not any(e['field'] in _OWNERSHIP_AXIS_FIELDS for e in errors)
...
class TestDeliverableSpineFields:
    ...
    def test_initiative_valid_string_ok(self):
        fm = _valid_handoff(initiative='delphi-pro-launch')
        errors = validate_frontmatter(fm, _HANDOFF_SCHEMA)
...
# bin/makima-revendor-handoff-schema.py). plan_ids/deliverable_ids are the
...
class TestArchivePathPrecedence:
    ...
    def test_archived_path_with_handoff_kind_resolves_to_handoff_archived(self):
        schemas = load_schemas(_MAKIMA_SCHEMAS_DIR)
...
class TestDriftCheck:
    def test_handoff_schema_matches_doe_head_after_dr084_revendor(self):
        ...
        1. `properties.additional_predecessors.description` — makima's local
        ...
        2. `x-schema-version` `7.1.0 -> 8.0.0`, `x-bump-class` `major` — DoE's
           ...
           UNMOVED `7.1.0` that makima had already vendored, deadlocking both trees:
        ...
        schema-hardening sub-step (docs/plans/2026-07-13-makima-auto-reconcile-open-handoffs.md
        ...
        Python cross-field rule as makima-local hardening on top of the vendored file,
        ...
        straight re-vendor, not a merge, so both constructs were dropped. makima
        ...
        dropped by DR-084 P0 widen (makima memo)"). This re-vendor pulls in bfbaac70,
    ...
    def test_plan_tasks_schema_matches_doe_head(self):
        ...
        because makima declared that contract here while DoE declared it on
        plan.schema.json. That divergence is GONE as of the 2026-08-06 re-vendor:
        makima now vendors plan.schema.json too (see the sibling test below), and
        ...
        assert 'grouping_approvals' not in vendored.get('properties', {}), (
            ...
            'Re-vendor via bin/makima-revendor-schema.py plan-tasks.'
        )
    ...
    def test_plan_schema_matches_doe_head_and_homes_grouping_approvals(self):
        ...
        compared makima's plan-tasks-declared block against DoE's
        ...
        contract in different files; now that makima vendors plan.schema.json
...
# work/striker/2026-07-21 (pushed to origin/work/striker/2026-07-21) — it is
...
# doc-new's self-scaffold fallback (project-makima commit b0a336c4, then
...
# on DoE's work/striker/2026-07-21 (fetch-reachable, not yet merged to
# origin/main).
#
...
# work/delphipro/2026-07-21to26 (fetch-reachable, not yet merged to origin/main).
# Note the class: an optional top-level *object*, so it is NOT the
# top-level-array-additive class makima's structural-tolerance ratification
...
_QUEUE_SCHEMA_PINS = {
    ...
    # HEAD) by bin/makima-revendor-schema.py bug-backlog.
    ...
    #   into makima's mirrors. Also takes DoE's additive predecessor_handoff
    ...
    #   2026-08-17-project-makima-em-adopt-stable-id-spec-backlinks-or-tell-
    #   us-to-stop.md
    'bug-backlog': "2d81501bfaed77ed433a2a160e69f4e51684d992",
    # Pin moved 2026-08-17 to cd70f651f95503ac2d8979b6900ba905c910a75a (DoE
    # HEAD) by bin/makima-revendor-schema.py cross-repo-commitment.
    ...
    # Pin moved 2026-08-14 to 13f3307000d4685243d60220145fc494383a0839 (DoE
    # HEAD) by bin/makima-revendor-schema.py debt-backlog.
    #   C7 (4b8e485c6) rewrote cross-repo citations to repo-qualified ids
    #   across 458 files and swept 16 vendored schemas as collateral, breaking
    #   the byte-identity the vendored contract rests on. Restoring mirror
    #   parity: the id-form convention is DoE-ratified (they committed it at
    ...
    # HEAD) by bin/makima-revendor-schema.py improvement-queue.
    ...
    # HEAD) by bin/makima-revendor-schema.py improvement-queue.
    #   config-edit was authorable in DoE's enum but absent from makima's
    ...
    #   makima's copy already carried both members, so this takes the version
    ...
    # 9b5a08fd2) by bin/makima-revendor-schema.py improvement-queue.
    #   Equal-version (1.2.0) content drift on case_against.description:
    #   makima carried the pre-697b7d451 'not yet populated by the harvest
    ...
    # Pin moved 2026-08-17 to cd70f651f95503ac2d8979b6900ba905c910a75a (DoE
    # HEAD) by bin/makima-revendor-schema.py lesson-entry.
    #   scheduled re-vendor pass: sync non-major drifted schemas from DoE HEAD
    # Pin moved 2026-08-17 to 2d81501bfaed77ed433a2a160e69f4e51684d992 (DoE
    # HEAD) by bin/makima-revendor-schema.py lesson-entry.
    #   DoE landed the stable-id spec-backlink sweep (DoE-claude@3f9df69d9 +
    ...
    # HEAD) by bin/makima-revendor-schema.py review-findings.
    ...
    #   on the left endpoint, actioning makima's 2026-08-14 reviewed-range-
    ...
    # HEAD) by bin/makima-revendor-schema.py review-findings.
    #   DoE 359e41da5 shipped reviewed_targets, the class-2 answer to makima's
    ...
    #   structural tolerance, and none was on record for makima.
    ...
    # HEAD) by bin/makima-revendor-schema.py review-trail.
    #   DoE vendored makima's 1.2.0 scope_kind enum at 6baac04a3; the ahead-
    ...
    # bin/makima-revendor-schema.py's own docstring for why the FIRST
    ...
    # project-opticon drops into priority-intent-inbox/ for priority.drain
...
# Ahead-pin registry: entries here declare "makima's vendored copy is
...
#   reason (str, required)      — why makima is allowed to lead DoE here.
...
#
# review-trail: makima bumped to 1.2.0 (closing scope_kind from an
...
# below (git diff <doe_ref> -- that path in DoE-claude is empty). makima's
...
# Schemas where makima deliberately leads DoE, awaiting their upward vendor.
...
# example: makima closed `scope_kind` to an enum ahead of DoE to fix a live
...
# an ordinary byte-pin through bin/makima-revendor-schema.py. That is the whole
...
class TestDivergenceKind:
    ...
    def test_prose_only_detail_still_warns_about_losing_local_prose(
        ...
        drops makima's prose — the risk is real (regressed once, commit
...
class TestPinnedQueueSchemaDrift:
    ...
    def test_review_trail_matches_pinned_sha(self):
        # Was an ahead-pin for a few hours on 2026-08-10 while makima led DoE
        ...
        # convergence was then taken through bin/makima-revendor-schema.py,
...
class TestGateEvidenceLegsShape:
    ...
    def test_valid_frontmatter_field_leg_ok(self):
        fm = _valid_handoff(gate_evidence={
            'covers_prose': True,
            'legs': [{
                'leg_id': 'l1', 'kind': 'frontmatter-field', 'repo': 'project_makima',
    ...
    def test_valid_test_node_id_leg_ok(self):
        ...
        fm = _valid_handoff(gate_evidence={
            'covers_prose': True,
            'legs': [{
                'leg_id': 'l1', 'kind': 'test-node-id', 'repo': 'project_makima',
...
class TestCfAwaitingGateNotPickupReady:
    ...
    Spec backlink:
    cross-repo/inbox/2026-08-06-market-intelligence-em-pickup-ready-true-under-unmet-gate.md
    """
