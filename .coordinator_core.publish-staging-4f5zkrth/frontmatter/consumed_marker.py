...
remains the home of CONSUMED_MARKER_RE, so the write-time path (the makima
port of normalize-consumed-frontmatter.js) and the read-time path (the
makima port of query-records.js) are greppably aligned, mirroring the JS
...
# from the JS original's own Patrik F4 fix.
#
