"""coordinator_core.frontmatter.schema_shape -- makima's own semantic
...
Why this exists: makima vendors DoE's schemas under
...
NEGATIVE SPEC -- what this is NOT:
  ...
  - NOT a byte hash of the file. A byte hash trips on reindentation, key
    ...
    validator's behaviour -- and on a schema DoE and makima hold
  ...
  - NOT a cross-repo comparison. Three gates already compare makima's copy to
  ...
  - NOT an `x-*` prefix glob. The authoring annotations stripped here are a
    ...
    `handoff.schema.json` is consumed by makima's cockpit-contract summaries
