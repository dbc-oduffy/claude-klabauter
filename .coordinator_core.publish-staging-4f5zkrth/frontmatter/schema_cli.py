...
Negative-spec:
    - Does NOT support COORDINATOR_SCHEMAS_DIR override — schema_validate.describe()/
      validate() always read makima's own vendored schema set
      (coordinator_core/frontmatter/schemas/); makima vendors a single fixed schema
      ...
      schema-cli.js's env-override — makima has no consumer-test schema-dir
...
async def _op_schema_describe(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    Params:
        schema_name (str): required. The vendored schema name to describe
            ...
            reads makima's own fixed vendored schema set, never a per-repo path.
