...
Invocation:
    ...
    --dump-op-timeouts — Read-only surface: print
                         ...
                         makima's real per-op dispatch-timeout budgets instead
...
Exit codes:
    ...
    2 — JSON-RPC error result printed to stdout, error.code ==
        ...
        structurally-wedged contract-pin error (e.g. ContractPinError: makima's own
...
def _dispatch_argv_body(argv: list, cwd: str, *, allow_warm: bool) -> None:
    ...
    #     something. market-intelligence-em ran the survey end-to-end and
    ...
    # 5. Op registration is now LAZY (F6 / makima-windows-portability § C4):
    ...
    #     project-makima-44 fixed for `detached_spawn`, through a different
...
def main() -> None:
    ...
    """
    # Lazy op registration (F6 / makima-windows-portability § C4) is
