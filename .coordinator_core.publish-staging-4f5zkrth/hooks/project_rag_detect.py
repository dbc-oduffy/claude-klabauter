"""
coordinator_core.hooks.project_rag_detect — SessionStart banner: generic
project-RAG freshness (Port of: project-rag-detect.sh, DoE d39ab164,
2026-07-16 / project-rag-detect.ps1, W4b bash-to-python-migration cohort).
...
exit, no stdout):
  ...
  SKIP          — fail-open message when a required probe (stat/git) itself
                  fails
  ""            — kill-switch set, holodeck context detected, or no
                  .project-rag/manifest.json marker found walking up from cwd

Disposition: PORT (Patrik adjudication item, W4b §2.8). Both `.ps1` and `.sh`
...
Holodeck dedupe: `.holodeck/` or `Saved/HolodeckProjectRag` found walking up
from cwd => silent exit; the holodeck-specific hook owns that banner.

Called IN-PROCESS by the thin DoE SessionStart stub
(coordinator/hooks/scripts/project-rag-detect.py), mirroring
...
def detect_banner(cwd: str) -> str:
    """Compute the project-rag freshness banner for cwd. "" means silent exit."""
    if os.environ.get("COORDINATOR_HOOK_PROJECT_RAG_DETECT_DISABLED") == "1":
        return ""

    # --- Holodeck dedupe: positive context detection ---
    if _find_marker_upward(cwd, ".holodeck") is not None:
        return ""
    if _find_marker_upward(cwd, os.path.join("Saved", "HolodeckProjectRag")) is not None:
        return ""

    # --- Generic project-RAG detection via marker file ---
    manifest_path = _find_marker_upward(cwd, os.path.join(".project-rag", "manifest.json"))
    if not manifest_path:
        return ""

    project_rag_dir = os.path.dirname(manifest_path)
    repo_root = os.path.dirname(project_rag_dir)

    db_path = os.path.join(project_rag_dir, "graph.db")

    if not os.path.isfile(db_path):
        return (
            f"project-rag: UNINITIALIZED — marker found at {manifest_path} but "
            "no graph.db; run the project-RAG indexer before querying"
        )

    try:
        db_mtime_epoch = Path(db_path).stat().st_mtime
    except OSError:
        return "project-rag: SKIP — could not stat graph.db (fail open)"
    ...
    if shutil.which("git") is None:
        return "project-rag: SKIP — git not found (fail open)"
    ...
    if delta_raw is None:
        return "project-rag: SKIP — git rev-list failed (fail open)"
    try:
        delta = int(delta_raw)
    except ValueError:
        return "project-rag: SKIP — git rev-list failed (fail open)"
    ...
    if delta == 0:
        return "project-rag: fresh (HEAD aligned)"

    base_msg = (
        f"project-rag: STALE — {delta} commits behind HEAD; "
        "project-RAG queries may miss recent changes"
    )
