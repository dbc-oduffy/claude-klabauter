...
This module is the makima half of the auto-push reimplementation; the DoE half
...
per DR-047 (DoE owns contract/generator, makima owns engine). Do NOT edit DoE's
...
# project-rag-em 2026-08-18 with a live 5.5h-resident instance, root-caused to
...
def branch_gate(branch: str) -> tuple[bool, str | None]:
    ...
    Case-sensitivity note (Patrik R1 F7 + 2026-05-07 HTTPS-autopush spinoff):
...
def _module_provenance() -> str:
    ...
    execs `coordinator/bin/coordinator-auto-push`, which resolves MAKIMA_ROOT at
...
def log_failure(
    ...
    shared log per repo -- a push failure is a repo-level fact, and makima
...
# so project-rag's and project-opticon's daily ceremonies would each have
...
def _cockpit_publish_script(repo_root: str) -> Path | None:
    ...
    below. It cannot fire in project-rag, project-opticon, or any other
...
# the window. Measured 2026-08-20 on work/striker/2026-08-18to20 (trailing
...
def _peer_commit_within_window(repo_root: str, branch: str, now: float) -> bool | None:
    ...
    ### DECISION (measured 2026-08-20, work/striker/2026-08-18to20, trailing
    36h): 2079 commits, median inter-commit gap 20s -- a median of 10 peer
...
def _respawn_env() -> dict:
    ...
    `_makima_package_root()` ahead of any pre-existing `PYTHONPATH` --
    ...
    env = dict(os.environ)
    env[_ENV_SYNC] = "1"
    package_root = _makima_package_root()
...
def _detach_and_run(repo_root: str, branch: str) -> None:
    ...
    # fleet repo, whose cwd is the committing repo (NOT makima), so
    ...
    # 2026-07-20 on win32: `-m` from cwd=X:/project-rag ->
    ...
    # exit 0. See `test_spawn_detached_push_child_survives_import_when_spawned_outside_makima`.
