...
def plugin_home(tmp_path: Path) -> Path:
    ...
    _write_agent_md(plugins / "claude-unreal-holodeck" / "game-dev" / "agents" / "staff-game-dev.md", "staff-game-dev")
    ...
    _write_agent_md(manifest_install_root / "plugin" / "agents" / "project-rag-researcher.md", "project-rag-researcher")
...
def test_resolve_roster_unions_all_three_sources(doe_root: Path, plugin_home: Path) -> None:
    ...
    assert "project-rag:project-rag-researcher" in roster  # leg 4, manifest + plugin/ nesting
...
def test_repo_style_plugin_leg_alone(tmp_path: Path) -> None:
    ...
    assert "holodeck-control:ue-asset-author" in names
...
def test_manifest_style_plugin_leg_with_plugin_subdir_nesting(tmp_path: Path) -> None:
    ...
    _write_agent_md(install_root / "plugin" / "agents" / "project-rag-researcher.md", "project-rag-researcher")
        ...
        json.dumps(
            {
                "version": 2,
                "plugins": {
                    "project-rag@project-rag": [{"scope": "user", "installPath": str(install_root)}]
                },
            }
        ),
        encoding="utf-8",
    ...
    assert "project-rag:project-rag-researcher" in names
...
def test_manifest_leg_failure_does_not_blank_filesystem_legs(tmp_path: Path) -> None:
    ...
    _write_agent_md(
        plugins_root / "claude-unreal-holodeck" / "game-dev" / "agents" / "staff-game-dev.md",
        "staff-game-dev",
    )
    plugins_root.mkdir(parents=True, exist_ok=True)
    (plugins_root / "installed_plugins.json").write_text("{not valid json", encoding="utf-8")
...
def test_live_roster_allows_harness_builtins_and_regression_plugin_types() -> None:
    ...
    assert "project-rag:project-rag-researcher" in roster
...
def test_live_roster_allows_via_check_not_just_resolve_roster(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for subagent_type in (
        ...
        "Plan",
        "general-purpose",
        "project-rag:project-rag-researcher",
