...
hook-path git-probe spawn sites (`project_rag_detect._git` and
...
from coordinator_core.hooks import context_pressure_precompact, project_rag_detect
...
def test_project_rag_detect_git_passes_suppression_kwargs_on_windows():
    with mock.patch(
        ...
        result = project_rag_detect._git("fake-repo-root", "log", "-1")
...
def test_project_rag_detect_git_unchanged_on_posix():
    with mock.patch(
        "coordinator_core.win_portability.no_console_creationflags",
        return_value={},
    ) as fake_flags, mock.patch("subprocess.run", return_value=_fake_run()) as fake_run:
