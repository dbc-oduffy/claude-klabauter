...
to go — the `holodeck` codename in particular, since the banner used to read
"...via holodeck-docs MCP.", a substring whose scrub-token adjacency
(`holodeck` immediately followed by `-docs`) was measured (see run-report
...
"holodeck-control@claude-unreal-holodeck") are functional identifiers the
...
def test_banner_drops_named_adjacency_hazard_string(tmp_path):
    ...
    holodeck-docs MCP." -- must not appear at all: the fix drops the
    ...
    assert "via holodeck-docs MCP" not in result.banner
    assert "holodeck-docs" not in result.banner
