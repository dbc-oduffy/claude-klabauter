...
CLASSIFY_CASES = [
    ...
    (
        "dead-ref",
        "error: src refspec work/striker/2026-08-10 does not match any\n"
...
def test_detach_and_run_windows_respawn_sets_sync_seam_and_no_infinite_loop(monkeypatch, tmp_path):
    ...
    assert "-m" not in captured["argv"], "respawn must not use -m (breaks outside makima)"
    ...
    assert auto_push._makima_package_root() in pythonpath
...
def test_main_internal_exception_logs_resolved_module_provenance(monkeypatch, tmp_path):
    ...
    The hook resolves this module from MAKIMA_ROOT at fire time, so an
...
# (cross-repo/archive/2026-07-25-project-makima-em-cockpit-publish-use-a-github-action-not-a-makima-directive.md,
# DoE-claude).
# ---------------------------------------------------------------------------
...
def test_respawn_env_injects_makima_package_root_on_pythonpath():
    env = auto_push._respawn_env()
    entries = env["PYTHONPATH"].split(os.pathsep)
    assert auto_push._makima_package_root() in entries
...
def test_spawn_detached_push_child_actually_imports_when_spawned_outside_makima(tmp_path):
...
def test_detach_and_run_windows_leg_child_actually_imports_when_spawned_outside_makima(tmp_path):
    """Same real-spawn regression guard as
    test_spawn_detached_push_child_actually_imports_when_spawned_outside_makima,
    ...
    """
    fake_repo = tmp_path / "not-makima-2"
    fake_repo.mkdir()
    (fake_repo / ".git").mkdir()
    ...
    makima_root = str(Path(auto_push.__file__).resolve().parents[2])
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            (
                "import sys; "
                f"sys.path.insert(0, {makima_root!r}); "
                "from coordinator_core.hooks import auto_push; "
...
# resolves locally (the branch-rename incident: `work/striker/2026-08-10`
# renamed to `work/striker/2026-08-10to11`, leaving the old name's record
...
def test_drain_pending_push_dead_ref_already_on_origin_drops_no_log(monkeypatch, tmp_path, capsys):
    ...
    (tmp_path / ".git").mkdir()
    auto_push._write_pending_record(
        repo_root, "work/striker/2026-08-10", "abc123", time.time() - 1, os.getpid()
    )
    monkeypatch.setattr(auto_push, "_run_git", lambda root, args: {
        # The old name never resolves; the resolved current branch is the
        # renamed one.
        ("branch", "--show-current"): "work/striker/2026-08-10to11",
        ("for-each-ref", "--format=%(refname:short)", "refs/heads/"): "work/striker/2026-08-10to11",
        ("fetch", "origin", "work/striker/2026-08-10to11"): "",
    }.get(tuple(args)))
    monkeypatch.setattr(auto_push, "_is_ancestor", lambda root, sha, ref: True)
...
def test_drain_pending_push_dead_ref_reachable_from_current_branch_retargets_and_pushes(
    ...
    (tmp_path / ".git").mkdir()
    auto_push._write_pending_record(
        repo_root, "work/striker/2026-08-10", "abc123", time.time() - 1, 4242
    )
    monkeypatch.setattr(auto_push, "_run_git", lambda root, args: {
        ("branch", "--show-current"): "work/striker/2026-08-10to11",
        ("for-each-ref", "--format=%(refname:short)", "refs/heads/"): "work/striker/2026-08-10to11",
        ("fetch", "origin", "work/striker/2026-08-10to11"): None,  # fetch fails -> not superseded
        ("remote", "get-url", "origin"): "https://github.com/org/repo.git",
        ("rev-parse", "work/striker/2026-08-10to11"): "abc123",
    }.get(tuple(args)))

    def _fake_is_ancestor(root, sha, ref):
        # Locally reachable from the CURRENT branch tip.
        return ref == "work/striker/2026-08-10to11" and sha == "abc123"
    ...
    assert push_calls == ["work/striker/2026-08-10to11"], "retargeted push must use the CURRENT branch"
...
def test_drain_pending_push_dead_ref_sha_none_with_current_branch_retargets_and_pushes(
    ...
    (tmp_path / ".git").mkdir()
    auto_push._write_pending_record(
        repo_root, "work/striker/2026-08-18", None, time.time() - 1, 4242
    )
    monkeypatch.setattr(auto_push, "_run_git", lambda root, args: {
        ("branch", "--show-current"): "work/striker/2026-08-18to19",
        ("for-each-ref", "--format=%(refname:short)", "refs/heads/"): "work/striker/2026-08-18to19",
    }.get(tuple(args)))
    monkeypatch.setattr(auto_push, "is_windows_bash", lambda: False)
    ...
    assert push_calls == ["work/striker/2026-08-18to19"], (
...
def _init_hook_mirror_repo(tmp_path: Path) -> Path:
    ...
    subprocess.run(
        ["git", "config", "user.email", "oracle@makima.test"], cwd=str(repo_root), check=True,
    )
    subprocess.run(
...
def _commit_env(makima_root: str, *, suppress: bool) -> dict:
    """Env for the `git commit` subprocess that fires the real hook. Rung 1
    of `cc_invoke._resolve_makima_root()` (env var already set -> fast-path
...
def _init_ceremony_repo(tmp_path: Path) -> tuple[Path, Path]:
    ...
    _git("init", "-q", "-b", "main")
    _git("config", "user.email", "oracle-arm3@makima.test")
...
def test_oracle_arm1_suppressed_scoped_git_commit_default_one_interpreter_start(tmp_path):
    ...
    makima_root = str(Path(auto_push.__file__).resolve().parents[2])
    env = _commit_env(makima_root, suppress=True)
...
def test_oracle_arm2_unsuppressed_wsc_tail_default_two_on_nt_one_on_posix(tmp_path):
    ...
    repo_root = _init_hook_mirror_repo(tmp_path)
    makima_root = str(Path(auto_push.__file__).resolve().parents[2])
    env = _commit_env(makima_root, suppress=False)
