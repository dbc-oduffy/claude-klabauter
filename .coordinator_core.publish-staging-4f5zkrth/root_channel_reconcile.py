...
THE DEFECT SHAPE, three times in one day:
  ...
  1. 2026-08-22, install dogfood on `delphipro`. The pointer file
  ...
  2. 2026-08-22, independently. DoE's `_engine_root.resolve_makima_root`
     answers `repos.claude_klabauter` (classed `resolved-engine`) while
     makima's `trusted_root_guard._makima_root` answers
     `repos.project_makima` and never reads that key at all -- one ladder
     ...
     makima-root-but-is-never-trusted.yaml`.
...
_ROOTS: Tuple[_RootSpec, ...] = (
    ...
    _RootSpec(
        name="project_makima",
        purpose="the live makima working tree (source, never a dispatch target)",
        channels=(
            _ChannelSpec("registry", "repos.project_makima"),
            _ChannelSpec("pointer", ".makima-root"),
            _ChannelSpec("registry", "engine.working_repos.project_makima"),
        ),
    ),
)
