# coordinator → project-rag producer contract (frozen)
...
> engine (**makima**) exposes to the **project-rag** data plane. It defines exactly what
> makima emits as authoritative disk-truth and which engine ops are observable, so that
> project-rag can build its CQRS projection / typed query surface against a stable input
> **without further co-design**. makima *emits*; project-rag *ingests and projects*.
>
> **Who consumes this.** A context-less project-rag EM building `workstate_store` — a
> derived, rebuildable projection over makima's disk-truth (capability-vs-custody: rag owns
> the projection mechanism, makima holds custody of the source bytes). Everything needed to
...
>
> **What this is NOT.** This is the *input* project-rag builds against — not project-rag's
> output. It does not specify project-rag's HTTP projection responses, its pull-vs-SSE
...
> contract follow the reader-widen-before-writer-flips co-ratified bump protocol; project-rag
...
>   from the retired "rag is the fleet system-of-record / makima is store-less" framing to
>   capability-vs-custody: rag's `workstate_store` is a derived, rebuildable projection over
>   makima's disk-truth, not a system-of-record makima is store-less against; makima holds
...
>   is discharged by two 2026-07-25 memos already in project-rag's inbox —
>   `2026-07-25-project-makima-em-pm-ruling-rag-is-capability-not-custody.md` and
>   `2026-07-25-project-makima-em-workstate-store-is-projection-not-custody-correction.md` —
...
>   C6 opened: makima owns this in-spine contract-text edit; **rag-EM is the accepting
...
>   cockpit-contract D13/D21 gate: makima owns this in-spine contract-text edit; **rag-EM is
...
>   `opticon-invoke-producer-contract.md` 2026-07-05 changelog for the parallel transport
...
>   existing behavior, no wire change; both rag and opticon derive the path themselves and hit
>   the same gap.
...
project-rag ingests from makima across **two planes**, in priority order:

1. **The disk-truth emission artifact** — `cockpit-emission.json`, a single
   schema-versioned envelope makima writes to disk. This is the **authoritative,
   primary** surface project-rag projects from. It contains every work-state section
...
2. **The observable op surface** — a small set of read-only JSON-RPC ops
   ...
   makima's mutating ops. **[SUPERSEDED by DR-215 (2026-07-06)]** — the ops are still live
...
Boundary invariant (capability-vs-custody, DR-236): **makima holds custody of its own
disk-truth and does not write into rag's `workstate_store`.** makima emits authoritative
disk-truth; project-rag owns the projection capability — `workstate_store` is a derived,
...
> 2026-07-05, landed 2026-07-06) retired makima's resident daemon and its UDS/HTTP invoke
...
>
> See `coordinator_core/contract/opticon-invoke-producer-contract.md`'s 2026-07-05 changelog
> entry for the parallel transport story on Channel A (opticon's `fleet.*` surface), and this
...
~~- **Socket path (POSIX):** `/tmp/coordinator-svc-<uid>/<hash16-of-canonical-repo-root>.sock`
  ...
  hexdigest (lowercase). A consumer deriving the socket path itself (project-rag calling ops,
  opticon on the fleet.* channel) MUST use the identical canonicalization; a single
...
**Invariant:** project-rag never invokes `artifact.emit`, `backlog.record`, or any `hooks.*`
op. makima owns emission; rag consumes the emitted disk-truth. rag's only writes are to its
own store.
...
consumer assumption: **makima owns a change-detection obligation** — any future release that alters
...
**This is the authoritative surface project-rag projects from.** makima's `artifact.emit`
op (MUTATING, makima-owned) assembles and writes a single schema-versioned envelope to the
...
**This producer contract references it; it does NOT redefine it.** project-rag vendors the
...
makima already emits conformant provenance on git-backed sources; the D9 null-for-local_fs
...
project-rag's projection MUST record which envelope `schema_version` it built against and
...
minor/additive `schema_version` bumps, forward-compatibility is project-rag's job: rag
tolerates a higher minor than the one it was built against and passthrough/drops unknown
additive keys (the same posture opticon's `checkSchemaVersion` takes, and rag's own
quarantine-tolerant ingestion already assumes). makima does **not** hold emission open
...
1. **Producer-side process hold (new, 2026-07-25).** makima holds merge-to-main and production
   ...
   readiness back to makima via the usual PM/cross-repo coordination channel.
...
This was previously documented (2026-07-08) as a co-ratified protocol with project-rag where
makima unwound the producer-side hold unilaterally as redundant with rag's quarantine-tolerant
...
This is the deterministic "did a landed commit close this work item" fact opticon's `recs-05`
(B4) code-complete auto-assert consumes — makima emits it via the `commit_closures` section
porter; opticon reads it from `cockpit-emission.json` (this contract's canonical surface),
...
time (DECISION-4). Multi-commit resolution ("is item X actually closed") is opticon's read-side
query over these rows — e.g. "any row for `(repo, item_id)` with `reachable_on_default_branch
== true`" — not a makima write-time concern.
...
**No opticon-side or rag-side git access.** makima performs the one bounded `git log` scan and
the reachability check; rag/opticon consume the emitted array only.
...
"content-hash is the disambiguation makima ships".
...
### 3.3 Wire-emission of the change-signal (what makima emits for rag to consume)
...
using that same sha256-content-hash convention. It is the frozen shape makima's emitter will
carry.
...
> *defined* here but is **not yet emitted** by `coordinator_core/ops/emit/`. makima must wire
...
> `provenance.path` + the on-disk body, but the contract's intent is that makima emits it.
...
this contract** and are **project-rag's own decisions**:

- **project-rag's HTTP projection response shapes** — `GET /v1/coverage`,
...
- **The CQRS projection implementation** itself (the qffs/tc-5 lineage; supersedes the
  pre-beachhead `2026-07-02-pcore-08-readmodel-ownership.md` memo) — rag's build.
- **Channel A — project-opticon invoke-envelope** — a deferred-with-seam, NOT built now and
  NOT part of this contract. opticon is store-less and routes verbs to the authoritative
  plane; makima is one outward target, not an action bus. Do not couple rag's projection to
  Channel A.
- **A makima-side HTTP invoke surface / CQRS HTTP adapter** — pcore-10 (internal) /
  pcore-11 (opticon-facing), both downstream; not this contract.
- **The dual-write ban.** `workstate_store` is rag's projection, built and owned by rag's
  capability code; makima MUST NOT write into it. makima emits disk-truth and holds custody
  of its own `state/`; makima does NOT own the store or the opticon-facing query surface
  built over it (capability-vs-custody, `docs/decisions/DR-236-state-is-disk-truth-workstate-store-is-pro.md`).

---

## 6. Open design questions flagged for the makima EM
...
1. **`content_hash` is defined but not yet emitted.** `compute_stamp` is an internal cache
   ...
   each section porter + an additive `schema_version` bump is follow-on makima engine work
   ...
   is an open makima task.** Recommend surfacing to the PM as a small follow-on.
...
<!-- producer-contract: pcore-06 Channel B — coordinator→project-rag. Frozen post pcore-03 + pcore-09. Amended 2026-07-10 for DR-215 transport supersession (UDS retired, cc_invoke command-type is current). -->
