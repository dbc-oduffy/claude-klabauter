...
> when the step is `MIXED` — the forced mechanical/judgment split) so that project-makima's
...
> makima side**. DoE-claude *classifies and produces rows*; makima *reads rows and scaffolds*.
>
> **Who consumes this.** A context-less DoE-claude EM building the census procedure/tooling,
> and makima's own compute-layer scaffolder implementation. Everything needed to emit a
...
> `MIXED`, nor how a census file is assembled, stored, or delivered to makima. See § "Out of
...
> were delegated to makima (§ 2.3, § 2.4) and only they can say the result is what they meant.
> **§ 6 states the exact one-line confirmation that closes the gate.** Until it lands, treat every
> field here as negotiable on DoE-claude's say-so, not makima's — but note that the DRAFT
> exemption is makima-local and does not reach the artifact-shape-contract bundle, which vendors
...
>   project-makima's counter-proposal reply to
...
- **project-makima** — reads a conforming census; the compute-layer scaffolder consumes rows
...
- **A single `x_producer` object, open inside — adopted.** The row itself stays
  ...
  needs no makima change to add a field.
...
- **Census file delivery/storage** — the format the census is packaged in, its filename, its
  location, or how it reaches makima. This contract fixes the shape of one row; it does not
...
they described fields, makima built a schema, and only they can say the schema is what they meant.
...
**That DRAFT exemption is makima-local and does not reach the bundle.** Because DoE-claude's
