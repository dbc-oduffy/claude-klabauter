# coordinator → project-opticon fleet.* invoke-envelope producer contract (FROZEN)
...
> this is a contract with a sibling repo and re-cutting it unilaterally is not makima's call. Treat
...
> envelope that the coordinator control-plane engine (**makima**) exposes to
> **project-opticon**. It defines the op namespace, per-op request/response wire shapes,
> and the four resolved co-design seam questions so that opticon can build its `/fleet`
> connector, Next.js route, and WS2 UI against a stable wire contract.
> makima *acts*; opticon *triggers and displays*.
>
> **Who consumes this.** A context-less opticon EM building the `/fleet` connector +
...
>
> **What this is NOT.** This is the *producer contract* — not opticon's consumer
> side. It does not specify the opticon connector implementation, the Next.js route shape,
...
> (co-design complete 2026-07-04 — project-opticon EM accepted all four seams + wire shape;
...
> protocol; project-opticon is notified via cross-repo memo before any breaking change.
>
> **Changelog (post-freeze):**
> - **2026-07-25 (citation re-point):** Tri-plane boundary spec-backlink and the § 0
>   opticon-facing-query-surface citation re-pointed to
...
>   <!-- Review: patrik F0 — replaced efcab05 (memo-sweep, zero coordinator_core files) with fb0bdeb (C9 closeout, pinned by trigger memo); F3 — reworded change-class claim to distinguish axes honestly -->
...
>   safe here because no opticon connector has shipped (confirmed by the trigger memo) and
>   this amendment is co-ratified via that memo. Source: inbound memo
>   `cross-repo/inbox/2026-07-05-global-multiplex-superseded-frozen-opticon-transport.md`.
> - **2026-07-04 (FROZEN, v1.0):** co-design complete — opticon accepted all four seams + wire
>   shape; Delta 1 candidates[] fields pinned; §6 execution model resolved SYNCHRONOUS
>   (async job-handle shape reserved for future long-running ops); makima-internal execution
...
> - Held handoff (Channel A gate): `state/handoffs/2026-07-04_160343_pcore-06-channel-a-opticon-invoke-envelope.md`
> - Inbound co-design memo: `cross-repo/inbox/2026-07-04-2026-07-04-opticon-fleet-invoke-envelope-codesign.md`
> - Amendment trigger: `cross-repo/inbox/2026-07-05-global-multiplex-superseded-frozen-opticon-transport.md`
...
opticon triggers makima's `fleet.*` ops via the **Unix-domain socket** (UDS) + JSON-RPC 2.0
...
1. `dry_run:true` — compute and return the candidate set (the human-readable preview opticon
...
NOT a general-purpose query surface. Opticon's steady-state `/fleet` state display comes from
**project-rag's typed query surface**, not from polling makima's `dry_run`. makima does NOT
own the opticon-facing query surface (capability-vs-custody boundary;
...
**Execution model for `dry_run:false` — RATIFIED SYNCHRONOUS (DR-211).** The makima-side
...
The `fleet.*` ops use the **identical transport as Channel B** (coordinator → project-rag).
This section is the authoritative Channel-A pin; the Channel-B frozen contract
(`coordinator_core/contract/project-rag-producer-contract.md § 1.1`) carries the same
...
- **Socket path (POSIX):**
  ...
  **Negative-spec (source: `coordinator_core/ipc.py:801-802`, `socket_path()` docstring):**
  ...
  - Do NOT return `sentinel_dir(repo_root)/'sock'` — path length hazard (macOS;
    ...
    the correct address.
    <!-- Review: patrik F6 — restored source reason "path length hazard (macOS)" from ipc.py:802 docstring; prior text silently rewrote the justification -->
...
- **Windows:** named-pipe/localhost stub is `TODO(D3/pcore-01)` — not part of this frozen
  contract. opticon should target the POSIX UDS surface for slice 1.
...
<!-- Review: patrik F2/F5 — corrected message to real prefixed form (ipc.py:274/496); F5 noted inconsistent fidelity vs exact _auth_token quotes; engine pre-auth guard now fires -32602 BEFORE auth for fleet.* missing this field -->
...
- opticon MUST **re-read the token file on every connect**.
- opticon MUST NOT cache a token across a possible service restart.
- On a `-32600 "invalid token"` response, re-read the token file and retry once (the service
  ...
  <!-- Review: patrik F4 — mid-session rotation recovery step; stale token after service restart is indistinguishable from bad token without the retry; reconnect + re-read is the correct recovery shape -->
...
<!-- Review: patrik F2 — full distinguishable taxonomy added; engine-state change since review landed pre-auth routing-key guard so all codes below are now emitted in the documented order; F5 — "branch on code" note added -->
...
fleet.* ops are **MUTATING** (git-mv archival). opticon **triggers**; it does not observe
the op surface for read-only queries (use rag's query surface for that).

| op | classification | opticon calls? |
|----|----------------|----------------|
...
| any other makima op | read-only or MUTATING | **no** — opticon calls only fleet.* |
...
`provenance.path` convention so opticon can correlate fleet op results against rag's
projection. `from_path` is redundant with `id` and is NOT in the contract. `to_path`
(the archive/ destination) is makima-internal layout and MUST NOT appear in the contract —
...
**Decision: accept opticon's proposal.** The `dry_run:false` path re-verifies each item's
...
- **Idempotent replay.** Re-issuing the same `candidate_ids` after a partial failure
  ...
  via T3 re-verify as a no-op skip. opticon's "retry failed" UX is a safe re-call, not a
  special path.
...
- `failed: [{id, reason}]` = per-item failures (lock, dirty tree, permission, git-mv
  conflict) — for opticon UX to render "7 archived, 3 failed: \<reasons\>".
...
| `2` | **DETERMINATE-PARTIAL**: `failed[]` is non-empty. The batch outcome IS known — exactly which items succeeded and which failed are enumerated. opticon branches on `failed[].length`, not on `exit_code`. |
...
`goal.append`) are the ops that may omit `_origin_worktree` — opticon never calls those.
...
<!-- Review: patrik F1 — dropped stale "ungated" adjective (two occurrences); "ungated" contradicts §5.1 + the amendment's own changelog; the admission-precondition argument (idempotent+commutative+git-reversible, orthogonal to auth) is unchanged -->
...
safe. opticon's retry/concurrency UX should be coded against the `failed[]` surface, not
...
version-skew-safe: a pre-B makima daemon receiving `mode:"adjudicate-in-flight"` (from an
opticon already updated to slice B) yields a clean `exit_code:1` setup-error. It does NOT
...
enum-extension safe across opticon/makima version skew. Slice B can ship as a purely
...
- Token tier for `fleet.*` (MUTATING): **READ_WRITE** — **enforced on UDS** (not merely
  ...
  fail-closed"`.
  <!-- Review: patrik F2 — READ_WRITE is now ENFORCED on UDS (scope-tier gates 4/5 landed post-review); -32601 denials documented here to match the error taxonomy in §1.1 -->
...
- fleet.* ops over HTTP require a **READ_WRITE** token (two-tier model:
  `coordinator_core/authz/token.py`; tokens rotate on every service start; opticon MUST
...
single-round-trip; the makima-side execution model (handler-mutates synchronously, scoped-pathspec
...
(opticon presents the dry_run preview for explicit approval before act). A synchronous round-trip
...
a contract break. Old opticon clients receiving an unexpected `job_id` field fail loud (unknown
shape), not silently wrong — the same fail-closed discipline as the mode enum unknown-value guard.

### Makima-internal implementation obligations (DO NOT reshape the wire)

The following items are makima-internal implementation requirements tracked separately as a
reconciliation spinoff. They do **NOT** appear in or reshape the frozen wire contract. opticon
...
makima's single event loop. The round-trip is synchronous from the wire perspective; the
implementation must be non-blocking from the event-loop perspective.

These are makima-internal implementation requirements, now RATIFIED per DR-211
...
The underlying sweeps are bash in `~/.claude` (a soft makima prereq):
...
<!-- Review: code-reviewer — added parenthetical on cs_sweep_actioned_memos clarifying no slice-1 fleet.* op maps to it; prevents opticon EM reading §7 as implying a dropped/missing op --> The op registry is Python in `coordinator_core`.
...
implementation slice. This is called out so opticon and the co-design reviewer see it is
...
- **Consumer side** (opticon `/fleet` connector, Next.js route, WS2 UI dashboard) —
  project-opticon's own design surface.
...
- **Cloud / HTTP invoke of fleet.*** — later slice, gated on pcore-05 + the single-writer
  queue (DR-208).
- **pcore-10 / pcore-11** (internal HTTP invoke surface / opticon-facing envelope impl) —
...
- **Channel-B §1.1 socket-algorithm amendment** — the global-multiplex socket algorithm is
  ACTIONED for Channel A in this contract (§1.1 above). Channel B (project-rag producer
  contract, `coordinator_core/contract/project-rag-producer-contract.md`) still carries its
...
- **opticon's `/fleet` state display** — comes from project-rag's typed query surface;
  `dry_run:true` is NOT a polling mechanism for that display.

---

<!-- producer-contract: pcore-06 Channel A — coordinator→project-opticon fleet.* invoke-envelope. FROZEN (co-design complete 2026-07-04; amended 2026-07-05 global-multiplex reader-widen v1.1). -->
