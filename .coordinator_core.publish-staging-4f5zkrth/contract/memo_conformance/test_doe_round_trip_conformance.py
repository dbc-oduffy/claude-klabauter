...
round-trip conformance gate for makima's native memo verbs.

Purpose: run the round-trip conformance fixture
(``coordinator/bin/test_cross_repo_memo_roundtrip.py``) -- now MAKIMA-OWNED, resolved
from makima's own tree via ``_makima_root()`` -- and assert makima's real ``memo.send``
...
fixture out of DoE-claude's ``coordinator/bin/`` into makima's own ``coordinator/bin/``
(``/Users/oduffy/X/DoE-claude/coordinator/bin/cross-repo-memo-roundtrip.test.py`` no
...
fixture is now resolved from makima's own tree and a missing fixture FAILS (not skips)
...
side), not makima's op; makima's equivalent collision semantics are already covered
locally by ``coordinator_core/ops/fleet/tests/test_memo_send.py``, and the round-trip
fixture's own Collision 1/1b/2 + AC9 cases already dispatch makima's REAL memo.send op
...
cannot see, both of which need a subprocess to observe:
  ...
  1. ``MAKIMA_ROOT`` pinning. The fixture's real-op sites (1, 2, 4, 5 + AC9) call its own
     ``skip_test()`` and ``return`` when the ambient resolver ladder cannot find a makima
     ...
     side. This wrapper pins ``MAKIMA_ROOT`` to the enclosing checkout (honoured first by
     ``cc_invoke._resolve_makima_root``) so the real-op sites cannot degrade, and asserts
...
Spec backlink: pln-memo-tool-rebuild-makima-owns--bd5745 § C8 (A7/A8)
Spec backlink: docs/decisions/DR-210-makima-native-tooling-ownership-strangler.md § 2(a)/2(b)
Oracle: coordinator/bin/test_cross_repo_memo_roundtrip.py (makima-owned as of 2026-07-21;
resolved from makima's own tree, no vendoring, no foreign-clone dependency)
"""
...
# ---------------------------------------------------------------------------
# Known-bad tracking -- makima-owned fixture defects, tracked pending a fixture-side
...
# than patched here (makima did not own DoE-claude/coordinator/bin/ at the time). DoE
...
# `_bin_dir()` (dirname(__file__)), which resolved inside makima's checkout after the
...
#
# To track a NEW makima-owned fixture defect: add its fixture test NAME (prefix-matchable)
# here, never swallow-all -- so a genuinely new makima regression is never silently
...
def test_doe_round_trip_conformance_fixture() -> None:
    """makima's memo.send op clears the 5-site + collision + AC9 round-trip fixture.

    The fixture is makima-owned (relocated 2026-07-21 from DoE-claude's
    coordinator/bin/ into makima's own coordinator/bin/) -- a missing fixture here is a
    ...
    fixture = makima_root / "coordinator" / "bin" / "test_cross_repo_memo_roundtrip.py"
    if not fixture.is_file():
        pytest.fail(
            f"round-trip conformance fixture not found at {fixture} -- expected in "
            "makima's own tree since the 2026-07-21 executable-surface migration "
            "(it is no longer hosted by DoE-claude)"
        )
        return

    env = {**os.environ, "MAKIMA_ROOT": str(makima_root)}
    ...
    assert not unexpected, (
        ...
        f"known-bad set -- makima contract regression suspected: {sorted(unexpected)}\n"
    ...
    assert not skipped, (
        "round-trip conformance fixture LOUD-SKIPPED real-op site(s) despite MAKIMA_ROOT "
        f"being pinned to {makima_root} -- the memo.send op was never dispatched, so the "
    ...
    assert result.returncode == 0 or tolerated, (
        ...
        "known-bad failure that would explain it -- makima contract regression, inner "
