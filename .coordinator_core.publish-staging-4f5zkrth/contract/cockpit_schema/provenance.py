...
Provenance is a real schema, not a label (Camelia P1-D2): `observed_at` and
...
class ProvenanceEnvelope(BaseModel):
    ...
    def _check_anchorless_guard(self) -> "ProvenanceEnvelope":
        # Identity-space-freeze anchorless guard (Camelia F6, tightened): a fact
...
class ContentHash(BaseModel):
    ...
    by makima on each work-state record so project-rag can invalidate its
    projection cache on change (keyed on (source_path, hex)). NOT mtime, NOT
    git-rev. Frozen shape: makima coordinator_core/contract/change-signal.schema.json.
    Spec: project-rag-producer-contract.md § 3.2 (hash convention), § 3.3 (wire shape).
    """
