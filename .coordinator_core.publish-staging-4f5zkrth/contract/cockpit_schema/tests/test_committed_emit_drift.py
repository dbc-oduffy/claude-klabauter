...
pending follow-up. `CommitClosure` is makima-net-new with no DoE `index.ts` counterpart;
...
def test_every_committed_schema_file_matches_fresh_emit(tmp_path, schema_dir):
    ...
    assert drifted == [], (
        "committed DoE cockpit-contract schema/*.json is out of sync with the makima "
