...
@skip_no_schema
def test_roadmap_dag_edge_schema_version_bumped_to_315():
    schema = load_schema("roadmap-dag-edge")
    assert schema.get("version") == "3.15.0", (
        "roadmap-dag-edge.schema.json version was not bumped to 3.15.0 alongside "
        f"the type widen — got {schema.get('version')!r}"
    )
