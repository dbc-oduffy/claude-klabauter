...
locks the NARROWED enums in makima's own vendored frontmatter schema pair
...
Unlike ``test_verify_superseded_retirement.py`` this is NOT DoE-clone-gated
(``@skip_no_doe``) — the schema pair and the handoff corpus it scans are both
local to this repo, not read from the DoE clone.
...
# Version FLOOR, not an equality pin. makima re-vendors this schema pair from
# DoE rather than authoring it, so an equality pin goes red on every upstream
...
def test_schema_pair_versions_at_or_past_the_c7_floor():
    ...
    and will keep drifting — makima re-vendors this pair from DoE rather than
