...
This package does not host the emit engine (DR-047: makima's `artifact.emit`
...
def test_non_meta_repo_week_goal_not_cross_repo_aggregate_sentinel():
    artifact = {
        **_base_authoring_artifact(),
        "repo": "dbc-oduffy/project-opticon",
        "period": "week",
        "period_value": "2026-W29",
    }
    wire = _project_goal_artifact_to_wire(artifact)
...
def test_emitted_machine_goal_same_repo_different_machine_is_distinct_declaration():
    ...
    from_striker = {
        **_base_authoring_artifact(),
        "goal_id": "2026-W29-shared-repo-goal-striker",
        "repo": "dbc-oduffy/project-rag",
        "declared_by_machine": "striker",
    }

    wire_b = _project_goal_artifact_to_wire(from_machine_b)
    wire_striker = _project_goal_artifact_to_wire(from_striker)

    assert zod_safe_parse_ok(Goal, wire_b)
    assert zod_safe_parse_ok(Goal, wire_striker)
    assert wire_b["repo"] == wire_striker["repo"]
    assert wire_b["declared_by_machine"] != wire_striker["declared_by_machine"]
