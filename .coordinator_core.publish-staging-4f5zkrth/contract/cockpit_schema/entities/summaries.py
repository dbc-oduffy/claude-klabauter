...
external handoff carrying `status: superseded`. The cockpit emitter (makima's
...
class ForeignOriginTriple(BaseModel):
    ...
    so opticon can render without a stub-less dangle back to a record it
    doesn't ingest.

    Spec backlink: DoE-claude:pln-structured-originating-session-8b505c § C7;
    shape mirrors opticon's hand-authored swap seam (project-opticon
...
class HandoffSummary(BaseModel):
    ...
    """Resolved commit sha + date when the workstream shipped. Opticon wants the date for timeline rendering. D9: nullable, never optional."""
    ...
    # contract) so a bare id suffices for opticon to resolve. origin_goal_id
    ...
    `{id, kind, label}` triples so opticon can render without a back-lookup.
    D9: nullable, never optional.
    """
...
class BacklogItemSummary(BaseModel):
    ...
    from_repo: str = Field(
        description=(
            ...
            "'project-opticon-em'). This is NOT a repository identity. "
            "Free-text and unvalidated; known to drift (e.g. '.claude', "
            "bare 'project-makima', underscore variants). Do NOT use it as "
