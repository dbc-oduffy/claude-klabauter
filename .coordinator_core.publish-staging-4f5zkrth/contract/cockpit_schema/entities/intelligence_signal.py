"""
IntelligenceSignal — market-intelligence signal fact (Level-2 aggregate).
Pydantic port of DoE `coordinator/cockpit-contract/src/entities/intelligence-signal.ts`
(Zod source).

Second of two v2.16.0 market-intelligence emission entities. Per-repo (D25
...
measurement_basis) reconciling market-intelligence's producer join/coverage
ask with opticon's descriptor ask into one coherent union (D29).

v2.19.0 widen: types the `confidence_interval` field deferred at D29/v2.18.0
now that its wire shape is confirmed by market-intelligence + opticon (D30).
...
Spec backlink: cross-repo/inbox/2026-07-14-project-opticon-em-market-intel-cockpit-contract-widen.md
Spec backlink: cross-repo/inbox/2026-07-14-project-makima-em-intelligence-signal-widen-conflict.md
Spec backlink: cross-repo/inbox/2026-07-14-market-intelligence-em-doe-intelligence-signal-confidence-interval-type-confirmed.md
...
class IntelligenceSignal(BaseModel):
    ...
    content_key: str | None = Field(
        description=(
            ...
            "projection-render join anchor (project-rag fleet identity). "
            "Nullable on the shared contract; market-intelligence enforces "
    ...
    claim_id: str | None = Field(
        description=(
            "Informational: project-rag producer-local surrogate id. NOT a "
    ...
    confidence_interval: tuple[float, float] | None = Field(
        description=(
            ...
            "market-intelligence + opticon 2026-07-14 (see D30)."
        )
    )
