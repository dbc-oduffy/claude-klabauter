...
class CrossRepoMemoSummary(BaseModel):
    ...
    frontmatter). A memo authored by `.claude-prime` to `geneva-mvp` read
    from the `.claude-prime` tree has `repo: "dbc-oduffy/.claude-prime"`,
    `from: ".claude-prime"`, `to: "geneva-mvp"`.
    """
