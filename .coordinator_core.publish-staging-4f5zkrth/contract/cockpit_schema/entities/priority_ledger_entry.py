...
what the PM (or opticon, via the intent-drain seam) actually SET, one row per
...
priority-ledger-for-opticon.yaml` commits DoE to publishing the ledger shape
to opticon, so `ENTITY_SCHEMAS` membership (and the DoE-side JSON-schema
...
schema for external reference/validation without makima owning a populated
array in the emitted snapshot yet. `CommitClosure` is the opposite shape
(SECMAP + collector, no `ENTITY_SCHEMAS`) precisely because makima itself is
