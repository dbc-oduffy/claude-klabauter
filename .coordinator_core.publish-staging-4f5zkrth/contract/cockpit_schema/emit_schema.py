...
# DoE's committed schema/*.json — as EM/PM/Patrik-adjudicated, NOT a
...
# and rides the already-landed owner-casing clause amendment (makima
# 4a73368b, DoE-claude 39a65721), which shipped without its own bump so it
# would ride this one. MINOR, not MAJOR, is deliberate: project-opticon's
...
# a warning), so a minor bump lets opticon keep ingesting without a
...
# DECISIONS.md) — opticon confirmed this class in their 2026-07-28 reply.
...
# of any consumer reply; opticon's 2026-07-28 reply corroborates it as
...
# with no degraded state — the defect project-opticon-em reported 2026-08-11.
#
...
# census (opticon and rag both pass), and specified the D39 source-first release
# sequence: makima widens and regenerates into DoE's schema/ out-dir, DoE
# commits the bundle and advances the release tag, makima re-vendors, and only
...
#
# D39 source-first release sequence, unchanged from 3.11.0: makima widens and
# regenerates into DoE's schema/ out-dir, DoE commits the bundle and advances
# the release tag, makima re-vendors, and only then does the emitter's output
...
# (opticon and rag) and claude-central-em's ratification of this MINOR
...
# 2026-08-20-project-makima-em-cockpit-schema-additive-minor-bump-for-the-human-axis.md
...
# and the C9 chunk body for the full "why a switch" rationale: opticon's
...
# (project-opticon) previously had no way to compute — see
# cross-repo/inbox/2026-08-21-project-opticon-em-vendored-contract-lacks-x-baton-class-mapping.md.
# makima's own vendored copy under `ops/emit/_vendor/cockpit-contract/` is
...
def _schema_out_dir(out_dir: str | os.PathLike[str] | None = None) -> Path:
    ...
    # project-makima repo, silently writing every *.schema.json + bundle
    ...
    # project-makima repo root (coordinator_core/contract/cockpit_schema/), so
