...
Five invariants under test:
  ...
  (c) Regression at makima's own auto-load dir: match_schema with a memo
...
_MAKIMA_SCHEMAS_DIR = Path(__file__).resolve().parent.parent / "frontmatter" / "schemas"
...
class TestEmitSchemas:
    ...
    def test_refuses_to_write_into_auto_load_dir(self) -> None:
        ...
        with pytest.raises(RuntimeError, match="auto-load"):
            emit_schemas(out_dir=_MAKIMA_SCHEMAS_DIR)
    ...
    def test_generated_files_not_present_in_auto_load_dir(self) -> None:
        ...
        names = {p.name for p in _MAKIMA_SCHEMAS_DIR.glob("*.schema.json")}
...
class TestMatchSchemaRegressionBothPaths:
    """(c) match_schema(path, {"kind": "proposal"}, load_schemas(makima_dir))
...
class TestByKindNeverArmedAtRealDeliveryShape:
    ...
    TestMatchSchemaRegressionBothPaths class only loads _MAKIMA_SCHEMAS_DIR,
...
class TestToRepoFieldPresentAndOptional:
    ...
    extension on both schemas that makima's generation permanently
