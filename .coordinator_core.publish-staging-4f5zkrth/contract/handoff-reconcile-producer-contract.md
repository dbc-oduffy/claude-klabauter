...
> **What this is.** The producer-side wire contract for makima's `handoff.reconcile_open` op —
...
>
> **Boundary authority.** `/Users/oduffy/X/DoE-claude/docs/decisions/DR-047-doe-makima-boundary-redraw-contract-vs-e.md`
> is the operative DoE↔makima boundary authority underlying this op's design: **makima owns the
...
> makima's grammar pin, `coordinator_core/contract/auto-reconcile-policy.grammar.md`). The
...
> grammar**, not a makima-hardcoded named constant — a DoE-side threshold edit (e.g. widening the
> mechanical-commit denylist) changes matcher behavior with zero makima code change.
>
> **Spec backlinks.**
> - Plan (source of truth): `docs/plans/2026-07-13-makima-auto-reconcile-open-handoffs.md` § C4/C5
...
`dry_run` **defaults to `true`** — this is the resolved outcome of Patrik review finding index 4:
...
**Cross-handoff attribution guard** (Patrik review, finding index 2): even when all three signals
...
matcher behavior with zero makima code change.
...
- **Abandoned-blocker → surface.** A `blocked_by` member that is `abandoned` routes to `surface`
  ...
  surface) is a makima extension flagged to DoE as a proposed spec addition** — the canonical
...
- **NARROW+SURFACE composite** (Patrik review, finding index 1 — major): a `narrow` verdict whose
...
makima verbs, and the partial `/pickup` Step 3d retirement described in § 6. See the plan's C6 task
...
<!-- producer-contract: makima handoff.reconcile_open op — turnable-on, op name RATIFIED (DoE 2026-07-13),
     no live caller yet. Spec backlink: pln-makima-auto-reconcile-pass-off-425848 § C5. -->
