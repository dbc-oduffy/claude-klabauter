...
> coordinator control-plane engine (**makima**) derives and returns via the `deliverable.rollup` op.
...
> schema — without further co-design on the makima side.
> makima *derives and resolves*; DoE *composes the prose* and owns the completion entry.
>
...
> does not cover rag ingest or opticon display of the rollup payload — those are out of scope for
> slice-1 (see § "Out of scope"). See § "Out of scope" for the full exclusion list.
>
> **Status: FROZEN — v1.0, frozen 2026-07-06.** Freeze gate cleared on the makima side: the
...
>   `coordinator_render_rollup.py` is **makima-resident**, and it is **count-agnostic** over
...
>   **Superseded same-day**: the "makima notifies DoE, DoE widens, DoE acks" framing this entry
...
>   confirming readiness, and makima flipped the writer to emit the field on every response
...
> - **2026-07-06 (op-gap amend, post-freeze v1.0):** initiative-entity resolution root relocated from the scanned-worktree `state/initiatives/` to the makima central-state root (worktree-local fail-open fallback); section-3 negative-spec amended for the env-miss-only non-git machine-local registry subprocess. Schema/wire shape UNCHANGED — op-gap, no version bump, no re-vendor required.
...
>
> **Spec backlinks.**
> - Plan (source of truth): `docs/plans/2026-07-06-makima-deliverable-spine-factsupply-op.md`
> - C0 findings: `docs/plans/2026-07-06-makima-deliverable-spine-factsupply-op.c0-findings.md`
...
makima's `deliverable.rollup` op (`COMPUTE_ONLY` — DR-208 five-question classification in
...
- **makima** — derives initiative forward-edges from its deliverable-spine read-model; owns this
...
- **project-rag / project-opticon** — no consumer role for this op in slice-1. The rollup payload
...
edge; makima includes only what it can resolve to a real `state/initiatives/<id>.yaml` file.
...
- `state/sizings/*.yaml` — **LANDED 2026-08-13.** Whole-document YAML sizing objects (no
  ...
  established the reader is count-agnostic and makima-resident (see Changelog).
...
> the single `makima-strangler` initiative); transitive resolution is **0**. The op returns an
...
makima's `deliverable.rollup` op resolves only what it can pin unambiguously:
...
resolved from the **makima central-state root** (`coordinator_state_root --central`, confirmed
makima-resident under DR-209 tri-plane state-placement law), with a **worktree-local fail-open
fallback** when the makima root is unresolvable (`MAKIMA_ROOT` env-miss AND `machine-local get
repos.project_makima` returns nothing). For makima's own worktree the central directory and the
worktree-local directory coincide (`coordinator_makima_root` IS makima's repo root), so
makima-own rollups resolve correctly via either branch.
...
carve-out (env-miss only, memoized once per process):** on `MAKIMA_ROOT` env-miss, the op may
invoke `machine-local get repos.project_makima` (a non-git, read-only registry lookup) at most
once per process lifetime to resolve the makima central-state root; the result is memoized at
...
1. makima notifies DoE via cross-repo memo, naming the added/changed field and the effective date.
...
3. makima begins emitting the new field/value only after DoE confirms.
...
populated). This is a `resolution_mode` enum widen — DoE widens its render before makima emits
...
- **`cc_invoke` shim / DoE-side fail-open wiring** — the call site, error handling, and
  TWO-SIGNAL rc interpretation live in DoE's code. makima authors the contract and the op;
...
- **rag ingest / opticon display of the rollup payload** — not requested for this op. The fields
  go to DoE's entry prose. If a durable-store or display need surfaces later, that is a separate
  contract and a separate rag/opticon coordination surface.
- **A JSON-Schema sibling of this contract** — natural follow-on after freeze and DoE's vendored
  pin confirmation; not this plan.

---

<!-- producer-contract: makima deliverable.rollup op — coordinator→DoE structured fields.
