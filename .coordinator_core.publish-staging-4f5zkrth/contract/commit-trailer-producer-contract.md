...
> coordinator control-plane engine (**makima**) derives and emits via the `commit.anchors` op.
> It defines the trailer key set, per-key source, grain, cardinality, and durable-join semantics,
> so that **project-rag** can build its typed ingest and **project-opticon** can build its
> intent→reality display against a stable schema — without further co-design on the makima side.
> makima *derives and stamps*; rag *ingests and serves*; opticon *displays over rag's surface*.
>
> **Who consumes this.** A context-less project-rag EM building the commit-edge ingest path, and
> a context-less project-opticon EM building the intent→reality display. Everything needed to
...
> opticon's display UI. It does not specify rag's temporal model, watermark driver, or
> `workstate-query` surface shape. It does not specify opticon's filter UI or query-mapping
> entries. See § "Out of scope — not our surface".
>
> **Status: FROZEN — co-design closed 2026-07-05, v1.0.** All three freeze-gate items are
> resolved (see § 0, Freeze-gate items): item 1 (rag↔opticon `workstate-query` v8 alignment)
> closed bilaterally by rag + opticon; item 2 (`source_kind: git_commit` enum) notify-at-freeze
> → DoE/central; item 3 (makima `commit.anchors` op + invocation shim) shipped. Sibling EMs
...
>   here, re-argued against review feedback (Zolí, F3), reasoning kept rather than silently edited.
...
>
>   **§ 5.2 step 3 gate is split, not blanket-deferred.** Step 3 ("makima begins emitting only
>   after both consumers confirm") gates *rag's and opticon's own decision to consume* the new
>   surface — theirs to make, not makima's landing gate. It does NOT gate makima's own internal
...
> - **2026-08-10 (producer-count correction):** § 1.2's "makima has TWO independent producers of
...
>   **zero** producers fleet-wide (0 commits in project-makima's entire history carried it)
...
>   is "a context-less project-rag EM" — precisely who would take "stores" as a residency licence
>   (PM-ratified at delphi-store 2026-07-25: project-rag owns capability code, not custody of data).
...
>   Source: `cross-repo/inbox/2026-07-25-project-opticon-em-commit-trailer-contract-residency-phrasing.md`.
...
>   makima subsystem (the emit-time `commit_closures` section porter) now reads. Documented here
>   so this table stays the single registry of recognized commit trailers (Patrik F9). Source:
...
>   Item 1 (rag↔opticon `workstate-query` v8 alignment) closed bilaterally (rag `d5370eeab`
>   trailer-edge-query-spec + `4bff8ce43` trailer-edge-key-pin; opticon confirmed; DR §7 resolved).
>   Item 2 (`source_kind: git_commit`) notify-at-freeze → DoE/central (ships with rag surface (b)).
>   Item 3 (makima `commit.anchors` op + shim) shipped (`043b214` C1–C4, `9f3ad55` shim,
>   `da13148` workstream-complete). Trigger memo:
>   `cross-repo/inbox/2026-07-05-opticon-rag-alignment-closed-freeze-gate-a.md`.
...
>   walk retired in favour of direct minted-id edges; opticon `Nature:` filter facet accepted;
>   vendored-pin confirmed by both consumers.
>
> **Spec backlinks.**
> - Plan (source of truth): `docs/plans/2026-07-04-makima-commit-anchor-stamper.md`
...
> - DoE ratification memo: `cross-repo/inbox/2026-07-04-makima-stamper-ratify-and-wedge-closed.md`
...
makima's `commit.anchors` op (`COMPUTE_ONLY` — see plan D2, AC2) derives a structured git trailer
block from the live operational read-model and returns it as text. The existing
`coordinator-prepare-commit-msg` hook (DoE surface) stamps that block onto every coordinator-session
commit, extending the already-in-use `Session-Id:` trailer injection.

The result is a set of **six trailer keys** per commit — five derived by makima, one already-in-place
...
`Closes: <ID>` themselves) that makima's *emit-time* `commit_closures` section porter reads via
a git-native trailer scan (see `project-rag-producer-contract.md § 2.5`). It is included here so
this table remains the single source of truth for every commit trailer makima's tooling
...
- **makima** — derives anchors from its live read-model; stamps trailers; owns this contract.
- **project-rag** — ingests the trailer stream from git history into its typed store; owns the query surface.
- **project-opticon** — displays the intent→reality graph over **rag's** `workstate-query` surface,
  not directly from makima.

**Producer precision guarantee.** A *present* trailer key is a high-confidence edge; makima stamps only
what it can resolve unambiguously. An *absent* key is a safe null, never a guess. A wrong stamped anchor
is worse than none — opticon renders it with confidence as a false edge.

### Freeze-gate items (three items — all resolved at freeze 2026-07-05)

| # | Item | Owner | Status |
|---|------|-------|--------|
| 1 | **rag ↔ opticon `workstate-query` v8 alignment** — rag exposes trailer edges keyed on logical-repo identity so opticon adds one query-mapping entry, not a second transport. | rag + opticon bilateral | **RESOLVED** — closed bilaterally (rag `d5370eeab` + `4bff8ce43`; opticon confirmed; DR §7 resolved 2026-07-05) |
| 2 | **`source_kind: git_commit`** provenance enum extension — rag needs a new `source_kind` value for the commit stream; the enum is coordinator-owned and routes via DoE/central on freeze notification. | coordinator (DoE surface, notified at freeze) | **notify-at-freeze** — DoE/central notified at freeze; ships with rag surface (b), not a pre-freeze blocker |
| 3 | **makima ships `commit.anchors` op + invocation shim + finalizes this contract** (plan AC1/AC2/AC7) → DoE wires the `prepare-commit-msg` hook (gated on this delivery). | makima (op + shim) → DoE (hook wiring) | **SHIPPED** — `043b214` C1–C4, `9f3ad55` shim, `da13148` workstream-complete |
...
|-------------|--------|-------|-------------|-------------------|---------------|
| `Plan:` | plan file present in commit diff; OR active plan in read-model (D2 DD#1 freshness-gated) | display / human-legible | 0..1 | **NO** — rename-fragile path | `docs/plans/2026-07-04-makima-commit-anchor-stamper.md` |
| `Plan-Id:` | `plan_id` frontmatter field of the same plan source | plan-grain | 0..1 | **YES** — durable `pln-…` minted id | `pln-makima-commit-anchor-stamper-q-29b891` |
| `Deliverable-Id:` | `deliverable_id` frontmatter field (DR-207 deliverable-spine key) | deliverable-grain | 0..1 | **YES** — durable `dlv-…` minted id; cross-entity join | `dlv-makima-commit-anchor-stamper-queryable-g-2064ae` |
...
Both come from the same frontmatter makima already reads; emitting `Plan-Id:` is free. `Plan:` is
...
**The key is `Deliverable-Id:`, never bare `Deliverable:` (ruled 2026-07-28).** makima has THREE
...
DoE's hook), and `coordinator_core/git/commit_trailers.py`'s `compute_missing_trailer_args`,
...
(makima's `coverage.py` DAG attribution and `execute_plan_assemble/close_out_and_stamp.py`,
project-rag's `workstate_store` ingest), it is 84-commits-to-4 in live history, and it matches the
...
nowhere. The four `Deliverable:`-stamped commits in makima history predate the ruling; they are
...
- **Reader:** makima's *emit-time* `commit_closures` section porter (not the commit-time
...
- **Emitted shape:** `coordinator_core/contract/project-rag-producer-contract.md § 2.5`
...
renamed or removed without a breaking-change bump. opticon consumes `Nature:` as a first-class filter
...
This distinction is not academic. project-rag-em circulated
...
commits it was meant to catch (`cross-repo/inbox/2026-08-20-project-rag-em-wave-commit-
...
the PROPOSED trailer producer contract for the makima commit-anchor stamper.
Mirrors the opticon/rag contract shape. Freezes on co-design close.

Nature: docs
Plan: docs/plans/2026-07-04-makima-commit-anchor-stamper.md
Plan-Id: pln-makima-commit-anchor-stamper-q-29b891
Deliverable-Id: dlv-makima-commit-anchor-stamper-queryable-g-2064ae
...
2026-08-08, `cross-repo/inbox/2026-08-08-project-rag-em-commit-token-trailer-no-closed-key-set.md`
and `...-project-opticon-em-commit-token-trailer-no-key-enumeration-here.md`):
...
- **opticon** — never parses a message at all; it consumes rag's typed projection, so an emptied
...
makima's `commit.anchors` op resolves only what it can pin unambiguously from the live read-model:
...
This guarantee is **asymmetric by design**: the precision cost of a false edge (opticon renders a
...
### 4.1 project-rag (ingest)

rag ingests the trailer set from git history into its typed store. Makima's producer side is agnostic
...
- The `(source_path, content_hash)` cache-invalidation key for rag's projection cache — specified
  in `coordinator_core/contract/project-rag-producer-contract.md § 3` (the R5 change-signal).
...
`artifact-shape-contract/`); DoE/central will be notified at contract freeze so the enum can be
extended additively. This extension follows reader-widen-before-writer-flips — rag widens its
validator to accept `git_commit` before makima emits it.
...
**rag ↔ opticon query-surface alignment (freeze-gate item 1).** opticon requires trailer edges
exposed via rag's `workstate-query` v8 envelope keyed on logical-repo identity. Rag's trailer-edge
query must land joinable in that envelope. This is a rag↔opticon bilateral surface; makima carries
the freeze flag but does not own or constrain the surface shape.

### 4.2 project-opticon (display)

opticon displays the intent→reality graph over **project-rag's `workstate-query` surface** — NOT
directly from makima or from raw git history. The tri-plane boundary is load-bearing here:
makima emits authoritative disk-truth; rag is the durable system-of-record and query surface.
opticon queries rag, not makima.

**`Nature:` as a first-class filter facet.** opticon consumes `Nature:` as an additive-only filter
dimension. Treat the enum vocabulary (§ 1.4) as stable and versioned — new values arrive via the
reader-widen-before-writer-flips protocol, never as silent renames. opticon widens its filter set
before makima emits a new value.
...
**rag and opticon MUST vendor-pin a snapshot of this contract.** They MUST NOT reference
...
- **opticon:** vendor under `contract/upstream/commit-trailer-producer-contract.md`.

On any contract bump (post-freeze), rag and opticon each receive a cross-repo memo identifying
...
1. makima notifies rag and opticon via cross-repo memo, naming the added/changed key and the
   effective date.
...
3. makima begins emitting the new key/value only after both consumers confirm.
...
### 6.4 `Nature:` as display/filter facet (opticon — ACCEPTED)

**Resolution:** opticon accepted `Nature:` as a first-class filter facet. The enum is additive-only;
reader-widen-before-writer-flips governs additions. opticon does NOT treat `Nature:` as a join key.

### 6.5 Vendored-pin (both consumers — CONFIRMED)

**Resolution:** both rag (confirmed in co-design reply) and opticon (confirmed in reply) acknowledged
...
- **rag's ingest implementation** — temporal model, watermark driver, `git log` walk mechanics,
  `(source_path, content_hash)` cache-invalidation implementation. project-rag's own design surface.
- **rag's `workstate-query` HTTP projection shapes** — the v8 surface shape, query endpoint design,
  SSE/pull mechanics. project-rag's output, not makima's.
- **opticon's display UI and query-mapping entries** — filter facets, intent→reality graph rendering,
  the single query-mapping entry opticon adds to join rag's workstate-query surface. project-opticon's
  own design surface.
- **The DoE append-hook wiring** — editing `coordinator-prepare-commit-msg` to call `commit.anchors`
  and inject the returned block. DoE surface (`coordinator-prepare-commit-msg` lives in
  `DoE-claude/coordinator/bin/`); makima proposes via memo + PM-relay per
  `CLAUDE.md § Cross-repo writes`. The hook wiring is the DoE consultation deliverable (AC6 ratified).
...
- **The makima invocation shim** (`bin/makima-commit-anchors.py`) — the fail-open shim the DoE hook
...
<!-- producer-contract: makima commit-anchor stamper — coordinator→git-history trailer schema.
