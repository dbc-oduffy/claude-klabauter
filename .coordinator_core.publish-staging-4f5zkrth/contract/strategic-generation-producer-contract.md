...
> control-plane engine (**makima**) derives and writes via the `strategic.generate` op. It defines
...
> makima *derives and writes the draft*; the human ceremony (DoE's refresh skill) *reconciles and
...
> green, and AC7 (dogfood — makima's own draft generated end-to-end) passes.
>
> **Changelog:**
...
>
> **Spec backlinks.**
> - Plan (source of truth): `docs/plans/2026-07-11-makima-strategic-self-description-generation-leg.md`
...
makima's `strategic.generate` op (**MUTATING**, scope `"none"`, fleet-generic, explicit
...
- **makima** — derives `version_highlights[]` (Track A) and `competitors[]` (Track B) from
...
- **market-intelligence-team / repo-setup** — upstream *providers* of Track B raw signal (market-
...
|---|---|---|---|
| `target_root` | `str` | **yes** | Repo root to generate a draft for. Canonicalized and validated via `coordinator_core.cartography._guard.path_guard(target_root, ".")` before any read/write — fail-loud on a missing/invalid root, never silently falls back to the makima repo itself. |
...
**Inputs (checked in this fallback order — first available source wins per makima repo; makima is
...
2. `state/week-changelog/*.md` — the **primary path today**. Each dated changelog entry
   (`state/week-changelog/YYYY-MM-DD-*.md`) is a candidate entry; makima's own repo is verified
   tag-less (0 git tags), so this is the path that actually populates makima's own draft.
...
**Inputs (both PROVIDED — sourced by an upstream owner, not derived by makima from repo state):**

1. **Market-intel snapshot diff** — owned and produced by the market-intelligence-team; this op
...
| `note` | market-intel snapshot diff (market-intelligence-team), when available | Nullable — absent snapshot signal for a given peer emits `note: null`, never a guess |
| `provenance` | literal constant | **always** `"generated"` — see § 4 |
...
- **Sourcing Track B's raw signal** — producing the market-intel snapshot diff is
  market-intelligence-team-owned; producing the peer identity marking is repo-setup-owned. This op
...
- **Cadence/scheduling** — `strategic.generate` is invokable on demand and nudged (never
  ...
  `docs/plans/2026-07-11-makima-strategic-self-description-generation-leg.md` § C5(b); this
...
<!-- producer-contract: makima strategic.generate op — draft artifact for DoE's refresh ceremony.
