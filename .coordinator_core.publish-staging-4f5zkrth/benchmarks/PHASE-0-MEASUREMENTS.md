...
| gating statistic | `min` (Camelia C2 — a p50 over small N is a coin-flip; `min`-vs-RAW-target is the resolved gate) |
| runner_isolation_mode | shared |
| machine | darwin (dev, delphipro) |
...
Benchmarked `records.query` (worktree-scoped) against **makima's real `state/` tree** in addition to
the pinned synthetic fixture (N=40 each):

| target | min | p50 |
|---|---|---|
| synthetic fixture | 61.94 | 64.17 |
| real makima `state/` | 64.63 | 68.26 |
...
repo (`project-makima`, ~600+ tracked files for `tree`, a 40-file sample for `symbols`/`edges`):
