...
DR-047 (contract/data lives with DoE, engine with makima) after the 2026-07-22
...
Two live layouts (mirrors coordinator_data_root.py's docstring):
  ...
  2. Split-repo    — coordinator_core lives in project-makima while the data
...
DR backlink:   docs/decisions/DR-047-doe-makima-boundary-redraw-contract-vs-e.md (DoE-side)
...
def _colocated_root() -> Path:
    ...
    coordinator_core/data_root.py -> parent.parent == the makima repo root
    ...
    makima REPO root bare (`parent.parent`, no `/ "coordinator"` suffix), so
    ...
    returned makima's OWN `docs/` tree (which exists) instead of ever
