...
this repo is the makima-owned SSOT per the C1 re-siting (engine-subject: a
constant consumed by makima's own Check 7 cannot live in a DoE hook script).

Derivation (2026-07-27, plan `docs/plans/2026-07-27-doctrine-envelope-allocation.md`
chunk C1, per Zolí F10 review-integration finding):
...
leave under it (DoE's hook: 100-byte margin; makima's Check 7: 2000-byte
...
convention DoE's `_claude_md_ledger.py` (C7a) already defined and reads for
its own PreToolUse admission gate. This module does NOT import that DoE
module -- `check_claude_md_size.py`'s own docstring already rules out this
engine resolving "the DoE repo" and reading its working data, and that
ruling stands: a cross-repo Python import would invert the coordinator-
claude-depends-on-project-makima direction. What this module does instead
...
the two enforcement points (DoE's PreToolUse hook, this engine's commit-time
...
(a roadmap-dir file under `project-makima state/roadmap/boot-envelope/`,
...
#: makima's Check 7 import these rather than carrying independent literals.
...
def is_governed_claude_md(
    ...
    (DoE-claude's own repo-root CLAUDE.md, project-makima's, any sibling
...
def is_claude_md_class(path: Union[str, Path]) -> bool:
    ...
    every file a DoE session can encounter that carries CLAUDE.md-shaped
...
#: convention -- mirrors DoE's `_claude_md_ledger._LEDGER_PATH_OVERRIDES`
...
class RatchetWatermarkError(Exception):
    ...
    ratchet it claims to enforce. Mirrors DoE's `_claude_md_ledger.LedgerError`
...
def surface_slug(surface: str) -> str:
    ...
    `_LEDGER_PATH_OVERRIDES` entry. Mirrors DoE's `_claude_md_ledger.
...
def resolve_ledger_path(repo_root: Union[str, Path], surface: str) -> Path:
    ...
    convention DoE's `_claude_md_ledger.resolve_ledger_path` already uses, so
