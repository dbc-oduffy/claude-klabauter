"""
coordinator_core.tracker_envelope — the shard-to-envelope fold for opticon's
`ingest_emission` (sat-07 C4).

Purpose: our event store is per-machine JSONL shards
(`tracker_store.EVENTS_SHARD_GLOB = "events.*.jsonl"`); opticon's
...
does not assemble anything opticon-side.
...
not opticon's pinned wire spec the way DEC-23's per-item `projects: string[]`
shape is (DR §4.5 / their `recs-05-action-item-population.md`). The EM has
an open cross-repo question out to project-opticon-em on the exact top-level
key names; this module is non-blocking in both directions on the answer —
opticon quarantines unknown top-level keys replayably, so an extra or
differently-named key here is a tolerable forward-compat cost, not a crash,
and this chunk does not gate on opticon's reply. Correct the two constants
...
# STATED ASSUMPTION — see module docstring. Not opticon-ratified; correct in
# place if project-opticon-em's answer to the open cross-repo question names
...
def build_ingest_envelope(repo_root: Path) -> dict:
    ...
    object for opticon's `ingest_emission` (AC12).
