...
def _dispatch_coordinator_doc_new(args: list[str], repo_root: Path) -> dict[str, Any]:
    ...
    makima_bin = resolve_operator_config()["makima_bin"]
    cli = str(Path(makima_bin) / "coordinator-doc-new.py")
    proc = subprocess.run(
...
def _dispatch_lint_frontmatter(args: list[str], repo_root: Path) -> dict[str, Any]:
    ...
    makima_bin = resolve_operator_config()["makima_bin"]
    cli = str(Path(makima_bin) / "lint-frontmatter.py")
    proc = subprocess.run(
        [sys.executable, cli, *args], cwd=repo_root, capture_output=True, text=True, **_NO_CONSOLE
    )
    if proc.returncode != 0:
...
def _dispatch_session_claim_cli(args: list[str], repo_root: Path) -> dict[str, Any]:
    ...
    makima_bin = resolve_operator_config()["makima_bin"]
    cli = str(Path(makima_bin) / "session-claim-cli.py")
    proc = subprocess.run(
        [sys.executable, cli, *args], cwd=repo_root, capture_output=True, text=True, **_NO_CONSOLE
    )
    if proc.returncode != 0:
...
def _reconcile_claim_from_ledger(
    ...
    THE DEFECT THIS CLOSES (live repro, project-makima 2026-08-07). The claim
...
def _dispatch_handoff_supersede_predecessor(args: list[str], repo_root: Path) -> dict[str, Any]:
    ...
    asserts no supersession of it. The C0 makima-em seam decision
    ...
    Live repro (project-makima, 2026-08-03, `/handoff` over a `status: open`
    ...
    def _cleanup_successor() -> None:
        ...
        # to a non-handoff kind (the open C0 makima-em seam decision noted
...
def _resolve_makima_bin() -> Path:
    """`makima_bin` for the generator-load seam below -- the SAME
    ...
    return Path(resolve_operator_config()["makima_bin"])
...
def _load_doc_new_module() -> Any:
    ...
    Resolved through `resolve_operator_config()["makima_bin"]` -- the SAME
    ...
    script_path = _resolve_makima_bin() / "coordinator-doc-new.py"
