...
class TestResolvedAddressWiringAC6:
    ...
    def test_a_reachable_claimed_by_prints_the_resolved_address(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda owner_id: reach.ResolveResult(
                outcome="reachable", session_id=owner_id, address="project-makima-57 [b2afcd]"
            ),
        )
        capsys.readouterr()
        ba.resolve_lineage("handoff", str(predecessor), tmp_path)
        captured = capsys.readouterr()
        assert "project-makima-57 [b2afcd]" in captured.err
    ...
    def test_an_ambiguous_claimed_by_renders_no_address(self, tmp_path, capsys, monkeypatch):
            ...
            lambda owner_id: reach.ResolveResult(
                ...
                candidates=[
                    reach.Candidate(
                        ...
                        name="project-makima-11",
                        ref="aaaaaa",
                        address="project-makima-11 [aaaaaa]",
                    ),
                    reach.Candidate(
                        session_id="session-bbbbbb",
                        name="project-makima-11",
                        ref="bbbbbb",
                        address="project-makima-11 [bbbbbb]",
                    ),
                ],
            ),
        )
        ...
        assert "already held by a live baton" in captured.err
        assert "send-message-address" not in captured.err
        assert "project-makima-11 [aaaaaa]" not in captured.err
        assert "project-makima-11 [bbbbbb]" not in captured.err
...
class TestRoadmapBatonExclusion:
    """2026-08-05 market-intelligence-em cross-repo memo: a `kind:
