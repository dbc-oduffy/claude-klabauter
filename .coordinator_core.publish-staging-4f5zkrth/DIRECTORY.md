...
| `records_query.py` | `records.query` | COMPUTE_ONLY — queries makima's own state records |
...
present-but-empty-by-design in `resolvers.py`'s skeleton. makima is not the data path for
market-intelligence (routes to opticon `ingestEmission` directly); do not add a section here.
...
| `shell_rc_guard.py` | `install.write_shell_rc_guard_block` | Idempotent sentinel-guarded MAKIMA_CLONE shell-rc block installer |
...
producer-contract docs for makima-owned ops, e.g. `handoff-reconcile-producer-contract.md`
