...
Run: cd /Users/oduffy/X/project-makima && python3 -m pytest coordinator_core/test_pickup_assemble.py -q
"""
...
class TestBareSlugSuffixFallback:
    ...
    `cross-repo/inbox/2026-07-28-project-rag-em-pickup-bare-slug-unresolvable.md`:
...
class TestRevisionShaFallback:
    ...
    def test_abbreviated_sha_not_in_clone_names_the_clone_not_the_filename(self, tmp_path):
        """`cross-repo/inbox/2026-08-15-project-rag-em-pickup-cannot-resolve-a-memo-by-its-delivery-sha.md`:
...
# ---------------------------------------------------------------------------
# AC-6 perf-optimization regression tests (Patrik review, `ac6-patrik-
...
def _watch_calls(func_name: str):
    ...
    below (Patrik review § 9) — each test differs only in which module-
...
# truth table, incl. the self-holder row (Zolí review, F2) and both sides of
...
class TestTreeQuiescence:
    ...
    def test_prose_scope_entry_never_lands_in_dirty_list(self, tmp_path):
        ...
        repo = tmp_path / "repo"
        _init_repo(repo)
        prose = "project-makima: coordinator/bin/ (apply entrypoint + auto-fire hook — makima direct-commit granted)"
    ...
    def test_sibling_repo_resolved_through_registry_not_hardcoded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            pa, "registry_get",
            lambda key: str(sibling) if key == "repos.project_makima" else None,
        )

        result = pa.compute_tree_quiescence(repo, ["project-makima: coordinator_core/dag.py"])
    ...
    def test_windows_drive_letter_sibling_path_resolves_via_pathlib(self, tmp_path, monkeypatch):
        ...
        windows_root = "C:\\Users\\test\\X\\project-makima"
        monkeypatch.setattr(
            pa, "registry_get",
            lambda key: windows_root if key == "repos.project_makima" else None,
        )
        repo = tmp_path / "repo"
    ...
    def test_sibling_prefix_matches_the_real_no_space_plan_shape(self, tmp_path, monkeypatch):
        """Real plans write `- project-makima:coordinator_core/x.py` with NO
...
class TestBareDictJudgmentPointBuildersCarryRecommendation:
    """Patrik second-pass finding #4: the four (five, per live disk —
...
class TestJudgmentPointConstructorRequiresRecommendation:
    """AC5b/Zolí F4: `build_judgment_point`'s `recommendation` parameter is
...
class TestUntrustedGateConstructorCannotCarryARecommendation:
    """AC5b/Zolí F4 (recommendation-forbidden tier): the discriminator is
...
class TestApplyHaltsAndIgnoresRecommendationContent:
    ...
    Scoped predicate (Patrik second-pass finding #5): the halt decision is
...
class TestSplitArtifactArgsBulletLines:
    ...
    def test_pm_pasted_bullet_list_yields_two_paths(self):
        ...
        raw = (
            "- X:\\project-makima\\cross-repo\\inbox\\2026-08-11-doe-claude-em-"
...
class TestSplitArtifactArgsBraceExpansion:
    ...
    def test_pm_invocation_string_resolves_to_two_real_inbox_paths(self):
        ...
        raw = (
            "X:\\project-makima\\cross-repo\\inbox\\2026-08-07-doe-claude-em-"
...
class TestRepoBasenameReanchor:
    def test_second_path_with_repo_basename_prefix_reanchors(self, tmp_path):
        ...
        result = pa.brief("/project-makima/cross-repo/inbox/m2.md", repo_root=repo)
    ...
    def test_and_joined_reanchor_path_resolves(self, tmp_path):
        ...
        results = pa.brief_multi(
            "state/handoffs/h1.md AND /project-makima/cross-repo/inbox/m2.md",
            repo_root=repo,
        )
...
class TestKindDispatchGuidanceAndRecommendation:
    ...
    def test_guidance_is_descriptive_not_steering(self):
        """Zolí, Design Call #1 rider — guidance describes what a disposition
...
class TestDispositionValueAwarePredicateZoliV2Finding1:
    """Zolí v2 finding 1 (CRITICAL) — a NON-terminal disposition on a
...
class TestEMContentChannelZoliV2Finding2:
    """Zolí v2 finding 2 (MAJOR) — the EM-content channel: a `j-kind`
...
class TestCommitOnGrabHaltZoliV2Finding3:
    """Zolí v2 finding 3 (MAJOR) — a grab-halt run (fires the unconditional
...
class TestApplyFailsLoudOnTerminalArtifactWithDroppedDecisions:
    ...
    def test_decisions_on_already_terminal_memo_fails_loud_not_silent_ok(self, tmp_path):
        repo = tmp_path / "repo"
        _init_repo(repo)
        name = "2026-07-26-project-opticon-em-fleet-state-memo-ingest-landed.md"
