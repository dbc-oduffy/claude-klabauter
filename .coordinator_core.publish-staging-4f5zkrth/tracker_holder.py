...
entry:
    ...
    tracker.holder_repo = "delphi_store"     # names a repos.<key>, not a path
...
`delphi_store`, resolving (as of this seeding) to whatever path
`machine-local get repos.delphi_store` reports on this machine.
...
already under registry management (`repos.delphi_store`) as a second source
of truth, and turns "is the holder the same repo as delphi_store?" into a
...
`delphi_store`, never a raw path). A second, bespoke, raw-absolute-path env
...
resolved holder root is not makima's own root
(`_makima_source_tree()` — the checkout, not the engine root), raising if
...
`repos.<key>` NAME. The agreed wire shape from opticon is an owner-qualified
`"<owner>/<repo>"` slug instead: opticon's repo identity is `UNIQUE (owner,
...
install-time/doctor-cadence concern (e.g. a future `makima-doctor-probe.py`
...
opticon's legacy encoding used `""` to mean "no owner", which this module
...
#: The makima SOURCE TREE key. Deliberately NOT the engine root: this module's
#: brightline compares a `repos.*` holder clone path against makima's own clone
...
_MAKIMA_SOURCE_TREE_KEY = "repos.project_makima"


def _makima_source_tree() -> Path:
    """Resolve the makima SOURCE TREE (the checkout), never the engine root.
    ...
    """
    value = registry_get(_MAKIMA_SOURCE_TREE_KEY)
    if not value:
        raise RuntimeError(
            "cannot check the holder brightline: the machine-local registry "
            f"has no '{_MAKIMA_SOURCE_TREE_KEY}' entry, so makima's own source "
            "tree is unknown and a holder that IS makima cannot be refused. "
            "Remediation: machine-local set "
            f"{_MAKIMA_SOURCE_TREE_KEY} /path/to/project-makima"
        )
    return Path(value)
...
def holder_repo_root() -> Path:
    ...
    holder root is makima's own SOURCE TREE (`_makima_source_tree()`, which
    honours no env rung) —
    ...
    makima_root = _makima_source_tree()
    if resolved.resolve() == makima_root.resolve():
        raise RuntimeError(
            f"designated holder repo '{holder_key}' resolves to "
            f"'{resolved}', which is makima's own root — the holder repo "
            "must never be makima itself (brightline guard). Remediation: "
            f"point {repos_key} at a different repo, not makima"
        )
...
def write_root_for(*, owning_repo: str | None, repo_root: Path) -> Path:
    ...
    already guards holder-root != makima-root; this guard needs the caller's
    ...
    if owning_repo is None:
        ...
        raise RuntimeError(
            "owning_repo is the empty string — this is opticon's legacy "
