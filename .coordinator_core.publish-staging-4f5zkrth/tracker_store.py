...
events (opticon-ratified §4.3 — an event that has not cleared reconcile
...
opticon's globally-unique primary key, so nothing below it in the tuple is
reachable, and ``logical_clock`` is written for a future opticon merge
...
Negative-spec:
  - Do NOT make ``logical_clock`` authoritative or invent a merge policy —
    that is opticon's contract to deliver (DEC-10). The field is written
  ...
  - Do NOT collapse ``OBSERVED_SET_UNKNOWN`` into ``{}`` or a trusted
    ...
    caller). That collapse is the exact over-claim opticon's property
...
class _UnknownObservedSet:
    ...
    is the exact over-claim opticon's property (c) exists to prevent — see
...
def read_events(*, repo_root: Path) -> list[dict]:
    ...
    if shard_dir.is_dir():
        ...
        # opticon-consumption-seam.md § RULING 2026-08-20.
...
def rotate_month(*, repo_root: Path, month: str, machine: str | None = None) -> int:
    ...
    partition-on-write — docs/plans/2026-08-18-sat-06-opticon-consumption-
...
def _well_formedness(sequences: list[int]) -> tuple[int | None, int | None]:
    """opticon's A12 well-formedness predicate over ONE shard's full
    ...
    exactly the defect this predicate exists to catch (opticon's
    ...
    Conformance oracle: project-opticon
...
def fold_observed_set(*, repo_root: Path) -> dict | None:
    ...
    having it inferred from a naive empty-vector default (opticon's named
    ...
    # (see docs/plans/2026-08-18-sat-06-opticon-consumption-seam.md §
...
def resolve_observed_set(marker: dict, *, repo_root: Path) -> dict:
    ...
    Resolution granularity is PER COMPONENT, not per marker (opticon's
    ...
    Negative-spec — a future editor must NOT:
      - Collapse ``OBSERVED_SET_UNKNOWN`` into ``{}`` or into a trusted
        ...
        over-claim opticon's property (c) exists to prevent.
    ...
    Well-formedness predicate adopted from opticon's ratified § VALIDATOR
    ...
    the prior caveat here pended opticon's answer, which now names this
    contract.
    ...
    for slug, claim in observed_set.items():
        ...
        # docs/plans/2026-08-18-sat-06-opticon-consumption-seam.md §
        ...
        for line in lines:
            ...
            # regardless of claimed_seq — opticon's well-formedness predicate
        ...
        # opticon's A12 well-formedness predicate, over the SAME
...
def resolve_observed_set_for_event(
    ...
    Implements § Design's event→marker mapping — the answer to opticon's
    ...
    *event*'s own ``"machine"`` and ``"sequence"`` fields.
    ...
    adopted opticon's ratified § VALIDATOR CONTRACT (A12) well-formedness
    ...
    ``cross-repo/inbox/2026-08-18-project-opticon-em-tmrg-03-ordering-contract-and-prefix-closure-ownership.md`` § (e).
    """
...
#
# Normative spec: project-opticon
...
# C3 task text. opticon owns the comparator as a SPECIFICATION end to end
...
def _resolve_self_component(
    ...
    """opticon's A14 self-normalisation: recompute *machine*'s own
    ...
    *machine*'s own shard cannot — under this module's own append
    ...
    Applies opticon's DEC-5 / § SELF-SHARD APPLICABILITY: the
    ...
    § C4; project-opticon docs/wiki/tracker-observed-set-and-concurrency.md
...
def resolve_observed_set_union_for_event(
    ...
    """``W(E)`` — opticon's § SELF-COMPONENT RECONCILIATION (A10): the union
...
def compare_observed_set_vectors(vector_a, vector_b) -> str:
    """The causal comparator (opticon's § MIXED-TYPE DOMINATION COMPARE,
    ...
    *vector_a* / *vector_b* are each either the bare ``OBSERVED_SET_UNKNOWN``
    ...
    trusted-maximum over-claim opticon's contract exists to prevent).
    ...
    Spec backlink: docs/plans/2026-08-18-tmrg-03-ordering-contract-in-tracker-store.md
    § C3; project-opticon docs/wiki/tracker-observed-set-and-concurrency.md
...
def compare_events_causal_order(event_a: dict, event_b: dict, *, repo_root: Path) -> int:
    ...
    opticon's globally-unique primary key), never to ``applied_at``:
    ...
    SAME machine sharing one fold marker (opticon's § SELF-COMPONENT
    ...
    CONSUMPTION CONSTRAINT (opticon's wiki, same section name — a hard
...
def _fallback_tiebreak_key(event: dict) -> tuple[str, str]:
    ...
    demand ``machine``; opticon's applied-event shape guarantees ``id``) —
