...
Negative-spec (hard-won):
    - The core is read-mostly for coordinator substrate. Handlers MUST NOT write coordinator
      ...
      .git/coordinator-sessions/ only:
        ...
        layer — not coordinator substrate (makima's disk-truth custody) and not rag's
      ...
      HANDOFF lifecycle ops (4, DR-212) — sanctioned MUTATING in-place frontmatter-mutation
        ...
        **Transport-bound vacated 2026-07-06 by DR-215:** in-process command-type
        ...
        Ownership authority: makima owns the authoritative mutation mechanism for handoff
      ...
      MEMO send op (1, DR-214-send-class) — sanctioned MUTATING cross-tree write category
        ...
        Q-c HARD (AC4): refuses when the makima engine is unreachable — no legacy
      ...
      SOVEREIGN-TRACKER append-ordered event-store class (library-only, zero ops, DR-241) —
        ...
        fleet-wide and never makima's tree on another repo's behalf (DEC-11); MUST NOT
      ...
      Backlinks:
        docs/plans/2026-07-04-pcore-04-advisory-hook-ops-makima-engine.md § AC4 / C7
...
# fallback to makima. none whatsoever ... I want that shit to fail hard every
...
# unstamped project-makima tree). This gate closes that gap at the one seam
...
#: makima. none whatsoever ... I want that shit to fail hard every time if it
...
#: which resolves via `_engine_root.resolve_makima_root()` to the STAMPED
...
#: `_resolve_makima.resolve_makima_root_with_class()` returns
...
def _is_dispatch_engine_stamped() -> bool:
    ...
    same tradeoff `coordinator/lib/resolve-makima/_resolve_makima.py`
    already made for the identical reason (see that module's own
    `_is_stamped_engine_root` docstring: "cannot import
    coordinator_core... its entire job is to FIND makima, so it cannot
    presuppose makima is already importable"). This is that module's
...
class CallerFacingValidationError(ValueError):
    ...
    cross-repo/inbox/2026-08-15-project-rag-em-wsc-review-trail-skips-silently.md.
...
# table's comment block — Patrik F11: this is MORE true after the override retirement,
...
# the relocated reconcile-before-retry negative-spec (Patrik F11) and the DEC-2
...
# cross-repo/archive/2026-07-13-project-rag-ue-addon-em-wsc-commit-30s-timeout-friction.md
#
...
def _owning_module_is_ceremony(method: str) -> bool:
    ...
    (`docs/decisions/DR-344-the-brightline-process-budget-for-makima.md`), so
...
# sibling repo makima does not own. Do not "fix" this back to declared-path
...
def _lazy_import_and_lookup(method: str) -> Optional[Callable]:
    ...
    Lazy op registration (F6 / makima-windows-portability § C4): imports fire each
...
async def _dispatch_message_impl(msg: dict) -> dict:
    ...
    #
    # Lazy op registration (F6 / makima-windows-portability § C4): the op-registry
