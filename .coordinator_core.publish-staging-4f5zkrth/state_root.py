...
Encodes the ratified state-placement taxonomy and DoE/makima plane routing so
...
Five routing rules (verbatim from the bash oracle's header):
  ...
  Rule 4  central=True, no subject, no artifact  [BACKWARD-COMPAT DEFAULT]
            -> <coordinator_engine_root()>/state
            Every existing --central caller resolves to makima, unchanged.
  ...
  Published-mirror guard (applies to every rule that resolves makima engine
  state — Rules 2, 4, and 5's meta-repo branch — AND Rule 5's sibling-repo
  branch): DR-132's two-tier gate means the resolved makima root can now be
  ...
  candidate git root rather than only "my own" makima root: before treating
  a non-meta cwd git root as its own state root, it asks
  `coordinator_core.engine_root.published_engine_mirror_path()` (the same
  `repos.claude_klabauter` discriminator `_makima_state()` already uses,
  ...
  makima engine root" (Rules 2/4/5-meta) or "this is just some sibling repo"
...
Negative-spec (faithfully reproduced — do NOT "fix" mid-port):
    ...
    - Does NOT fall back to makima when doctrine DoE root fails (Rule 1).
...
#: (`coordinator/lib/resolve-makima/_resolve_makima.py`) rather than loading
...
def _doe_state() -> str:
    """Rule 1 helper: DoE doctrine state root. Fail-loud; no makima fallback."""
    doe = coordinator_doe_root()
    if not doe:
        raise StateRootError(
            "coordinator_state_root: cannot resolve DoE doctrine root — "
            "repos.doe_claude is not set. Does NOT fall back to makima for the "
...
def _makima_state() -> str:
    """Rule 2/4/5 helper: makima engine state root. Raises StateRootError on
    failure OR when the resolved makima root is a published engine mirror
    ...
    if resolution_class == _RESOLUTION_RESOLVED_ENGINE_LITERAL:
        raise StateRootError(
            ...
            "project-makima (e.g. unset any published-mirror registration "
            "for this session), or set COORDINATOR_ENGINE_ROOT explicitly to a working "
            "tree."
        )
    return _state_of(makima_root)
...
def coordinator_state_root(
    ...
    if meta:
        # Meta-repo -> central state is in makima.
        return _makima_state()
    ...
    # `repos.claude_klabauter` discriminator `_makima_state()` already
...
def print_map() -> str:
    ...
    # map reflects the same published-mirror guard `_makima_state()` applies
