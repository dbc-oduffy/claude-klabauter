...
Port of DoE-side `coordinator/bin/snippet-registry` (bash CLI, 477 LoC,
...
Modules:
  registry.py — reads `snippets/registry.toml`, resolves per-snippet
                ...
                TOML parsing via `tomllib`/`tomli`. Consumed by DoE-side
  ...
  verify.py   — the shared verify/--fix/--list engine. Per-snippet
                ...
                per-script code branches. Consumed by DoE-side
