...
``oduffy`` + backslash + ``foreign`` losing its separators), because
...
(confirmed this session: `ls` on ``X:`` + backslash + ``project-makima`` +
backslash + ``CLAUDE.md`` errors `cannot access 'X:project-makimaCLAUDE.md'`,
...
# rooted, never a tracked makima repo artifact.
GENERATES = []
...
def probe_armed(
    interpreter: str, makima_root: Union[str, Path], *, timeout: float = 10.0
) -> Tuple[bool, str]:
    ...
    probe_src = (
        "import sys\n"
        f"sys.path.insert(0, {str(makima_root)!r})\n"
