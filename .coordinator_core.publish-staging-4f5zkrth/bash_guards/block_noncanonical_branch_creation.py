...
offer a rename), which would let `work/DELPHIPRO/2026-07-13` sail through
...
e.g. `git checkout -b "work/delphipro/$(date +%F)"` -- is a DIFFERENT
...
(`"work/delphipro/$(date +%F)"`) yields `"work/delphipro/ "` (an internal/
trailing literal space -- `name != name.strip()`); the unquoted form
(`work/delphipro/$(date +%F)`) yields `"work/delphipro/"` (shlex splits at
...
def _looks_unsafe(name: str) -> bool:
    ...
    e.g. `"work/delphipro/$(date +%F)"`) -- neither `resolved.tokens` nor
...
def _canonical_example(today: str) -> str:
    ...
    message's remediation text -- e.g. `work/delphipro/2026-08-01` -- via
