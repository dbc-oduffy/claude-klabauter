...
Negative spec for the public API -- what it deliberately does NOT do:
  ...
  - It does NOT self-apply the ``position`` label as an allow/deny policy.
    ...
    undeterminable) -- makima classifies, the CALLING guard decides what a
...
project-opticon-em measured pnpm v11 running the WHOLE suite (642-643
...
(no "R10") -- DR-088's Tier-T definition is unchanged, only makima's scope
...
backlink: cross-repo/inbox/2026-07-30-project-opticon-em-tier-u-guard-
...
ruled.md's sibling chain; original ``2026-08-01-project-rag-em-test-suite-
...
refuse. Reported by project-rag, relayed via
...
COLLECTION. Confirmed live via project-rag-em, which saturated a contested
...
makima's scope DERIVATION for this shape changed. Spec backlink:
cross-repo/inbox/2026-08-14-project-rag-em-em-level-expensive-command-guard-
question.md
...
Spec backlink (R9 precision public API): cross-repo/inbox/2026-07-28-market-intelligence-em-dispatched-agent-scoped-test-breadth.md (DoE-claude repo)
...
def _classify_pytest(args: Sequence[str], testpaths: Sequence[str],
    ...
    definition is unchanged; only makima's scope DERIVATION for this one
...
def _classify_package_manager(base: str, args: Sequence[str],
    ...
    Measured by project-opticon-em on pnpm v11 with ``"test": "vitest
    run"`` (``cross-repo/inbox/2026-07-30-project-opticon-em-tier-u-guard-
    bypassed-by-pnpm-arg-forwarding.md``):
    ...
    makima's scope DERIVATION for this shape changed. Not a new DR-088
...
def _classify_tox_nox(base: str, args: Sequence[str]) -> str:
    ...
    ``2026-08-01-project-rag-em-test-suite-approval-gate-bypass.md`` (not
...
def _normalized_segments(cmd: str) -> List[Tuple[str, ...]]:
    ...
    was the bug (cross-repo/inbox/2026-07-25-project-opticon-em-tier-f-
...
def _cfg_segments_satisfied(cfg_segments: Sequence[Tuple[str, ...]],
    ...
    pnpm run test``) -- see
    ``cross-repo/archive/2026-07-25-project-opticon-em-tier-f-escape-hatch-
    ...
    set is still fully present (opticon's fix survives) AND no segment
...
def _configured_test_cmds(repo_root: Optional[str]) -> List[ConfiguredCmd]:
    ...
    project-makima, the one repo where the by-path shim exists and works.
    ...
    coordinator/bin/`` holds the resolver in project-makima ONLY -- the
...
#: shape the project-rag memo flagged. The doctrine still genuinely lives in
...
def _pytest_directory_args(segments_argv: Sequence[Sequence[str]],
    ...
    *"the caller authored or touched"*; the mechanism enforced path-scoped and
    ...
    Glob-expansion closing (2026-08-03, project-rag Finding 2, relayed via
    ...
    Hot-path cost, measured 2026-08-03 on makima at ~4.2k files rather than
    ...
    ``PreToolUse`` guard on this repo and the walk is bounded by makima's own
...
def classify_command_precision(command: str, *, cwd: Optional[str] = None) -> List[PrecisionMatch]:
    ...
    Spec backlink: cross-repo/inbox/2026-07-28-market-intelligence-em-dispatched-agent-scoped-test-breadth.md (DoE-claude repo)
    """
