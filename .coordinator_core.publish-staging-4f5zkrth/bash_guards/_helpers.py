...
2. Two new shared predicates that had no Python port before this module
   ...
   - ``is_confined_findings_agent(effective_type)`` -- port of DoE's
...
Parity oracles:
  Port of: coordinator-session.sh \xa7 _cs_is_confined_findings_agent (DoE e34f2484, 2026-07-22)
  Port of: coordinator-safe-name.sh \xa7 csn_check (DoE 721a71f4, 2026-07-21)
  Ported from the retired DoE bash guard
  ``block-reviewer-bash-outside-allowlist.sh`` (deleted 2026-07-16, DoE
  ``2f8b8450``).
...
#: ``"project-makima docs/reference/guard-override-keys.md"``. Not
...
#:
#: These guards are not makima-local: DoE's PreToolUse shim resolves this
...
OVERRIDE_KEYS_DOC_DISPLAY = "project-makima " + OVERRIDE_KEYS_DOC
...
def operator_override_note(
    ...
    *,
    ...
    market-intelligence-em plus two prior siblings): the previous render --
    ...
    prior rounds already tried and the source memo (2026-08-11-project-rag-
...
def is_confined_by_roster_absence(effective_type: str) -> bool:
    ...
    DoE's policy YAML, ``coordinator/agents/*.md``, and the plugin discovery
