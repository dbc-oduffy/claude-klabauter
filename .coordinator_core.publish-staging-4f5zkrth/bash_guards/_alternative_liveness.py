...
def _trigger_validate_frontmatter_schema_advisory() -> Optional[Dict[str, Any]]:
    ...
    makima's own vendored `_VENDORED_SCHEMAS_DIR`; `_load_doe_registry()`
...
#: Zoli's review (2026-07-29, coordinatoreng-director-1d83280e, finding 4):
...
def _split_alternative_text(text: str) -> List[str]:
    ...
    ``git -C X:\\project-makima\\coordinator_core status`` tokenizes its path as
    ``X:project-makimacoordinator_core`` -- a path that cannot exist, which
...
def _classify_backtick_span(span: str) -> Optional[Alternative]:
    ...
    if len(argv) == 2 and argv[1].startswith("-"):
        ...
        # BOTH through as the detail tuple (Zoli's review, finding 5:
...
def extract_alternatives(hso: Dict[str, Any], *, override_route_known: bool = False) -> List[Alternative]:
    ...
    # message. This is the position-anchoring Zoli's review required
    ...
    if not alts:
        ...
        if _ALT_CUE_RE.search(text):
            ...
            if not alts:
                ...
                # for `<makima-root>`-style templates elsewhere), never a
...
def _resolve_on_path_or_settings_home(name: str) -> Optional[str]:
    ...
    outside makima's own tree uses) -- never a hardcoded absolute path."""
...
def probe_flag(alt: Alternative) -> Verdict:
    ...
    Zoli's review (finding 5): a bare flag extracted with no accompanying
...
def probe_harness_capability(alt: Alternative, guard: Optional[str] = None) -> Verdict:
    ...
    corroboration -- Zoli's review (2026-07-29,
...
def probe_override(alt: Alternative, guard: str, baseline: GuardFireResult) -> Verdict:
    ...
    Zoli's review (finding 6) caught two problems with the prior
