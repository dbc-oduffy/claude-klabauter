...
must NOT be able to move (Zolí F1: the harness Bash tool contract states "Working directory
...
read-settings-home-first" call in that plan's Problem section):
  ...
  2. A settings-home hub, `$(coordinator-settings-home)/project-makima/write-bump-anchor/
     ...
     worktree, a bare checkout, or a `cd` outside any git repo. This is makima's first out-of-repo
     ...
     `$(coordinator-settings-home)/project-makima/`, never the settings-home root or an ad-hoc
...
#: `$(coordinator-settings-home)/project-makima/write-bump-anchor/` — the durable-data-plane
...
_SETTINGS_HOME_ANCHOR_SUBDIR = ("project-makima", "write-bump-anchor")
...
def _settings_home_anchor_dir(env: Optional[dict] = None) -> str:
    """`$(coordinator-settings-home)/project-makima/write-bump-anchor`, resolved via the
