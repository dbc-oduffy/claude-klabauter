...
def check_destructive_rm(cmd: str, session_id: str = "") -> Optional[Dict[str, Any]]:
    ...
    for seg in _split_segments(cmd):
        ...
        for tgt, tgt_expanded in targets:
            ...
            # 2026-08-12-project-rag-em-git-index-lock-reaper.md. This leg
            ...
            if tgt_is_dir or recursive:
                ...
                # and `~/X/project-makima` were all ALLOWED because `~` and
...
# repo -- 4 cross-contamination commits on work/striker/2026-06-14,
...
# project-makima, a shared tree this DoE-claude EM commits into directly and
...
def _is_hazard_repo(git_root: str) -> bool:
    ...
    (``repos.*``) tracks (project-makima, DoE-claude, and every other
...
#: subcommand-not-literally-adjacent-to-`git` problem. Market-intelligence-em
...
def check_blanket_git_add(
    ...
    for seg in _awk_quote_aware_split(cmd):
        ...
        # market-intelligence-em scoped-commit-guard-asymmetry finding):
        ...
        for tok in after.split():
            ...
            # just spelled differently (market-intelligence-em
...
def _skip_leading_env_and_wrappers_idx(buf: str, i: int) -> int:
    ...
    while i != prev:
        ...
        if m2:
            ...
            # pass, confirmed live -- Patrik staff-eng review Finding 3):
...
def check_validate_commit(
    ...
    # coordinator_core.claude_md_budget.is_governed_claude_md (makima-owned
...
def _bt_python3_invocation_cache_path() -> str:
    ...
    Prefers makima's own `state/cache/` (this repo's disk-truth substrate,
...
def _paths_match_prefix(root: str, target: str) -> bool:
    ...
    guard's own repo (`project-makima`, "Windows is first-class" per this
...
def _bt_commit_operand_scan(
    ...
    whether bare-positional or after `--` (2026-08-05 opticon-incident fix):
...
def _bt_commit_has_amend_flag(seg_tokens: List[str]) -> bool:
    ...
    - The amend-ownership gate (project-rag-em cross-repo memo, 2026-08-15,
...
def check_git_commit_safe_commit_advise(
    ...
    AMEND GATE (project-rag-em cross-repo memo, 2026-08-15): a segment
    ...
    for seg_index, (seg_tokens, pipe_before) in enumerate(segments):
        ...
        if _bt_git_resolved_subcommand(seg_tokens) != "commit":
            continue
        # Amend-ownership gate (project-rag-em cross-repo memo, Finding 2):
        ...
        # cross-repo memo, project-rag-em, 2026-08-15, "amend has no safe
...
def _bt_is_redirection_token(tok: str) -> bool:
    ...
    this module, per the 2026-08-05 opticon incident fix: a surviving
...
def _bt_is_bare_redirection_token(tok: str) -> bool:
    ...
    2026-08-05 opticon-incident follow-up review): `_bt_tokenize_full_command`
