...
`/x/project-makima/scratch/t.txt`.
"""
...
def test_resolve_relative_version_independent_for_msys_absolute_target(
    ...
    result = shapes.resolve_relative(
        "/x/project-makima", "/x/project-makima/scratch/t.txt"
    )
    assert result == "X:\\project-makima\\scratch\\t.txt"  # abs-path-ok: brief-pinned AC1 fixture (C1 brief), synthetic
...
def test_translate_msys_path_drive_mount_to_native(_windows_ntpath):
    assert shapes.translate_msys_path("/c/Users/oduffy/x.txt") == "C:\\Users\\oduffy\\x.txt"  # abs-path-ok: brief-pinned AC2 example (C1 brief), synthetic
...
def test_translate_msys_path_leaves_native_drive_absolute_unchanged(_windows_ntpath):
    assert (
        shapes.translate_msys_path("C:\\Users\\oduffy\\x.txt")  # abs-path-ok: brief-pinned AC2 example (C1 brief), synthetic
        == "C:\\Users\\oduffy\\x.txt"  # abs-path-ok: brief-pinned AC2 example (C1 brief), synthetic
    )
    assert shapes.translate_msys_path("C:/Users/oduffy/x.txt") == "C:/Users/oduffy/x.txt"  # abs-path-ok: brief-pinned AC2 example (C1 brief), synthetic
...
def test_resolve_relative_posix_relative_target(_posix_host):
    assert shapes.resolve_relative("/x/project-makima", "scratch/t.txt") == (
        "/x/project-makima/scratch/t.txt"
    )
...
def test_resolve_relative_posix_absolute_target(_posix_host):
    assert (
        shapes.resolve_relative("/x/project-makima", "/x/other/t.txt")
        == "/x/other/t.txt"
    )
...
def test_resolve_relative_posix_never_returns_none(_posix_host):
    for target in ["/tmp/x", "/x/foo", "/c/Users/x", "relative/path.txt", ""]:  # abs-path-ok: same AC4 fixture set, synthetic
        assert shapes.resolve_relative("/x/project-makima", target) is not None
...
def test_resolve_relative_untranslatable_target_returns_none(_windows_ntpath, target):
    assert shapes.resolve_relative("/x/project-makima", target) is None
