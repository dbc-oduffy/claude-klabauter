...
class TestSubstrateTable:
    ...
    def test_canonical_daily_branch_passes(self):
        assert guard.check(_payload("git checkout -b work/delphipro/2026-07-13")) is None

    def test_closeout_suffix_denies(self):
        _reason(guard.check(_payload("git checkout -b work/delphipro/2026-07-13-closeout")))

    def test_numeric_suffix_denies(self):
        _reason(guard.check(_payload("git checkout -b work/delphipro/2026-07-13-2")))

    def test_ci_green_suffix_denies(self):
        _reason(guard.check(_payload("git checkout -b work/delphipro/2026-07-13-ci-green")))
    ...
    def test_mixed_case_daily_branch_denies(self):
        ...
        _reason(guard.check(_payload("git checkout -b work/DELPHIPRO/2026-07-13")))
...
class TestPowerShellIdiomDialectNeutral:
    ...
    def test_canonical_daily_branch_still_passes_under_semicolon_chain(self):
        assert guard.check(
            _payload("Get-Location; git checkout -b work/delphipro/2026-07-13")
        ) is None
