...
Genesis: coordinator:staff-eng (Patrik) review, DoE-claude
...
the moment a migration (Patrik's proposed ``_command_position.py`` layer)
...
XFAIL_BYPASSES: Dict[Tuple[str, str], str] = {
    ...
    # cells below are the live remainder of Patrik's Finding 0.
    ("no-verify", "setsid_wrapper"): (
        "LIVE BYPASS: check_no_verify does not recognize `setsid` as a "
        "passthrough wrapper -- Patrik staff-eng review 2026-07-29 Finding 0"
    ),
    ...
    ("destructive-rm", "setsid_wrapper"): (
        ...
        "same class as Patrik staff-eng review Finding 0/2"
    ),
    ...
    ("destructive-git-clean", "setsid_wrapper"): (
        ...
        "2026-07-29, same class as Patrik staff-eng review Finding 0/2"
    ),
    ("destructive-git-clean", "busybox_wrapper"): (
        ...
        "corpus 2026-07-29, same class as Patrik staff-eng review Finding 0/2"
    ),
    ...
    # Patrik's Finding 3 -- this is the guard that should have caught the
    ...
    ("runaway-find", "setsid_wrapper"): (
        ...
        "include `setsid` -- Patrik staff-eng review 2026-07-29 Finding 3, "
        "verified live (`setsid find` allows)"
    ),
    ("runaway-find", "busybox_wrapper"): (
        "LIVE BYPASS: check_runaway_find's `_FIND_WRAPPER_WORDS` does not "
        "include `busybox` -- Patrik staff-eng review 2026-07-29 Finding 3, "
