...
def test_ac11_enumerates_all_repos_star_entries_not_just_two_named_keys(tmp_path, monkeypatch):
    """Widened per the plan's Zoli finding 12 -- ANY `repos.*` key counts,
    not only `repos.doe_claude` / `repos.project_makima`."""
    reg_dir = tmp_path / "registry"
    ...
    _write_registry(
        ...
        project_makima=str(tmp_path / "makima-placeholder"),
...
def test_ac1_target_is_publish_destination_false_for_every_repos_value(tmp_path, monkeypatch):
    ...
    reg_dir = tmp_path / "registry"
    project_makima_root = tmp_path / "project-makima"
    project_makima_root.mkdir()
    ...
    _write_registry(
        reg_dir,
        project_makima=str(project_makima_root),
    ...
    assert applicability.target_is_publish_destination(str(project_makima_root)) is False
...
def test_d4_publish_destination_owner_reads_the_merged_view_across_both_files(
    ...
    assert applicability.publish_destination_owner(str(klabauter_root)) == "project-makima-em"
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "un-run there. Owner: project-makima-em (chunk C9a, "
