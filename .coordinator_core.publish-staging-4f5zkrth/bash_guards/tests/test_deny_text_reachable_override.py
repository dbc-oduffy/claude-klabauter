...
def test_deny_reason_grant_tie_branch_stays_within_word_budget() -> None:
    ...
    ("project-makima " + the relative path, 2 words), then
    `_resolve_override_keys_doc_display()` upgraded it to resolve the
    makima root (env var, then the `.makima-root` pointer file -- never the
    ...
    (no space inside a path, vs. "project-makima " + the relative path being
