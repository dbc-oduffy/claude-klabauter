...
`.makima-root` pointer file), falling back to the repo-qualified hint
...
"project-makima docs/reference/guard-override-keys.md"). This module pins
...
class TestOverrideKeysDocDisplayIsPortable:
    ...
    def test_constant_is_repo_qualified_relative_form(self) -> None:
        assert h.OVERRIDE_KEYS_DOC_DISPLAY == "project-makima " + h.OVERRIDE_KEYS_DOC
    ...
    @pytest.mark.parametrize(
        "makima_root_env",
        [
            "/Users/someoperator/project-makima",
            r"C:\Users\someoperator\project-makima",
            "/home/ci-runner/checkouts/makima",
        ],
    )
    def test_resolver_ignores_machine_root_state(
        self, monkeypatch: pytest.MonkeyPatch, makima_root_env: str
    ) -> None:
        ...
        regardless of what `MAKIMA_ROOT` names is the direct regression test
        ...
        does."""
        monkeypatch.setenv("MAKIMA_ROOT", makima_root_env)
        assert h._resolve_override_keys_doc_display() == h.OVERRIDE_KEYS_DOC_DISPLAY
...
class TestOperatorOverrideNoteIsPortable:
    ...
    def test_note_is_stable_regardless_of_machine_root_state(
        ...
        monkeypatch.setenv("MAKIMA_ROOT", "/Users/someoperator/project-makima")
        assert (
