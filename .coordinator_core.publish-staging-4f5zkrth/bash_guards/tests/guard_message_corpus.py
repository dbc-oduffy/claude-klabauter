...
def _branch_set_precedence_setup(
    ...
    mp.setattr(
        ...
        lambda cwd=None: [("work/delphipro/2026-07-31", fixed_now - 3600)],
    )
    ...
    return {
        _CMD_OVERRIDE_KEY: "git checkout -b work/delphipro/2026-08-03",
...
ADVISORY_REWRITE_ROWS: List[CorpusRow] = [
    ...
    CorpusRow(
        ...
        "git checkout -b work/delphipro/2026-08-03",
        True,
        _REWRITE,
        False,
...
from coordinator_core.hooks import project_rag_detect as _hook_project_rag_detect
...
def _fire_project_rag_detect() -> Optional[Dict[str, Any]]:
    with tempfile.TemporaryDirectory(prefix="guard-message-corpus-hooks-prd-") as scratch:
        ...
        banner = _hook_project_rag_detect.detect_banner(str(scratch_dir))
        if not banner:
            return None
        return {"hookSpecificOutput": {"additionalContext": banner}}


def _fire_project_rag_detect_control() -> Optional[Dict[str, Any]]:
    with tempfile.TemporaryDirectory(prefix="guard-message-corpus-hooks-prd-ctrl-") as scratch:
        banner = _hook_project_rag_detect.detect_banner(scratch)
        if not banner:
            return None
        return {"hookSpecificOutput": {"additionalContext": banner}}
...
HOOK_ROWS: List[HookRow] = [
    ...
    HookRow("project_rag_detect", "fire-uninitialized", True, _fire_project_rag_detect),
    HookRow("project_rag_detect", "control-no-marker", False, _fire_project_rag_detect_control),
...
#       `postuse_advisory_dispatch`, `project_rag_detect`,
...
# is therefore the makima-side upper bound on the shim-relayed surface, not
