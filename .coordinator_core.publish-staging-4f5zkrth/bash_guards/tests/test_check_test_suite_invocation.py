...
def test_classify_command_core_chained_fast_test_cmd_classifies_both_segments_as_tier_f():
    """Regression (2026-07-25 opticon Tier-F-unreachable report), UPDATED
...
def test_matches_configured_cmd_containment_for_a_chained_configured_command():
    """Regression (2026-07-25 opticon Tier-F-unreachable report): a chained
...
class TestGrantLeg:
    ...
    def test_em_tier_f_chained_no_grant_denied_as_tier_u(self, grant_repo, free_mutex, monkeypatch):
        """Regression (2026-07-25 opticon Tier-F-unreachable report),
...
# coordinator.local.md. This is the fix for the live repro: makima's own
...
class TestR6DeclaredUnscopedFastTier:
    def test_declared_unscoped_fast_tier_verbatim_allowed_no_grant(
        ...
        """The live repro: makima's own configured fast_test_cmd, invoked
        VERBATIM by the top-level EM with the declaration present and no
        Tier-U grant on disk, is allowed."""
        monkeypatch.setattr(
            guard, "_configured_test_cmds",
            lambda root: [guard.ConfiguredCmd("fast_test_cmd", _MAKIMA_FAST_TEST_CMD, 0)],
        )
        monkeypatch.setattr(
            guard, "_fast_tier_unscoped_declaration",
            lambda root: "marker-based fast/full split; no path subset is meaningful",
        ...
        assert guard.check(
            _payload("with-suite-mutex -- " + _MAKIMA_FAST_TEST_CMD, grant_repo)
        ) is None
    ...
    def test_absent_declaration_denies_the_same_unscoped_fast_test_cmd(
        ...
        out = guard.check(_payload(_MAKIMA_FAST_TEST_CMD, grant_repo))
        reason = _reason(out)
        assert "Tier-U" in reason
        assert "authorization grant" in reason
    ...
    def test_superset_invocation_still_denied_despite_declaration(
        ...
        superset = _MAKIMA_FAST_TEST_CMD + " --extra-unscoped-thing"
    ...
    def test_declared_unscoped_fast_tier_never_widens_subagent_rung(
        ...
        out = guard.check(_payload(_MAKIMA_FAST_TEST_CMD, grant_repo, agent_id=_AGENT_ID))
        reason = _reason(out)
        assert reason.startswith("Run the tests you actually touched:")
    ...
    def test_declaration_never_covers_full_test_cmd(
        ...
        monkeypatch.setattr(
            guard, "_configured_test_cmds",
            lambda root: [guard.ConfiguredCmd("full_test_cmd", _MAKIMA_FAST_TEST_CMD, 0)],
        )
        monkeypatch.setattr(
            guard, "_fast_tier_unscoped_declaration",
            lambda root: "marker-based fast/full split; no path subset is meaningful",
    ...
    def test_chained_declared_fast_test_cmd_verbatim_still_reaches_grant_exit(
        ...
        chained command (opticon's real case: ``pnpm run typecheck && pnpm
        ...
        ``cross-repo/archive/2026-07-25-project-opticon-em-tier-f-escape-
    ...
    def test_chained_declared_fast_test_cmd_plus_scoped_extra_segment_still_denied(
        ...
        chained = _MAKIMA_FAST_TEST_CMD + " && " + _MAKIMA_FAST_TEST_CMD + " tests/test_x.py"
        out = guard.check(_payload(chained, grant_repo))
        reason = _reason(out)
        assert out is not None
        assert "authorization grant" in reason
...
class TestConfiguredCmdReachability:
    ...
    grant required of the EM) purely because it names a directory:
      ...
      1. the resolver was loaded only from ``<repo_root>/coordinator/bin/``,
         which exists in project-makima alone -- every other repo silently
    ...
    def narrowing_repo(self, tmp_path, monkeypatch):
        """market-intelligence's real shape (observed 2026-07-30): the fast
...
def test_r9_deny_instructs_reporting_the_substitution_to_the_dispatcher(
    ...
    """Inbound memo gap (market-intelligence-em, 2026-07-28): when R9 denies
...
# R9 -- unexpanded-glob routearound close (2026-08-03, project-rag Finding 2)
#
...
# fleet shapes named in the incident report (project-rag-ue-addon's
# ``bin/run-fast-tests.py``/``bin/run-full-test-suite.py --yes``, and
# project-rag's ``run_tier_tests.py``, whose static-regex token was removed
# by this fix).
# ---------------------------------------------------------------------------
...
class TestDynamicPrefilterLeg:
    ...
    def ue_addon_repo(self, tmp_path, monkeypatch):
        """project-rag-ue-addon's real ``coordinator.local.md`` shape."""
        (tmp_path / ".git").mkdir()
        (tmp_path / "coordinator.local.md").write_text(
            "---\n"
    ...
    def test_project_rag_run_tier_tests_denies_without_static_token(
            self, tmp_path, monkeypatch, free_mutex):
        """project-rag's shape, with the hand-added ``run_tier_tests`` static
