...
def _clean_bump_env(monkeypatch, tmp_path):
    ...
    subdirectory rather than `delenv`'d (binds AC13, DoE finding-#6-adjacent
    ...
    project-makima/`. Isolated per-test via `tmp_path`, matching every
    ...
    `gettempdir()` patch. DoE finding #6 describes this exact hazard as
...
def _set_anchor(monkeypatch, env, session_id: str, extra: dict | None = None) -> None:
    ...
    (DoE finding #5 / AC5). Mirrors `test_bump_foreign_repo_write._set_anchor`
...
def test_agent_memory_store_write_never_bumps(env, monkeypatch):
    ...
    _set_anchor(monkeypatch, env, session_id)
    memory_dir = env["home"] / ".claude" / "projects" / "-Users-oduffy-X-some-project" / "memory"
