...
def _build_stub_tree(tmp_path: Path) -> Dict[str, Path]:
    ...
    # `makima_root` is a SEPARATE fixture tree from `coord_root` -- the
    # executable `bin/` surface (`claude-doe`, `gen-makima-root-pointer.py`)
    # migrated wholesale to project-makima in commit `b644d5a9` (2026-07-22),
    # so production code now resolves these two paths under
    # `<makima_root>/coordinator/bin/...`, distinct from the DoE clone's
    ...
    # distinguish "resolved coord_root" from "resolved makima_root" by
    # accident.
    makima_root = tmp_path / "makima"
    (makima_root / "coordinator" / "bin").mkdir(parents=True)

    # gen-makima-root-pointer.py is invoked as `python3 <path>` via a real
    # subprocess (Step 3.5a.1b, advisory, out of C13's scope) -- must be
    # valid Python, not a shell stub.
    makima_pointer = makima_root / "coordinator" / "bin" / "gen-makima-root-pointer.py"
    makima_pointer.write_text(
        "import os, sys\n"
        'with open(os.environ["CALL_LOG"], "a") as f:\n'
        '    f.write("gen-makima-root-pointer.py " + " ".join(sys.argv[1:]) + "\\n")\n'
        'sys.exit(int(os.environ.get("RC_GEN_MAKIMA_ROOT_POINTER_PY", "0")))\n'
    )
    ...
    wrapper_src = makima_root / "coordinator" / "bin" / "claude-doe.py"
...
def test_full_success_returns_zero_and_calls_every_phase_in_order(stub_env):
    rc = maximalist.run(
        ...
        makima_root=str(stub_env["makima_root"]),
...
def test_claude_doe_wrapper_missing_source_is_fatal(stub_env):
    (stub_env["makima_root"] / "coordinator" / "bin" / "claude-doe.py").unlink()
    rc = maximalist.run(
        check_only=False,
        non_interactive=True,
        coord_root=str(stub_env["coord_root"]),
...
# imports THIS repo's coordinator_core.install.gen_settings_hooks -- so makima
...
def _real_makima_clone_root() -> Path:
...
def test_seed_project_makima_argv_composed_with_key_and_derived_path(stub_env, monkeypatch):
    ...
    expected_root = _real_makima_clone_root()
    assert (expected_root / "coordinator_core").is_dir()
    makima_seed_calls = [c for c in captured if "repos.project_makima" in c]
    assert makima_seed_calls == [
        ["fake-ml-cli", "set", "repos.project_makima", str(expected_root)]
    ]


def test_seed_project_makima_check_only_does_not_invoke_machine_local(stub_env, monkeypatch):
    ...
    assert [c for c in captured if "repos.project_makima" in c] == []


def test_seed_project_makima_machine_local_absent_degrades_to_note(stub_env, monkeypatch, capsys):
    ...
    other lookup, e.g. gen-makima-root-pointer.py's own python3/python probe,
    ...
    assert "repos.project_makima" in err


def test_seed_project_makima_missing_coordinator_core_warns_and_skips(stub_env, monkeypatch, tmp_path, capsys):
    ...
    fake_file = tmp_path / "not-makima" / "some-unrelated-bin" / "install" / "maximalist.py"
    ...
    # discriminating assertion is "no repos.project_makima _run call" (below),
...
def test_env_var_propagated_to_subprocess_phase(stub_env, monkeypatch, tmp_path):
    ...
    # Rewrite the real subprocess-invoked phase (gen-makima-root-pointer.py)
    ...
    makima_pointer = stub_env["makima_root"] / "coordinator" / "bin" / "gen-makima-root-pointer.py"
    makima_pointer.write_text(
        "import os, sys\n"
