...
Spec backlink: pln-makima-pure-python-shop-retire-0f8aee § C12
"""
...
#        makima link/import failed before this module's own main() could
...
def _seed_machine_local_registry(confirm: bool, non_interactive: bool) -> None:
    ...
    itself only a polyglot trampoline back onto that same makima module (see
    ...
    the load-bearing ``repos.project_makima`` entry itself -- that
    ...
    try:
        makima_root_str, _resolution_class = coordinator_engine_root_with_class()
        makima_root = Path(makima_root_str)
    except RuntimeError as exc:
        print(f"[post-toolchain] WARNING: cannot resolve MAKIMA_ROOT to locate machine-local: {exc}", file=sys.stderr)
...
# ONLY if it runs the SAME `_resolve_makima_root` -> `_resolve_claude_klabauter_root`
...
# (git unavailable, dirty makima tree failing publish's own dirty-tree gate,
...
# bounds is `coordinator/bin/publish.py` — makima's own compute — so it is
...
def _register_engine_key(key: str, value: str) -> bool:
    ...
    `<makima_root>/coordinator/bin/machine-local` — a binary deleted in
...
def provision_stamped_engine(
    makima_root: Path, timeout: int = _PUBLISH_ROUND_ADVISORY_BUDGET_SECS
) -> bool:
    ...
    project-makima is even cloned -- see the engine-root resolution guard
    below) and `scripts/setup.py`'s `register_makima_root` (project-makima's
    own AUTHORITATIVE registration surface, which is where this reliably has
    a resolved `makima_root` to work with).
    ...
    publish_script = makima_root / "coordinator" / "bin" / "publish.py"
...
def _run_post_toolchain_steps(plugin_root: Path, args: _Args) -> int:
    ...
    # DoE-claude CLAUDE_PLUGIN_ROOT entirely and into makima's OWN checkout
    ...
    # must resolve off coordinator_makima_root(), never plugin_root. Unlike
    # site 3 below, an unresolvable engine root here is fail-loud: this is
    # install-path code and a broken/absent makima checkout at this point
    ...
    try:
        makima_root_for_preflight_str, _resolution_class = coordinator_engine_root_with_class()
        makima_root_for_preflight = Path(makima_root_for_preflight_str)
    except RuntimeError as exc:
        print(f"[post-toolchain] ERROR: cannot resolve the engine root for setup preflight: {exc}", file=sys.stderr)
        return EXIT_FAIL

    coordinator_tree_root = makima_root_for_preflight / "coordinator"
    if coordinator_tree_root.is_dir():
        ...
        print(
            ...
            f"(resolved engine root={makima_root_for_preflight}).",
            file=sys.stderr,
        )
    ...
    # fresh box this usually no-ops here (project-makima is not yet cloned,
    # so the engine root is unresolvable) -- `scripts/setup.py`'s own
    # `register_makima_root` is the AUTHORITATIVE call site for that case
    # and calls the same function once project-makima's own installer runs.
    # This call site exists for the re-run/already-registered case.
    try:
        makima_root_for_engine_str, _resolution_class = coordinator_engine_root_with_class()
        provision_stamped_engine(Path(makima_root_for_engine_str))
    except RuntimeError:
        print(
            "[post-toolchain] Skipping stamped-engine provisioning: the engine root not yet "
            "resolvable (project-makima not cloned yet). scripts/setup.py provisions it "
            "once project-makima is installed.",
        )
