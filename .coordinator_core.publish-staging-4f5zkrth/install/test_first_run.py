...
def test_run_post_toolchain_never_calls_ensure_venv(tmp_path, monkeypatch):
    ...
    makima_root.mkdir(parents=True)
...
def test_run_post_toolchain_preflight_calls_setup_chain_walker_in_process(tmp_path, monkeypatch):
    ...
    when the makima-side `coordinator/` tree is present (post b644d5a9
    ...
    coordinator_makima_root() -- never plugin_root -- and must NOT spawn
    bash. Formerly `bash <plugin_root>/scripts/setup.sh --preflight`."""
    plugin_root = tmp_path / "coordinator"
    (plugin_root / "bin").mkdir(parents=True)
    (plugin_root / "scripts").mkdir(parents=True)
    makima_root = tmp_path / "makima"
    coordinator_tree_root = makima_root / "coordinator"
...
def test_run_post_toolchain_preflight_failure_is_non_fatal(tmp_path, monkeypatch, capsys):
    ...
    (plugin_root / "scripts").mkdir(parents=True)
    makima_root = tmp_path / "makima"
    (makima_root / "coordinator").mkdir(parents=True)
    monkeypatch.setenv("MAKIMA_ROOT", str(makima_root))
...
def test_seed_registry_missing_makima_root_warns_and_returns(monkeypatch, capsys):
    """MAKIMA_ROOT unresolvable -> WARNING + manual-registration hint
    ...
    def _raise(*a, **k):
        raise RuntimeError("simulated MAKIMA_ROOT resolution failure")
    ...
    err = capsys.readouterr().err
    assert "cannot resolve MAKIMA_ROOT to locate machine-local" in err
...
def test_seed_registry_happy_path_writes_expected_entries(monkeypatch, tmp_path, capsys):
    ...
    out = capsys.readouterr().out
    assert "Registering repos.project_makima = /x/project-makima" in out
    ...
    assert "\"repos.project_makima\" = '/x/project-makima'" in content
    assert "\"repos.doe_claude\" = '/x/DoE-claude'" in content

    from coordinator_core.machine_resolver import registry_get

    assert registry_get("repos.project_makima") == "/x/project-makima"
...
def test_seed_registry_write_failure_warns_and_continues(monkeypatch, tmp_path, capsys):
    ...
    err = capsys.readouterr().err
    assert "failed to register repos.project_makima" in err
...
def test_journal_records_registered_repo_keys(monkeypatch, tmp_path, _journal_env):
    ...
    assert keys == {"repos.project_makima", "repos.doe_claude"}
...
def test_journal_omits_failed_registration_from_written_entries(monkeypatch, tmp_path, _journal_env):
    ...
    assert resolution is not None
    keys = {e.key for e in resolution.entries}
    assert keys == {"repos.project_makima"}
...
def test_journal_empty_entries_on_missing_makima_root(monkeypatch, _journal_env, capsys):
    """MAKIMA_ROOT unresolvable is a definitive 'resolved to nothing' for
