...
family (machine-local family, claude-home family, resolve-makima family,
...
def test_sample_forwarders_execute_and_resolve_their_real_target(tmp_path, monkeypatch):
    ...
    # A written forwarder's runtime resolution ladder (_resolve_makima.py,
    # exec'd BY the forwarder as a fresh subprocess) is independent of this
    # install pass's MAKIMA_ROOT env-var shortcut -- it consults the
    # settings-home machine-local registry / `.makima-root` sentinel, per its
    ...
    # project-makima" -- an unstated prereq this test surfaces by supplying
    ...
    (ml_dir / ".makima-root").write_text(str(_REPO_ROOT) + "\n", encoding="utf-8")
