"""
coordinator_core.install.test_resolve_makima_publisher_only — keeps
``_resolve_makima.PUBLISHER_ONLY_TARGETS`` honest against what
``coordinator/bin/`` actually imports.

Why this file exists. ``resolve_makima_root_with_class()`` diverts any
session whose own repo root is not the live makima checkout to the published
...
naming them is hand-maintained inside ``_resolve_makima.py`` because that
...
percolate-dispatching CLI cannot ship silently broken for every non-makima
session on the box.
...
boxes that legitimately have no live makima checkout. Both directions are
asserted.
...
_MODULE_PATH = _REPO_ROOT / "coordinator" / "lib" / "resolve-makima" / "_resolve_makima.py"
_BIN_DIR = _REPO_ROOT / "coordinator" / "bin"

_spec = importlib.util.spec_from_file_location("_resolve_makima_under_test_publisher_only", _MODULE_PATH)
...
def test_publisher_only_targets_matches_what_bin_actually_imports():
    ...
    for every non-makima session and dies on an import that cannot succeed.
    ...
    declared = set(resolve_makima.PUBLISHER_ONLY_TARGETS)
...
@pytest.mark.parametrize("name", sorted(resolve_makima.PUBLISHER_ONLY_TARGETS))
...
def test_predicate_accepts_bare_and_suffixed_spellings(spelling):
    ...
    assert resolve_makima._is_publisher_only_target(spelling) is True
...
def test_predicate_rejects_ordinary_targets(spelling):
    ...
    assert resolve_makima._is_publisher_only_target(spelling) is False
