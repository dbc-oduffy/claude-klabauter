...
def _fake_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        mod, "coordinator_engine_root_with_class", lambda: ("/resolved/makima/clone", "live-working-tree")
    )
...
def test_first_invocation_appends_block(tmp_path):
    ...
    assert 'export MAKIMA_CLONE="/resolved/makima/clone"' in text
...
def test_makima_root_resolution_failure_skips_write_without_raising(tmp_path, monkeypatch):
    def _raise():
        raise RuntimeError("MAKIMA_ROOT unresolvable")

    monkeypatch.setattr(mod, "coordinator_engine_root_with_class", _raise)
    result = write_shell_rc_guard_block(check_only=False)
    assert result["modified"] is False
    assert result["already_present"] is False
...
def test_relocation_self_heal_with_unterminated_stale_block_does_not_eat_rc_file(tmp_path):
    ...
    rc_path = tmp_path / ".zshrc"
    begin, end = mod._sentinel_markers("MAKIMA_CLONE")
    ...
    result = mod._write_block_to_file(
        rc_path, begin, end, 'export MAKIMA_CLONE="/resolved/makima/clone"', check_only=False, label="MAKIMA_CLONE",
    )
...
# --- Finding 2 (P1, shell injection): path_entry / makima_clone must be
...
def test_makima_clone_body_escapes_metacharacters(tmp_path, monkeypatch):
    monkeypatch.setattr(
        mod, "coordinator_engine_root_with_class", lambda: ('/tmp/$(touch /tmp/PWNED)', "live-working-tree")
    )
    result = write_shell_rc_guard_block(check_only=False)
    assert result["modified"] is True
...
def test_journal_records_empty_tuple_on_makima_root_resolution_failure(monkeypatch, _journal_env):
    def _raise():
        raise RuntimeError("MAKIMA_ROOT unresolvable")
