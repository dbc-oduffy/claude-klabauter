...
# disk in project-rag/pyproject.toml and claude-unreal-holodeck/pyproject.toml).
...
# `project-rag-symbol-extract` lives under project-rag/packaging/symbol-
...
# on. Excluded here specifically because project_makima's own `[symbols]`
...
# extra.
_FIRST_PARTY_EXTRA_NAMES: Tuple[str, ...] = ("project-rag-symbol-extract",)
...
PARITY_LOCKSTEP_GROUPS: Tuple[Dict[str, object], ...] = (
    {
        ...
        "members": (
            ("claude_unreal_holodeck", "scripts/lib/pypi-overrides.txt"),
            ("project_rag", "project_rag_scripts/pypi-overrides.txt"),
        ),
        "rule": (
            "claude-unreal-holodeck scripts/lib/pypi-overrides.txt header: "
            "'Parity peer: ../project-rag/project_rag_scripts/pypi-overrides.txt "
    ...
    },
    {
        ...
        "rule": (
            "project-rag project_rag_scripts/pypi-overrides.txt header: "
...
def _flatten_pyproject_deps(pyproject: dict) -> List[str]:
    ...
    Extras must not be dropped — the fleet's one real conflict (project-rag's
...
def _first_party_sibling_names(rows: Sequence[Dict[str, str]]) -> Set[str]:
    ...
    key with underscores turned to dashes, e.g. ``project_rag`` ->
    ``project-rag``) — used to drop a manifest's bare self-reference to a
    sibling contributing repo (e.g. project-rag-ue-addon's undecorated
    ``project-rag`` dependency line), the same "one first-party entry"
...
def _parity_member_specs(
    ...
    holodeck's ``gpu_sidecar/requirements.txt:10`` torchvision mention a
    non-declaration.
    """
...
def check_parity_lockstep(
    ...
    rot this exists to catch, and it has already happened once — project-rag's
    ...
    NOT hold in lockstep. project-rag pins torchvision ``~=0.27.0`` in
