...
project-rag-ue-addon): `prereq_probe.py` + `manifest_reader.py` +
...
two makima-native additions, and returns the list of lines. main(argv) is
a thin CLI parity aid (mirrors the bash
file's own standalone `if [[ BASH_SOURCE == 0 ]]` entrypoint) — NOT a
registered op; this module has no makima-mutating side effect (read-only
...
try:
    ...
    # project-rag-ue-addon) whose only interest is the probe functions
...
def probe_ue() -> str:
    ue_root = os.environ.get("HOLODECK_UE_ROOT")
    if ue_root:
        found = _ue_check_dir(ue_root)
        if found:
            return emit_line("ue", "pass", "advisory", f"UnrealEditor found via HOLODECK_UE_ROOT: {found}", "")
        return emit_line(
            "ue", "warn", "advisory",
            f"HOLODECK_UE_ROOT is set but UnrealEditor not found under it: {ue_root}",
