...
and ``machine-local/.makima-root`` each land via their own install step, with
...
``makima.settings_home.complete`` doctor probe (bin/makima-doctor-probe.py)
share one oracle instead of two that can drift apart.

A forwarder COUNTS only when its body is the one this makima root's
...
count reports that state green -- it was green on delphipro on 2026-08-22
...
- ``coordinator-claude coordinator/docs/wiki/machine-local-registry.md``
  ...
  while a machine-local interpreter pin still names it -- makima installs it
  only under ``--allow-venv-fallback``. See ``_FIXED_MEMBERS``.
- ``coordinator-claude coordinator/docs/install/AGENT.md`` § Fail-loud
  makima resolution names ``<settings-home>/machine-local/.makima-root`` as
...
member makima itself is responsible for installing
...
# shape plus makima's one additive member. kind is "dir" or "file".
#
...
# PURPOSE -- only when makima's own completeness check has grounds to demand it.
...
_FIXED_MEMBERS: tuple[tuple[str, str, str, str, bool], ...] = (
    ...
    (
        ...
        "dir",
        "DoE machine-local-registry.md §4e settings-home inventory; "
        "makima INSTALL.md § Dependency provisioning item 5 (--allow-venv-fallback)",
        False,
    ),
    (
    ...
    (
        "machine-local/.makima-root (fail-loud rung-2 pointer)",
        "machine-local/.makima-root",
        "file",
        "DoE docs/install/AGENT.md § Fail-loud makima resolution, rung 2",
        True,
    ),
    (
        ...
        "file",
        "makima coordinator_core/install/substrate.py :: _install_bin_resolvers, rm_family",
        True,
    ),
    (
        ...
        ".percolate-identity",
        "file",
        "makima scripts/setup.py :: install_percolate_identity (additive, not a DoE member)",
        True,
    ),
)
...
def expected_forwarders(makima_root: Path) -> dict[str, str]:
    ...
    (bin/makima-doctor-probe.py), emits a pure-JSON envelope on stdout by
...
def forwarder_body_is_ours(path: Path, target: str) -> bool:
    """True iff `path` holds the forwarder THIS makima root's installer
    ...
    shim (`_resolve_claude_klabauter` rather than `_resolve_makima`) or
...
def check_settings_home(settings_home_path: Path, makima_root: Path) -> SettingsHomeReport:
    ...
    try:
        expected = expected_forwarders(makima_root)
...
def format_report_lines(report: SettingsHomeReport) -> list[str]:
    ...
    because DoE named the namespace, not demanded because makima installs it.
    """
    lines: list[str] = []
