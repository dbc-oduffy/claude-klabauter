...
_INSTALLED_VERSIONS: Dict[tuple, str] = {
    ...
    # line 32 (holodeck's transformers 5.3 uses is_offline_mode) and the
    # C7 dispatch brief naming project-rag as the 5.9.0 side.
    ("project_rag", "transformers"): "5.9.0",
    ("claude_unreal_holodeck", "transformers"): "5.3.0",
    # survey line 272: "fifa has transformers 5.10.2 installed".
    ("fifa_26wcle_data", "transformers"): "5.10.2",
    # survey headline table (§ "Torch — one build, triplicated" /
    # Python-version-spread table): project-rag and holodeck's main
    # .venv both carry 2.12.0+cu130.
    ("project_rag", "torch"): "2.12.0+cu130",
    ("claude_unreal_holodeck", "torch"): "2.12.0+cu130",
    ...
    ("claude_unreal_holodeck", "huggingface-hub"): "0.36.2",
    # survey § "Declared-vs-installed divergences" — project-rag's own
    ...
    ("fifa_26wcle_data", "scipy"): "1.17.1",
    ("market_intelligence", "scipy"): "1.18.0",
}
...
def test_huggingface_hub_flagged_as_major_advance_against_holodeck():
    """The one advance the C7 dispatch brief names explicitly as already
    known and contested: holodeck's `gpu_sidecar/requirements.txt` caps
    ...
    major-version advance against holodeck, not silently assumed."""
    entries = build_pin_advance_report()
    matches = [
        e
        for e in entries
        if _normalize(e.package) == "huggingface-hub"
        and e.repo == "claude_unreal_holodeck"
    ]
    assert matches, "expected holodeck's own huggingface_hub declaration in the report"
    entry = matches[0]
...
def test_print_pin_advance_report(capsys):
    ...
    report = render_pin_advance_report(entries)
    print(report)
    assert "claude_unreal_holodeck" in report
