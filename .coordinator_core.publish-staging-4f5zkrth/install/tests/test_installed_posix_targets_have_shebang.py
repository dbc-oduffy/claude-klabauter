...
def test_the_wrapper_source_maximalist_installs_is_the_one_this_guard_checks():
    ...
    source = (_MAKIMA_ROOT / "coordinator_core" / "install" / "maximalist.py").read_text(
        encoding="utf-8"
    )
    assert 'os.path.join(makima_root, "coordinator", "bin", "claude-doe.py")' in source, (
