...
def test_shaped_clause_with_resolution_derives_the_pinned_down_entries() -> None:
    ...
    resolutions = {
        0: ClauseResolution(
            entries=(
                WriteSurfaceEntry(kind="machine-local-key", key="repos.project-makima"),
    ...
    assert entries == (
        ReceiptEntry(writer_id="register-discovered-repos", kind="machine-local-key", key="repos.project-makima"),
