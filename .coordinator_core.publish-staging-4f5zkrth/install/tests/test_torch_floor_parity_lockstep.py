...
Negative spec: project-rag's torchvision pins deliberately diverge between its
...
def test_comment_only_mention_is_not_a_declaration(tmp_path, monkeypatch):
    """holodeck's gpu_sidecar/requirements.txt names torchvision only inside a
...
def test_moved_peer_file_fails_loud(tmp_path, monkeypatch):
    """The rot this check exists to catch. project-rag's own header already
