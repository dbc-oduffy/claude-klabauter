...
def test_klabauter_front_case_composes_current_engine_clone(tmp_path, monkeypatch):
    ...
    checkout (which is deliberately unstamped — the makima-dev case below),
...
def test_makima_dev_case_self_unstamped_falls_to_none(tmp_path, monkeypatch):
    """AC — the makima-dev case: no published root registered, and the
