...
makima's own tree): DoE-side surfaces (``templates/``, ``whoami/``,
``schemas/``) still resolve off ``CLAUDE_PLUGIN_ROOT``; makima-side surfaces
(``coordinator/lib/``, ``coordinator/bin/``) resolve off the makima root via
...
Env:
    ...
    MAKIMA_ROOT        — optional; short-circuits engine-root resolution
...
module lives in a different repo (makima) and has no such co-location — the
...
# settings-home is outside the makima repo tree.
GENERATES = []
...
def _load_setup_template_manifest(makima_root: Path):
    """Load ``<makima_root>/coordinator/lib/setup-templates-manifest.py``'s three
    ...
    The manifest lives in makima's OWN ``coordinator/lib/`` tree (b644d5a9's
    executable-surface relocation moved ``lib/`` out of the DoE-claude
    ``CLAUDE_PLUGIN_ROOT`` entirely), so this resolves off ``coordinator_makima_root()``,
    ...
    manifest = makima_root / "coordinator" / "lib" / "setup-templates-manifest.py"
...
def _resolve_bin_templates_manifest_root() -> Path:
    """Resolve the makima root `_load_bin_templates_manifest` reads from,
    when a caller does not already have one on hand.

    Rung 1: co-located — `bin-templates-manifest.py` lives in makima's OWN
    ...
    Rung 2: `coordinator_makima_root()`'s registry chain, for the
    ...
    if (colocated / "coordinator" / "lib" / "bin-templates-manifest.py").is_file():
        return colocated
    _makima_root_str, _resolution_class = coordinator_engine_root_with_class()
    return Path(_makima_root_str)
...
def _load_bin_templates_manifest(makima_root: Path) -> "_BinTemplatesManifest":
    """Load ``<makima_root>/coordinator/lib/bin-templates-manifest.py``'s
    ...
    manifest = makima_root / "coordinator" / "lib" / "bin-templates-manifest.py"
    if not manifest.is_file():
        raise SubstrateFatalError(
...
#
# Placement note (Patrik F4, settled, not negotiable): the tracked-ness
...
def _resolve_directory_tracked_set(dest_dir: Path) -> Optional["FrozenSet[str]"]:
    ...
    this chunk uses is TRACKED-NESS, not repo ownership (Patrik finding 0): a
...
def _install_one(
    ...
    *, force_overwrite: bool = False,
    ...
    executes whichever mechanism it is told to (Patrik F4).
...
_AGENT_FORWARDER_MARKER = "from _resolve_makima import exec_cli"
...
def _write_agent_forwarder(name: str, dst: Path, check_only: bool, *, target: str) -> None:
    """Naked-Python forwarder that resolves and execs the makima-resident
    CLI at ``<makima-root>/coordinator/bin/<target>``, per the ratified
    resolve-makima-bin contract (DoE-claude
    ``coordinator/snippets/resolve-makima-bin.md``, DoE commit ``ad7fb0d1``).
    ...
    of exactly this. See
    cross-repo/inbox/2026-07-23-claude-central-em-makima-pickup-assemble-heads-up.md
    § 0.
    ...
    The ladder
    is extracted ONCE into ``_resolve_makima.py`` (installed alongside every
    ...
    surface into project-makima's own ``coordinator/bin/`` — the forwarder
    ...
    See ``_resolve_makima.py``'s own module docstring for the full ladder
...
# coordinator-claude bin forwarder for {name} — resolves makima's
# `coordinator/bin/` directory via the co-located `_resolve_makima.py`
# shim (the ratified resolve-makima-bin contract, DoE-claude
# coordinator/snippets/resolve-makima-bin.md) and execs `{target}` there.
...
def _write_agent_cmd_forwarder(
    ...
    makima-side ``<target>.py`` directly — the resolve-makima-bin ladder
    must exist in exactly ONE place (``_resolve_makima.py``, exec'd by the
    Unix half), so re-deriving makima-root resolution in batch here would
    ...
    Formerly this ``.cmd`` half was a VERBATIM COPY of makima's own
    ...
    ``_resolve_makima.py``, etc.) plus the generated Unix-half forwarders —
    never the makima-side ``.py``/``.sh`` targets themselves. Every launcher
    ...
    ``target`` is the on-disk filename in makima's ``coordinator/bin/`` this
...
REM Unix-half forwarder ("%~dp0{name}"), which resolves makima's
REM coordinator/bin/ via _resolve_makima.py and execs the real target
...
def _write_agent_ps1_forwarder(
    ...
    makima-side ``<target>.py`` directly, so the resolve-makima-bin ladder
...
# Unix-half forwarder ("$_here/{name}"), which resolves makima's
# coordinator/bin/ via _resolve_makima.py and execs the real target
...
# templates/bin or makima's coordinator/lib/claude-home, NOT from
...
# with a resolve-makima-bin forwarder stub. Some of these names ALSO exist
...
def _derive_agent_helper_target_map(agent_bin: Path) -> "dict[str, str]":
    ...
    from makima's own ``coordinator/bin/`` directory listing. This is the
...
def _sweep_orphaned_agent_helpers(
    ...
    (covers ``_machine_local.py``/``_claude_home.py``/``_resolve_makima.py``)
...
def run(setup_only: bool = False, check_only: bool = False, allow_venv_fallback: bool = False) -> int:
    ...
    # makima's own coordinator/ tree; requiring lib/ here as well would be a
    ...
    # makima-side precondition — coordinator/lib/ and coordinator/bin/ now live
    # in the makima checkout, resolved via the canonical resolver, never by
    # __file__-walking or a hardcoded sibling name.
    try:
        _makima_root_str, _resolution_class = coordinator_engine_root_with_class()
        makima_root = Path(_makima_root_str)
    except RuntimeError as exc:
        print(str(exc), file=sys.stderr)
        return 1

    _write_dispatch_root_bake(makima_root, _makima_root_str, _resolution_class)

    makima_lib = makima_root / "coordinator" / "lib"
    if not makima_lib.is_dir():
        print(
            f"install-substrate: makima-side {makima_lib} not found "
            f"(resolved engine root={makima_root}).\n"
            "The project-makima checkout is broken/incomplete, or repos.project_makima "
            "resolves to the wrong tree. Remediation:\n"
            "  (a) confirm 'machine-local get repos.project_makima' points at a real "
            "project-makima checkout,\n"
            "  (b) git fetch/pull the existing project-makima checkout to restore "
    ...
    for required in (ml_templates, ml_bin, ch_bin, setup_src):
        if not required.is_dir():
            print(
                ...
                "The coordinator plugin install (or the project-makima checkout) is "
                ...
                "  (c) verify COORDINATOR_ENGINE_ROOT / repos.project_makima resolves to a complete "
                "project-makima checkout,\n"
    ...
    try:
        setup_files, setup_exec_files, setup_hook_files = _load_setup_template_manifest(makima_root)
...
# `_CH_FAMILY_FILES` and `_RM_FAMILY_FILES` are sourced from MAKIMA'S OWN
# tree (`coordinator/lib/claude-home/` and `coordinator/lib/resolve-makima/`
...
_RM_FAMILY_FILES = ("_resolve_makima.py",)


def _static_bin_family_names(makima_root: "Optional[Path]" = None) -> "frozenset[str]":
    ...
    `makima_root`: resolved via `_resolve_bin_templates_manifest_root()`
    ...
    already-resolved `makima_root_resolved` explicitly instead of paying
    ...
    """
    root = makima_root if makima_root is not None else _resolve_bin_templates_manifest_root()
...
def _install_bin_resolvers(
    ...
    *, python3_cmd_resolved_bin: str,
    ...
    # the bin-templates manifest (C12) — makima's own coordinator/lib/,
    # loaded below via `makima_root_resolved` — must be available before
    ...
    try:
        _makima_root_resolved_str, _resolution_class = coordinator_engine_root_with_class()
        makima_root_resolved = Path(_makima_root_resolved_str)
    except RuntimeError as exc:
        raise SubstrateFatalError(f"install-substrate: {exc}") from exc

    bin_manifest = _load_bin_templates_manifest(makima_root_resolved)
    ...
    def rm_family(dst_dir: Path, prefix: str) -> None:
        # _resolve_makima.py is installed ONCE per bin dir, alongside every
        # emitted forwarder — see its own module docstring for why the
        # resolve-makima-bin ladder now lives here instead of duplicated
        # inline in each forwarder body.
        _install_one(resolve_makima_lib / "_resolve_makima.py", dst_dir / "_resolve_makima.py", False, prefix, check_only)
    ...
    # `.cmd` twins are sourced from makima's OWN coordinator/bin/, resolved
    ...
    agent_bin = makima_root_resolved / "coordinator" / "bin"
    resolve_makima_lib = makima_root_resolved / "coordinator" / "lib" / "resolve-makima"
    ...
    rm_family(bin_dst, "resolve-makima")
    ...
    all_current_names = (
        _static_bin_family_names(makima_root_resolved)
...
WRITE_SURFACE = WriteSurfaceDeclaration(
    ...
    clauses=(
        ...
        # 3-entry tuple (`_CH_FAMILY_FILES`), sourced from makima's own
        ...
        # Clause 11 — the resolve-makima family: a STATIC 1-entry tuple
        # (`_RM_FAMILY_FILES`), sourced from
        # `coordinator/lib/resolve-makima/` — a distinct source tree/family
