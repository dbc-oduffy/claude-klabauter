...
orchestration body it imports) is makima-resident post-port (DR-047), so
the real precondition to reach ``main()`` here is: makima cloned AND
``repos.project_makima`` registered via machine-local, OR ``MAKIMA_ROOT``
...
Documented divergence from the bash oracle (structural, not a scope-drop):
  ...
  - Phase 3 Step 1 (install-substrate) is called via a **direct in-process
    ...
    oracle's ``env PYTHONPATH=<makima_root> python3 -m
    ...
    oracle's ``machine-local get repos.project_makima`` resolution (whose
    entire purpose was locating a *separate* makima checkout to construct
...
def _registry_get_for_check(key: str) -> Optional[str]:
    ...
    ``test_seed_project_makima_check_only_does_not_invoke_machine_local`` to
...
def _claude_home_cli_argv(*args: str) -> List[str]:
    ...
    This ladder differs from ``bin/makima-doctor-probe.py::_resolve_machine_local``
...
def _resolve_coordinator_live_path() -> str:
    ...
    used to perform before handing off to the (already makima-native)
...
def _install_claude_doe_wrapper(
    ...
    surface migrated wholesale to project-makima in commit ``b644d5a9``
    (2026-07-22), so ``claude-doe`` now lives at
    ``<makima_root>/coordinator/bin/claude-doe.py``, not under the DoE clone.
    ...
    inconsistency. ``makima_root`` is an explicit param (not a
    ...
    wrapper_src = os.path.join(makima_root, "coordinator", "bin", "claude-doe.py")
...
def run(
    ...
    ``makima_root`` is REQUIRED (not defaulted here) -- the phases it feeds
    (``claude-doe`` wrapper install, ``gen-makima-root-pointer.py``) resolve
    ...
    makima checkout and machine-local registry the moment a test omitted the
    ...
    with _env_overlay({}):
        try:
            return _run_body(
                check_only, non_interactive, coord_root, doe_clone, claude_home_dir, makima_root,
                allow_venv_fallback=allow_venv_fallback,
            )
...
def _run_body(
    ...
    *,
    ...
    # -- Phase 3 Step 3 (partial) -- best-effort seed repos.project_makima --
    # Breaks the makima<->coordinator install-order circularity (cross-repo ask
    # 2026-07-22, claude-central-em "Ask 1"): makima-first installs advisory-skip
    # this key in scripts/setup.py::register_makima_root() because machine-local
    ...
    # it here, from the makima clone that is running THIS install, closes the
    # loop from the makima side without waiting on a coordinator-side fix.
    #
    # This block MUST run before Step 3.5a.1b (gen-makima-root-pointer.py) below:
    # that advisory step resolves the makima root via REPO_PROJECT_MAKIMA or
    # `machine-local get repos.project_makima`, which is exactly the read this
    ...
    # This code runs FROM the makima clone (coordinator_core/install/maximalist.py),
    # so the clone root is derived from this file's own location instead.
    makima_clone = Path(__file__).resolve().parents[2]
    if check_only:
        current = _registry_get_for_check("repos.project_makima")
        if current == str(makima_clone):
            orch.skip_note(f"repos.project_makima registry key already seeded and verified ({makima_clone})")
        else:
            orch.skip_note(f"Seed repos.project_makima registry key -- check-only (would seed: {makima_clone})")
    elif not (makima_clone / "coordinator_core").is_dir():
        # Mirrors gen-makima-root-pointer.py's own sanity guard: a bare
        # directory-existence check on the wrong marker cannot distinguish
        # "the makima clone" from "some directory" (an unrelated 18-entry
        ...
        print(
            f"WARN: derived makima clone root {makima_clone} has no coordinator_core/ subdir -- "
            "skipping repos.project_makima seed (best-effort, not fatal)",
            file=sys.stderr,
        )
    else:
        orch.phase_header("Seed repos.project_makima registry key (best-effort)")
        from coordinator_core.install._shared import resolve_machine_local_cli
        ...
        if ml_argv is None:
            ...
            ml_argv = [found] if found else None
        if ml_argv:
            rc = _run([*ml_argv, "set", "repos.project_makima", str(makima_clone)], env=env)
            if rc == 0:
                _verify_registry_seed("repos.project_makima", str(makima_clone))
            else:
                print(
                    "WARN: machine-local set repos.project_makima failed -- gen-makima-root-pointer.py "
                    "(Step 3.5a.1b below) may not resolve this run",
                    file=sys.stderr,
                )
        else:
            print(
                "NOTE: machine-local not yet on PATH in this shell -- repos.project_makima not seeded; "
                "open a new shell and re-run to persist the registry key.",
                file=sys.stderr,
            )
    ...
    # -- Step 3.5a.1b -- gen-makima-root-pointer.py (advisory) --
    # Same migrated-`bin/` bug class as `_install_claude_doe_wrapper`'s
    # `claude-doe` below: this script lives at
    # `<makima_root>/coordinator/bin/gen-makima-root-pointer.py` post
    # b644d5a9, not under the DoE clone's `coord_root/bin/`.
    py_bin = shutil.which("python3") or shutil.which("python")
    if py_bin:
        makima_pointer_args = ["--check-only"] if check_only else []
        orch.run_advisory(
            "gen-makima-root-pointer.py (Step 3.5a.1b -- <settings-home>/machine-local/.makima-root pointer)",
            [
                py_bin,
                os.path.join(makima_root, "coordinator", "bin", "gen-makima-root-pointer.py"),
                *makima_pointer_args,
            ],
            env=env,
        )
        ...
        print(
            "WARN: no python3/python interpreter found on PATH -- skipping gen-makima-root-pointer.py (Step 3.5a.1b)",
            file=sys.stderr,
        )
    ...
    # `bin/` migration, so `coord_root` (not `makima_root`) is correct here.
    ...
    _install_claude_doe_wrapper(coord_root, claude_home_dir, check_only, orch, makima_root, settings_bin)
    ...
    # coordinator_core.install.gen_settings_hooks -- i.e. makima was spawning a
    # cold bash.exe to PATH-probe python to import back into makima. Called
    ...
    try:
        _build_and_persist_receipt(Path(makima_root))
    except Exception as exc:  # noqa: BLE001 -- recording leg must never break the install
...
def main(argv: List[str]) -> int:
    ...
    # `coordinator_core` only ever runs from inside the real makima clone --
    # this module lives at `<makima_root>/coordinator_core/install/maximalist.py`,
    ...
    # inside `run()` itself.
    makima_root = str(Path(__file__).resolve().parents[2])
...
WRITE_SURFACE = WriteSurfaceDeclaration(
    ...
    clauses=(
        ...
        # wrapper binary from `<makima-root>/coordinator/bin/claude-doe.py`
            ...
            entries=(
                WriteSurfaceEntry(
                    ...
                    reason=(
                        ...
                        "<makima-root>/coordinator/bin/claude-doe.py to a "
...
Not declared here (deliberately, not an oversight):
  - The best-effort `repos.doe_claude` / `repos.project_makima`
