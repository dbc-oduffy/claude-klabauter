"""
coordinator_core.install.test_forwarder_trust_guard — resolve-makima-bin
...
``_resolve_makima.py`` ladder module (coordinator/lib/resolve-makima/).

``b644d5a9`` (DoE, 2026-07-22) relocated DoE-claude's entire executable
surface into project-makima's own ``coordinator/bin/`` — the prior forwarder
...
a full replacement covering the resolve-makima-bin ladder instead.

M1 (forwarder-ladder extraction): the ladder moved out of the emitted
forwarder body and into a single shared ``_resolve_makima.py`` module,
installed alongside every forwarder. ``_write_forwarder`` below now also
copies the real on-disk ``_resolve_makima.py`` into the same tmp_path
...
_resolve_makima.py module) to disk and executes the forwarder as a
subprocess against a fixture ``machine-local/`` + fake makima-root tree —
...
Spec backlink:
    DoE-claude coordinator/snippets/resolve-makima-bin.md (DoE commit ad7fb0d1)
...
def _write_forwarder(tmp_path: Path, name: str = "cross-repo-memo", target: str | None = None) -> Path:
    ...
    shutil.copyfile(_RESOLVE_MAKIMA_SRC, tmp_path / "_resolve_makima.py")
    return dst


def _make_makima_fixture(root: Path, target_name: str = "cross-repo-memo", sentinel_executable: bool = True) -> None:
    """Build a fixture makima root: coordinator/bin/ with an executable
    ...
    if sentinel_executable and os.name == "nt":
        # `_resolve_makima.py`'s Windows-side executability probe checks
...
def test_registry_rung_wins_over_sentinel(tmp_path: Path):
    ...
    (the .makima-root sentinel) also resolves — to a DIFFERENT, non-fixture
...
def test_sentinel_only_resolves(tmp_path: Path):
    """With no registry.local.toml at all, the .makima-root sentinel alone
    ...
    sentinel_root = tmp_path / "sentinel-makima"
    _make_makima_fixture(sentinel_root)
    (ml_dir / ".makima-root").write_text(str(sentinel_root), encoding="utf-8")
...
def test_missing_both_fails_loud(tmp_path: Path):
    ...
    assert "cannot resolve project-makima" in result.stderr
    assert "machine-local set repos.project_makima" in result.stderr
...
def test_coordinator_bin_present_but_sentinel_absent_message(tmp_path: Path):
    ...
    assert result.returncode == 1
    assert "stale or partial project-makima migration" in result.stderr
...
def test_py_suffixed_cli_forwarder_execs_py_target(tmp_path: Path):
    ...
    root = tmp_path / "wsc-close-makima"
    _make_makima_fixture(root, target_name="wsc-close.py")
    (ml_dir / ".makima-root").write_text(str(root), encoding="utf-8")
...
def test_write_agent_cmd_forwarder_targets_installed_name_not_stem(tmp_path: Path):
    ...
    `.cmd` destination name and never the makima-side `.py` target. This is
...
def test_forwarder_missing_target_exits_127_without_traceback(tmp_path: Path):
    ...
    root = tmp_path / "missing-target-makima"
    bin_dir = root / "coordinator" / "bin"
    bin_dir.mkdir(parents=True)
    sentinel = bin_dir / "archive-stamp-cli"
    sentinel.write_text("#!/bin/sh\necho SENTINEL\n", encoding="utf-8")
    ...
    if os.name == "nt":
        # See `_make_makima_fixture`'s matching comment: the Windows-side
...
def test_derive_agent_helper_names_excludes_reserved_names(tmp_path: Path):
    ...
    a resolve-makima-bin forwarder stub."""
    agent_bin = tmp_path / "coordinator" / "bin"
...
# cross-repo/inbox/2026-07-23-claude-central-em-makima-
...
def test_forwarder_trailing_slash_and_crlf_normalized(tmp_path: Path):
    ...
    root = tmp_path / "crlf-makima"
    _make_makima_fixture(root)
    (ml_dir / ".makima-root").write_bytes((str(root) + "/\r\n").encode("utf-8"))
...
# makima-side .py at the install destination" break survived undetected.
# This gate runs the REAL install loop into a tmpdir and asserts BOTH halves
# resolve to a real file THERE — not in makima's own coordinator/bin/.
# ---------------------------------------------------------------------------
...
def _make_install_bin_resolvers_fixture(tmp_path: Path) -> "tuple[Path, Path, Path]":
    ...
    ``coordinator/lib/resolve-makima/`` (the makima-side tree,
    MAKIMA_ROOT-pointed), plus ``templates/bin`` (``ml_bin``) and
    ...
    ``_resolve_makima.py`` copy is the REAL module, keeping the fixture
    honest against drift in that module's own name/exclusion behavior.

    Returns ``(makima_root, ml_bin, ch_bin)``.
    """
    makima_root = tmp_path / "fixture-makima"
    agent_bin = makima_root / "coordinator" / "bin"
    resolve_makima_lib = makima_root / "coordinator" / "lib" / "resolve-makima"
    agent_bin.mkdir(parents=True)
    resolve_makima_lib.mkdir(parents=True)
    shutil.copyfile(_RESOLVE_MAKIMA_SRC, resolve_makima_lib / "_resolve_makima.py")
    # `_install_bin_resolvers` loads coordinator/lib/bin-templates-manifest.py
    # (C12) off the resolved makima root before its write loops run — copy
    # the REAL manifest into this fixture root rather than hand-authoring a
    # second copy that could drift from it.
    shutil.copyfile(
        _BIN_TEMPLATES_MANIFEST_SRC,
        makima_root / "coordinator" / "lib" / "bin-templates-manifest.py",
    )
    ...
    return makima_root, ml_bin, ch_bin
...
def test_install_bin_resolvers_agent_helper_pairs_resolve_at_destination(
    ...
    the co-located Unix-half sibling, never the makima-side .py file).
    ...
    which is never present at bin_dst — only inside makima's
    own coordinator/bin/. The generator fix makes it pass by construction.
    """
    makima_root, ml_bin, ch_bin = _make_install_bin_resolvers_fixture(tmp_path)
    monkeypatch.setenv("MAKIMA_ROOT", str(makima_root))
    ...
    for dest_dir in (bin_dst,):
        for name, target in sorted(mapping.items()):
            ...
            assert (agent_bin / target).is_file(), (
                ...
                f"fixture coordinator/bin/ — makima-side target is dead"
            )
...
def test_sweep_against_real_coordinator_bin_derivation_does_not_remove_live_forwarders(
    ...
    current forwarder set from makima's OWN coordinator/bin/, write real
