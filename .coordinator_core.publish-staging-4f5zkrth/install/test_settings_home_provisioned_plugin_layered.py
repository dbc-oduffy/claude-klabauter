...
This is makima's half of a declare/prove split agreed with DoE-claude on
...
after install; makima owns a test proving the plugin-layered path provisions it.
The split is on TREE OWNERSHIP, not on who noticed — DoE's own version of this
cited makima `file:line` anchors they hold no gate over, and one had already
...
HAPPENS, and that the families makima itself owns arrive. When DoE's
...
def test_plugin_layered_install_provisions_the_settings_home_bin(monkeypatch, tmp_path):
    ...
    # tree via MAKIMA_ROOT rather than a fixture. The assertions below are
    ...
    # The makima-owned family specifically: these are sourced from our own tree
    ...
    assert not missing_ch, (
        f"makima-owned claude-home family absent from the provisioned settings-home: "
