...
def _patch_env(monkeypatch, *, makima_root: Path, settings_home: Path, lock_root: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: (str(makima_root), "live-working-tree"),
    )
    monkeypatch.setattr(
...
class TestFailureDegradesSilently:
    def test_unresolvable_makima_root_is_silent_noop(self, tmp_path, monkeypatch, capsys):
        def _raise():
            raise RuntimeError("no makima root")
