"""
coordinator_core.install.test_resolve_makima_exec_cli — unit coverage of
``exec_cli``'s Windows/POSIX platform branch in
``coordinator/lib/resolve-makima/_resolve_makima.py``.

Companion to ``test_resolve_makima.py`` (which covers ``resolve_makima_bin_dir``'s
...
``resolve_makima_bin_dir`` and run the target INSIDE the pytest process —
...
``test_resolve_makima.py`` — the module under test is installed standalone
...
_spec = importlib.util.spec_from_file_location("_resolve_makima_under_test_exec_cli", _MODULE_PATH)
...
#: That is makima's cold-path rule (CLAUDE.md § Runtime conventions); the
#: mechanical guard for it, `coordinator/tests/test_cold_path_remediation_is_runnable.py`,
#: enumerates its subjects in `COLD_PATH_MODULES` — and `_resolve_makima.py`
...
class _OSNameProxy:
    ...
    on a later, unrelated test. Patching the NAME ``resolve_makima.os``
    itself (via ``monkeypatch.setattr(resolve_makima, "os", proxy)``)
...
def _patch_os_name(monkeypatch, name: str) -> "_OSNameProxy":
    ...
    monkeypatch.setattr(resolve_makima, "os", proxy)
    return proxy


def _patch_root_resolution(monkeypatch, bin_dir: Path) -> None:
    """Point ``exec_cli``'s post-C4b root resolution at *bin_dir* directly,
    bypassing ``resolve_makima_root_with_class()``'s registry-then-sentinel
    ladder and ``_validate_bin_dir()``'s on-disk sentinel probe.

    Mirrors the pre-C4b convention (monkeypatching ``resolve_makima_bin_dir``
    wholesale) for the two calls ``exec_cli`` makes today:
    ``resolve_makima_root_with_class()`` (root + resolution class) then
    ``_validate_bin_dir(root)`` (dir + sentinel validation). Patching only
    ``resolve_makima_bin_dir`` (as the pre-C4b tests did) intercepts
    ...
    ``MakimaResolutionError``.
    """
    monkeypatch.setattr(
        resolve_makima,
        "resolve_makima_root_with_class",
        lambda: (str(bin_dir), resolve_makima.RESOLUTION_LIVE_WORKING_TREE),
    )
    monkeypatch.setattr(resolve_makima, "_validate_bin_dir", lambda root: root)
    monkeypatch.setattr(resolve_makima, "resolve_makima_bin_dir", lambda: str(bin_dir))
...
def _write_sentinel(coord_bin: Path) -> None:
    """Write the executable ``archive-stamp-cli`` sentinel ``resolve_makima_bin_dir``
    ...
    if os.name == "nt":
        # `_resolve_makima.py`'s Windows-side executability probe is
...
def _write_forwarder(bin_dir: Path, forwarder_name: str, target: str) -> Path:
    ...
    co-located ``_resolve_makima.py`` and call ``exec_cli(target)``."""
    content = (
        "import sys\n"
        "from pathlib import Path\n"
        "sys.path.insert(0, str(Path(__file__).resolve().parent))\n"
        "from _resolve_makima import exec_cli  # noqa: E402\n"
        f"exec_cli({target!r})\n"
    )
...
def _invoke_posix_subprocess(
    ...
    *,
    ...
    ``COORDINATOR_SETTINGS_HOME`` + ``registry.local.toml``) at a tmp makima
    root, and invoke it as a genuinely separate process."""
    settings_home = tmp_path / "settings-home"
    bin_dir = settings_home / "bin"
    bin_dir.mkdir(parents=True)
    shutil.copy(_MODULE_PATH, bin_dir / "_resolve_makima.py")
    ...
    if resolution_ok:
        makima_root = tmp_path / "makima-root"
        coord_bin = makima_root / "coordinator" / "bin"
        ...
        # TOML literal string (single quotes) — makima_root is a raw
        ...
        # companion test_resolve_makima.py.
        (ml_dir / "registry.local.toml").write_text(
            f"[repos]\nproject_makima = '{makima_root}'\n", encoding="utf-8"
        )
    # else: leave machine-local/ empty -> _resolve_makima_root raises
    # MakimaResolutionError inside the forwarder's own process.
...
def _invoke_nt_inprocess(
    ...
    *,
    ...
    if resolution_ok:
        ...
        def _raise() -> str:
            raise resolve_makima.MakimaResolutionError("ERROR: boom\n")

        monkeypatch.setattr(resolve_makima, "resolve_makima_root_with_class", _raise)

    _patch_os_name(monkeypatch, "nt")

    original_argv = list(sys.argv)
    with pytest.raises(SystemExit) as excinfo:
        resolve_makima.exec_cli(_FIXTURE_TARGET_NAME, argv=argv)
...
def test_posix_forwarder_execs_no_shebang_no_exec_bit_target_via_real_subprocess(tmp_path):
    ...
    in-process monkeypatch of ``resolve_makima_bin_dir`` would pass
    ...
    fixture_path = tmp_path / "makima-root" / "coordinator" / "bin" / _FIXTURE_TARGET_NAME
...
# ---------------------------------------------------------------------------
# sys.path regression -- `_run_target_in_process` must put the makima root
...
def test_run_target_in_process_puts_makima_root_on_sys_path(tmp_path):
    real_makima_root = _MODULE_PATH.resolve().parents[3]
    assert (real_makima_root / "coordinator_core" / "__init__.py").is_file(), (
        "sanity: this test relies on the real on-disk makima root actually "
    ...
    driver.write_text(
        ...
        f"code = m._run_target_in_process({str(target)!r}, [], {str(real_makima_root)!r})\n"
        "sys.exit(code)\n",
        encoding="utf-8",
    )
    ...
    assert result.returncode == 0, (
        f"target import failed under a process without the makima root "
...
def test_windows_branch_never_calls_os_execv(tmp_path, monkeypatch):
    ...
    with pytest.raises(SystemExit) as excinfo:
        resolve_makima.exec_cli("archive-stamp-cli", argv=[])
...
def test_posix_branch_execs_via_sys_executable_with_target_path_and_argv(tmp_path, monkeypatch):
    ...
    with pytest.raises(SystemExit):
        resolve_makima.exec_cli("archive-stamp-cli", argv=["--foo"])
...
def test_posix_branch_execv_oserror_of_any_cause_still_exits_127(tmp_path, monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit) as excinfo:
        resolve_makima.exec_cli("archive-stamp-cli")
