"""
coordinator_core.install.test_resolve_makima — direct unit coverage of
``coordinator/lib/resolve-makima/_resolve_makima.py``'s reject paths.

This is the module the forwarder-ladder extraction (M1) pulled the
resolve-makima-bin ladder into. Before the extraction, these reject paths
...
_MODULE_PATH = (
    Path(__file__).resolve().parents[2] / "coordinator" / "lib" / "resolve-makima" / "_resolve_makima.py"
)

_spec = importlib.util.spec_from_file_location("_resolve_makima_under_test", _MODULE_PATH)
assert _spec is not None and _spec.loader is not None
resolve_makima = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = resolve_makima
_spec.loader.exec_module(resolve_makima)


def _make_makima_fixture(root: Path, sentinel_executable: bool = True) -> None:
    ...
    if os.name == "nt" and sentinel_executable:
        ...
        # on Windows -- `_resolve_makima.py`'s own `_is_executable` probe
...
def test_missing_both_registry_and_sentinel_raises_distinct_message(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(resolve_makima.MakimaResolutionError, match="cannot resolve project-makima"):
        resolve_makima.resolve_makima_bin_dir()


def test_traversal_segment_rejected(tmp_path: Path, monkeypatch):
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    (ml_dir / ".makima-root").write_text(str(tmp_path / "some" / ".." / "makima"), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    with pytest.raises(resolve_makima.MakimaResolutionError, match="traversal segment"):
        resolve_makima.resolve_makima_bin_dir()
...
def test_root_does_not_exist_rejected(tmp_path: Path, monkeypatch):
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    (ml_dir / ".makima-root").write_text(str(tmp_path / "nonexistent-makima"), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    with pytest.raises(resolve_makima.MakimaResolutionError, match="does not exist"):
        resolve_makima.resolve_makima_bin_dir()
...
def test_root_resolved_but_coordinator_bin_missing_distinct_message(tmp_path: Path, monkeypatch):
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    incomplete_root = tmp_path / "incomplete-makima"
    incomplete_root.mkdir()
    (ml_dir / ".makima-root").write_text(str(incomplete_root), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    with pytest.raises(resolve_makima.MakimaResolutionError, match="has no coordinator/bin/ directory") as excinfo:
        resolve_makima.resolve_makima_bin_dir()
...
def test_coordinator_bin_present_but_sentinel_absent_distinct_message(tmp_path: Path, monkeypatch):
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    stale_root = tmp_path / "stale-makima"
    (stale_root / "coordinator" / "bin").mkdir(parents=True)
    (ml_dir / ".makima-root").write_text(str(stale_root), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    with pytest.raises(resolve_makima.MakimaResolutionError, match="stale or partial project-makima migration") as excinfo:
        resolve_makima.resolve_makima_bin_dir()
...
def test_sentinel_appearing_mid_rename_resolves_instead_of_raising(tmp_path: Path, monkeypatch):
    ...
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    root = tmp_path / "mid-rename-makima"
    (root / "coordinator" / "bin").mkdir(parents=True)
    (ml_dir / ".makima-root").write_text(str(root), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))
    ...
    monkeypatch.setattr(resolve_makima.time, "sleep", _write_sentinel_on_first_sleep)

    assert resolve_makima.resolve_makima_bin_dir().replace("\\", "/") == (
...
def test_sentinel_absent_across_reprobe_still_raises(tmp_path: Path, monkeypatch):
    ...
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    root = tmp_path / "genuinely-absent-makima"
    (root / "coordinator" / "bin").mkdir(parents=True)
    (ml_dir / ".makima-root").write_text(str(root), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    monkeypatch.setattr(resolve_makima.time, "sleep", lambda seconds: None)

    with pytest.raises(
        resolve_makima.MakimaResolutionError,
        match="stale or partial project-makima migration",
    ) as excinfo:
        resolve_makima.resolve_makima_bin_dir()
...
def test_present_sentinel_never_pays_the_retry_sleep(tmp_path: Path, monkeypatch):
    ...
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    root = tmp_path / "happy-path-makima"
    _make_makima_fixture(root)
    (ml_dir / ".makima-root").write_text(str(root), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    slept: list = []
    monkeypatch.setattr(resolve_makima.time, "sleep", lambda seconds: slept.append(seconds))
...
def test_non_executable_sentinel_rejected(tmp_path: Path, monkeypatch):
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    root = tmp_path / "non-exec-sentinel-makima"
    _make_makima_fixture(root, sentinel_executable=False)
    (ml_dir / ".makima-root").write_text(str(root), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    with pytest.raises(resolve_makima.MakimaResolutionError, match="stale or partial project-makima migration"):
        resolve_makima.resolve_makima_bin_dir()
...
def test_valid_fixture_resolves_bin_dir(tmp_path: Path, monkeypatch):
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    root = tmp_path / "valid-makima"
    _make_makima_fixture(root)
    (ml_dir / ".makima-root").write_text(str(root), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    # `resolve_makima_bin_dir` returns `makima_root + "/coordinator/bin"`
    # where `makima_root` is read back verbatim from `.makima-root` --
    ...
    assert resolve_makima.resolve_makima_bin_dir().replace("\\", "/") == root.as_posix() + "/coordinator/bin"


def test_registry_rung_wins_over_sentinel(tmp_path: Path, monkeypatch):
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    registry_root = tmp_path / "registry-makima"
    _make_makima_fixture(registry_root)
    (ml_dir / "registry.local.toml").write_text(
        f'"repos.project_makima" = \'{registry_root}\'\n', encoding="utf-8"
    )
    (ml_dir / ".makima-root").write_text(str(tmp_path / "sentinel-makima-nonexistent"), encoding="utf-8")
    monkeypatch.setenv("MACHINE_LOCAL_REGISTRY_DIR", str(ml_dir))

    assert resolve_makima.resolve_makima_bin_dir().replace("\\", "/") == registry_root.as_posix() + "/coordinator/bin"
...
def test_settings_home_prefers_claude_home(monkeypatch, tmp_path):
    ...
    assert resolve_makima._settings_home() == (tmp_path / "sb" / ".coordinator-claude-settings")
...
def test_settings_home_falls_back_to_userprofile_when_no_home(monkeypatch, tmp_path):
    ...
    assert resolve_makima._settings_home() == (tmp_path / "winhome" / ".coordinator-claude-settings")
...
def test_settings_home_fails_loud_when_home_unresolvable(monkeypatch):
    ...
    monkeypatch.setattr(resolve_makima.os.path, "expanduser", lambda p: p)
    with pytest.raises(resolve_makima.MakimaResolutionError, match="cannot resolve the coordinator settings home"):
        resolve_makima._settings_home()
...
def test_standalone_shim_imports_no_coordinator_core(shim_path: Path):
    ...
    the stdlib importable — their whole job is to LOCATE the makima tree, so
    ...
    module-scope package import to ``_resolve_makima.py``. Every one of the
...
# this pair (test_resolve_makima.py, test_resolve_makima_exec_cli.py) both
# import the module under test IN-PROCESS, inside makima's own pytest
...
import sys
import _resolve_makima as mod

assert callable(mod.exec_cli), "exec_cli is not callable after import"
assert callable(mod.resolve_makima_bin_dir), "resolve_makima_bin_dir is not callable after import"
...
def _run_standalone_import_probe(shim_source: Path, tmp_path: Path) -> "subprocess.CompletedProcess[str]":
    ...
    *tmp_path* — mirroring the settings-home ``bin/`` layout the shim's own
    ...
    prepend; nothing here adds makima's root).
...
_RESOLVE_MAKIMA_SOURCE = (
    Path(__file__).resolve().parents[2] / "coordinator" / "lib" / "resolve-makima" / "_resolve_makima.py"
)


def test_resolve_makima_shim_importable_standalone_via_subprocess(tmp_path: Path):
    """Regression, 2026-07-29 bootstrap-inversion outage: the module whose
    entire job is locating project-makima had a module-scope
    ...
    result = _run_standalone_import_probe(_RESOLVE_MAKIMA_SOURCE, tmp_path)
    assert result.returncode == 0, (
        "coordinator/lib/resolve-makima/_resolve_makima.py failed to import "
...
def _resolve_installed_shim_path() -> Path:
    ...
    ``_resolve_makima._settings_home()``'s precedence, sufficient only to
    ...
    if override:
        return Path(override) / "bin" / "_resolve_makima.py"
    home = os.environ.get("CLAUDE_HOME") or os.environ.get("HOME") or os.environ.get("USERPROFILE") or ""
    if not home:
        return Path()
    return Path(home) / ".coordinator-claude-settings" / "bin" / "_resolve_makima.py"
...
@pytest.mark.skipif(
    ...
    reason=(
        "no installed <settings-home>/bin/_resolve_makima.py found on this machine "
...
# ---------------------------------------------------------------------------
# DR-132 two-tier ladder — `resolve_makima_root_with_class()`. Mirrors
# DoE-claude `coordinator/hooks/scripts/_engine_root.py`'s
# `resolve_makima_root_with_class()` step order; a conformance fixture
...
def test_live_tree_wins_when_session_is_the_source_tree(tmp_path: Path, monkeypatch):
    ...
    OWN root must equal ``_resolve_makima_root``'s resolved value, not
    membership in a registered ``engine.working_repos.*`` set (retired)."""
    ml_dir = tmp_path / "machine-local"
    ml_dir.mkdir()
    live_root = tmp_path / "live-makima"
    _make_makima_fixture(live_root)
    (ml_dir / ".makima-root").write_text(str(live_root), encoding="utf-8")
    ...
    root, cls = resolve_makima.resolve_makima_root_with_class()
    assert root == str(live_root)
    assert cls == resolve_makima.RESOLUTION_LIVE_WORKING_TREE
...
def test_published_wins_when_session_is_not_the_source_tree(tmp_path: Path, monkeypatch):
    ...
    assert cls == resolve_makima.RESOLUTION_RESOLVED_ENGINE
...
def test_absent_engine_target_does_not_divert_even_when_session_confirmed_different(
    ...
    assert resolve_makima.resolve_engine_target(ml_dir) is None
    assert resolve_makima._is_makima_source_tree(ml_dir) is False
...
def test_no_klabauter_byte_identical_to_today(tmp_path: Path, monkeypatch):
    ...
    assert root == str(live_root)
    assert cls == resolve_makima.RESOLUTION_LIVE_WORKING_TREE
    assert resolve_makima.resolve_makima_bin_dir() == root + "/coordinator/bin"
...
def test_both_unresolvable_raises(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(resolve_makima.MakimaResolutionError, match="cannot resolve project-makima"):
        resolve_makima.resolve_makima_root_with_class()
...
def test_klabauter_only_resolves_published_engine(tmp_path: Path, monkeypatch):
    ...
    # No repos.project_makima, no .makima-root sentinel -- the
...
def test_unstamped_published_root_is_denied(tmp_path: Path, monkeypatch):
    ...
    assert resolve_makima._resolve_published_engine(ml_dir) is None
    with pytest.raises(resolve_makima.MakimaResolutionError, match="cannot resolve project-makima"):
        resolve_makima.resolve_makima_root_with_class()
...
def test_installed_settings_home_shim_importable_standalone_via_subprocess(tmp_path: Path):
    ...
    at ``<settings-home>/bin/_resolve_makima.py``, not the
