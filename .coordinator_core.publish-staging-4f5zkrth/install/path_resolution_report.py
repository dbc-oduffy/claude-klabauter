...
chain actually worked, consumed by both entry points the plan names:
  ...
  - "the chain-walk" — the `makima.entrypoints.path_resolved` doctor probe
    (`bin/makima-doctor-probe.py`, registered in `bin/doctor-probes.toml`),
    ...
    chain-walker's no-recursion gap (`makima_seam_resolvable` self-confirms
    and never recurses into makima's own manifest) rather than closing it —
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    distinct from the doctor-probe registration (bin/makima-doctor-probe.py
