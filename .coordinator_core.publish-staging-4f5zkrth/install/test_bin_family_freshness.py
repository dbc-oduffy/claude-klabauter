...
def test_every_static_family_member_is_byte_identical_to_its_source(monkeypatch, tmp_path):
    ...
    for f, _exec_bit in _CH_FAMILY_FILES:
        ...
        src_by_name[f] = src
    # _resolve_makima.py's source is fixed by _install_bin_resolvers to the
    # REAL repo tree (coordinator/lib/resolve-makima/), not a synthetic
    # fixture — resolved via MAKIMA_ROOT below, exactly like a real install.
    for f in _RM_FAMILY_FILES:
        src_by_name[f] = _REPO_ROOT / "coordinator" / "lib" / "resolve-makima" / f

    # coordinator_makima_root() Rung 1 short-circuits on MAKIMA_ROOT being
    # set — this repo checkout IS the makima root, so this is a same-repo
