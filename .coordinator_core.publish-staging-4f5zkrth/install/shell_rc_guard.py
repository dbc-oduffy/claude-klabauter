...
single-file/MAKIMA_CLONE call shape (see § Two calling shapes below).
...
single-purpose ``MAKIMA_CLONE``-export writer; generalized
...
(Zolí F2). Every consumer — the ``MAKIMA_CLONE`` export, the claude-CLI PATH
...
§ Two calling shapes:
  1. **Legacy single-file** (``rc_files`` omitted): resolves exactly one rc
     file via ``$SHELL`` (mirrors the original ``MAKIMA_CLONE``-only writer;
...
choice for the ``MAKIMA_CLONE`` case (`distinct-ops-new.tsv` row
`install-shell-rc-guard-block`: "Bakes MAKIMA_CLONE path into the written
...
§ CI limit (Zolí F3 — stated explicitly, not implied): rc-file writes do
...
Mandated resolver used (legacy MAKIMA_CLONE default body only):
`coordinator_core.engine_root.coordinator_engine_root()` (this op's ONE
cross-repo-shaped fact — the makima clone's own root — per the plan's
...
def _sentinel_markers(sentinel_id: str) -> Tuple[str, str]:
    ...
    `sentinel_id="MAKIMA_CLONE"` this reproduces the pre-generalization
    ...
    return (
        f"# --- coordinator makima {sentinel_id} shell-init guard [generated] ---",
        f"# --- end coordinator makima {sentinel_id} shell-init guard ---",
    )


SENTINEL_BEGIN, SENTINEL_END = _sentinel_markers("MAKIMA_CLONE")
...
def _resolve_rc_path() -> Path:
    ...
    could not take effect and `uninstall_legs`' MAKIMA_CLONE strip leg
...
def _shell_dquote_escape(value: str) -> str:
    ...
    directory via `_build_path_entry_body`, and the legacy MAKIMA_CLONE
    ...
    *,
    ...
    Args:
        body: literal block content (excluding sentinels). Ignored if
            ...
            omitted and `sentinel_id == "MAKIMA_CLONE"`, the legacy
            default body (`export MAKIMA_CLONE="<resolved clone root>"`)
            is auto-resolved via `coordinator_makima_root()`.
    ...
    if path_entry is not None:
        ...
        if sentinel_id != "MAKIMA_CLONE":
            raise ValueError(
                "write_shell_rc_guard_block: body or path_entry is required "
                f"for sentinel_id={sentinel_id!r}"
            )
        try:
            makima_clone, _resolution_class = coordinator_engine_root_with_class()
        except RuntimeError as exc:
            print(f"[shell-rc-guard] skip: could not resolve makima clone root: {exc}", file=sys.stderr)
            # Discovery (the makima clone root) came up empty — resolved
            ...
            return {"results": {}, "modified": False, "already_present": False}
        body = f'export MAKIMA_CLONE="{_shell_dquote_escape(makima_clone)}"'
...
async def _handler(params: dict, repo_root=None) -> dict:
    ...
    single-file MAKIMA_CLONE shape — the op contract is unchanged by this
...
WRITE_SURFACE = WriteSurfaceDeclaration(
    ...
    clauses=(
        ...
        #     this module — today's two known callers pass "MAKIMA_CLONE"
