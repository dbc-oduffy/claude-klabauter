...
# hardcoded real-user path") -- the operator's home tree, not makima's own.
GENERATES = []
...
def uninstall_remove_shim() -> bool:
    ...
    # ---- (d) #10 (patrik F0): claude-doe wrapper ----
    ...
    # may have written -- MAKIMA_CLONE (single file, same resolver the
    ...
    # (e.g. test isolation). MAKIMA_CLONE's `_resolve_rc_path()` stays
    ...
    for sentinel_id, candidate_files in (
        # Review: code-reviewer (Finding 3, P2) — MAKIMA_CLONE deliberately
    ...
    *,
    ...
    if mode == "full-remove":
        ...
        coord_bin_names = (
            ...
            # _resolve_makima.py: the shared resolve-makima-bin ladder module
        ...
        # note above this leg's test). Best-effort: a makima checkout that
        ...
        try:
            ...
            agent_bin = Path(_makima_root_str) / "coordinator" / "bin"
