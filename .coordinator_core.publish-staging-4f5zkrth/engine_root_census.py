...
for the retired MAKIMA_ROOT fallback.

What this is: a durable, append-only observation sink. A row here means some
process on this box still had `MAKIMA_ROOT` exported when
...
fires when `MAKIMA_ROOT` is merely SET IN THE ENVIRONMENT, never when it
...
`MAKIMA_ROOT` by hand to drive a ceremony trips the counter by verifying;
...
scatter the series across roots, so a reader anchored on makima would see a
...
def record_fallback_read(
    ...
    *,
    ...
    """Best-effort, append-only record that `site` read the `MAKIMA_ROOT`
    fallback.
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Report engine-root fallback reads (stale MAKIMA_ROOT pins) "
