...
Exit-code contract (main()), UNCHANGED from the bash oracle — no makima-engine
transport call exists inside this module (no cc_invoke / IPC round-trip):
    ...
    1  — fail-loud: not inside a git repository; a HAND fence pair is absent or
         ...
         unparseable HAND region); or the makima/state-root resolver failed
         (coordinator_makima_root_with_class() raised, including the
         published-engine-mirror refusal reused from
         coordinator_core.state_root._makima_state).
...
from coordinator_core.state_root import _makima_state as _guarded_makima_state
...
def _resolve_state_root(repo_root: str) -> str:
    ...
    Returns `<makima_root>/state` when repo_root IS the coordinator meta-repo,
    else `<repo_root>/state`.

    Raises RuntimeError (coordinator_makima_root's own remediation text) when
    repo_root IS the meta-repo but MAKIMA_ROOT cannot be resolved — matches the
    ...
    `coordinator_core.state_root._makima_state`'s own published-mirror guard
    (class-AWARE `coordinator_makima_root_with_class`, not the class-less
    `coordinator_makima_root` this function used to call directly) rather than
