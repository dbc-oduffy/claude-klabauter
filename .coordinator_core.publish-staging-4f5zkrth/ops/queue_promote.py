...
outbox is NOT makima-rooted (see the "Makima root" note below, corrected
2026-07-22 after 72 outbox YAMLs from a DoE-claude session landed in makima's own
...
materially (design decision, strang-08 Design decisions section):
    ...
    - DoE root: NO cwd fallback on unresolvable DoE-claude root (C12 negative-spec,
      mirrored onto the DoE-rooted seam — same negative-spec as the makima-rooted
...
makima — makima is just one of many senders), so ``caller_worktree`` is not used for
...
GENERATES = []  # writes only into DoE-claude's central state/lessons-outbox/, never makima's own tree
...
class _DoeUnresolvable(RuntimeError):
    ...
    makima-rooted queue.append central scope (see ``coordinator_doe_root``'s own
...
def _outbox_root() -> str:
    ...
    Resolution:
        ...
        2. ``<doe_root>/state/lessons-outbox/`` via ``coordinator_doe_root()`` — the
           ...
           repo), NOT makima. Matches the documented CLI oracle contract
    ...
    Spec backlink: pln-stop-the-rot-makima-state-home-placement-4cc787 § C12 / AC13
    ...
    *,
    ...
    Parameters:
        ...
        caller_worktree — caller's worktree root (for from_repo fallback; not used for
                          path routing since outbox is always central makima state).
...
def _queue_promote_handler(
    ...
    makima central state, so ``caller_worktree`` is used only for ``from_repo`` fallback,
