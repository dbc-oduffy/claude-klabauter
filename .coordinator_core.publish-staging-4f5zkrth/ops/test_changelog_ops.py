...
def test_main_positive_backfills_gap(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.chdir(repo)
    # 2026-08-11 fix (cross-repo/inbox/2026-08-11-project-rag-em-backfill-
...
def test_main_help_does_not_run_backfill(tmp_path: Path, monkeypatch, capsys) -> None:
    """Item 1 (cross-repo/inbox/2026-08-11-project-rag-em-backfill-changelog-
...
#
# Regression cover for the project-rag-em memo 2026-07-20: the predecessor bash
...
class TestUpsertReviewed:
    def test_upsert_replaces_only_reviewed_line_rest_byte_identical(
        ...
        assert (
            "**Reviewed:** sha_range=aaa1111..bbb2222 reviewer=patrik verdict=pass diff_loc=12"
            in content
        )
        ...
        expected = _CURATED_BLOCK_TMPL.format(
            ...
            machine=machine,
            reviewed_line=(
                "**Reviewed:** sha_range=aaa1111..bbb2222 reviewer=patrik "
