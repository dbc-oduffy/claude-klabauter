...
GENERATES = []  # merges into a caller-supplied target file (a consumer repo's .claude/em-context.md), never a fixed makima artifact
...
def main(argv: List[str], coordinator_root: Optional[str] = None) -> int:
    ...
    templates are DoE-resident, not makima-resident) but defaults to None so
