...
produce byte-identical file content to their DoE oracle CLIs
...
Oracle (a): [DoE] coordinator/bin/workday-complete-step9-append-changelog.py
Oracle (b): [DoE] coordinator/bin/backfill-week-changelog-gaps.py
"""
...
# output, matching what the DoE oracle CLIs themselves read — a mocked git
...
# ---------------------------------------------------------------------------
# inject_anchor oracle resolution — this oracle is a makima-owned in-repo CLI
...
_MAKIMA_ROOT = Path(__file__).resolve().parents[3]
_INJECT_ANCHOR_ORACLE = (
    _MAKIMA_ROOT / "coordinator" / "bin" / "workday-complete-backfill-inject-anchor.py"
)
...
_requires_append_oracle = pytest.mark.skipif(
    ...
    reason="step9-append-changelog oracle not available (DoE not configured or absent)",
)
_requires_backfill_oracle = pytest.mark.skipif(
    not _ORACLE_BACKFILL_AVAILABLE,
    reason="backfill-week-changelog-gaps oracle not available (DoE not configured or absent)",
)
...
@pytest.mark.real_home  # oracle subprocess resolves MAKIMA_ROOT via the real machine-local
# registry / <settings-home>/machine-local/.makima-root pointer — see note below.
class TestAppendDayByteParity:
    ...
    oracle's MAKIMA_ROOT resolver (coordinator-makima-root.sh, sourced transitively via
    ...
    finds neither, and hard-fails with "repos.project_makima is not set" — even on a
    ...
    ``coordinator_makima_root`` resolves correctly outside the quarantine). The write
...
class TestBackfillGapsByteParity:
    ...
    subprocess, whose MAKIMA_ROOT resolver then fails to find the real settings-home
    pointer / machine-local registry entry and hard-fails with "repos.project_makima is
...
class TestAppendDaySmoke:
    ...
    comparing against the oracle. These run even when DoE is not installed.
    """
    ...
    def test_compose_block_format(self):
        ...
        block = _compose_block(
            ...
            rc_validate="0",
            rc_plugin_suite="0",
            reviewed_lines=["code: Patrik found 1 finding (P2, applied)."],
        ...
        assert "**Reviewed:** code: Patrik found 1 finding (P2, applied)." in block
...
class TestInjectAnchorByteParity:
    ...
    Unlike append_day/backfill_gaps, this oracle is a makima-owned CLI resolved
