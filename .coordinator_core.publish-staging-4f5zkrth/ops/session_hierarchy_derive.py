...
Purpose: query all handoffs (active + archived) in makima's OWN checkout, derive
...
makima's own worktree, resolved via ``Path(__file__)`` + ``git rev-parse``, NOT any
caller-supplied ``repo_root`` — post-migration, handoffs live in makima
(``coordinator_makima_root``/state/handoffs, archive/handoffs) regardless of which
...
MUTATES = ["state/session-hierarchy.*.json"]  # per-machine full-rebuild shard; filename varies by machine_slug(), e.g. state/session-hierarchy.delphipro-local.json / state/session-hierarchy.striker.json (both tracked)
...
def _engine_worktree_root() -> Optional[Path]:
    """Resolve makima's OWN worktree root via ``Path(__file__)`` + ``git rev-parse``.
...
async def _handler(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    parity but IGNORED — this op always targets makima's own checkout (see
