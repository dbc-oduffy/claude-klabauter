...
BINDING CONSTRAINTS (from the bash oracle's own Patrik-reviewed header, preserved):
...
Port of: classify-dispatch-shape.sh (DoE b5a4192c, 2026-07-20, 339 lines)
...
def _resolve_plan_file(plan_slug: str, script_dir: Optional[str]) -> Optional[str]:
    ...
    is known — the DoE-side trampoline's own directory, NOT this makima module's
...
def main(argv: List[str], *, script_dir: Optional[str] = None) -> int:
    ...
    caller); see module docstring § OFFER SHAPE. `script_dir` is the DoE-side
