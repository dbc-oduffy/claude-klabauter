...
decide final promotion — that judgment belongs to DoE's C6 background-Workflow
LLM triage wave, which consumes this op's candidate list as its input, not a
replacement for it.

Two-tier read model:
  1. If a memo's frontmatter already carries ``distill_fate`` (DoE's C2 schema:
     ...
     is a thin forward-compatible read; conformance-recheck when DoE's
  ...
  2. Otherwise (the common case today — no memo in the wild carries
     ...
     is DoE's cascade's job, not this pre-filter's).
...
# distill_fate values (DoE's forward C2 schema) that count as a promote signal.
...
def _handler(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    Params:
        ...
        project_slug (str, optional): the auto-memory project-slug directory
            ...
            ``-Users-oduffy-X-project-makima``. Absent → auto-memory corpus is
