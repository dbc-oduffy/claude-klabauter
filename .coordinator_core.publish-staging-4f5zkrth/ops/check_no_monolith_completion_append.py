...
port of DoE's check-no-monolith-completion-append.sh trampoline.
...
Six allowed exceptions (excluded from firing), evaluated per matched line:
  ...
  6. Files whose *path* contains a `/tests/` segment — the tripwire's own
     ...
     `/hooks/scripts/tests/` (that dir stayed DoE-resident post-2026-07-22
     ...
     `coordinator/bin/tests/` under makima) to the general `/tests/` segment
...
Port of: check-no-monolith-completion-append.sh (DoE 894d4bc6, 2026-07-22)
...
Negative-spec:
    ...
    - Directory scan is the bash oracle's original five discovery-surface
      ...
      migration actually populated under a makima-side `--root`
      ...
      five stayed DoE-resident (coordinator-claude) post-migration and so
      never exist under makima's own `coordinator/`; the four
      ...
      caller pointing `--root` at either a DoE-resident (co-located, pre-
      split) tree or a makima-resident (post-split) tree gets a live scan
...
def _default_root() -> str:
    ...
    # analogous "own path" to derive from (it's a makima-side import, not the
