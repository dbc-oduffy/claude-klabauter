...
Spec backlinks:
  ...
  - Wire contract (FROZEN): coordinator_core/contract/opticon-invoke-producer-contract.md §2.1, §3, §5
...
#   makima-initiated commit; without them that branch is never selected on
#   Windows.
#
...
def _setup_error(mode, dry_run, reason: str) -> dict:
    ...
    (contract §2.1) is not expanded by setup errors; opticon branches on exit_code:1
    ...
    Reaching THIS process's stderr is where makima's guarantee stops.  A setup error
...
def build_act_result(
    ...
    The annotation still reaches makima-internal consumers that read
