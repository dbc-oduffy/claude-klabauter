...
retires) against an arbitrary ``repo_root`` — makima's own or a consumer repo's
(project-rag, project-opticon).

Mapping (mirrors makima's own C5+C8 live-data migration, ``e2cf1a08``, byte-for-byte
...
reinterpret):
  ...
  - ``deployment_state: abandoned`` splits on positive succession proof, checked
    ...
    ``superseded_by`` field (foreign-corpus tolerance only — NOT a makima
    ...
    tree" and "this work died" are different claims (project-opticon's
    ...
    because the work died. Evidence from a real consumer corpus (project-opticon,
...
Live-data migration oracle (makima's own corpus, same mapping): e2cf1a08
...
Negative-spec:
    - Reads ``status``/``deployment_state`` comment-aware (a trailing ``# ...``
      ...
      project-opticon's corpus carried
    ...
    - ``claimed_by``/``consumed_by`` session-id equality across two records is
      ...
      follows from that. (Rejected candidate from project-opticon's own audit:
...
# written onto the wrong axis. Observed in project-rag's corpus on records dated
...
# origin_handoff (added 2026-07-23 per project-opticon's dr084 memo). A
...
def _resolve_superseded_by(
    ...
    ``superseded_by`` is NOT a makima handoff-schema field — it lives only on
    ...
    shapes encountered when this op runs against a CONSUMER tree (project-rag
    et al.), never as an endorsement of ``superseded_by`` on makima's own
    handoffs. Mirrors a sibling-repo lesson: project-rag's own migrator
...
def _plan_one(
    ...
    if deployment_raw == "abandoned" or is_repair:
        ...
        repair_prefix = "REPAIR: deployment_state: superseded (invalid — " \
            "DoE 2026-06-26 retired 'superseded' as a handoff STATUS; the ruling's " \
        ...
        if successor is not None:
            ...
            if is_repair:
                fm_text = _insert_raw_line_after(
                    ...
                    f"normalized to abandoned per DoE's 2026-06-26 status retirement, "
...
def plan_migration(repo_root: str) -> Dict[str, Any]:
    ...
    exact set without reading every change line — see project-opticon's
...
def _discharge_continued_archival(
    ...
    the archival call:
      ...
      - Guard-retain records are committed IN PLACE via
        ...
        "zero abandoned/superseded records remain" on makima's own tree).
