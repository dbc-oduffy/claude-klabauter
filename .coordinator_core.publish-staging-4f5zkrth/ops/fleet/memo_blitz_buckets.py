...
instead of on grep-grade bucketing. Four outputs, one read of the inbox:
  ...
  - `bucket_summary` — the counts-by-kind, counts-by-sender, and the resolved
                    ...
                    load-bearing structural fact, not a nicety: project-rag's
...
Provenance: cross-repo/inbox/2026-07-28-project-rag-em-inbox-blitz-proven-
pattern.md (project-rag-em's pattern report + proposal, adopted 2026-07-28 with
...
Negative-spec:
  ...
  - Does NOT CONFIRM a supersession — it emits candidates. Confirming that a
    ...
    live asks; project-rag was strict about this deliberately and said so.
...
# load-bearing one: project-rag's failure was 16 days of accretion where no
...
# correspondent" and earns its own bucket. Half is the threshold project-rag's
...
def _supersession_candidates(records: list[dict]) -> list[dict]:
    ...
    can weigh them differently, ranked strongest-first:
      ...
      - `same-sender-same-locus` — same sender, later date, overlapping cited
        loci. This is the inference project-rag ran by hand; against
...
def _build_candidates(
    ...
    for record in records:
        ...
        # break-class defect in project-rag's run.
        if record["kind"] == "fyi":
            bucket = "fyi"
