...
repo (2026-07-17, project-rag-em). Registered as "memo.list" via @register_op;
...
Two modes, selected by whether `to` is supplied:
  ...
  - Resolution mode (`to` supplied): resolve `to` through the ONE shared
    ...
    would use — correct only for makima-origin sends, silently wrong for
    every other caller (reported by DoE/claude-central-em: `cross-repo-memo
    --dry-run` previewed `makima-engine`-namespaced filenames for DoE-origin
...
def _receiver_shortname_aliases(manifest_aliases: dict, repo_key: str) -> set:
    ...
    `receiver_em_to_repo_key()` accepts either form (e.g. `holodeck` or
    `holodeck-em` both resolve `repos.claude_unreal_holodeck`) — both forms
