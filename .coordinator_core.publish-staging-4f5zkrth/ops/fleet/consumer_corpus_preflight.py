...
CONSUMER trees (DoE-claude, project-makima, project-rag, project-opticon,
project-rag-ue-addon, claude-unreal-holodeck, market-intelligence), never just
...
strength of a test that scanned ONLY makima's own corpus, while makima's ops
execute INSIDE consumer repos against THEIR corpora — at retirement time
project-rag was 100% un-migrated and project-opticon ~96% un-migrated, and
...
two repos (``project-rag-ue-addon``, ``claude-unreal-holodeck``) that were not
even in ``FLEET_REPO_KEYS``, and the other 25 (``project-opticon``, which WAS
...
first cut of this hardening would have gated a re-vendor on project-makima's
own 25 correctly-archived ``spinoff-roadmap`` records (0 live), and dozens more
across project-rag and market-intelligence. A gate that fires on correct data
...
archive dir INSIDE ``state/handoffs/`` (makima's hidden ``.archive/``,
claude-unreal-holodeck's non-hidden ``archive/`` subdir) is governed by the
...
(archived) — both vendored copies, authoritative for makima's engine; this
...
FLEET_REPO_KEYS: Dict[str, str] = {
    ...
    "project-makima": "project_makima",
    "project-rag": "project_rag",
    "opticon": "project_opticon",
    "project-rag-ue-addon": "project_rag_ue_addon",
    "claude-unreal-holodeck": "claude_unreal_holodeck",
...
NON_FLEET_EXCLUDED_KEYS: Dict[str, str] = {
    "repos.zzztest-normcheck": "per-machine smoke-test fixture (registered path is /tmp scratch), not a repo",
    "repos.holodeck-python-audit": "UE consumer-project Saved/ scratch dir (python audit recall log), not a git working tree",
    "repos.delphios": "DelphiOS release/binary-assembly shell (packages the assembled fleet into one binary) — not an authoring EM working tree",
    "repos.geneva_mvp": "standalone product repo (Geneva MVP), not part of the coordinator EM fleet",
    "repos.fifa_stats": "standalone product repo (FIFA stats), not part of the coordinator EM fleet",
    "repos.fifa_26wcle_data": "standalone data repo, distinct from fifa_stats, not part of the coordinator EM fleet",
    "repos.experiments": "generic per-machine scratch/experiments repo, not part of the coordinator EM fleet",
    "repos.dronesim": "standalone product repo (DroneSim), not part of the coordinator EM fleet",
    "repos.nimbalyst": "standalone product repo (nimbalyst), not part of the coordinator EM fleet",
    "repos.delphi_store": "standalone product repo (Delphi Store), not part of the coordinator EM fleet",
}
...
def _iter_handoff_md_files(repo_root: Path) -> List[Path]:
    ...
    archive directories under ``state/handoffs/`` — hidden (makima's own
    ``state/handoffs/.archive/``) OR NOT (claude-unreal-holodeck's non-hidden
