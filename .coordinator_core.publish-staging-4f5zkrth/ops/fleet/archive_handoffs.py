...
uniformly to both branches:
  ...
  Branch B — terminal deployment_state (NEW, 2026-07-13):
    B1. deployment_state in _TERMINAL_DEPLOYMENT_STATES (frontmatter) — regardless of
        ...
        state (makima's status enum is only {open, claimed} post-DR-084; no cross-field
        rule forbids open+shipped) produced when work ships off-baton (never claimed) —
        e.g. makima's auto-reconcile engine or /workstream-complete Step 2.7
...
Negative-spec:
  ...
  - resolve_live_session_ids and cs_claim_holder_live are both sync subprocess.run
    ...
    Patrik F3; DR-211 D4).  Check 4 (Finding 3, C1, 2026-07-14) consults
  ...
  - _handle_preview_handoffs / _handle_act_handoffs do NOT build a unified cross-family
    ...
    separate per-family callables (Patrik F3).
  ...
  - The heir branch does NOT change handoff_children.py / "handoff.has_live_children"
    (a FROZEN wire op — coordinator_core/contract/project-rag-producer-contract.md,
    coordinator_core/contract/opticon-invoke-producer-contract.md) or
...
async def _is_terminal(
    ...
    *,
    ...
    deployment_state: in_flight → abandoned in-place BEFORE archiving (Patrik
    ...
    wire display string (contract §2.1), NOT a makima frontmatter key.
    ...
    in asyncio.to_thread per Key Decision / Patrik F3 / DR-211 D4 async mandate.
...
# subtle behavior drift hides (Patrik F3, strang-11 B8 C1).
...
async def _handle_preview_handoffs(
    ...
    *,
    ...
    for handoff_path in live_paths:
        ...
        # display string (contract §2.1), NOT a makima frontmatter key.
        ...
        # first (coordinator_core/contract/opticon-invoke-producer-contract.md
        ...
        # rejected: status is a documented display field opticon may filter/key
