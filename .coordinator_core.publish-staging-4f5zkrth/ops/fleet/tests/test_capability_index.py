...
# the dispatch brief ("Tests must not depend on DoE or project-rag being
...
_PROJECT_RAG_ROOT = _candidate_repo_root("PROJECT_RAG_ROOT", lambda: registry_get("repos.project_rag"))
...
@pytest.mark.skipif(
    _PROJECT_RAG_ROOT is None, reason="project-rag checkout not found — set PROJECT_RAG_ROOT to pin it",
)
class TestProjectRagRealExemplar:
    def test_real_project_rag_manifest_aggregates_cleanly(self, tmp_path: Path, monkeypatch) -> None:
        real_manifest_path = _PROJECT_RAG_ROOT / "state" / "capabilities" / "manifest.json"
        if not real_manifest_path.is_file():
            pytest.skip("project-rag checkout present but state/capabilities/manifest.json absent")
