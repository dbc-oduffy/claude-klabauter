...
Ratifying spinoff: state/handoffs/2026-07-21_184526_makima-check-addressee-verb.md
...
Spec backlink: state/handoffs/2026-07-21_184526_makima-check-addressee-verb.md
"""
...
class TestMatch:
    def test_self_resolves_to_same_repo_as_to(self, tmp_path, monkeypatch):
        ...
        self_repo.mkdir()
        claude_home = _make_claude_home(tmp_path, {"project_makima": str(self_repo)})
        ...
        result = _run(
            _memo_check_addressee(
                {"dry_run": True, "to": "project-makima-em"}, repo_root=common_dir
            )
        )
...
class TestMismatch:
    def test_to_resolves_to_a_different_registered_repo(self, tmp_path, monkeypatch):
        ...
        claude_home = _make_claude_home(
            tmp_path,
            {"project_makima": str(self_repo), "project_rag": str(other_repo)},
        )
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
        ...
        result = _run(
            _memo_check_addressee(
                {"dry_run": True, "to": "project-rag-em"}, repo_root=common_dir
            )
        )
...
class TestUnresolvedWithSuggestion:
    def test_near_miss_receiver_gets_did_you_mean_suggestion(self, tmp_path, monkeypatch):
        ...
        claude_home = _make_claude_home(
            tmp_path,
            {"some_self_repo": str(self_repo), "project_makima": str(makima_repo)},
        )
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
        ...
        result = _run(
            _memo_check_addressee(
                {"dry_run": True, "to": "makima-em"}, repo_root=common_dir
            )
        )
