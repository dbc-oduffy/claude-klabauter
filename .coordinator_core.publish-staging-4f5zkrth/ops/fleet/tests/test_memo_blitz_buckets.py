...
Spec backlink: cross-repo/inbox/2026-07-28-project-rag-em-inbox-blitz-proven-pattern.md;
...
class TestBucketing:
    def _dominant_inbox(self, tmp_path: Path) -> Path:
        ...
        inbox.mkdir()
        for i in range(6):
            _write_memo(inbox, f"2026-07-2{i % 8}-project-makima-em-thread-{i}.md")
        _write_memo(inbox, "2026-07-21-project-rag-em-other.md", sender="project-rag-em")
        _write_memo(
            inbox, "2026-07-22-opticon-em-status.md", sender="opticon-em", kind="fyi",
        )
        return inbox
    ...
    def test_dominant_correspondent_resolved_and_bucketed(self, tmp_path):
        ...
        assert summary["dominant_sender"] == "project-makima-em"
    ...
    def test_fyi_wins_over_dominant(self, tmp_path):
        ...
        inbox.mkdir()
        for i in range(5):
            _write_memo(inbox, f"2026-07-2{i}-project-makima-em-ask-{i}.md")
        _write_memo(inbox, "2026-07-26-project-makima-em-note.md", kind="fyi")
        candidates = _build_candidates(inbox, 10, 7, TODAY)
        ...
        assert buckets["2026-07-26-project-makima-em-note.md"] == "fyi"
        assert buckets["2026-07-20-project-makima-em-ask-0.md"] == "dominant"
...
class TestTrigger:
    ...
    def test_age_leg_alone_fires(self, tmp_path):
        # The load-bearing leg: one memo, 16 days old — project-rag's actual
