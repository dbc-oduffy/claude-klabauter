...
def _make_claude_home(tmp_path: Path, receiver_repos: dict[str, Path]) -> Path:
    ...
    receiver_repos: {registry_key_suffix: repo_path} e.g. {"project_rag": Path("/...")}
...
# guard-plus-scoped-to-q.md — routed schema question from project-rag-em's
...
#
# state/audits/2026-08-07-makima-install-dogfood-friction.md § F14: a
...
class TestScopedToShaResolvabilityGate:
    ...
    def test_unresolvable_sha_refusal_names_the_frame_of_reference(
        ...
        cross-repo/inbox/2026-08-18-project-rag-em-scoped-to-sha-refusal-
        message-frame.md.
        """
        receiver_repo = _make_receiver_git_repo(tmp_path)
        claude_home = _make_claude_home(tmp_path, {"project_rag": receiver_repo})
...
class TestEnvelopeShape:
    ...
    def test_dry_run_candidate_fields(self, tmp_path, monkeypatch):
        ...
        assert candidate["receiver"] == "project-rag-em"
    ...
    def test_act_acted_item_fields(self, tmp_path, monkeypatch):
        ...
        assert acted.get("receiver") == "project-rag-em"
    ...
    def test_act_written_memo_has_schema_valid_frontmatter(self, tmp_path, monkeypatch):
        ...
        assert '"project-rag-em"' in content  # to: quoted
    ...
    def test_act_with_custom_from_id(self, tmp_path, monkeypatch):
        ...
        params = {**_base_params(dry_run=False), "from_id": "project-makima-em"}
        result = _run(_memo_send(params))

        assert result["exit_code"] == 0
        content = Path(result["acted"][0]["id"]).read_text(encoding="utf-8")
        assert '"project-makima-em"' in content
...
class TestDeliveredMemoCommit:
    ...
    def test_post_commit_hook_suppressed_no_auto_push_driven(self, tmp_path, monkeypatch):
        ...
        Empirically confirms the one behavior Patrik flagged he had not
    ...
    def test_no_active_branch_receiver_left_uncommitted_no_branch_created(
        ...
        and dirty — branch-creation was REMOVED (2026-07-21, Patrik
...
class TestPathTraversalRejection:
    ...
    def test_dotdot_traversal_in_to_is_rejected(self, tmp_path, monkeypatch):
        ...
        traversal_to = "project-rag-em/../../etc"
    ...
    def test_containment_check_rejects_inbox_not_in_allowed_set(self, tmp_path):
        ...
        all_repos = {"repos.project_rag": str(registered_repo)}
...
class TestRegistryEnumeratedAllowedSet:
    ...
    def test_unregistered_receiver_dry_run_rejected(self, tmp_path, monkeypatch):
        ...
        # Registry has only project-rag registered
        receiver_repo = _make_receiver_git_repo(tmp_path)
        claude_home = _make_claude_home(tmp_path, {"project_rag": receiver_repo})
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
    ...
    def test_unresolved_to_suggests_nearest_registered_receiver(
        ...
        """
        receiver_repo = _make_receiver_git_repo(tmp_path)
        claude_home = _make_claude_home(tmp_path, {"project_makima": receiver_repo})
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))

        with caplog.at_level("ERROR"):
            result = _run(_memo_send(
                _base_params(to="makima-em", dry_run=False),
            ))

        assert result["exit_code"] == 1
        assert "project-makima-em" in caplog.text
        inbox = receiver_repo / "cross-repo" / "inbox"
        written = [f for f in inbox.iterdir() if f.name != ".gitkeep"]
    ...
    def test_registry_key_convention(self):
        ...
        assert _receiver_em_to_repo_key("project-rag-em") == "repos.project_rag"
        assert _receiver_em_to_repo_key("doe-claude-em") == "repos.doe_claude"
        assert _receiver_em_to_repo_key("project-makima-em") == "repos.project_makima"
        # Without -em suffix (bare shortname is also accepted)
        assert _receiver_em_to_repo_key("project-rag") == "repos.project_rag"
...
class TestInternalHelpers:
    ...
    def test_read_registry_repos_loads_local_overrides(self, tmp_path, monkeypatch):
        ...
        assert "repos.project_rag" in result
        assert result["repos.project_rag"] == str(tmp_path / "rag-repo")
...
class TestCollisionFailLoud:
    ...
    Prior art: Patrik finding #2b; improvement-queue 2026-06-23-hot-shared-branch-a-cross-
...
class TestSupersedesRedelivery:
    ...
    def test_redelivery_filename_disambiguates_via_supersedes_slug(self):
        ...
        """
        today = "2026-07-21"
        base = _memo_filename(today, "project-rag-em", "my-topic")
        redelivered = _redelivery_filename(today, "project-rag-em", "my-topic", "my-topic")
    ...
    def test_redelivery_filename_never_carries_the_senders_absolute_path(self):
        """project-opticon-em, 2026-08-17: `--supersedes <absolute path>` slugged
        ...
        """
        today = "2026-08-17"
        sender = "project-opticon-em"
        topic = "settings-home-ladder-rung-order-is-yours-to-call"
        predecessor = f"{today}-{sender}-{topic}.md"
        for ref in (
            f"/Users/oduffy/X/DoE-claude/cross-repo/inbox/{predecessor}",
            f"C:\\Users\\oduffy\\X\\DoE-claude\\cross-repo\\inbox\\{predecessor}",
...
class TestMemoSendThreeSeamRegistration:
    ...
    at THREE distinct seams to dispatch correctly:
      ...
      3. ops/__init__.py — _EAGER_OP_MODULES entry (the table
         `ipc._eager_import_all()`'s registry-miss fallback iterates to
         trigger the op module's register_op() side-effect and populate
         ipc._REGISTRY).
    ...
    def test_memo_send_registered_at_all_three_seams(self):
        ...
        assert "coordinator_core.ops.fleet.memo_send" in eager_module_paths, (
            ...
            "ops/__init__.py's _EAGER_OP_MODULES table — the "
            "ipc._eager_import_all() registry-miss fallback would no "
            "longer guarantee 'memo.send' registers."
        )
...
class TestReceiverAliasResolution:
    ...
    def test_receiver_alias_resolves_via_manifest(self, tmp_path, monkeypatch):
        """holodeck-em → repos.claude_unreal_holodeck via manifest alias lookup."""
        claude_home = _make_claude_home(tmp_path, {})
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
        _make_doe_manifest(tmp_path, claude_home, [
            {
                "registryKey": "claude_unreal_holodeck",
                "dirBasename": "claude-unreal-holodeck",
                "shortname": "holodeck",
            }
        ])

        # holodeck-em strips to shortname "holodeck"; aliases["holodeck"] = "claude_unreal_holodeck"
        result = _receiver_em_to_repo_key("holodeck-em")
        assert result == "repos.claude_unreal_holodeck", (
            f"holodeck-em must resolve via manifest alias to repos.claude_unreal_holodeck; "
            f"got: {result}"
        )

    def test_receiver_convention_when_no_alias(self, tmp_path, monkeypatch):
        """project-rag-em → repos.project_rag via convention when no alias entry in manifest."""
        claude_home = _make_claude_home(tmp_path, {})
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
        # Manifest present but only has holodeck alias — project-rag is not aliased
        _make_doe_manifest(tmp_path, claude_home, [
            {
                "registryKey": "claude_unreal_holodeck",
                "dirBasename": "claude-unreal-holodeck",
        ...
        result = _receiver_em_to_repo_key("project-rag-em")
        assert result == "repos.project_rag", (
            f"project-rag-em must fall back to convention (no alias); got: {result}"
        )
    ...
    def test_alias_graceful_degradation_when_manifest_absent(self, tmp_path, monkeypatch):
        ...
        assert _receiver_em_to_repo_key("project-rag-em") == "repos.project_rag"
        assert _receiver_em_to_repo_key("holodeck-em") == "repos.holodeck"
...
class TestComposedMemoYamlSchemaValid:
    """Parse makima's REAL _compose_memo output as YAML and assert schema shape.

    Closes the residual conformance gap the DoE round-trip fixture cannot catch:
    the fixture SIMULATES the makima write shape at most sites rather than driving
    makima's own op, and test_act_written_memo_has_schema_valid_frontmatter (above)
    only asserts via substrings ('status: open' in content) — neither actually
    parses makima's real emission with a YAML parser. These two tests take
    ...
    place in the suite that structurally certifies makima's actual emission shape.

    Review: patrik — finding 2 (the most important note): nothing currently runs
    makima's real composed emission through a structural schema check.
    """
    ...
    def test_compose_memo_output_parses_as_valid_yaml_frontmatter(self):
        ...
        assert frontmatter["to"] == "project-rag-em"
    ...
    def test_act_written_memo_parses_as_valid_yaml_frontmatter(self, tmp_path, monkeypatch):
        """A live act call's on-disk memo — makima's REAL emission, not a simulated fixture —
...
class TestCentralReceiverResolution:
    ...
    def test_claude_central_em_resolves_to_doe_claude_path(self, tmp_path, monkeypatch):
        """claude-central-em (the canonical makima-side central alias) resolves — was broken."""
    ...
    def test_non_central_receiver_unchanged(self, tmp_path, monkeypatch):
        """A non-central receiver (project-rag-em) resolves to its own repo, unaffected."""
        rag_repo = _make_receiver_git_repo(tmp_path, "rag-repo")
        doe_claude_repo = _make_receiver_git_repo(tmp_path, "doe-claude-repo")
        claude_home = _make_claude_home(
            tmp_path, {"project_rag": rag_repo, "doe_claude": doe_claude_repo}
        )
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
        _make_doe_manifest(tmp_path, claude_home, [])

        result = _run(_memo_send(
            _base_params(to="project-rag-em", dry_run=False, topic="non-central-unchanged")
        ))
...
class TestCanonicalReceiverIdStamping:
    ...
    def test_non_central_receiver_stamped_unchanged(self, tmp_path, monkeypatch):
        """A non-central receiver (project-rag-em) is stamped as-is — no rewrite."""
        ...
        result = _run(_memo_send(
            _base_params(to="project-rag-em", dry_run=False, topic="non-central-stamp")
        ))

        assert result["exit_code"] == 0, result
        memo_path = Path(result["acted"][0]["id"])
        assert self._read_to_field(memo_path) == "project-rag-em"
    ...
    def test_manifest_absent_canonicalization_is_passthrough(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_send(
            _base_params(to="project-rag-em", dry_run=False, topic="no-manifest-passthrough")
        ))
...
# Spec: cross-repo/inbox/2026-07-28-project-rag-em-inbox-blitz-proven-pattern.md
# ===========================================================================
...
class TestSpaceParam:
    ...
    def test_space_renders_into_frontmatter(self):
        content = _compose_memo(
            from_id="makima-engine", to="project-rag-em", topic="t", title="T",
...
class TestSenderOutboxSentStamp:
    ...
    def test_successful_send_stamps_outbox_copy_sent_with_evidence(
        ...
        outbox_path = _write_outbox_draft(
            sender_repo, topic, to="project-rag-em", title="Stamp test",
        )

        result = _run(_memo_send(
            _base_params(dry_run=False, topic=topic, to="project-rag-em"),
    ...
    def test_failed_dispatch_does_not_stamp_outbox_copy(self, tmp_path, monkeypatch):
        ...
        outbox_path = _write_outbox_draft(
            sender_repo, topic, to="project-rag-em", title="Collision test",
        )
    ...
    def test_missing_outbox_draft_is_silently_skipped(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="no-outbox-draft-here", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_already_sent_outbox_copy_is_not_reclobbered(self, tmp_path, monkeypatch):
        ...
        outbox_path = _write_outbox_draft(
            sender_repo, topic, to="project-rag-em", title="Already sent",
        )
        outbox_path.write_text(
...
class TestSentMemoLedger:
    ...
    def test_lifecycle_form_produces_both_sent_stamp_and_ledger_line(
        ...
        outbox_path = _write_outbox_draft(
            sender_repo, topic, to="project-rag-em", title="Lifecycle ledger test",
        )
    ...
    def test_second_send_appends_rather_than_truncates(self, tmp_path, monkeypatch):
        ...
        result_1 = _run(_memo_send(
            _base_params(dry_run=False, topic="append-first", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result_1["exit_code"] == 0, result_1
        result_2 = _run(_memo_send(
            _base_params(dry_run=False, topic="append-second", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
    ...
    def test_delivered_to_is_receiver_relative_never_absolute_home_path(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="home-path-check", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_ledger_write_failure_does_not_fail_the_send(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ledger-write-failure", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_ledger_line_carries_delivery_commit_reason_and_retried(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ac8-commit-fields", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
        delivery_commit = result["acted"][0]["delivery_commit"]
    ...
    def test_ledger_line_carries_real_delivery_sha(self, tmp_path, monkeypatch):
        """project-rag-em cross-repo memo, 2026-08-15 ("pickup cannot resolve
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="sha-ledger-test", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
        delivery_commit = result["acted"][0]["delivery_commit"]
    ...
    def test_ledger_line_sha_null_on_noop_and_commit_failure(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="sha-noop-test", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0
        delivery_commit = result["acted"][0]["delivery_commit"]
        ...
        result2 = _run(_memo_send(
            _base_params(dry_run=False, topic="sha-fail-test", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
...
class TestSenderLedgerCommitLeg:
    ...
    def test_send_commits_ledger_in_sender_repo_and_stamps_entry(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ac1-ledger-commit", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_ledger_commit_changed_path_set_is_a_single_path(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ac2-single-path", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_commit_failure_never_fails_the_send(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ac4-commit-failure", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_ledger_commit_sha_is_an_ancestor_of_head(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ac5-ancestry", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
        sha = result["acted"][0]["sender_ledger_commit"]
    ...
    def test_fresh_clone_untracked_ledger_does_not_retry(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ac5-fresh-clone-no-retry", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_commit_leg_uses_asyncio_to_thread_not_direct_subprocess(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="ac8-to-thread", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_ledger_append_uses_asyncio_to_thread_not_direct_call(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="p2-1-append-to-thread", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
...
class TestSentByRoundTrip:
    ...
    def test_delivered_memo_and_sent_ledger_row_agree_on_sent_by(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="c7-roundtrip", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_sent_by_unresolved_sentinel_says_so_not_silent_omission(
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="c7-unresolved", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
    ...
    def test_sent_by_never_a_resolved_address(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_send(
            _base_params(dry_run=False, topic="c7-not-address", to="project-rag-em"),
            repo_root=str(sender_repo / ".git"),
        ))
        assert result["exit_code"] == 0, result
