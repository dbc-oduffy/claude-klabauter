...
Spec backlink: pln-memo-tool-rebuild-makima-owns--bd5745 § C2
"""
...
class TestEnumerationMode:
    def test_enumerates_all_registered_receivers(self, tmp_path, monkeypatch):
        ...
        holo_repo = tmp_path / "claude-unreal-holodeck"
        rag_repo.mkdir()
        holo_repo.mkdir()
        claude_home = _make_claude_home(
            tmp_path,
            {"project_rag": str(rag_repo), "claude_unreal_holodeck": str(holo_repo)},
        )
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
        ...
        assert ids == {"repos.project_rag", "repos.claude_unreal_holodeck"}
        for c in receivers:
...
class TestResolutionMode:
    def test_resolved_to_reports_destination_inbox(self, tmp_path, monkeypatch):
        ...
        result = _run(_memo_list({"dry_run": True, "to": "project-rag-em"}))
    ...
    def test_unresolved_to_suggests_nearest_match(self, tmp_path, monkeypatch):
        """C4 parity: makima-em -> suggests project-makima-em, same as memo.send."""
        makima_repo = tmp_path / "project-makima"
        makima_repo.mkdir()
        claude_home = _make_claude_home(tmp_path, {"project_makima": str(makima_repo)})
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))

        result = _run(_memo_list({"dry_run": True, "to": "makima-em"}))

        candidate = result["candidates"][0]
        assert candidate["resolved"] is False
        assert "project-makima-em" in candidate["note"]
...
class TestResolvedFilename:
    def test_resolved_to_plus_topic_yields_resolved_filename_matching_send(
        ...
        result = _run(
            _memo_list(
                {"dry_run": True, "to": "project-rag-em", "topic": "example-topic"}
            )
        )
    ...
    def test_non_makima_caller_preview_matches_send_shared_derivation(
        self, tmp_path, monkeypatch
    ):
        """The defect this closes: a non-makima caller's from_id must produce
        ...
        # that was exactly the defect (preview always showed makima-engine
        # regardless of the caller's actual from_id).
        assert "makima-engine" not in candidate["resolved_filename"]
        assert "claude-central-em" in candidate["resolved_filename"]

    def test_makima_origin_caller_still_previews_correctly(self, tmp_path, monkeypatch):
        """Guard against regressing the currently-accidentally-correct case:
        a caller that supplies NO from_id (makima-origin / engine-default
        ...
        result = _run(
            _memo_list({"dry_run": True, "to": "project-rag-em", "topic": "smoke"})
        )
    ...
    def test_unknown_caller_identity_fails_loud_not_engine_fallback(
        ...
        assert "makima-engine" not in str(result)
    ...
    def test_invalid_topic_fails_loud_locked_to_memo_send_regex(self, tmp_path, monkeypatch):
        ...
        result = _run(
            _memo_list({"dry_run": True, "to": "project-rag-em", "topic": bad_topic})
        )
    ...
    def test_empty_string_topic_fails_loud_not_coerced_to_absent(self, tmp_path, monkeypatch):
        ...
        result = _run(
            _memo_list({"dry_run": True, "to": "project-rag-em", "topic": "   "})
        )
...
class TestNoWriteProof:
    ...
    def test_resolution_leaves_filesystem_unchanged_resolved(self, tmp_path, monkeypatch):
        ...
        before = _snapshot(tmp_path)
        _run(_memo_list({"dry_run": True, "to": "project-rag-em"}))
        after = _snapshot(tmp_path)
