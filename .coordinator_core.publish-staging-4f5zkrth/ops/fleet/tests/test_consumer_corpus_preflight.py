...
class TestScanRepoKindCounts:
    def test_scan_repo_kind_counts_splits_live_and_archived_exactly(self, tmp_path: Path) -> None:
        ...
        # Archived — NON-hidden nested dir under state/handoffs/ (holodeck shape).
...
class TestRunPreflightRepoResolution:
    def test_run_preflight_reports_exact_counts_and_unresolvable_bucket_separately(
        ...
        _handoff(makima_root, "archive/handoffs/2026-07/c.md", "spinoff")

        # project-rag: registered, but the path does not exist on disk on THIS machine.
        missing_path = tmp_path / "not-cloned-here" / "project-rag"

        # Every other fleet repo (opticon, rag-ue-addon, holodeck, market-intelligence)
        ...
        assert repos["project-makima"]["resolved"] is True
        assert repos["project-makima"]["counts_live"] == {}
        assert repos["project-makima"]["counts_archived"] == {"spinoff": 1}
        assert repos["project-makima"]["total"] == 1

        # project-rag: registered but path absent — UNRESOLVABLE, never a "zero records" claim.
        assert repos["project-rag"]["resolved"] is False
        assert "does not exist on disk" in repos["project-rag"]["reason"]

        # opticon: key never registered — UNRESOLVABLE, never a "zero records" claim.
        assert repos["opticon"]["resolved"] is False
        assert "not registered" in repos["opticon"]["reason"]

        unresolvable_names = {entry["repo"] for entry in report["unresolvable"]}
        assert unresolvable_names == {
            "project-rag", "opticon", "project-rag-ue-addon",
            "claude-unreal-holodeck", "market-intelligence",
        }
        ...
        assert "counts_live" not in repos["opticon"]
    ...
    def test_run_preflight_all_resolvable_and_scanned_is_exit_zero(
        ...
        assert report["repos"]["project-makima"]["resolved"] is True
        assert report["repos"]["project-makima"]["counts_live"] == {}
        assert report["repos"]["project-makima"]["counts_archived"] == {}
        assert report["repos"]["project-makima"]["total"] == 0
...
class TestUnclassifiedReconciliation:
    ...
    def test_known_non_fleet_excluded_key_does_not_trip_unclassified(
        ...
        junk_path.mkdir()
        with open(registry_local, "a", encoding="utf-8") as fh:
            fh.write(f'"repos.zzztest-normcheck" = "{junk_path}"\n')
...
class TestOffEnumDetection:
    def test_retired_kind_value_on_a_live_record_trips_off_enum_live_and_exit_nonzero(
        ...
        roots = _register_all_fleet(tmp_path, monkeypatch)
        _handoff(roots["opticon"], "state/handoffs/a.md", "spinoff-roadmap")
        _handoff(roots["project-rag-ue-addon"], "state/handoffs/b.md", "spinoff-roadmap")
        ...
        assert off_enum_by_repo == {
            ("opticon", "spinoff-roadmap"): 1,
            ("project-rag-ue-addon", "spinoff-roadmap"): 1,
        }
        assert report["off_enum_archived"] == []
    ...
    def test_retired_kind_value_on_an_archived_record_does_not_gate(
        ...
        roots = _register_all_fleet(tmp_path, monkeypatch)
        _handoff(roots["project-makima"], "archive/handoffs/2026-07/c.md", "spinoff-roadmap")
        _handoff(roots["claude-unreal-holodeck"], "state/handoffs/.archive/d.md", "spinoff-roadmap")
        _handoff(roots["claude-unreal-holodeck"], "state/handoffs/archive/e.md", "spike-result")
    ...
    def test_a_value_off_even_the_archived_enum_warns_without_gating(
        ...
        roots = _register_all_fleet(tmp_path, monkeypatch)
        _handoff(roots["project-makima"], "archive/handoffs/z.md", "not-a-real-kind")
        ...
        assert report["off_enum_live"] == []
        assert report["off_enum_archived"] == [
            {"repo": "project-makima", "kind": "not-a-real-kind", "count": 1}
        ]
    ...
    def test_archive_dir_variants_are_both_classified_archived(
        ...
        _handoff(roots["claude-unreal-holodeck"], "state/handoffs/archive/b.md", "spinoff-roadmap")
        ...
        assert report["repos"]["claude-unreal-holodeck"]["counts_archived"] == {"spinoff-roadmap": 1}
        assert report["repos"]["DoE-claude"]["counts_live"] == {}
        assert report["repos"]["claude-unreal-holodeck"]["counts_live"] == {}
