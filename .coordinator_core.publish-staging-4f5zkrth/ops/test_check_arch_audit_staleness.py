...
Golden oracle (Port of: check-arch-audit-staleness.sh, DoE b5a4192c, 2026-07-20),
...
def test_resolve_state_root_meta_repo_routes_to_makima(monkeypatch, tmp_path):
    monkeypatch.delenv("CAAS_TEST_STATE_ROOT", raising=False)
    monkeypatch.setattr(caas, "_git_root", lambda: str(tmp_path))
    monkeypatch.setattr(caas, "_claude_home", lambda: str(tmp_path))
    monkeypatch.setattr(caas, "_makima_root", lambda: "/makima/root")
    assert caas._resolve_state_root() == str(Path("/makima/root") / "state")


def test_resolve_state_root_meta_repo_unresolvable_makima_returns_none(
    monkeypatch, tmp_path
):
    monkeypatch.delenv("CAAS_TEST_STATE_ROOT", raising=False)
    monkeypatch.setattr(caas, "_git_root", lambda: str(tmp_path))
    monkeypatch.setattr(caas, "_claude_home", lambda: str(tmp_path))
    monkeypatch.setattr(caas, "_makima_root", lambda: None)
