...
class TestFsProbePath:
    ...
    def test_identity_on_non_windows(self, monkeypatch):
        ...
        assert _fs_probe_path("/x/doe-claude") == "/x/doe-claude"
        assert _fs_probe_path("/home/oduffy/DoE-claude") == "/home/oduffy/DoE-claude"
...
class TestTierAGreedyDecode:
    ...
    def test_resolves_hyphenated_segment(self, tmp_path: Path):
        # "X:\dev\fifa-stats" encodes to "X--dev-fifa-stats"; the naive decode
        # (blanket - -> \) turns "fifa-stats" into "fifa\stats" which doesn't
        # exist. The greedy walk should recover "fifa-stats" as one segment.
        root = tmp_path
        (root / "dev" / "fifa-stats").mkdir(parents=True)
        out = _tier_a_greedy_decode("dev-fifa-stats", "X", str(root))
        assert out == "X:\\dev\\fifa-stats"
...
class TestDecodeProjectsDirName:
    def test_drive_letter_form(self):
        drive, rest, decoded = _decode_projects_dir_name("X--dev-fifa-stats")
        assert drive == "X"
        assert rest == "dev-fifa-stats"
    ...
    def test_posix_form_out_of_scope_gap(self):
        ...
        drive, rest, decoded = _decode_projects_dir_name("-Users-oduffy-X-DoE-claude")
        assert drive == ""
        assert decoded == "\\Users\\oduffy\\X\\DoE\\claude"
...
class TestSortUnique:
    def test_matches_locale_collation_not_ordinal(self):
        ...
        out = _sort_unique(["DoE-claude", "delphi-store", "DroneSim-md"])
        assert set(out) == {"DoE-claude", "delphi-store", "DroneSim-md"}
        assert len(out) == 3
