...
def test_stale_draft_emits_nudge(tmp_path, monkeypatch):
    ...
    _write_draft(f, to="project-rag-em", title="Shape memo for project-rag")
    _backdate(f, 30 * 3600)

    rc, out = _run([], {"COORDINATOR_OUTBOX_DIR": str(outbox)}, monkeypatch)
    assert rc == 0
    assert "Outbox draft test-topic staged 30h ago → project-rag-em  :: Shape memo for project-rag" in out
...
def test_fresh_draft_silent(tmp_path, monkeypatch):
    ...
    _write_draft(f, to="holodeck-em", title="Fresh memo")
    _backdate(f, 3600)
