...
cycle is detected. Returns the ordered ancestry list. This is makima's compute half
...
X") is project-rag's surface per
...
- Does NOT build the durable/transitive fleet-wide ancestry query — that is
  project-rag's surface (ratified tri-plane boundary). This op is the live,
...
def _handler(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    Params:
        ...
        repo (str):       Accepted and ignored for path resolution — path
                          ...
                          git common dir), not ``repo``. Negative-spec: makima
