...
def test_slugify_replaces_path_separators_with_dashes() -> None:
    assert _slugify_repo_root("/home/example/repos/project-makima") == (
        "-home-example-repos-project-makima"
    )
...
def test_slugify_encodes_drive_letter_colon() -> None:
    """Real Windows paths carry a drive-letter colon (``X:\\project-makima``),
    which Claude Code's own ``~/.claude/projects/<slug>/`` naming also
    encodes (verified on-disk: ``X--project-makima``). A test path without a
    ...
    assert _slugify_repo_root("X:\\project-makima") == "X--project-makima"
...
def test_slugify_encodes_every_character_in_the_documented_scheme():
    ...
    ``C:\Users\oduffy\.claude`` is ``C--Users-oduffy--claude`` on disk — the
