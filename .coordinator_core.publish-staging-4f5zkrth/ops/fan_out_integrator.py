...
coordinator/bin/fan-out-integrator.sh (DoE-claude, 469 lines, DOE-PORT clean-slate
migration, variant #1 — direct-import trampoline, no registered op; DoE keeps the
...
DEDICATED, higher exit code for a makima-import/transport failure is owned by the
...
module's `main()` never emits anything outside the three codes below):
    ...
    2 — invocation error (usage, environment — including this module's own
        ...
        since it is a purely local/deterministic DoE-side resolution, not a
        makima-engine transport failure; see the trampoline's own exit-3 contract
        for the transport-failure case, per docs/wiki addendum § 3b)

PLUGIN_ROOT resolution: this module needs to locate two DoE-side snippet files
(snippets/peer-scope-block.md, snippets/text-only-recovery-preamble.md) that live
in the DoE-claude repo, not in makima. `CLAUDE_PLUGIN_ROOT` env var is rung 1 and
...
or a future non-DoE-trampoline caller).
...
Port source: coordinator/bin/fan-out-integrator.sh (DoE-claude, 469 lines,
