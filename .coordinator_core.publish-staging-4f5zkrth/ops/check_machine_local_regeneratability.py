...
Negative-spec (do NOT "fix" while porting):
    - The bash oracle's ``COORDINATOR_OWNED_KEYS`` list is reproduced verbatim
      ...
      includes ``repos.fifa_26wcle_data`` / ``repos.project_opticon`` / ``repos.delphios``).
      ...
      ``repos.claude_klabauter`` was added deliberately — makima's own installer writes
      this key (``register_makima_root()``, DR-261 gives makima klabauter publishing), so
      it is makima-owned and belongs on the canonical list. This is the one reviewed,
    ...
    - Check 1's key match is exact-string, plus two named family-prefix arms (see
      ...
      NO production call-site in makima today. Check 1 in ``main()`` only iterates
...
# docstring negative-spec). "engine.working_repos" is DoE-declared on their plane; makima
...
def _declared_owned_keys(ml_dir: Path) -> List[str]:
    ...
    the dotted top-level table names (``repos.dronesim``, ``publish.targets``)
    ...
    module's own docstring), and makima supplies only the family-prefix match
    ...
    for toml_file in _tracked_toml_files(ml_dir):
        ...
        for key, value in data.items():
            ...
            if head not in COORDINATOR_OWNED_NAMESPACES:
                continue
            # Both TOML spellings of the same declaration. `["repos.dronesim"]`
            # (quoted) parses to a flat dotted top-level key; `[repos.dronesim]`
