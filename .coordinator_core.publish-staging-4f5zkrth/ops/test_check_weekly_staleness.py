...
def test_resolve_state_root_meta_repo_routes_to_makima(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cws, "_makima_root", lambda: "/makima/root")
    assert cws._resolve_state_root() == str(Path("/makima/root") / "state")
...
def test_resolve_state_root_meta_repo_unresolvable_makima_returns_none(
    ...
    monkeypatch.setattr(cws, "_git_root", lambda: str(tmp_path))
    monkeypatch.setattr(cws, "_claude_home", lambda: str(tmp_path))
    monkeypatch.setattr(cws, "_makima_root", lambda: None)
