...
def repo(tmp_path) -> StampRepo:
    ...
    r = StampRepo(root)
    r._git("init", "-b", "main")
    r._git("config", "user.email", "stamp-test@makima.test")
...
def test_post_commit_live_children_check_narrows_edge_kinds_so_a_live_spinoff_does_not_retain(
    ...
    tail's writer from stamping/shipping (project-opticon-em, 2026-08-05,
...
# survives verbatim (the project-makima 506748a0 incident shape, closed).
