...
Coverage:
  ...
  (e)  a concurrent sibling's own staged DELETION, outside the explicit
       ...
       inverse check (the exact 2026-07-07 project-opticon incident shape).
...
# isolation -- proves the 2026-07-07 project-opticon incident fix survives
...
def test_kpi_spawn_count_git_only_and_collapsed(tmp_path, monkeypatch):
    ...
    `git commit` by hand (the shape of two real incidents -- project-makima
...
def wsc_tail_repo(tmp_path) -> WscTailRepo:
    ...
    r = WscTailRepo(root)
    r._git("init", "-b", "main")
    r._git("config", "user.email", "wsc-tail-parity@makima.test")
...
def test_chain_terminal_commit_abort_stamps_never_evaluated_and_labels_nodes(
    ...
    """Regression for cross-repo/inbox/2026-08-04-project-rag-em-wsc-tail-
...
def test_archive_sweeps_do_not_fire_when_commit_pipeline_fails(wsc_tail_repo, monkeypatch):
    ...
    # Labelled-skip fix (cross-repo/inbox/2026-08-04-project-rag-em-wsc-tail-
...
# coverage.gate's "precommit.coverage_gate_fire"/"precommit.coverage_gate_join"
# spans were removed here (K-001, state/kill-ledger.md, 2026-08-16) along with
# tail_ops.run_coverage_gate itself -- the ceremony-close fire/join this
# instrumented no longer runs.
