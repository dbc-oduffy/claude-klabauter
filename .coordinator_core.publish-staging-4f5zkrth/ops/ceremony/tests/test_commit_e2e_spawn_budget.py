...
def _count_op_spawns_both_ways(fn) -> tuple[int, int, int, dict]:
    ...
    def _git_wrapper(*a, **kw):
        ...
        seam_argvs.append(list(argv) if argv is not None else [])
    ...
    return seam["n"], allc["n"], retries["extra"], result, seam_argvs


def _seam_subcommands(seam_argvs: list) -> list[str]:
    """Reduce a captured seam-argv list to its ordered git SUBCOMMAND names
    (`_git()`'s own `args` param excludes the leading `"git"` token, see
    that function's docstring, but several callers still lead with a global
    flag -- `-c core.quotepath=false`, `--no-optional-locks` -- ahead of the
    actual subcommand). Walks past leading flag tokens (`-c` consumes the
    following `key=value` token too) to find the first non-flag token. Byte
    content (shas, temp paths, messages) varies run to run and is
    deliberately not part of this shape; the subcommand sequence is what a
    refused/short-circuited run cannot coincidentally reproduce (C6,
    docs/plans/2026-08-22-a-commit-is-one-spawn-not-eleven.md)."""
    out: list[str] = []
    for argv in seam_argvs:
        i = 0
        cmd = ""
        while i < len(argv):
            tok = argv[i]
...
def _assert_argv_shape(shape: str, seam_argvs: list, required: list[str]) -> None:
    """C6: assert on ARGV, not only on a count. `required` names every git
    subcommand this shape's ladder must actually have issued, each exactly
    once unless repeated in the list -- a refusal path that short-circuits
    before staging (`refusal_path_unanswerable`'s own 0-spawn shape) or a
    rerouted spawn that still fires but off the counted seam (the
    `head_blobs`/`run_git` split this module's own manifest rationale
    documents) both fail this check even when a raw integer coincidentally
    matches the budget. This does not replace the exact-equality spawn-count
    assertion below -- the two are complementary, per this module's own
    `_assert_op_total` precedent for seam vs. op_total."""
    seen = _seam_subcommands(seam_argvs)
    missing = [cmd for cmd in required if cmd not in seen]
    assert not missing, (
        "%s: expected git subcommand(s) %r on the seam, missing %r -- observed "
        "sequence was %r. A low/matching spawn COUNT here can still mean the "
        "run refused early or a spawn moved off the counted seam; this shape "
        "check is what a bare integer cannot catch."
        % (shape, required, missing, seen)
    )
...
def test_green_path_spawn_count_matches_budget(tmp_path_factory):
    ...
    n, n_all, retries_fired, result, seam_argvs = _count_op_spawns_both_ways(_run)
    ...
    _assert_argv_shape(
        "green_path",
        seam_argvs,
        ["add", "commit", "interpret-trailers", "rev-parse"],
    )
...
def test_directory_pathspec_expansion_spawn_count_matches_budget(tmp_path_factory):
    ...
    _assert_argv_shape(
        ...
        seam_argvs,
        ["add", "commit", "interpret-trailers", "rev-parse", "diff"],
    )
...
def test_push_raced_path_spawn_count_and_sleep_match_budget(tmp_path_factory):
    ...
    _assert_argv_shape(
        "push_raced_path",
        seam_argvs,
        ["add", "commit", "interpret-trailers", "rev-parse", "push"],
    )
...
def test_deferred_diverged_commit_reaches_detached_push_spawn_count_matches_budget(
    ...
    _assert_argv_shape(
        ...
        seam_argvs,
        # C8a/C8b's in-process rewire of `_commit_scoped_private_index` took the
        # `read-tree`/`add`/`update-index --cacheinfo`/`write-tree`/`commit-tree`/
        # `update-ref` ladder off the seam entirely (`write_object` + a locked
        # `cas_ref`, both in process) -- `interpret-trailers` is the one native-git
        # call this staged-only (no worktree-sourced content) diverged commit still
        # issues. A reappearance of any ladder subcommand here means the fast path
        # regressed to the fall-back, not a false failure to fix by re-adding them.
        ["interpret-trailers"],
    )
