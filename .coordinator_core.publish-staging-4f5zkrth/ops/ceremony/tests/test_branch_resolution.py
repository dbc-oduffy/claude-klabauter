...
def test_disposition_grep_fallback_single_ignores_sibling_consumed_handoff(git_repo):
    ...
    makima-engine.md (doe_id 2026-07-09-wsc-resolve-mis-anchors-on-a-sibling-
...
def test_session_shape_handoff_path_absent_phantom_falls_through(repo):
    ...
    Originating incident: cross-repo/inbox/2026-07-11-project-rag-em-wsc-resolve-
...
# primary-path guard validate a file in a DIFFERENT repo (e.g. project-rag)
...
def test_session_shape_handoff_path_absolute_foreign_repo_rejected(repo, tmp_path):
    ...
    e.g. project-rag) whose frontmatter even has consumed_by: <sid> — proving
...
# consumed_handoff_paths, the 2026-07-13 project-opticon incident shape.
#
...
def test_sanitize_drops_foreign_absolute_from_merged_set(repo, tmp_path):
    ...
    foreign_repo = tmp_path / "project-opticon"
...
# Patrik-finding regression cases (F0 dedup, F2 STEP_2_7 plural evidence —
...
def test_n_handoffs_both_found_live_and_archived_scalar_is_first(repo):
    ...
    # Patrik F2 producer side: the STEP_2_7 node's own evidence dict must carry
...
def test_n_handoffs_dedup_pickup_named_handoff_also_found_by_scan(repo):
    """Patrik F0: session-shape.json pickup.handoff names a handoff that the
...
def test_grep_fallback_returns_all_matches_not_just_first(git_repo):
    """Patrik F6: with session-shape.json absent entirely (triggering
...
def test_c1_divergent_scalar_field_rejected_by_validate():
    """Patrik F4: a hand-edited/round-tripped context where consumed_handoff !=
...
def test_c1_divergent_predecessor_field_rejected_by_validate():
    """Patrik F4 parallel case: predecessor != predecessors[0] must also fail
    validate()."""
    ctx = PipelineContext(
        ceremony="wsc",
        scope_mode="",
...
# branch_resolution._resolve_branches. Extends project-makima commit
...
def test_env_override_shared_helper_agrees_with_bin_script(monkeypatch, tmp_path):
    ...
    keeps its OWN copy of the override-parsing logic (landed by project-makima
