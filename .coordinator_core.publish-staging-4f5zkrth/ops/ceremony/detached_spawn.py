...
- `clear_failures_log(repo_root)` — best-effort truncate, called by EVERY orientation-cache
  ...
  cross-repo/inbox/2026-07-28-project-rag-em-orientation-cache-housekeeping-flood.md); all
...
def _child_env(repo_root: str) -> dict:
    ...
    Mirrors `auto_push._respawn_env`'s `_makima_package_root()` injection -- the SAME
