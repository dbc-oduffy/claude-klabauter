...
def session_commit_count_attributed(worktree_root: Path, sid: str) -> dict:
    ...
    Patrik F4): the underscore name was being reached across packages by
    ...
    KNOWN UNDERCOUNT (Patrik F5, R-11/DR-319): `--grep=Session-Id:` only sees commits
...
#   cross-repo/archive/2026-07-12-project-rag-em-wsc-concurrent-tree-race.md
# ---------------------------------------------------------------------------
...
def _sanitize_consumed_handoffs(
    ...
    *,
    ...
    Defense-in-depth choke point (foreign-repo bleed defect, 2026-07-13 opticon
...
def _resolve_branches(  # noqa: C901  (complex but linear — each branch is 1 section)
    ...
    if is_env_override:
        ...
        if use_grep_fallback:
            ...
            if canonicalize(disposition) == PREDECESSOR_CONSUMED and consumed_handoff_path:
                ...
                # file in ANOTHER repo (e.g. project-rag).  _resolve_in_repo asserts the
            ...
            # anchored recovery scan just below (Patrik F0) re-derives every
            ...
            # Patrik F0: a DAG pickup can own N predecessor handoffs even when
    ...
    # Defense-in-depth final gate (foreign-repo bleed defect, 2026-07-13 opticon
    ...
    ctx.add_node(handle_d(STEP_2_7, resolving_op="coordinator-handoff-archive.sh:stamp-only",
                          evidence={
                              ...
                              # Patrik F2: _read_step_2_7_evidence reads THIS node (not
