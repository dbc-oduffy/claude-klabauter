...
def _validate_row(row: dict, *, governed: bool = False, plan_created: Optional[str] = None) -> list:
    ...
    Nor can they: `check_plan_tasks_source` hardcodes makima's own vendored
    schema, while the write guards deliberately resolve DoE's vendored
    corpus copy (which its own docstring notes has drifted from makima's),
...
# party one extra call — project-opticon-em read it as impossibility, could
...
# enforcement leg asked of makima by that plan's C8 memo.
...
def _to_repo_relative(path: str, worktree: Path) -> str:
    ...
    — a central-scope (makima) or lessons-outbox (DoE) write can legitimately
