...
gate_source is DATA, never a command (Review: zoli-cutover-review F3):
...
zoli-cutover-review F2, engaging with ``cartography_edges.py``'s own standing
...
zoli-cutover-review F2): ``value-vocabulary``'s implementation below always
...
Foreign-repo handling (Review: zoli-cutover-review F5): this module receives
``repo_roots`` — the set of repos the CALLER can actually resolve a local
path for (DoE + makima ``coordinator/bin/``, per D2). For every repo named in
...
below): landed in this same module file (Review: zoli-cutover-review F8/F14 —
...
DR-208 five-question affirmation (COMPUTE_ONLY; citing ``_cutover_gate``):
  ...
  alternatives, Review: zoli-cutover-review F7, PM-approved in full
...
#: never vendored into makima (unlike the frontmatter schemas
...
def _derive_value_vocabulary(
    ...
    This is the kind's fixed mode set (Review: zoli-cutover-review F2) — no
    ...
    for repo_name, root in scanned_roots.items():
        ...
        # Supplementary structural pass (Review: zoli-cutover-review F2 /
...
def derive(gate_source: Mapping[str, object], repo_roots: Mapping[str, Path]) -> dict[str, object]:
    ...
    Args:
        ...
        repo_roots: map of repo-name -> local filesystem root for every repo
            this process can actually resolve (DoE + makima
...
def resolve_cutover_schema(
    ...
    module, not a step inside either (Review: zoli-cutover-review F14).
...
#: two repos ("doe-claude" vs "DoE-claude", "project-makima" vs "makima") are
...
_MAKIMA_ROOT_ALIASES: tuple[str, ...] = ("project-makima", "project_makima", "makima")
...
def _build_repo_roots(doe_root: Path) -> dict[str, Path]:
    ...
    The makima root is THIS repo's own root, derived via an in-repo
    ...
    """
    makima_root = Path(__file__).resolve().parents[2]
    roots: dict[str, Path] = {}
    for alias in _DOE_ROOT_ALIASES:
        roots[alias] = doe_root
    for alias in _MAKIMA_ROOT_ALIASES:
        roots[alias] = makima_root
    return roots
...
# ---------------------------------------------------------------------------
# Signal-2 re-verification (Review: zoli-cutover-review F7, PM-approved in
...
#: `state/cross-repo-commitments/2026-07-21-makima-to-land-c9-a11-scoped-to-
...
async def _cutover_gate(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    if derived_count == 0:
        notes.append(
            ...
            "decision, Review: zoli-cutover-review F1)"
        )
        return _gate_reply(
            f"{header} VERDICT=INDETERMINATE", notes, 2, derivation_history_entry=history_entry
        )
    ...
    if foreign_unconfirmed:
        notes.append(
            ...
            "PASS (fail-closed on 'cannot see', Review: zoli-cutover-review F5)"
        )
        return _gate_reply(
            f"{header} VERDICT=INDETERMINATE", notes, 2, derivation_history_entry=history_entry
        )
