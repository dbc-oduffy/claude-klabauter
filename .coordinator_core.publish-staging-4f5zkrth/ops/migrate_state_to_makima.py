"""
coordinator_core.ops.migrate_state_to_makima — two-phase state/docs migration
from ~/.claude (CLAUDE_HOME) to the makima sibling repo
(COORDINATOR_ENGINE_SOURCE_ROOT).

Purpose: DR-059 bash-to-naked-Python port of the DoE-owned script
`coordinator/bin/migrate-state-to-makima.sh` (~346 lines). Implements the
...
Two subcommands:
  ...
  --finalize (C6b)
    1. Delta re-sync: copies any CLAUDE_HOME source file NEWER than its makima
       counterpart (captures writes that happened after --populate).
    2. Per-path guard: before removing ANY source file, confirms the makima
...
Spec backlink: pln-stop-the-rot-makima-state-home-placement-4cc787
               § Phase 2 / C6 / AC5 / § Execution Notes (C6a/C6b split)
               docs/plans/2026-07-15-bash-to-naked-python-engine-migration.md
Port of: migrate-state-to-makima.sh (DoE b5a4192c, 2026-07-20)
...
Exit codes (parity-critical, preserved from the bash oracle):
    ...
    1 — business error: no subcommand / unknown subcommand, CLAUDE_HOME not
        ...
        with no confirmed makima copy (fail-closed; nothing was removed)
...
def cmd_populate(claude_home: str, makima_root: str, out: TextIO) -> int:
    """--populate (C6a): copy source trees to makima; originals untouched."""
    print("=== migrate-state-to-makima --populate (C6a) ===", file=out)
    print(f"  Source (CLAUDE_HOME): {claude_home}", file=out)
    print(f"  Destination (COORDINATOR_ENGINE_SOURCE_ROOT): {makima_root}", file=out)
    print("", file=out)

    for src_rel, dst_rel in TREE_PAIRS:
        src = os.path.join(claude_home, src_rel)
        dst = os.path.join(makima_root, dst_rel)
...
def cmd_finalize(claude_home: str, makima_root: str, out: TextIO) -> int:
    ...
    """
    print("=== migrate-state-to-makima --finalize (C6b) ===", file=out)
    print(f"  Source (CLAUDE_HOME): {claude_home}", file=out)
    print(f"  Destination (COORDINATOR_ENGINE_SOURCE_ROOT): {makima_root}", file=out)
    print("", file=out)

    # --- Step 1: Delta re-sync — copy any source file NEWER than its makima
    # copy (or whose makima copy is missing). Captures writes that happened
    ...
    # is confirmed in makima before removing ANYTHING. Fail loud on any
    ...
    for src_rel, dst_rel in TREE_PAIRS:
        ...
        for src_file in _walk_files(src):
            ...
            if not os.path.isfile(dst_file):
                print(f"  GUARD FAIL: makima copy missing for: {src_file}", file=sys.stderr)
                print(f"             expected at: {dst_file}", file=sys.stderr)
                missing_count += 1

    if missing_count > 0:
        print("", file=sys.stderr)
        print(f"ERROR: {missing_count} source file(s) have no confirmed makima copy.", file=sys.stderr)
    ...
    print("  Guard PASSED: all source files confirmed present in makima.", file=out)
    print("", file=out)
    ...
    print(f"    Working data now lives in {makima_root}.", file=out)
    return 0


def _default_makima_root() -> str:
...
def _usage(out: TextIO) -> None:
    print("Usage: migrate-state-to-makima.sh --populate | --finalize", file=out)
    print("", file=out)
    print("  --populate   Copy ~/.claude working data to makima (C6a).", file=out)
...
def main(argv: List[str]) -> int:
    ...
    Environment overrides (mirror the bash oracle):
      COORDINATOR_ENGINE_SOURCE_ROOT  — override the makima repo path
                     (default: this module's own repo root, see
                     `_default_makima_root`). Read via
                     `coordinator_engine_source_root_env`, which falls back
                     to the dispatch variable (COORDINATOR_ENGINE_ROOT, or
                     its legacy spelling MAKIMA_ROOT) during the transition.
    ...
    if subcommand not in ("--populate", "--finalize"):
        ...
        print("Usage: migrate-state-to-makima.sh --populate | --finalize", file=sys.stderr)
        return 1
    ...
    # accessor rather than a bare MAKIMA_ROOT read.
    makima_root = coordinator_engine_source_root_env("migrate_state_to_makima") or _default_makima_root()
    makima_root = makima_root.rstrip("/")

    if subcommand == "--populate":
        return cmd_populate(claude_home, makima_root, sys.stdout)
    return cmd_finalize(claude_home, makima_root, sys.stdout)
