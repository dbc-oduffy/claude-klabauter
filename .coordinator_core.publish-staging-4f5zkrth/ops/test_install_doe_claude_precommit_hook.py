...
`coordinator_core.ops.install_makima_precommit_hook`'s own test-file
...
def test_missing_gate_script_advisory_fires_at_install_time(tmp_path, monkeypatch, capsys):
    ...
    is not expected to exist in project-makima's tree -- when this installer
...
def test_foreign_hook_ending_in_unconditional_exit_1_swallows_appended_gates(
    ...
    plumbing with the ratified makima installer; changing it here is out of
