...
_DEFAULT_SLUG_MAP = {
    "X--project-makima": "X:/project-makima",
...
def _build_fixture(tmp_path, project_slug="X--project-makima"):
    ...
    (tmp_path / "pytest-of-oduffy").mkdir(exist_ok=True)
...
def test_dry_run_is_default_and_mutates_nothing(tmp_path):
    ...
    old_scratch = tmp_path / "claude" / "X--project-makima" / _SID_DEAD_OLD / "scratchpad"
...
def test_live_session_never_touched(tmp_path):
    ...
    scratch = tmp_path / "claude" / "X--project-makima" / _SID_LIVE / "scratchpad"
...
def test_dead_but_recent_is_too_recent_not_reclaimed(tmp_path):
    ...
    scratch = tmp_path / "claude" / "X--project-makima" / _SID_DEAD_RECENT / "scratchpad"
    assert scratch.is_dir()


def test_dead_and_old_is_reclaimed_when_opted_in(tmp_path):
    _build_fixture(tmp_path)
    scratch = tmp_path / "claude" / "X--project-makima" / _SID_DEAD_OLD / "scratchpad"
    assert scratch.is_dir()
...
def test_own_session_never_reclaimed_even_if_dead_and_old(tmp_path):
    ...
    self_scratch = fixture_root / "claude" / "X--project-makima" / _SID_SELF / "scratchpad"
...
def test_project_slugs_filter_restricts_scope(tmp_path):
    _build_fixture(tmp_path, project_slug="X--project-makima")
    _build_fixture(tmp_path, project_slug="Y--other-project")

    result = _sweep(tmp_path, project_slugs=["X--project-makima"])
    slugs = {e["project_slug"] for e in result["entries"]}
    assert slugs == {"X--project-makima"}
...
def test_two_project_slugs_resolve_independent_liveness(tmp_path):
    ...
    result = _sweep(
        tmp_path,
        slug_to_root_map={"X--project-makima": "X:/project-makima"},
        reclaim=True,
    )

    mapped_entries = [
        e for e in result["entries"] if e["project_slug"] == "X--project-makima"
    ]
    unmapped_entries = [
...
def test_encode_project_slug_matches_harness_convention():
    assert _encode_project_slug("X:\\project-makima") == "X--project-makima"
    assert _encode_project_slug("X:/project-makima") == "X--project-makima"
...
def test_build_slug_to_root_map_drops_encoding_collisions(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: ["X:/project.makima", "X:/project-makima"],
    )
    mapping = _build_slug_to_root_map()
    # Both encode to "X--project-makima" — collision, dropped.
    assert "X--project-makima" not in mapping


def test_build_slug_to_root_map_keeps_unambiguous_roots(monkeypatch):
    monkeypatch.setattr(
        "coordinator_core.ops.scratchpad_sweep._known_repo_roots",
        lambda: ["X:/project-makima", "X:/DoE-claude"],
    )
    mapping = _build_slug_to_root_map()
    assert mapping["X--project-makima"] == "X:/project-makima"
...
def test_size_cut_stops_at_right_threshold_whole_cohorts_only(tmp_path):
    ...
    result = _sweep(
        tmp_path,
        slug_to_root_map={slug: "X:/project-makima"},
        project_slugs=[slug],
        reclaim=True,
...
def test_archive_shaped_file_is_detected_and_totaled(tmp_path):
    ...
    result = _sweep(
        tmp_path, slug_to_root_map={slug: "X:/project-makima"}, project_slugs=[slug]
    )
