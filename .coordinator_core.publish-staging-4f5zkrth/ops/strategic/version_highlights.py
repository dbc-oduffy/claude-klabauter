...
from repo history. Signal readers, in priority order:
  ...
  2. **Tag-less repo (makima's own path — 0 git tags verified)** — read
...
Spec backlink: pln-makima-generation-leg-machine--127c81 § C2
"""
...
def _summarize_plans_touched(field_value: str) -> str | None:
    ...
    for raw_entry in field_value.split(","):
        ...
        # (`.patrik-review`, `.prior-art-check`, ...), including the archival
        ...
        # (`.patrik-review`, `.sonnet-review`, `.node-map`, ...) baked into the
        # emitted slug.
