...
fall back on: `MAKIMA_ROOT`, `COORDINATOR_ENGINE_ROOT`, `COORDINATOR_ROOT`,
`CLAUDE_PROJECT_DIR`.
...
constant (e.g. `_MAKIMA_ROOT_ENV = "MAKIMA_ROOT"`) plus a bare read elsewhere --
...
ROOT_ENV_NAMES = frozenset(
    {"MAKIMA_ROOT", "COORDINATOR_ENGINE_ROOT", "COORDINATOR_ROOT", "CLAUDE_PROJECT_DIR"}
)
...
def _env_name_constants(path: Path, names: frozenset[str]) -> list[tuple[int, str]]:
    ...
    like `_MAKIMA_ROOT_ENV = "MAKIMA_ROOT"` (or any local alias of the same
...
def test_family_declares_no_root_env_name_constant(prefixes: tuple[str, ...]) -> None:
    ...
    This is the shape that bites: `_MAKIMA_ROOT_ENV = "MAKIMA_ROOT"` plus a bare
