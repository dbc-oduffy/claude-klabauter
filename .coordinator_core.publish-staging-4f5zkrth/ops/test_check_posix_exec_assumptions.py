...
_EXEMPTED_IN_MAKIMA = "coordinator/bin/plan-tasks-resolve"
...
def _repo_named(tmp_path: Path, dir_name: str) -> Path:
    ...
    one at a relpath project-makima holds an exemption for, one at a relpath
    ...
    _git(repo, "config", "user.email", "test@example.com")
    _git(repo, "config", "user.name", "Test")
    for relpath in (_EXEMPTED_IN_MAKIMA, "coordinator/scripts/control.py"):
        target = repo / relpath
...
def test_exemption_granted_for_one_repo_does_not_exempt_that_relpath_in_another(tmp_path):
    ...
    project-makima happens to hold an exemption for must still be REPORTED."""
    sibling = _repo_named(tmp_path, "some-unrelated-sibling")

    result = scan(sibling)

    assert repo_key_for_root(sibling) == "some_unrelated_sibling"
    assert _EXEMPTED_IN_MAKIMA in result["env_shebang"]
    assert _EXEMPTED_IN_MAKIMA in result["mode_100755"]
...
def test_exemption_granted_for_one_repo_still_applies_in_that_repo(tmp_path):
    ...
    makima_like = _repo_named(tmp_path, "project-makima")

    result = scan(makima_like)

    assert repo_key_for_root(makima_like) == REPO_PROJECT_MAKIMA
    assert _EXEMPTED_IN_MAKIMA not in result["env_shebang"]
    assert _EXEMPTED_IN_MAKIMA not in result["mode_100755"]
...
def test_real_tree_every_makima_exemption_is_actually_subtracted():
    """Repo-scoping must leave project-makima's OWN scan result unchanged:
    every relpath exempted under the project_makima key is still invisible in
    ...
    for cls, per_repo in EXEMPTIONS.items():
        for relpath in per_repo.get(REPO_PROJECT_MAKIMA, {}):
            assert relpath not in result[cls], (
                f"{relpath} is exempted for project_makima in class {cls} but "
...
def test_real_tree_ratchets_clean_against_committed_baseline():
    """Wiring proof for project-makima's own test tier: scanning THIS repo
...
_MAKIMA_PREFIX = "dist/mirror-native/"
...
def test_prefix_exclusion_hides_a_preserved_tree_from_every_non_tier_a_class(tmp_path):
    ...
    repo = _repo_with_preserved_tree(tmp_path, "project-makima", _MAKIMA_PREFIX)
    inside = f"{_MAKIMA_PREFIX}nested/check-thing.py"

    result = scan(repo)

    assert repo_key_for_root(repo) == REPO_PROJECT_MAKIMA
...
def test_prefix_exclusion_does_not_swallow_a_similarly_named_sibling_path(tmp_path):
    ...
    repo = _repo_with_preserved_tree(tmp_path, "project-makima", _MAKIMA_PREFIX)
    outside = f"{_MAKIMA_PREFIX.rstrip('/')}-scratch/check-thing.py"
    ...
    assert not is_prefix_excluded(outside, REPO_PROJECT_MAKIMA)
...
def test_prefix_exclusion_never_suppresses_tier_a_checkout_breakers(tmp_path):
    ...
    repo = _repo_with_preserved_tree(tmp_path, "project-makima", _MAKIMA_PREFIX)
    long_rel = _MAKIMA_PREFIX + "/".join(["preserved-deep-directory-segment"] * 7) + ".txt"
...
def test_prefix_exclusion_is_repo_scoped(tmp_path):
    ...
    sibling = _repo_with_preserved_tree(tmp_path, "some-unrelated-sibling", _MAKIMA_PREFIX)
    inside = f"{_MAKIMA_PREFIX}nested/check-thing.py"
...
def test_stale_exempt_prefix_is_red(tmp_path):
    ...
    project_makima key but has no preserved tree."""
    repo = tmp_path / "project-makima"
...
def test_stale_prefix_check_is_green_when_the_preserved_tree_exists(tmp_path):
    ...
    project-makima must carry a matching tracked file under every
    project_makima-keyed EXEMPT_PREFIXES entry, not only `_MAKIMA_PREFIX`,
    or this positive control would false-fire the moment a second grant
    (e.g. the frozen-fixture-bytes prefixes) is added."""
    repo = _repo_with_preserved_tree(tmp_path, "project-makima", _MAKIMA_PREFIX)
    for prefix in EXEMPT_PREFIXES.get(REPO_PROJECT_MAKIMA, {}):
        if prefix == _MAKIMA_PREFIX:
            continue
        target = repo / prefix / "specimen.py"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(_MULTI_CLASS_VIOLATOR, encoding="utf-8")
    _commit_all(repo, "seed remaining project_makima prefixes")
...
def test_real_tree_mirror_native_files_are_prefix_excluded():
    ...
    tracked = subprocess.run(
        ["git", "-C", str(_REPO_ROOT), "ls-files", _MAKIMA_PREFIX],
        capture_output=True,
        text=True,
        check=True,
        creationflags=_NO_WINDOW,
    ).stdout.split()
    assert tracked, f"{_MAKIMA_PREFIX} has no tracked files -- prefix is stale"
    ...
    for relpath in tracked:
        assert is_prefix_excluded(relpath, REPO_PROJECT_MAKIMA)
        for cls in PREFIX_EXCLUDABLE_CLASSES:
            assert relpath not in result[cls]
...
# 2026-08-03, project-rag thread): the same EXEMPT_PREFIXES admission test,
...
def test_is_prefix_excluded_backward_compatible_without_root():
    ...
    assert is_prefix_excluded("dist/mirror-native/thing.py", REPO_PROJECT_MAKIMA)
...
# ---------------------------------------------------------------------------
# --json CLI mode (2026-08-03, project-rag thread): machine-readable output
...
# test_real_tree_every_makima_exemption_is_actually_subtracted does not
...
def test_real_tree_no_stale_exemptions():
    ...
    project_makima in THIS repo must still produce a live hit for its
