...
def test_point1_direct_dep_accepts_flat_functional_probe_kind(tmp_path, capsys):
    ...
    manifest["direct_deps"] = [
        {"id": "project-makima", "functional_probe_kind": "makima_seam_resolvable"}
    ]
    path = _write_manifest(tmp_path, manifest)
    rc = main(["--manifest-path", str(path)])
    err = capsys.readouterr().err
...
def test_point6_dependency_root_requires_both_flag_and_env(tmp_path, capsys):
    ...
    manifest["configurable_locations"] = [
        {
            "name": "makima_root",
            "dependency_id": "makima",
            "discovery": {"candidates": ["../project-makima"]},
            "default": "../project-makima",
            "override": {"flag": "--makima-root"},  # env missing
        }
    ]
    path = _write_manifest(tmp_path, manifest)
    rc = main(["--manifest-path", str(path)])
    ...
    assert rc == 1
    assert (
        "configurable_locations[0] ('makima_root') is a dependency root "
    ...
    manifest["configurable_locations"] = [
        {
            ...
            "discovery": {"candidates": ["../project-makima"]},
            "default": "../project-makima",
            "override": {"flag": "--makima-root", "env": "MAKIMA_ROOT"},
        }
    ]
    path = _write_manifest(tmp_path, manifest)
    rc = main(["--manifest-path", str(path)])
...
def test_repo_root_probes_makima_layout_when_doe_layout_absent(tmp_path, capsys):
    """makima's own layout (`docs/install/...`, no `coordinator/` prefix) must
...
def test_repo_root_prefers_doe_layout_when_both_present(tmp_path, capsys):
    ...
    makima_dir = repo_root / "docs" / "install"
    makima_dir.mkdir(parents=True)
    (makima_dir / "agent-install-manifest.json").write_text(
...
def test_point2_object_valued_platform_leg_fails_not_skips(tmp_path, capsys):
    ...
    manifest = _manifest_with_setup_script(
        posix={"repo": "project_makima", "path": "scripts/setup.py"},
...
def test_point2_object_valued_programmatic_entry_point_leg_fails_not_skips(tmp_path, capsys):
    ...
    manifest = _compliant_manifest()
    manifest["programmatic_entry_point"] = {
        "posix": {"repo": "project_makima", "path": "scripts/install-maximalist.py"},
        "entry_point_contract": {"check_only_flag": "--check-only"},
    }
    path = _write_manifest(tmp_path, manifest)
