...
def _holodeck_registry(reg_dir: Path, *, backslash: bool = False) -> None:
    reg_dir.mkdir(parents=True, exist_ok=True)
    source_path = r"X:\\claude-unreal-holodeck" if backslash else "X:/claude-unreal-holodeck"
    (reg_dir / "registry.local.toml").write_text(
        "[plugin.mirrors.holodeck]\n"
...
def _holodeck_registry_cmdless(reg_dir: Path) -> None:
    reg_dir.mkdir(parents=True, exist_ok=True)
    (reg_dir / "registry.local.toml").write_text(
        "[plugin.mirrors.holodeck]\n"
        'propagation_mode = "copy_install"\n'
        'source_path = "X:/claude-unreal-holodeck"\n',
        encoding="utf-8",
    )
...
def test_no_scope_emits_all(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _holodeck_registry(reg_dir)
    out, _err, rc = _run_lister(monkeypatch, reg_dir, tmp_path, [])
    assert rc == 0
    assert "holodeck|X:/claude-unreal-holodeck|" in out
...
def test_consumer_repo_noop(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reg_dir = tmp_path / "regdir"
    _holodeck_registry(reg_dir)
    out, _err, rc = _run_lister(monkeypatch, reg_dir, tmp_path, ["--scope-repo", "X:/project-rag"])
    assert rc == 0
    assert out == ""
...
def test_own_source_repo_matches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reg_dir = tmp_path / "regdir"
    _holodeck_registry(reg_dir)
    out, _err, rc = _run_lister(monkeypatch, reg_dir, tmp_path, ["--scope-repo", "X:/claude-unreal-holodeck"])
    assert rc == 0
    assert "holodeck|X:/claude-unreal-holodeck|" in out
...
def test_consumer_scopes_out_cmdless_plugin_no_misconfig(
    ...
    reg_dir = tmp_path / "regdir"
    _holodeck_registry_cmdless(reg_dir)
    out, _err, rc = _run_lister(monkeypatch, reg_dir, tmp_path, ["--scope-repo", "X:/project-rag"])
    assert rc == 0
...
def test_backslash_source_path_matches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    reg_dir = tmp_path / "regdir"
    _holodeck_registry(reg_dir, backslash=True)
    out, _err, rc = _run_lister(monkeypatch, reg_dir, tmp_path, ["--scope-repo", "X:/claude-unreal-holodeck"])
    assert rc == 0
...
def test_norm_path_windows_drive_lowercases() -> None:
    assert _norm_path("X:/Claude-Unreal-Holodeck") == "/x/claude-unreal-holodeck"


def test_norm_path_windows_backslash_to_forward() -> None:
    assert _norm_path(r"X:\claude-unreal-holodeck") == "/x/claude-unreal-holodeck"
...
def test_norm_path_msys_form_folds_under_msys_ostype() -> None:
    assert _norm_path("/x/Claude-Unreal-Holodeck", ostype="msys") == "/x/claude-unreal-holodeck"
...
def test_norm_path_posix_case_preserved() -> None:
    assert _norm_path("/Users/alice/X/project-rag") == "/Users/alice/X/project-rag"
