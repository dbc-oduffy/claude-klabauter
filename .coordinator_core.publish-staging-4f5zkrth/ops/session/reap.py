...
Negative-spec:
  ...
  - Does NOT expose a dry_run/candidate_ids two-phase contract — this is not a fleet op;
    it has no opticon-facing reviewer loop.
...
def _reap_orphaned_claims(
    ...
    for subdir_name in _CLAIM_SUBDIRS:
        ...
        for claim_dir in sorted(claims_dir.iterdir()):
            ...
            # two-repo-rebuild.md § M3a, Zolí Finding 5): this automated
...
# § C5 (PM-ratified 2026-07-14; direction resolved by Patrik review, "adopt split,
# no Zolí round"). DR-211 does NOT govern this primitive — session.reap is declared
...
#
# Negative-spec (C5):
#   - Does NOT enumerate working-repos.yaml — makima has no fleet census here.
...
def _reap_claims_for_target(target_root: Path) -> Tuple[List[str], List[dict], List[dict]]:
    ...
    Op scope "none" per makima's target-resolution convention (fleet-generic ops
...
def _reap_session_touched_repos(target_roots: List[Path]) -> dict:
    ...
    repos and no others — makima does NOT enumerate working-repos.yaml and does
...
async def _handler_reap_claims_for_repos(
    ...
    Fleet-generic, op scope "none" per makima's target-resolution convention
    ...
    Patrik-review-resolved "adopt split, no Zolí round"): the PRIMITIVE is
    ...
    `/pickup` already resolves BATON_REPO) — makima does NOT enumerate
