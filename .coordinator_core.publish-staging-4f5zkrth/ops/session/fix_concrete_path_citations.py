...
def _default_registry_keys() -> List[str]:
    ...
    `MACHINE_LOCAL_REPOS_PROJECT_MAKIMA` does not invert to a unique dotted
...
def discover_families(
    ...
    # already keeps "project-rag-ue-addon" and "project-rag" from ever
...
def _locate_family(token: str, families: List[Family]) -> Optional[Tuple[Family, int]]:
    ...
    folder" (`project-makima` is a normalized substring of
    `project-makima-backup-2026` either way); requiring a full-segment
