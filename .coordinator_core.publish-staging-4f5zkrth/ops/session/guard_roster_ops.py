...
deregistered and its logic now runs as a pure-Python port with no makima call
...
Scope of "the advisory op set" (read from makima's tree, not asserted from
...
grouping is the only boundary makima's tree draws between "advisory" and
