...
project-makima) only fire on a git sync event. Nothing previously ran on
...
Sibling: coordinator/bin/coordinator-postsync-marker-resync-check
    (project-makima) — the git-sync-triggered leg of the same defense; this
...
GENERATES = []  # writes only a per-machine sentinel and kill-switch marker under settings-home, outside makima's own tracked tree
