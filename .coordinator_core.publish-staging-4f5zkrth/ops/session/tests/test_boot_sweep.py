...
Coverage:
  ...
  (b) non-heir consumed+in_flight skip-and-surface (DR-084 stop-gap, 2026-07-22) — a
      ...
      state/handoffs/ untouched (Patrik F0 / AC2, DR-084 no-automated-abandonment).
...
Spec backlinks:
  ...
  - Design-decision 3 / Patrik F0: fleet op is archival-only; boot entrypoint carries
...
Negative-spec:
  - Does NOT test a dry_run/candidate_ids round-trip — session.boot_sweep is a
    one-shot self-selecting sweep with no opticon reviewer loop (Anti-scope).
...
def boot_repo(tmp_path) -> BootRepo:
    ...
    _git("init", "-b", "main")
    _git("config", "user.email", "boot-sweep-test@makima.test")
...
def _build_test_repo(root: Path, dirs: list) -> BootRepo:
    ...
    _git("init", "-b", "main")
    _git("config", "user.email", "two-repo-test@makima.test")
...
def test_two_repo_gitroot_notes_commit_succeeds_under_gpgsign_true(tmp_path):
    ...
    Spec backlink: pln-strang-11-b8-session-init-boot-f78455 § C8b
