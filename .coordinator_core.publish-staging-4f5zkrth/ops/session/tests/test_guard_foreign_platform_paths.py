...
def _posix_settings():
    return {
        ...
        "extraKnownMarketplaces": {
            "project-rag": {"source": {"source": "directory", "path": "/Users/alice/X/project-rag"}}
        },
    }
    ...
    return {
        ...
        "extraKnownMarketplaces": {
            "claude-unreal-holodeck": {
                "source": {"source": "directory", "path": "C:\\Users\\alice\\.claude\\plugins\\claude-unreal-holodeck"}
            }
        },
    }
...
def test_prose_leak_banner_names_file_line_and_remedy():
    text = "line one\n- `X:\\claude-unreal-holodeck` — UE5 workbench\nline three"
    findings = detect_foreign_platform_paths_in_prose(text, host_is_windows=False)
    assert len(findings) == 1
...
def test_generic_single_word_segment_now_fires():
    ...
    the real leaked lines (`experiments`, `DelphiOS`) this fix targets."""
...
#
# `experiments` and `DelphiOS` are real, hyphen-free fleet repo names that were
...
def test_single_word_repo_name_delphios_fires():
    text = "- `X:\\DelphiOS` — Delphi Interactive repo."  # abs-path-ok: real leaked-corpus fixture under test
    findings = detect_foreign_platform_paths_in_prose(text, host_is_windows=False)
    assert len(findings) == 1
    banner = format_prose_banner(findings, "CLAUDE.local.md")
    assert "CLAUDE.local.md" in banner
...
def test_hyphenated_repo_name_claude_unreal_holodeck_fires():
    text = "- `X:\\claude-unreal-holodeck` — UE5 workbench."  # abs-path-ok: real leaked-corpus fixture under test
    findings = detect_foreign_platform_paths_in_prose(text, host_is_windows=False)
    assert len(findings) == 1
    banner = format_prose_banner(findings, "CLAUDE.local.md")
    assert "CLAUDE.local.md" in banner
...
def test_multi_segment_nested_path_dronesim_fires():
    text = "checked out at `E:\\dev\\DroneSim` on that box"  # abs-path-ok: synthetic nested-segment fixture under test
    findings = detect_foreign_platform_paths_in_prose(text, host_is_windows=False)
    assert len(findings) == 1
    banner = format_prose_banner(findings, "CLAUDE.local.md")
    assert "CLAUDE.local.md" in banner
