...
Decision branches:
    1. UE repo + project-rag daemon present  -> emit "offer to index" text
...
currently UE-only because an upstream `project-rag-ue-addon`
.uproject-abstention defect blocks indexing of non-UE repos. This is a known
upstream limitation in project-rag-ue-addon (tracked via cross-repo memo
project-rag-ue-addon-em); widen to non-UE only after that defect is resolved.
...
probe checks `project-rag status` on PATH.
...
Exit codes (parity-critical -- matches the bash oracle's contract exactly):
    ...
    2  RESERVED for the DoE trampoline's own makima-link/import failure
...
Port of: setup-rag-decision.sh (DoE 6fb5fb37, 2026-07-22)
...
Negative-spec:
    - Does NOT support "sourced" invocation (the bash oracle offered a dual
      ...
      (coordinator/skills/repo-setup/SKILL.md) invokes the DoE-side
      ...
      the DoE trampoline to the sh/python polyglot shape makes sourcing
...
GENERATES = []  # appends the un-indexed tripwire block to <root>/CLAUDE.md where root is a caller-supplied --root/cwd target repo (any repo /repo-setup targets), not a fixed path inside makima's own tree
...
def _daemon_present() -> bool:
    """Return True if the project-rag daemon is reachable.
    ...
    project_rag = shutil.which("project-rag")
    if not project_rag:
        return False
    try:
        result = subprocess.run(
...
def setup_rag_decision(argv: List[str]) -> int:
    ...
    if is_ue:
        ...
        if daemon_present:
            ...
            if dry_run:
                ...
                print("setup-rag-decision: branch 1 — UE repo with project-rag daemon detected.")
                print("  Offer: run project-rag index to build a semantic index for this repo.")
            return 0
        ...
        if dry_run:
            ...
            print("setup-rag-decision: branch 3 — UE repo detected but project-rag daemon is absent.")
            print("  Writing un-indexed tripwire to CLAUDE.md.")
            print("  To enable indexing: install and start the project-rag daemon, then re-run /repo-setup.")
    ...
    # because project-rag-ue-addon's .uproject-abstention defect blocks
    # indexing of non-UE repos (tracked via cross-repo memo
    # project-rag-ue-addon-em). Widen after that upstream defect is resolved.
    if dry_run:
        print(
            "[setup-rag-decision dry-run] branch 2: non-UE repo → tripwire "
            "(upstream .uproject-abstention defect, cross-repo memo project-rag-ue-addon-em)"
        )
    else:
        print("setup-rag-decision: branch 2 — non-UE repo. RAG indexing is not yet supported")
        print("  for non-UE repos: blocked by project-rag-ue-addon .uproject-abstention defect")
        print("  (cross-repo memo project-rag-ue-addon-em). Writing un-indexed tripwire to CLAUDE.md.")
    _write_tripwire(root, dry_run)
    return 0
