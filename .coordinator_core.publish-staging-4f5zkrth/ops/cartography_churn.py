...
coordinator_core.cartography.churn.compute_emergent_set. Promotes DoE survey
...
Wire params:
    ...
    excluded_dirs   (list[str], optional) — source-dir prefilter list
                                       ...
                                       DoE's tree the emergent set at that
...
with a real call site (refresh mode, DoE's survey.workflow.js chunk-K gate)
...
Negative-spec:
  - Does NOT apply the chunk-K threshold decision (emergent non-empty OR
    ...
    (DoE-owned, Part B, not executed here); this op only returns the
    ...
    judgment — is load-bearing and written into DoE's contract for this op
    family).
  - Does NOT write to any fleet store or state/ path — pure compute-and-
    return, no disk artifact (unlike ops/coverage_gate.py's opticon-facing
    artifact write).
  - Does NOT change `emergent`'s semantics (churned paths outside
    ...
    DoE's survey.workflow.js chunk-K gate) see no behaviour change.
