...
def test_mixed_prefixes_fall_back_to_unprefixed_namespace(tmp_path):
    ...
    _touch(tmp_path, "DR-HOLODECK-005-a.md", "DR-RAG-010-b.md")
    assert allocate_dr_number(tmp_path) == "DR-001"


def test_sole_shared_prefix_inferred_when_unambiguous(tmp_path):
    _touch(tmp_path, "DR-HOLODECK-005-a.md", "DR-HOLODECK-006-b.md")
    assert allocate_dr_number(tmp_path) == "DR-HOLODECK-007"


def test_explicit_prefix_overrides_disk_inference(tmp_path):
    _touch(tmp_path, "DR-010-unprefixed.md")
    assert allocate_dr_number(tmp_path, explicit_prefix="holodeck") == "DR-HOLODECK-001"


def test_explicit_prefix_scopes_independently_of_other_namespaces(tmp_path):
    _touch(tmp_path, "DR-HOLODECK-005-a.md", "DR-RAG-010-b.md")
    assert allocate_dr_number(tmp_path, explicit_prefix="HOLODECK") == "DR-HOLODECK-006"
...
def test_width_padding_is_per_namespace_not_global(tmp_path):
    ...
    _touch(tmp_path, "DR-0999-wide-unprefixed.md", "DR-HOLODECK-005-a.md")
    assert allocate_dr_number(tmp_path, explicit_prefix="HOLODECK") == "DR-HOLODECK-006"
...
# ---------------------------------------------------------------------------
# Frontmatter-carried ids — project-opticon date-named-record regression
# ---------------------------------------------------------------------------
# Spec backlink: cross-repo/inbox/2026-08-01-project-opticon-em-dr-allocator-frontmatter-id-blindness.md
...
def test_frontmatter_carried_id_extends_max_past_filename_led_records(tmp_path):
    # Reproduces the opticon scenario: 7 filename-led records DR-001..DR-007
...
def test_frontmatter_prefixed_namespace_participates_in_inference(tmp_path):
    _touch(tmp_path, "DR-OPTICON-001-a.md", "DR-OPTICON-002-b.md")
    _write_frontmatter_record(
        tmp_path, "2026-07-29-frontmatter-carried-record.md", "DR-OPTICON-004"
    )
    assert allocate_dr_number(tmp_path) == "DR-OPTICON-005"
