...
def _environment_requires_doe_clone() -> bool:
    ...
    ``CI=1``) plus a dedicated ``MAKIMA_REQUIRE_DOE_CONFORMANCE`` override for a
    ...
    """
    if os.environ.get("MAKIMA_REQUIRE_DOE_CONFORMANCE", "").strip().lower() in _TRUTHY:
        return True
...
def test_doe_clone_conformance_lane_ran_or_environment_is_documented_optional(
    ...
    if not _environment_requires_doe_clone():
        pytest.skip(
            "DOCUMENTED optional-skip lane: neither MAKIMA_REQUIRE_DOE_CONFORMANCE "
    ...
    try:
        ...
        pytest.fail(
            ...
            f"should-have-DoE-clone (CI or MAKIMA_REQUIRE_DOE_CONFORMANCE "
            f"truthy): {exc}"
        )
