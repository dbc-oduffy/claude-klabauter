...
def test_absent_and_unset_skips(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("MAKIMA_REQUIRE_DOE_CONFORMANCE", raising=False)
...
@pytest.mark.parametrize("env_var", ["MAKIMA_REQUIRE_DOE_CONFORMANCE", "CI"])
...
def test_falsy_env_values_do_not_trigger_requirement(
    ...
    """
    monkeypatch.setenv("MAKIMA_REQUIRE_DOE_CONFORMANCE", falsy_value)
    monkeypatch.delenv("CI", raising=False)
