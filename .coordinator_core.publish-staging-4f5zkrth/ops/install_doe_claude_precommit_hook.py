...
installer for DoE-claude, dispatched from project-makima.
...
Modelled closely on `coordinator_core.ops.install_makima_precommit_hook` —
...
Identity guard — the ONE load-bearing divergence from the makima installer's
shape. `install_makima_precommit_hook` resolves "is this the target repo"
from its OWN file location (`_self_repo_root()`), because that module ships
INSIDE project-makima. This module does not ship inside DoE-claude — it
lives in project-makima and targets a PEER repo — so there is no self-
...
THIS tree (project-makima). Its absence at install time fires the ordinary
...
`_BIN_SUBDIR` below is, exactly as in the makima installer, the single
...
apart. Unlike the makima installer's `_bin_dir()` (self-relative, no
...
`install_makima_precommit_hook`'s own "Exit-code clamping" section — a
...
Negative-spec:
    ...
    - No `bash` anywhere in the emitted hook body — the shim is `#!/bin/sh`
      and execs `$_py` directly, per project-makima's CLAUDE.md § Runtime
      conventions.
    ...
    - Does not install into DoE-claude's real, live tree, and does not
      install into project-makima's own live repo either — this module and
...
# docs/wiki/guard-messaging.md § Register). Repo-qualified ("project-makima
# <path>"), so it stays fleet-addressable when this hook fires inside
# DoE-claude's own tree, not just project-makima's.
...
def _bin_dir(repo_root: str) -> Path:
    ...
    `_resolve_doe_claude_target`). Unlike the makima installer's `_bin_dir()`
    — which is self-relative because that module ships inside its own
    target — this module ships in project-makima and has no self-relative
...
def _resolve_doe_root() -> str:
    ...
    is independently monkeypatchable in the makima installer. Returns `""`
...
def _gate_block(gate: _Gate) -> List[str]:
    ...
    `install_makima_precommit_hook._gate_block`'s docstring for the fuller
...
def _warn_if_gate_script_missing(repo_root: str, gates: List[_Gate]) -> None:
    ...
    exist in project-makima's tree; when this installer is exercised against
...
def _install_or_append_hook(repo_root: str, gates: List[_Gate]) -> int:
    ...
    # `install_makima_precommit_hook._install_or_append_hook`'s own comment
