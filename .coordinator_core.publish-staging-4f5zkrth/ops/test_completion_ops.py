...
def test_day_coverage_sweep_separates_foreign_authored_deliveries(repo: Path) -> None:
    ...
    ``prepare-commit-msg`` injected a ``Session-Id`` that no makima session
    ever owned.
    """
    day = "2026-07-20"
    iso = f"{day}T12:00:00+00:00"
    ...
    foreign_false_trailer_sha = _commit_on(
        ...
        "memo",
        "cross-repo: deliver Other thing memo from project-rag-em",
        iso_date=iso,
        trailer="Session-Id: sess-injected-by-receiver-hook",
    )
    # Negative 1 — no trailer, but makima's own work: a genuine orphan.
...
def test_day_coverage_sweep_separates_sibling_homed_sessions(
    ...
    the inverse "absent locally" test, which would fail open on every makima
    ...
    sibling.mkdir()
    _register_repos(monkeypatch, tmp_path, doe_claude=sibling, project_makima=repo)
