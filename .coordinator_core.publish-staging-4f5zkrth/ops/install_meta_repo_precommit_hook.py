...
generations of defect found in the same session:
  ...
  1. The ORIGINAL bash-ported version baked a literal
     ...
     surface into this repo (`project-makima/coordinator/bin/`); that literal
  ...
  `install_makima_precommit_hook.py`'s `_gate_block`): a gate's own exit code
...
def _py_resolve_line() -> str:
    ...
    implementation" reasoning, and `install_makima_precommit_hook.
