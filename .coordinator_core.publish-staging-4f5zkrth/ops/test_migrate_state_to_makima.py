"""
coordinator_core.ops.test_migrate_state_to_makima — behavior-parity tests for
the naked-Python port (DR-059).

Tests exercise the module against `tmp_path`-scoped throwaway CLAUDE_HOME /
MAKIMA_ROOT trees (hermetic; no shared/real repo state touched) and assert
...
from coordinator_core.ops import migrate_state_to_makima as mstm  # noqa: E402
...
def _run(argv, claude_home: Path, makima_root: Path, monkeypatch, capsys):
...
def test_no_args_prints_full_usage_and_exits_1(tmp_path, monkeypatch, capsys):
    ...
    makima_root = tmp_path / "makima_root"
    makima_root.mkdir()

    rc, out, err = _run([], claude_home, makima_root, monkeypatch, capsys)

    assert rc == 1
    assert out == ""
    assert "Usage: migrate-state-to-makima.sh --populate | --finalize" in err
    ...
    assert "--populate   Copy ~/.claude working data to makima (C6a)." in err
...
def test_unknown_subcommand_prints_short_usage_only_and_exits_1(tmp_path, monkeypatch, capsys):
    ...
    rc, out, err = _run(["--frobnicate"], claude_home, makima_root, monkeypatch, capsys)

    assert rc == 1
    assert out == ""
    assert err == (
        "ERROR: unknown subcommand: --frobnicate\n"
        "Usage: migrate-state-to-makima.sh --populate | --finalize\n"
    )
...
def test_missing_claude_home_exits_1(tmp_path, monkeypatch, capsys):
    ...
    rc, out, err = _run(["--populate"], missing_home, makima_root, monkeypatch, capsys)
...
def test_populate_copies_all_tree_pairs_and_leaves_originals_untouched(
    ...
    rc, out, err = _run(["--populate"], claude_home, makima_root, monkeypatch, capsys)
    ...
    for rel in [
        ...
        assert (makima_root / rel).is_file()
        # Originals untouched.
...
def test_populate_skips_absent_source_trees(tmp_path, monkeypatch, capsys):
    ...
    assert (makima_root / "docs/plans/plan1.md").is_file()
    assert not (makima_root / "state").exists()
...
def test_populate_is_idempotent(tmp_path, monkeypatch, capsys):
    ...
    _run(["--populate"], claude_home, makima_root, monkeypatch, capsys)
    # Overwrite source, re-populate -- destination should reflect the update.
    (claude_home / "state/a.txt").write_text("v2\n")
    rc, _out, _err = _run(["--populate"], claude_home, makima_root, monkeypatch, capsys)

    assert rc == 0
    assert (makima_root / "state/a.txt").read_text() == "v2\n"
...
def test_finalize_removes_source_after_successful_populate(tmp_path, monkeypatch, capsys):
    ...
    _run(["--populate"], claude_home, makima_root, monkeypatch, capsys)
    rc, out, err = _run(["--finalize"], claude_home, makima_root, monkeypatch, capsys)
    ...
    assert (makima_root / "state/foo/a.txt").is_file()
    assert (makima_root / "docs/plans/plan1.md").is_file()
...
def test_finalize_delta_resyncs_files_written_after_populate(tmp_path, monkeypatch, capsys):
    ...
    # an mtime strictly newer than the makima copy.
    src = claude_home / "state/a.txt"
    dst = makima_root / "state/a.txt"
    src.write_text("v2\n")
    newer = os.path.getmtime(dst) + 5
    os.utime(src, (newer, newer))

    rc, out, _err = _run(["--finalize"], claude_home, makima_root, monkeypatch, capsys)
...
def test_finalize_guard_fails_closed_when_makima_copy_missing(tmp_path, monkeypatch, capsys):
    """Fail-closed edge: if a source file cannot be confirmed present in
    makima (e.g. the delta-copy itself failed), --finalize MUST abort before
    ...
    makima_root = tmp_path / "makima_root"
    (makima_root / "state").mkdir(parents=True)
    ...
    assert rc == 1
    assert "GUARD FAIL: makima copy missing for:" in err
...
# ---------------------------------------------------------------------------
# MAKIMA_ROOT default resolution
# ---------------------------------------------------------------------------


def test_default_makima_root_is_this_modules_own_repo_root():
    root = mstm._default_makima_root()
    assert (Path(root) / "coordinator_core" / "ops" / "migrate_state_to_makima.py").is_file()
