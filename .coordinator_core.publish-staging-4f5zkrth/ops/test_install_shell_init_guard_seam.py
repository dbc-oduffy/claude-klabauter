...
second append); and the graceful skip when no project-makima guard script is
resolvable (or not executable).

Never touches a real rc file — every rc path in this suite is under
tmp_path, and REPO_PROJECT_MAKIMA is set explicitly per test so resolution
never falls through to the operator's real machine-local registry.

Converted 2026-08-16 (C7b): `resolve_makima_clone`'s tier-2 rung now reads
...
(`test_graceful_skip_when_makima_not_resolvable`) cannot read the
...
@pytest.fixture()
def makima_clone(tmp_path: Path) -> Path:
    """A fake project-makima checkout carrying shell-init-guard.py at the mode
    makima's real `bin/` uses: 100644, no execute bit. Every happy-path test
    below therefore doubles as a regression against re-introducing an
    exec-bit gate (bug fix, 2026-08-22)."""
    clone = tmp_path / "project-makima"
...
def _isolate_env(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("REPO_PROJECT_MAKIMA", raising=False)
...
def test_check_only_reports_would_install_and_does_not_mutate(
    capsys, monkeypatch, tmp_path, makima_clone
):
    monkeypatch.setenv("REPO_PROJECT_MAKIMA", str(makima_clone))
...
def test_live_run_writes_sentinel_block_and_preserves_prior_content(
    ...
    guard_src = str(makima_clone / "bin" / "shell-init-guard.py")
...
def test_graceful_skip_when_makima_not_resolvable(capsys, tmp_path):
    ...
    (bug fix, 2026-08-22: all three printed "project-makima not found")."""
    rc_path = tmp_path / ".bashrc"
    rc_path.write_text("# pre-existing content\n")
    ...
    out = capsys.readouterr().out
    assert (
        "shell_init_guard: skipped (project-makima not registered — "
        "set REPO_PROJECT_MAKIMA or machine-local repos.project_makima)"
    ) in out
...
def test_graceful_skip_when_guard_script_missing(capsys, monkeypatch, tmp_path):
    ...
    present."""
    empty_clone = tmp_path / "project-makima-empty"
    empty_clone.mkdir()
    monkeypatch.setenv("REPO_PROJECT_MAKIMA", str(empty_clone))
    rc_path = tmp_path / ".bashrc"
    rc_path.write_text("# pre-existing content\n")
...
def test_graceful_skip_when_guard_script_not_readable(capsys, monkeypatch, tmp_path):
    ...
    clone = tmp_path / "project-makima-unreadable"
    (clone / "bin").mkdir(parents=True)
    guard = clone / "bin" / "shell-init-guard.py"
    guard.write_text("#!/usr/bin/env python3\nprint('')\n")
    guard.chmod(0o644)
    monkeypatch.setenv("REPO_PROJECT_MAKIMA", str(clone))
    rc_path = tmp_path / ".bashrc"
    rc_path.write_text("# pre-existing content\n")
...
def test_non_executable_guard_still_installs(capsys, monkeypatch, tmp_path, makima_clone):
    """The exec bit is not a condition. makima's `bin/` is committed 100644
    ...
    guard = makima_clone / "bin" / "shell-init-guard.py"
...
def test_seam_never_consults_the_execute_bit(monkeypatch, tmp_path, makima_clone):
...
def test_rc_created_fresh_when_absent(monkeypatch, tmp_path, makima_clone):
