...
Purpose: COMPUTE_ONLY read op answering "what are the four opticon columns
...
addendum names as the replacement for opticon's retiring emission read.
...
records.query's own ``format=json`` returns is precisely what opticon asked
...
is served here, by this producer, rather than re-derived on opticon's side,
because opticon's own fleet-board code explicitly forbids re-deriving it —
the sole canonical derivation is `coordinator_core.frontmatter.baton_class
.baton_class`, and this op is opticon's only intended consumer of handoff
columns. Requested via cross-repo memo
cross-repo/inbox/2026-08-11-project-opticon-em-baton-class-has-no-producer-in-either-leg.md.
...
Spec backlink: pln-a-pull-surface-for-opticon-the-b8e2f3 § C3
"""
...
def _handler(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    Params (all optional):
        ...
        archive: bool — archive coverage, DEFAULT ON (True) for this op. Pass
                 ...
                 it has exactly one intended consumer (opticon's fleet
