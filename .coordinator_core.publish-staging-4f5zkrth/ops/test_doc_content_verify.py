...
class TestNegativeSurface:
    ...
    def test_known_good_cross_repo_citation_does_not_produce_finding(self):
        ...
        makima_root = Path(coordinator_engine_root())
        assert (makima_root / "coordinator" / "lib" / "install-substrate.py").exists(), (
        ...
        findings = verify_doc(
            ...
            sibling_checkers=[lambda token: (makima_root / token).exists()],
        )
        assert findings == []
    ...
    def test_exclusion_dollar_prefixed(self):
        assert is_excluded("$MAKIMA_ROOT/coordinator/bin/foo.py") is True
    ...
    def test_exclusion_dollar_prefixed_end_to_end(self):
        text = "```bash\npython3 $MAKIMA_ROOT/coordinator/bin/foo.py\n```\n"
        findings = verify_doc("README.md", text, repo_exists=lambda token: False)
        assert findings == []
...
class TestAC13HistoricalReplay:
    ...
    def test_true_positive_flagged_install_maximalist_when_no_sibling_root_resolvable(self):
        ...
        against project-makima shows the file landed there at commit `8a28a6ca`,
        ...
        makima-side file before removing the DoE-side one.
        ...
        a bare clone with no machine-local registry entry for project-makima.
    ...
    def test_install_maximalist_resolves_cross_repo_when_sibling_root_available(self):
        ...
        text = _git_show(doe_root, _B644D5A9_SHA, "README.md")
        makima_root = Path(coordinator_engine_root())
        assert (makima_root / "coordinator" / "scripts" / "install-maximalist.py").exists()
