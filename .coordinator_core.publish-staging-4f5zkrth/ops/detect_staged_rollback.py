...
``coordinator_core.ops.install_makima_precommit_hook._GATE_REGISTRY``, the
...
project-makima runs through ``main()`` here before it can land.
...
Negative-spec:
    ...
    - Does NOT scope the mass-deletion check to the pathspec a
      ...
      ``coordinator_core.ops.install_makima_precommit_hook`` and
    ...
    - Does NOT itself decide whether to block anything beyond its own exit
      ...
      (``coordinator_core.ops.install_makima_precommit_hook``) clamps
...
# section). Never hand-copy the "project-makima <path>" string here; both
...
# drop 1,709 pre-July claude-prime files reclaimed by DoE", a deliberate,
