...
in the fleet — makima's own `coverage.py` DAG attribution and `close_out_and_stamp`, and
project-rag's `workstate_store` ingest — read `Deliverable-Id` and silently saw nothing
...
Spec backlink: pln-makima-commit-anchor-stamper-q-29b891 § D2 / C1-op
...
Negative-spec (hard-won):
  ...
  - Does NOT emit any key it cannot resolve unambiguously; precision over recall at every
    key boundary (makima-commit-anchor-stamper.md § Anti-scope: a wrong anchor is worse than
    none because opticon renders it as a confident false edge).
...
# Spec backlink: pln-makima-commit-anchor-stamper-q-29b891 § D1
# ---------------------------------------------------------------------------

#: Closed enum of valid Nature values (slice-1, additive-only per opticon co-design).
...
def _resolve_plan_from_diff(
    ...
    from the DD#1 freshness gate (makima-commit-anchor-stamper.md § D2).
    """
...
def _resolve_anchor(worktree_root: Path, session_id: str) -> Optional[str]:
    ...
    (makima-commit-anchor-stamper.md § D1 co-design resolution: Anchor demoted from
