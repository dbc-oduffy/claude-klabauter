...
def _make_fake_plugin_root(base, version):
    ...
    # (coordinator_currency.py migrated to makima's own coordinator/lib/ --
    # see detect_onboarding_offer's fallback-branch comment) -- see the
    # _fake_makima_root autouse fixture below for that file.
    return proot


@pytest.fixture(autouse=True)
def _fake_makima_root(tmp_path, monkeypatch):
    """Autouse: pins MAKIMA_ROOT deterministically (env monkeypatch, never
    the real machine-local registry or a real makima checkout) to a
    synthetic makima root containing coordinator/lib/coordinator_currency.py
    ...
    makima_root = tmp_path / "fake-makima-root"
    (makima_root / "coordinator" / "lib").mkdir(parents=True, exist_ok=True)
    (makima_root / "coordinator" / "lib" / "coordinator_currency.py").write_text(
...
# ---------------------------------------------------------------------------
# Fallback-branch reachability -- MAKIMA_ROOT resolution (2026-07-22 repoint:
# coordinator_currency.py migrated out of plugin_root's lib/ into makima's
...
def test_fallback_unresolvable_makima_root_degrades_to_silent(tmp_path, monkeypatch):
    """MAKIMA_ROOT unresolvable (no env, no settings-home pointer, no
    ...
    repo = tmp_path / "case-unresolvable-makima-root"
    repo.mkdir()
    _init_git_repo(repo)
    _mark_onboarded(repo)
    _write_currency_stamp(repo, 1)
    ...
    def _raise():
        raise RuntimeError("coordinator_makima_root: cannot resolve MAKIMA_ROOT")

    monkeypatch.setattr(_mod, "coordinator_makima_root", _raise)

    out = detect_onboarding_offer(str(repo), str(plugin_root))
    assert out == ""


def test_fallback_reaches_probe_when_makima_root_resolved(tmp_path):
    """With MAKIMA_ROOT resolved (via the autouse fixture) and
