...
DoE-claude coordinator/ tree, NOT makima) — a stable, versioned JSON Schema contract
...
preserved there as the historical record; this port carries only the CURRENT pin):
  ...
  2.0.0  (2026-07-22) DR-084 P4 narrow: handoff status/deployment_state enum values
    ...
    (DoE's 4bd0e4f1 regen shipped the narrowed body still stamped 1.19.0).
  2.1.0  (2026-07-27) handoff-archived read-tolerance widening, MINOR: (1) `status`
    ...
    notes the bundle regen is makima-run and lagged the ratification — this bump
    discharges that lag.
  ...
  3.1.0  (2026-08-03) MINOR: derivation gains 'synthesized' — a lawful fourth
    ...
    its own producer documented as wrong). Governance-ratified DoE-side
    (cockpit-contract/DECISIONS.md D42, twin note artifact-shape-contract/
    DECISIONS.md) as the makima-side half of the D31 emitter-ownership split — D42
  ...
  3.2.0  (2026-08-03) MINOR: bundler now rewrites every `$defs.*` `$ref` carrying the
    ...
    project-rag-ue-addon-em (consumer-side containment, per-file demotion to a
  ...
  4.0.0  (2026-08-04) MAJOR: `$defs.review.properties.reviewer` enum narrows from persona
    names to agent-registry role slugs — `patrik`/`sid`/`camelia`/`pali`/`fru`/`zoli` are
    ...
    structurally the 2.0.0 case. Caught by DoE-side review before the close, not by a consumer.
  5.0.0  (2026-08-05) hnd- id pattern narrow: `handoff_id`, `predecessor_id` and
    ...
    (project-makima f67a1d859530) landed FIRST and the fleet-wide corpus sweep SECOND, so this
    narrow strands nothing — verified across 7 coordinator repos, 1588 id-field values, zero
    stranded. Mirrors handoff.schema.json's own 4.0.0 -> 5.0.0 (DoE 0391ab20c), which is the
  ...
  6.0.0  (2026-08-05) dlv- id pattern narrow: `deliverable_id`'s string arm gains
    ...
    project-rag-ue-addon. Hence case-permissive, `.`-admitting, and NOT anchored on
    `-[0-9a-f]{6}` (the stub-origin shape carries no hex suffix; the corpus holds
    trailing-dash-before-hex slug-truncation artifacts). Verified twice independently: makima
    swept 7 repos / 2200 non-null values, zero rejections; DoE re-derived on their own corpus,
    ...
    edit is DoE's: `coordinator/schemas/handoff.schema.json` is authored there and VENDORED
    here (`check_schema_drift` is a byte-for-byte tamper-check against DoE HEAD), so at this
    commit makima's vendored copy does NOT yet carry the pattern and this emitter's output body
    ...
    precondition: DoE's `coordinator/tests/test_artifact_shape_contract_freshness.py`
    ...
    project-rag/opticon a narrowed domain under an unchanged version. Both doors shut until
    this moves. No consumer is exposed by the ordering: the stamped artifact lives in DoE's
  ...
  6.1.0  (2026-08-13) `peer-set-entry` enters the bundle: DoE registered
    ...
    body it describes. DoE's `test_artifact_shape_contract_freshness.py` regenerates in-memory
    ...
    6.1.0. They regenerate, commit the bundle, and own the DECISIONS.md row on their side; makima
  ...
  7.0.0  (2026-08-14) `reviewed_range` pattern narrow, PLUS the `census-document`
    ...
    `^[0-9a-f]{4,64}\\^?\\.{2,3}[0-9a-f]{4,64}$` on both `$defs["review-findings"]` and
    `$defs["run-report"]`, narrowing previously-unconstrained strings to a hex-SHA-range
    shape. Non-additive by the 5.0.0/6.0.0 reasoning — a consumer holding 6.x semantics
    accepts ranges the narrowed contract rejects — so MAJOR, and the additive majority
    around it does not soften that: the bump rule below keys on the presence of a
    non-additive change, not its share of the delta. Source narrow: DoE 8a6c58257.
    FIVE DELTAS RODE THIS STAMP, NOT THE TWO THIS ENTRY ORIGINALLY NAMED. The constant moved
    here at 7dc74dd0c; DoE then held their emit until a peer session's four schema bumps
    committed at 993891f5c, rather than regenerate against a staged index, so all five landed
    in the one body they stamped 7.0.0. Recorded on their side in
    coordinator/artifact-shape-contract/DECISIONS.md's 7.0.0 row; recorded here because this
    history is makima's own account of what a stamp it owns covers, and naming two of five
    would leave the other three undiscoverable from this side. (1) `census-document`
    registered: `schema_count` 63 -> 64, `$defs` 77 -> 79, zero removed. The second new `$def`
    is `conversion-census-row`, THIS REPO'S row shape
    (coordinator_core/contract/conversion-census.schema.json) vendored into their envelope and
    hoisted out of its `$defs` — excluded from `schema_count` per the ProvenanceEnvelope
    convention (1.10.0 row). That vendoring is load-bearing for us: a change to our row shape
    changes their bundle body and owes a bump, and the row schema's own DRAFT status grants no
    exemption from that. (2)+(3) `review-findings` 3.0.0 -> 3.1.0 and `run-report`
    2.0.0 -> 2.1.0 WIDEN the very pattern this entry describes as narrowed — the committed
    regex is now `^[0-9a-f]{4,64}(?:\\^|~[0-9]+)?\\.{2,3}[0-9a-f]{4,64}$`, admitting a `~<N>`
    left endpoint alongside `^`. Net: narrowed against any pre-49dd0ecdc consumer, widened
    against the unstamped body, which makes 7.0.0 the first stamp under which the field's
    accepted-value set is honestly named. (4)+(5) `handoff` 8.0.0 -> 8.1.0 and
    `handoff-archived` 2.5.0 -> 2.6.0 adopt `reconcile_disposition` /
    `reconcile_disposition_reason` — optional strings, no `required` entry, no schema-level
    cross-field coupling. All four declare `x-bump-class: nested-field-additive`; note their
    declared version lives at `x-schema-version`, not `version` — only the emitted bundle uses
    a bare `version` key. Additive every one: the MAJOR is still paid for by the narrow alone.
    WHY THIS IS NOT THE USUAL CONSTANT-MOVES-FIRST SEQUENCING. 6.0.0 and 6.1.0 both
    record makima moving the stamp ahead of a body DoE had yet to regenerate. This bump
    is the inverse and the rule's actual failure case: the narrowed body ALREADY LANDED
    in DoE's committed bundle at 49dd0ecdc ("refresh five generated artifacts their
    source commits left stale") under an unmoved 6.1.0 stamp. Two different bundle
    bodies have therefore shared one version stamp since that commit — precisely what
    the rule forbids. This constant is closing an open violation, not preceding a
    planned one.
    WHY 7.0.0 AND NOT THE REQUESTED 6.2.0. DoE asked separately for 6.2.0 MINOR to
    register `coordinator/schemas/census-document.schema.json` (`schema_count` 63 -> 64,
    `$defs` +1), on the stated premise that the delta is entirely additive — "zero enum
    narrows, zero required-additions, zero property removals". That premise was true
    when written and stopped being true at 49dd0ecdc. Landing 6.2.0 as MINOR would
    stamp the already-committed pattern narrowing as additive and leave the violation
    open under a new number. The census registration is genuinely additive and is
    ...
    Cross-repo edits stay declined in both directions per DR-127: makima owns this
  ...
  8.0.0  (2026-08-14) `conversion-census-row` — the vendored row shape this repo owns —
    changes non-additively at a1f3b7f72. `mechanical_part` and `judgment_part` go
    `type: string` -> `type: object`, each carrying the generation-relevant fields for the
    half it names (`mechanical_part.candidate_op`; `judgment_part.judgment_kind` /
    `round_trip` / `revalidate_at_dispatch`). A consumer holding the 7.0.0 stamp reads both
    as strings and every MIXED row now fails it, so MAJOR — and, as at 7.0.0, the additive
    majority around it does not soften the class: the bump rule keys on the presence of a
    non-additive change, not its share. The additive remainder in the same commit: the
    per-half fields also appear at row level for the single-classification rows,
    `judgment_kind` (named enum, advisory | untrusted_gate) replaces the ordinal
    `judgment_tier` the request originally carried, `round_trip` and
    `revalidate_at_dispatch` split one lossy enum into two orthogonal fields, and
    `x_producer` enters as the producer-namespaced object.
    THE `anyOf` SOFTENING WAS DECLINED ON BOTH SIDES. Admitting `[string, object]` for the
    two halves would preserve 7.x consumers, but it makes the per-half fields optional in
    practice and returns the scaffolder to guessing on exactly the rows this revision
    exists to disambiguate. DoE independently reached the same refusal; no anyOf lands.
    SEQUENCING — the constant-moves-first shape of 6.0.0/6.1.0, not 7.0.0's violation-closing
    inverse. DoE's committed bundle (their 0ca980633) vendors the row pinned at dd2e30c7b,
    the pre-revision string-parts shape, and is therefore stale against the DRAFT it claims
    to vendor. They re-vendor at a1f3b7f72 and regenerate once this constant moves, and have
    stated they will not regenerate against 7.0.0 — so no committed bundle anywhere carries
    ...
    per DR-127: makima owns this constant, DoE owns the re-vendor, the regen, the bundle
...
Exit codes (parity-critical):
  ...
  2 — DEDICATED transport/config-failure code (coordinator root not resolvable via the
      ...
      the cross-repo split (this module runs makima-side, schemas live DoE-side) and is
...
# schema.json under the DoE-claude coordinator/ tree, explicitly NOT makima
# (see module docstring PURPOSE).
GENERATES = []

# Env var read by main() to locate the DoE-claude coordinator/ root (schemas/ input,
...
LIVENESS_MAPPING: dict = {
    ...
    "types": {
        ...
        "plan": {
            ...
            # prior literal-dict holdout): DoE-claude coordinator/tests/
        ...
        "roadmap": {
            "combination_rule": "single-axis",
            "axis": "status",
            "note": "Forward horizon/portfolio record. blocked resolves LIVE (in-flight), NOT the BLOCKED liveness bucket — that bucket is reserved for handoff awaiting_gate and plan deferred. Spec: qffs fleet-warp geneva Ask 1.",
            "mapping": _derive_status_mapping(
        ...
        "tracker": {
            "combination_rule": "single-axis",
            "axis": "status",
            "note": "Current status board / action register; one per repo. Spec: qffs fleet-warp geneva Ask 5 promote.",
        ...
        "health-status": {
            "combination_rule": "single-axis",
            "axis": "status",
            "note": "Periodic health summary/ledger. Liveness keys on status (lifecycle axis), NOT on health (posture axis — HEALTHY/WATCH/ACTION/CRITICAL). Spec: qffs fleet-warp geneva Ask 5 promote.",
...
_USAGE = (
    ...
    "artifact-shape-contract/; makima owns the sole regeneration path). The write is\n"
...
def main(argv: List[str]) -> int:
    """CLI entry: resolve the DoE-claude coordinator root from the environment and emit.
    ...
    regenerated bundle in DoE-claude's working tree instead of usage text. Any argv
