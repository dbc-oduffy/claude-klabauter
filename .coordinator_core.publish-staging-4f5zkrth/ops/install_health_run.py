...
see the DoE trampoline's own header note for the anti-pattern this collapses
...
Dual-anchor discovery (2026-07-22, closes the plugin_root-coupling defect):
  ...
  plus the makima-root sourced-lib `coordinator-makima-root.sh`) under a
  ...
  lives (if at all) under makima's OWN tree
  (`<makima_root>/coordinator/bin/install-health/`), alongside the
  orchestrator itself (`<makima_root>/coordinator/bin/install-health-run.py`,
  ...
  even for a still-live makima-owned drop-in.
  ...
  makima-side surface (the drop-in directory, and the `seed-skill-overrides`
  helper lookup) instead resolves off `coordinator_engine_root()`
  (`coordinator_core.engine_root`) — the canonical makima-root resolver
...
Native leg registry (`_NATIVE_LEGS`) — no globbing for makima-owned legs:
  All four makima-owned legs (`ensure_python3_exe_shim`,
  ...
  glob's own drop-in directory is gone; makima's own copy under
  ...
  `.sh`-drop-in ancestor at all — it is a makima-native addition to this
  ...
  sibling repo's routine housekeeping silently disabling a makima-owned
...
(exit 1) — every leg below needs a resolved makima root to run correctly
...
# Ordered list of (display_name, entrypoint) for every makima-owned
# install-health leg. Each entrypoint is normalized to a single
# ``(plugin_root, makima_root) -> int`` shape regardless of the underlying
# module's own main() signature, and runs UNCONDITIONALLY in `main()` below
# — no dependency on any file existing in DoE's (or any) drop-in directory,
...
#
# `seed-skill-overrides` needs `makima_root` to locate its DoE-resident-
# named-but-now-makima-resident helper (`<makima_root>/coordinator/bin/
...
# `makima_root` either — like the two OS-gated probes above, it derives
...
_NATIVE_LEGS = [
    ("ensure-python3-exe-shim", lambda plugin_root, makima_root: ensure_python3_exe_shim.main([])),
    ("check-windows-ssh-binary", lambda plugin_root, makima_root: check_windows_ssh_binary.main([])),
    (
        "seed-skill-overrides",
        lambda plugin_root, makima_root: seed_skill_overrides.main(
            [], plugin_root=plugin_root, helper_root=os.path.join(makima_root, "coordinator")
        ),
    ),
    (
        "check-bareword-path-provisioning",
        lambda plugin_root, makima_root: check_bareword_path_provisioning(plugin_root, makima_root),
    ),
]
...
def check_bareword_path_provisioning(plugin_root: str, makima_root: str) -> int:
...
def main(argv: List[str], script_path: Optional[str] = None) -> int:
    ...
    with env_overlay({"CHECK_ONLY": "1" if check_only else ""}):
        return _run_legs(plugin_root, makima_root, script_path)


def _run_legs(plugin_root: str, makima_root: str, script_path: Optional[str]) -> int:
    ...
    # whether bin/install-health/ exists under makima_root at all. The trust
    ...
    for leg_name, native_entrypoint in _NATIVE_LEGS:
        try:
            rc = native_entrypoint(plugin_root, makima_root)
    ...
    # with no native peer — resolved off makima_root (coordinator_engine_root()),
    # NOT plugin_root: this directory is makima's own tree
    # (<makima_root>/coordinator/bin/install-health/), not a DoE-side
    # surface, per the dual-anchor split (see module docstring).
    health_dir = os.path.join(makima_root, "coordinator", "bin", "install-health")
