...
makima-native ops. All are MUTATING (DR-208) and write coordinator substrate directly.
DR-216 is the reserved-noun-write carve-out authority for this op family.

Three ops registered here, byte-parity ports of makima-owned CLI writers:
    ...
    *,
    ...
    Params:
        ...
        dry_run: when True, compute the same {backfilled, skipped} result WITHOUT
            ...
            Added for cross-repo/inbox/2026-08-11-project-rag-em-backfill-
...
#: rather than asserting a status about them (project-rag memo 2026-07-20, item 3).
#:
...
#: `-check` segment (`.patrik-review.md`, `.sonnet-review.md`, `.review.md`,
...
def _as_of_sha(worktree: Path, date: str, commit_span: Optional[str] = None) -> Optional[str]:
    ...
    - date window → the last commit at or before `{date}T23:59:59`, the same
      `git rev-list -1 --before=…` probe project-rag's forensic audit used.
...
def _plans_touched(worktree: Path, date: str, commit_span: Optional[str] = None) -> str:
    ...
    weekly prior-week digest unusable. Fixed per project-rag-em memo 2026-07-20
    (`cross-repo/archive/2026-07-20-project-rag-em-changelog-plans-touched-hardcoded-status.md`).
...
def main(argv: List[str]) -> int:
    ...
    cross-repo/inbox/2026-08-11-project-rag-em-backfill-changelog-cli-three-
    ...
    returns the raw OS hostname (e.g. "Striker"), while every other artifact
    ...
    e.g. "striker"). On a case-sensitive filesystem the mismatch reads as a
