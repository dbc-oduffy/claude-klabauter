...
Exit codes (must match `main()`'s actual returns byte-for-byte — do not drift):
  ...
  5  TRANSPORT failure (caller-side only, not returned by this
     ...
     error / git not available" must not conflate that with "the makima link
...
Sibling-script resolution: this op is makima-resident but depends on one
...
def _resolve_scaffold_manifest_root(claude_home: str, coordinator_root: str) -> str:
    ...
    to project-makima") moved `check-install-divergence.py` here for good,
    and the trampoline (`coordinator/lib/bootstrap-repo.py`) now
    unconditionally points `COORDINATOR_ROOT` at makima's OWN `coordinator/`
