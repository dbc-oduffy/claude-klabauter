"""
coordinator_core.ops.probe_cwd_project_rag_relevance — Workstream-start
gift-shape signal for project-RAG visibility.

Purpose: Detect whether the current working directory has a project-RAG
...
Port of: probe-cwd-project-rag-relevance.sh (DoE b5a4192c, 2026-07-20), 342 lines
Port backlink: docs/plans/2026-07-15-bash-to-naked-python-engine-migration.md
Spec backlink (behavior): docs/plans/2026-05-20-portable-code-substrate.md § Chunk 3
DoE-side test oracle: coordinator/tests/test_probe_cwd_project_rag_relevance.sh
...
USAGE = """Usage: probe-cwd-project-rag-relevance.sh
...
def _debug(msg: str) -> None:
    if os.environ.get("COORDINATOR_DEBUG", "0") == "1":
        print(f"[debug] probe-cwd-project-rag-relevance: {msg}", file=sys.stderr)
...
def _claude_json_has_project_rag(claude_json_path: str) -> bool:
    ...
    try:
        ...
        print(f"skip: _claude_json_has_project_rag: with open(claude_json_path, \"r\", encoding=\"utf-8\") as fh: failed: {sys.exc_info()[1]}", file=sys.stderr)
        return False
...
def _whoami_project_kind() -> str:
    """Run the coordinator_whoami.project_rag probe via a resolved-Python
...
def main(argv: List[str]) -> int:
    ...
    rag_bound = _claude_json_has_project_rag(claude_json) or _registry_bound(ml_dir)
    ...
    if is_ue:
        ue_addon_sentinel = os.path.join(
            home_base, ".claude", "plugins", "project-rag-ue-addon", "data",
    ...
    if not is_ue:
        if mcp_healthy:
            print("[project-rag-relevance] healthy: project-rag MCP up for this cwd.")
        else:
            print(
                "[project-rag-relevance] broken: project-rag MCP not responding "
                "for this cwd. Run /project-rag:doctor to restore."
            )
    else:
        if not mcp_healthy:
            print(
                "[project-rag-relevance] p0-broken-ue: > Going alone on a UE "
                "codebase: project-rag MCP is the knowledge tool for this terrain."
            )
            print(
                "[project-rag-relevance] p0-broken-ue: > Restore it first — "
                "/project-rag:doctor. Without it you're grepping a 100K-file "
                "engine source by hand."
            )
        else:
            print(
                "[project-rag-relevance] healthy-ue: project-rag MCP up — "
            ...
            if not engine_corpus_ok:
                print(
                    "[project-rag-relevance] suggest-engine-corpus: Engine "
                    "corpus not loaded. Run /project-rag-ue-addon:doctor to bootstrap."
                )
