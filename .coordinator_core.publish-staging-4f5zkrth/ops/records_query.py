...
Purpose: COMPUTE_ONLY read op that queries makima's own state records
...
This is a per-repo live-state query surface (makima's class per the
...
Negative-spec:
  ...
  - Does NOT use ``read_fm_field`` for equality matching — it returns raw
    ...
    ``roadmap_id: "makima-strangler-2026-07-04"`` mis-match.  Uses
...
# archive location. Fleet-verified 2026-08-11 by project-opticon-em, who ran
# `handoff.columns` across all six coordinator repos on this disk and compared
# served rows against on-disk handoff files: DoE-claude 550/550, project-makima
# 431/431, project-rag 371/371, market-intelligence 194/194, project-opticon
# 169/169 — five of six exact on the globs above. The sixth, delphi-store, served
...
# on disk here, both locations present; makima's own tree is clean). That is
# residue from a one-off manual audit in their repo (delphi-store `838853f`),
# not a third live fleet convention.
#
# The fix belongs in their tree — move the files — and opticon has already routed
...
# delphi-store asks for the glob change directly, that is the standing answer,
# and opticon's independent read agrees with it.
...
#       ``fifa-stats/state/debt-backlog.md``'s ``| G-OVR | G | Build script
...
# `project-rag-ue-addon/state/bug-backlog.md`) does not match any branch
...
# an ``initiative`` FK — makima owns ``state/initiatives/``, so "which types
...
def _resolve_field(fm: dict, field: str):
    ...
    by project-rag-ue-addon-em against a 27-entry corpus whose true answer was
...
def _apply_sibling_exclusion(
    ...
    make this op depend on the full DoE schema tree at runtime — makima's
...
def _handler(
    ...
    if record_type not in _TYPE_TO_GLOB:
        ...
        # that names no type and offers no valid set. project-opticon-em hit
