...
#: (market-intelligence, 2026-07-22) by appending block items under a flow line.
...
def _apply_commits_fold(content: str, new_shas: list[str]) -> str:
    ...
    Negative-spec:
      ...
      - Flow-style ``commits: [...]`` → block-form rewrite is the ONE permitted byte-level
        ...
        corrupting the entry. Source: cross-repo memo 2026-07-22 (market-intelligence-em).
...
def _collect_chain_session_ids(
    ...
    (b) claimed_by (new; consumed_by fallback for pre-migration handoffs): from handoffs
        ...
        makima-central when worktree_root IS the meta-repo) and archive/handoffs/ whose
...
def _resolve_handoff_dirs(worktree_root: Path) -> Tuple[List[Path], Optional[str]]:
    ...
    resolving the central makima root then raised, so the caller falls back to the
...
#: committing across the tree boundary: makima's own sends land in the
#: receiver's tree, never here, so the sender slug is by construction foreign
#: (empirically: claude-central-em, doe-claude-em, project-opticon-em,
#: project-rag-em, market-intelligence-em, project-rag-ue-addon-em,
#: holodeck-em, delphi-store-em — never a makima slug across all 602
...
def _sibling_homed_session_ids(
    ...
    in DoE-claude that commits into makima's tree therefore leaves those
    ...
    its own home repo), makima owes no completion entry for such a session and
    ...
    subagent while working in makima's tree provisions a share directory here
    ...
    it is the tempting cheap predicate and it fails OPEN. 36 of 85 makima-homed
    ...
    orphan commits and 6 of 87 makima-homed ones — a baton marker, not a home
...
def _foreign_delivery_commits(
    ...
    PREDICATE — conjunction of two facts, both required:
      1. the subject is the delivery subject makima's own memo-send engine
    ...
    deliveries; the other 37 are makima's own commits that simply lost the
    ...
    still have no makima session behind them.
...
def day_coverage_sweep(worktree_root: Path, day: str) -> dict:
    ...
    Unclaimed commits are partitioned four ways:
      - FOREIGN-AUTHORED: a sibling repo's engine committed the commit
        ...
        makima session authored it, so no completion entry can ever claim
      ...
      - GENUINELY ORPHANED: none of the above — no trailer, or a trailer
        matching nothing known, on a commit some makima session did author.
