...
docs/plans/2026-07-26-memo-disposition-flip-op-and-hand-edit-hole.md; makima owns
...
Return contract (AC6, makima-client op-result):
...
def _normalize_block_scalar_summary(fm_text: str, memo: str) -> str:
    ...
    ``replace_fm_field``/``remove_fm_field`` both refuse (block-scalar guard, Patrik F1)
...
# frontmatter SHAPE is coordinator-claude's contract to own, not makima's
...
def _action(memo: str, params: dict) -> dict:
    ...
    Spec backlink: DoE-claude:pln-rebuild-distill-on-makima-engi-9509ac § C3
