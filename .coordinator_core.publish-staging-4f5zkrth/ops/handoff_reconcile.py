...
must pass `dry_run=false` explicitly to transition anything (Patrik review, finding
...
makima's `status` axis is `{open, claimed}` under DR-084 (reads stay dual-tolerant on
...
Per-verdict routing (Patrik review, finding index 3 — minor: route the C3 structured
...
of this ruling against the 2026-08-01 ruling it re-keys, and docs/plans/2026-07-13-makima-
...
Spec backlink: pln-makima-auto-reconcile-pass-off-425848 § C4
...
CLOSED:
  - Schema declaration: landed upstream at DoE-claude's SSOT and vendored in
    ...
    `cross-repo/inbox/2026-08-14-project-makima-em-reviewed-range-pattern-and-direction-blind-parity-gate.md`.
...
def _ancestor_evidence(
    ...
    """C6 per-ancestor EVIDENCE GATE (Patrik F2, MAJOR) — never infer shipped-ness
...
async def _handle_auto_ship(
    ...
    if not auto_ship_enabled:
        ...
        try:
            ...
            # F4 error isolation (Patrik review) — a mid-loop raise (e.g. a
...
async def _handle_in_flight_blocked_by_retirement(
    ...
    2026-08-04-market-intelligence-em-pickup-jgate-cleared-strands-gate-
    ...
    def mutate(old_text: str) -> str:
        ...
        # Act-time re-verification (mirrors _gate_cascade_clear's own Patrik
...
async def _handler(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    for handoff in open_handoffs:
        ...
        if deployment_state == _AWAITING_GATE_STATE:
            ...
            # 2026-07-13-makima-auto-reconcile-open-handoffs.md § D5 ("evidence
