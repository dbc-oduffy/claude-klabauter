...
class TestRenderContract:
    ...
    def test_one_initiative_one_line(self, monkeypatch, tmp_path, capsys):
        ...
        _patch_handler(
            monkeypatch,
            {
                ...
                "artifacts_matched": 1,
                "advances_initiatives": [
                    {"id": "makima-strangler", "label": "Makima Strangler", "status": "active"}
                ],
            },
        )
        ...
        out = capsys.readouterr()
        assert rc == 0
        assert out.out == "advances initiative Makima Strangler (makima-strangler)\n"
    ...
    def test_scan_incomplete_true_appends_partial_scan_qualifier(
        ...
        assert rc == 0
        assert out.out == (
            "advances initiative Makima Strangler (makima-strangler) (partial scan)\n"
        )
