...
not) — that indirection existed ONLY because the DoE-side caller was bash
...
stamp_abort_count) rather than one global boolean:
  - `promoted == 0 and norc_count == 0 and unknown_error_count == 0 and
    ...
    quiet. This is the "zero candidates at all" case: DoE's `/workday-start`
...
unconditional-0 contract no longer holds. A makima-link/import failure at
the DoE-side trampoline layer still degrades to exit 0 with a loud stderr
...
(passed explicitly by the DoE trampoline, derived the same SCRIPT_DIR-
...
Port of: promote-shipped-in-flight-stubs.sh (DoE b5a4192c, 2026-07-20)
...
def main(argv: List[str], *, repo_root: Optional[str] = None) -> int:
    ...
    # — neither is a caller-visible failure, and DoE's /workday-start runs
