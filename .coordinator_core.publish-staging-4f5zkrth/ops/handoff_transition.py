...
Verb contracts (mirrored from the JS spec):
  ...
  claim — pickup-time transition (params: handoff_path, session_id, at).
    ...
    - Fail-loud on empty session_id (Patrik P2): exit_code=1, no write.
  ...
  gate-cascade-clear — structured blocked_by cascade-clear (params: handoff_path,
    ...
    - ACT-TIME RE-VERIFICATION (Patrik F0, load-bearing): before removing any
...
def _claim(handoff_path: str, session_id: str, at: str, worktree: Path, repo_root: Path) -> dict:
    ...
    Fail-loud on empty session_id (Patrik P2) — never write claimed_by: empty.
...
def _registered_sibling_repo_named_in_prose(prose: str) -> Optional[str]:
    ...
    naming convention (e.g. "DoE-claude" -> "doe_claude", "project-makima"
    -> "project_makima" — the same convention CLAUDE.md's own prose uses
...
def _resolve_blocker_deployment_state(blocker_id: str, worktree: Path) -> _BlockerState:
    ...
    Act-time re-verification (Patrik F0): scans state/handoffs/ (live) first, then
...
def _gate_cascade_clear(
    ...
    Act-time re-verification (Patrik F0): each blocker_id is independently
    ...
    def mutate(old_text: str) -> str:
        ...
        # Act-time re-verification (Patrik F0, load-bearing): re-resolve EACH
        ...
        #
        # Clause-ownership match (Patrik/Slice-B follow-up, C8): the former
