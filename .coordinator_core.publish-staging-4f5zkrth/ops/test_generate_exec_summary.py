...
def test_t10_resolved_engine_class_refuses_write_loudly(repo, tmp_path, monkeypatch, capsys):
    ...
    When the meta-repo branch resolves via `coordinator_makima_root_with_class`
    ...
    write makima state into the mirror. Reuses
    `coordinator_core.state_root._makima_state`'s own guard -- this test
    monkeypatches `coordinator_makima_root_with_class` as SEEN THROUGH
    ...
    monkeypatch.setattr(state_root_mod, "coordinator_makima_root_with_class", _fake_with_class)
...
def test_t11_live_working_tree_class_unchanged_behavior(repo, tmp_path, monkeypatch):
    ...
    live_root = str(tmp_path / "live-makima-checkout")
...
def test_link_rewriting_does_not_double_prefix_stray_double_relative_target():
    text = "[source](../../archive/daily-summaries/2026-07-12-delphipro.md)"
    rewritten = mod._rewrite_managed_links(text)
    assert rewritten == "[source](../archive/daily-summaries/2026-07-12-delphipro.md)"
...
def test_rewritten_managed_links_resolve_on_disk_relative_to_output_dir(tmp_path):
    ...
    target_file = repo_dir / "archive" / "daily-summaries" / "2026-07-12-delphipro.md"
    target_file.write_text("hi", encoding="utf-8")

    for raw_target in (
        "archive/daily-summaries/2026-07-12-delphipro.md",
        "../archive/daily-summaries/2026-07-12-delphipro.md",
        "../../archive/daily-summaries/2026-07-12-delphipro.md",
    ):
