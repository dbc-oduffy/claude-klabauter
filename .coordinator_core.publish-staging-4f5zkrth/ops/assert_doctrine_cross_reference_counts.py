...
a cross-repo caller would otherwise silently read makima's own tree instead of
...
Spec backlinks:
  ...
  - Fence example (read-only reference, not ported verbatim):
    coordinator/skills/plan/SKILL.md § Test Surface (DoE-claude, AC3 grep-assert)
