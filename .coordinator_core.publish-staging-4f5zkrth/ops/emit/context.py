...
emits under its own identity — ``~/.claude`` reaches slug ``dbc-oduffy/.claude-prime``
...
coordinator script/lib clone (``<makima-root>/coordinator`` on a current install; the
...
META_REPO_NAME_FALLBACK = "dbc-oduffy/.claude-prime"
...
def _remote_url_to_slug(url: str) -> Optional[str]:
    ...
    Handles the common SSH and HTTPS forms:
        git@github.com:dbc-oduffy/.claude-prime.git  -> dbc-oduffy/.claude-prime
        https://github.com/dbc-oduffy/.claude-prime  -> dbc-oduffy/.claude-prime
    """
    url = url.strip()
    if not url:
        return None
...
class EmitContext:
    ...
    Fields (parity with bash:117-122 one-time capture):
        ...
        coordinator_root   — the live post-W4.2-cutover coordinator script/lib clone,
                             ...
                             ``<makima-root>/coordinator`` on a current install; the
        ...
        repo_name          — EMITTING REPO's own slug (e.g. ``dbc-oduffy/project-makima``);
                             resolved from ``repo_root``'s git remote.  This is the ``repo``
                             field on every provenance row — the rag/opticon ingest key.
        ...
        full_enrichment    — cadence tier for this emission. THE single cadence signal; both
                             ...
                             notices it; silently stale data in a rag/opticon join key is not.
    ...
    def provenance(
        ...
        method is NOT the sole provenance constructor in makima; a future re-vendor
