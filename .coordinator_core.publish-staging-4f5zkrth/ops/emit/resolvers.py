...
The twenty-two section porters under `sections/` were deliberately NOT removed. Seven have
...
surface currently being built for project-opticon. The rest are producer libraries with no
aggregate caller: the artifact was the defect, not the producers, and opticon's
...
# EMPTY-BY-DESIGN, PERMANENTLY. makima is NOT the data path for market-intelligence — that
# data routes market-intelligence -> opticon's `ingestEmission` plane DIRECTLY, never through
...
# (cross-repo/inbox/2026-07-14-project-makima-em-cockpit-v2160-source-seam.md, ## EM Response).
#
...
def _resolve_central_state_root(coordinator_root: Path, cwd: Path) -> Path:
    ...
    ``$(_csr_makima_root)/state``. The bash lib's own ``_csr_makima_root`` is itself
    ...
    on the makima-root resolver, never on the coordinator/cwd it's invoked from.
    ...
    fallback on a durable-settings-home-only machine (unset ``repos.project_makima``); a
    ...
    try:
        ...
        warnings.warn(
            ...
            "repos.project_makima is unset in the machine-local registry, or a "
            "partially-installed/durable-settings-home-only machine — verify "
            "`machine-local get repos.project_makima` resolves before trusting emitted state.",
            stacklevel=2,
        )
...
def resolve_coordinator_root() -> Path:
    ...
    2026-07-22 de-node cutover then retired ``bin/query-records.js`` fleet-wide (makima's own
    production dependency on it dropped to zero -- see
    ``cross-repo/archive/2026-07-22-project-makima-em-query-records-positive-clearance-de-node-cutover-landed.md``
    -- and DoE-claude/coordinator/bin now carries neither the ``.js`` oracle nor a ``.py``
    port), so ``bin/query-records.py`` (makima's own native, de-node-durable reader) is now
    ...
    doctrine; rung 5 is the sole CLI-shellout exception, see below):
      ...
      2. Co-located ``<makima-repo-root>/coordinator`` (2026-07-22 executable-surface
    ...
    raise RuntimeError(
        ...
        "<makima-repo-root>/coordinator tree, machine-local registry, and "
