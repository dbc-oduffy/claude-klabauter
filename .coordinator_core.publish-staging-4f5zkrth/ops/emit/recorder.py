...
Fleet-wide backlog totals are aggregated by opticon across N per-repo shards.
...
def record(ctx: Optional[EmitContext] = None) -> dict:
    ...
    fleet-wide totals are aggregated by opticon across N per-repo shards.
...
def _backlog_record(params: dict, repo_root=None) -> dict:
    ...
    # collision detection here is out of scope; opticon handles deduplication at aggregation time.
