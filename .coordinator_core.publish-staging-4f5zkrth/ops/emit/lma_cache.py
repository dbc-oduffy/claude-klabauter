...
back through the whole corpus. Measured ~0.86s of a 3.5s emit on the DoE-claude corpus
...
Why not ``coordinator_core.cache``:
    ...
    falsely hit. The cache also has to survive across processes, because makima is a
...
rag/opticon, so a wrong hit is worse than no cache at all):
...
Cache location: ``<coordinator-settings-home>/project-makima/emit-lma-cache/<slug>.json``,
...
EMITTING repo, which is routinely a sibling clone (makima emits for DoE-claude), and a
...
_CACHE_SUBDIR = ("project-makima", "emit-lma-cache")

# Generator-provenance: writes only to the durable-data-plane cache file under
# settings_home()/project-makima/emit-lma-cache/<slug>.json -- deliberately
# not in-repo (see module docstring "Cache location") -- never into makima's
# own tracked tree.
GENERATES = []
