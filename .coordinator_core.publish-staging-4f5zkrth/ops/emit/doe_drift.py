...
fail-loud drift-check when makima's vendored schema-version lags the DoE-HEAD
...
Resolution contract (DoE emission-conformance-contract.md § CD-1/CD-2):
  ...
  - **Version band — min_supported, not == HEAD.** Fail loud ONLY when makima's pinned
...
Spec backlink: docs/decisions/DR-210-makima-native-tooling-ownership-strangler.md § 2
Oracle: /Users/oduffy/X/DoE-claude/coordinator/docs/wiki/emission-conformance-contract.md
"""
...
# Written by the re-vendor step (bin/makima-revendor-cockpit-contract.py); ``ABSENT`` when the ref was not
...
class DriftWarning(UserWarning):
    ...
    freshness gate is disabled until ``bin/makima-revendor-cockpit-contract.py`` is re-run.
...
class AheadOfReleaseWarning(UserWarning):
    ...
    Purpose: signal the expected reader-first window state — makima has re-vendored a
    ...
    ``bin/makima-revendor-cockpit-contract.py``'s ``_post_vendor_drift_check`` kills on
...
def check_version_band(pinned_version: str, fixture: dict) -> None:
    ...
    if pinned_t < min_t:
        raise DriftError(
            f"makima vendored schema-version {pinned_version} is below the DoE "
            f"min_supported_contract_version {min_supported_str}.  Re-vendor the "
            "cockpit-contract pin (python3 bin/makima-revendor-cockpit-contract.py) to satisfy the version-band gate "
...
def check_freshness(doe_clone: Optional[Path] = None) -> None:
    ...
    DoE publishes would break makima before re-vendor can run; a loud, distinct,
    ...
    if pin_sha is None:
        ...
        try:
            ...
            warnings.warn(
                ...
                "Run python3 bin/makima-revendor-cockpit-contract.py to populate the pin when DoE publishes the ref.",
                stacklevel=2,
            )
            return
        ...
        if origin_sha is not None:
            ...
            # failure the instant DoE publishes would break makima before
            # bin/makima-revendor-cockpit-contract.py runs.
            warnings.warn(
                ...
                "disabled.  Re-vendor via python3 bin/makima-revendor-cockpit-contract.py to arm the gate and enable "
    ...
    if origin_sha != pin_sha:
        ...
        if is_ahead is True:
            warnings.warn(
                ...
                "This is expected during the DR-203 reader-first re-vendor window — makima "
        ...
        raise DriftError(
            ...
            "Re-vendor the cockpit-contract (python3 bin/makima-revendor-cockpit-contract.py) to update the pin."
        )
