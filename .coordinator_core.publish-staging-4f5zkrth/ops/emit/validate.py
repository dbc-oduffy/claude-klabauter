...
entire entity array against the frozen cockpit-contract schema. The makima engine validates
against a VENDORED PIN (DR § DD#3: "makima-produce validates against a vendored pin … no
...
Version guards (plan § D4 / patrik F5, reworked 2026-07-21):
    - The content-based version-desync guard now cross-checks the vendored schema bundle
      ``.version`` against MAKIMA'S OWN ``CONTRACT_VERSION``
...
def assert_pin_integrity() -> None:
    ...
    if missing:
        raise ContractPinError(
            "makima cockpit-contract vendored pin is incomplete — missing: "
            + ", ".join(str(p) for p in missing)
            + ". Re-run the vendor step (python bin/makima-revendor-cockpit-contract.py) "
...
def assert_version_consistency() -> str:
    ...
    The guard now cross-checks the vendored schema bundle's ``.version`` against MAKIMA'S OWN
    ...
    from coordinator_core.contract.cockpit_schema import CONTRACT_VERSION as _makima_contract_version

    if schema_version != _makima_contract_version:
        raise ContractPinError(
            "cockpit-contract version desync — makima's own CONTRACT_VERSION="
            f"{_makima_contract_version} but the vendored schema bundle .version="
            ...
            "lags — re-vendor: python bin/makima-revendor-cockpit-contract.py\n"
...
def contract_declares_backlog_history() -> bool:
    ...
    the coordinator lands it (contract v2.7.0; DoE+PM convention call + Zolí review); this
...
def _validate_emission_scope(envelope: dict, ctx, schema_version: str) -> None:
    ...
    Spec backlink: pln-makima-emission-scope-conforma-1f0dbb § C1, D25 §6.
    """
    try:
...
def _compiled_entity_validator(entity_name: str):
    ...
    if not schema_path.exists():
        raise ContractPinError(
            ...
            "(python bin/makima-revendor-cockpit-contract.py) if the pin is stale."
        )
