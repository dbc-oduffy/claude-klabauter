...
**34.7%** of a 2,924-file ``git ls-files`` union on the DoE-claude corpus — 1,910 tracked
...
(2026-07-29):
  ...
  - Do NOT collapse a subagent's touches onto its parent session. That is the same lossy
    ...
    the thing that would need a new contract field; route that through DoE rather than
    inventing it here.
...
Port of: emit-cockpit-snapshot.sh (DoE 07eedcfb, 2026-07-19) — § SECTION 8.14,
...
Producer: coordinator/bin/derive-file-attribution.py — claude-klabauter's own file, resolved via
  ``ctx.coordinator_root`` (a ``__file__`` walk from this repo, not a DoE/upstream path);
...
# cache, DoE-claude corpus) gave 14.12s, 2.56s and 4.87s, varying with OS page-cache state and
...
def _validate_prior_file_attributions(rows: Any, repo_root: Path) -> Optional[str]:
    ...
    Why this exists at all: DoE-claude's on-disk emission (2026-07-28, schema 3.7.0) carries
...
def collect(ctx: EmitContext) -> tuple[list[dict], list[dict]]:
    ...
    (557 on the DoE-claude corpus), so it runs only on the full-enrichment tier. For this
    ...
    DoE-claude's on-disk emission — is REJECTED and recomputed, not reused. Unvalidated reuse
