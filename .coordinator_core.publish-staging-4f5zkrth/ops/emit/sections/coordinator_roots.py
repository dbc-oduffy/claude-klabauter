...
bash-to-Python port). That is a live remote call on makima's emission path, which does not
belong here — makima emits authoritative disk-truth; GitHub repo metadata is remote state,
and the fleet query surface belongs to opticon (see project CLAUDE.md § Project Overview).
...
Consumer deduplication contract (S3-F7 deferral — NOT a makima fix; opticon's responsibility):
Post-cutover, opticon receives coordinator_roots entries for project-makima from TWO sources:
(1) the meta-repo's fleet census (working-repos.yaml walk), and (2) project-makima's own
self-report (element [0] of its own snapshot). Opticon MUST deduplicate coordinator_roots by
``repo`` slug across all ingested snapshots (latest-wins / self-report-authority). Failure to
deduplicate would cause project-makima to appear twice in the fleet-census view. This is a
tracked follow-up on opticon's side (self-report-authority model). makima emits correctly per
...
def collect(ctx: EmitContext) -> tuple[list[dict], list[dict]]:
    ...
    element [0] is the EMITTING repo (project-makima when project-makima is emitting, ~/.claude
