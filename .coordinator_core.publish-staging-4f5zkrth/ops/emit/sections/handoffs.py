...
that justified retiring it (commit 5372260e) held only for makima's OWN corpus — this section
also runs against consumer repos' `state/handoffs/` + `archive/handoffs/` trees (project-rag,
project-opticon, etc.), which were verified un-migrated on 2026-07-23 (project-rag: 100% old
vocabulary; project-opticon: majority old vocabulary, including ``deployment_state: abandoned``
records).

    MEASUREMENT SUPERSEDED IN PART, 2026-08-11 — do not cite the opticon half above as current.
    project-opticon-em re-measured their own corpus through the ``handoff.columns`` CLI and
    ...
    The "majority old vocabulary" description of opticon is ~3 weeks stale and was cited from
    ...
    project-rag-em re-measured their own ``state/handoffs/`` + ``archive/handoffs/`` trees on
    request: ``claimed`` 245, ``open`` 104, ``superseded`` 16, ``active``/``consumed`` **zero**.
    Their corpus is uniformly NEW vocabulary; "project-rag: 100% old vocabulary" describes
    ...
    **The shim STAYS, and the exit condition below is NOT met** — but on one reason now, not
    two. Both consumer legs (opticon 2026-08-11, rag 2026-08-11) are measured migrated, so the
    un-migrated-corpus reason is discharged. What survives is opticon's own argument for
...
permanently-grandfathered axis (2026-06-26, Patrik-ratified DO-NOT-NARROW invariant on
...
surviving old tokens — not on a timer, not on makima's own corpus alone.
...
strings), UNLIKE the ``plan_id`` dead-join above:
  ...
  - ``origin_plan_id``    — same ``pln-*`` value as the ``plan_id`` field above (the C7
    ...
    already resolvable elsewhere in this contract — so a bare id suffices for opticon;
...
def _derive_handoff_id(repo: str, path: Optional[str]) -> tuple[str, str]:
    ...
    cockpit corpus aggregates records across repos (makima, DoE-claude, project-rag,
    project-opticon, …) and a timestamp-prefixed basename (``2026-07-19-foo.md``) is exactly the
...
def collect(ctx: EmitContext) -> tuple[list[dict], list[dict]]:
    ...
    for rec in raw_records:
        ...
        # no new key on the wire until opticon has vendored it.
        if _human_axis_on:
