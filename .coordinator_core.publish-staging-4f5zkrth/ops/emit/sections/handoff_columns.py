"""The four opticon columns (``status``, ``deployment_state``, ``predecessor``,
``shipped_in``), callable without an ``EmitContext`` or an emit envelope.

Extracted out of ``sections/handoffs.py`` (C1, plan
``docs/plans/2026-08-11-pull-surface-for-opticon-the-four-columns-and-the-archive.md``) so a
...
def _coerce_legacy_abandoned(fm: dict) -> tuple[str, Optional[str], Optional[str]]:
    ...
    docstring): a dead-holder node WITH a successor, and a deliberate stop WITHOUT one. makima's
    ...
    consumer repos this section also ingests from (project-rag, project-opticon) carry
...
def compute_handoff_columns(fm: dict, repo_root: Path) -> dict:
    """Compute the four opticon columns for ONE record's parsed frontmatter.
