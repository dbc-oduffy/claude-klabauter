...
Covers every acceptance criterion from strang-02:
  ...
  AC_Q-a-band:
    ...
    - ``min_supported_contract_version`` past makima's pin → DOES fail-loud.

  AC_Q-b:
    - Fixture read from ``coordinator/cockpit-contract/conformance/emission-conformance.json``
      in the DoE local clone at ``repos.doe_claude``; NOT co-vendored in makima's _vendor/.
...
class TestFixtureResolution:
    """AC_Q-b: fixture lives in the DoE clone, not co-vendored in makima's _vendor/."""
    ...
    def test_fixture_read_from_doe_clone(self) -> None:
        """Fixture is read from repos.doe_claude (not makima's _vendor/)."""
    ...
    def test_fixture_not_in_vendor_tree(self) -> None:
        """The fixture MUST NOT be co-vendored in makima's _vendor/ tree.
...
class TestVersionBand:
    ...
    def test_contract_version_bump_alone_does_not_fail(self) -> None:
        ...
        min_supported only after the re-vendor window closes.  makima must NOT fail
        ...
        # makima's pin is 2.5.0 — MUST pass.
    ...
    def test_min_supported_past_pin_fails_loud(self) -> None:
        ...
        makima pin must fail loud — the grace window has closed.
        """
    ...
    def test_missing_min_supported_warns_not_raises(self) -> None:
        ...
        we warn rather than breaking makima (graceful degradation).
        """
...
class TestFreshnessRef:
    ...
    def test_pin_ahead_of_tag_warns_not_raises(self, tmp_path: Path) -> None:
        ...
        Reader-first window: makima re-vendors v2.6.0 while the tag still points to
    ...
    def test_pin_absent_but_ref_now_on_origin_emits_drift_warning(
        ...
        ``bin/makima-revendor-cockpit-contract.py`` has not been re-run so the pin file
        ...
        break makima before bin/makima-revendor-cockpit-contract.py can run.  The
