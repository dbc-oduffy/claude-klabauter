...
The break-class risk here is a stale hit: ``last_modified_at`` feeds rag/opticon, so a cache
...
def test_non_monotonic_histories_agree_with_the_oracle(repo: Path) -> None:
    ...
    break-class: ``last_modified_at`` is a rag/opticon join key.
    """
...
def test_cache_path_lives_under_the_durable_data_plane(repo: Path, tmp_path: Path) -> None:
    ...
    """
    path = lma_cache.cache_path(repo)
    assert path.is_relative_to(tmp_path / "settings-home" / "project-makima")
