...
the observable-contract lock the plan review demanded; see docs/plans/2026-07-21-makima-
...
class TestResolveCoordinatorStateRoot:
    ...
    def test_meta_repo_routes_to_makima_root(self, monkeypatch, tmp_path: Path) -> None:
        monkeypatch.setattr(routine_signals, "_cws_git_root", lambda cwd=None: str(tmp_path))
        monkeypatch.setattr(routine_signals, "_cws_claude_home", lambda: str(tmp_path))
        monkeypatch.setattr(routine_signals, "_cws_makima_root", lambda: "/makima/root")
        assert _resolve_coordinator_state_root(tmp_path) == str(Path("/makima/root") / "state")

    def test_meta_repo_unresolvable_makima_returns_none(self, monkeypatch, tmp_path: Path) -> None:
        monkeypatch.setattr(routine_signals, "_cws_git_root", lambda cwd=None: str(tmp_path))
        monkeypatch.setattr(routine_signals, "_cws_claude_home", lambda: str(tmp_path))
        monkeypatch.setattr(routine_signals, "_cws_makima_root", lambda: None)
        assert _resolve_coordinator_state_root(tmp_path) is None
