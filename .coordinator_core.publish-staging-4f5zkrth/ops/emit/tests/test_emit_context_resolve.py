...
class TestRemoteUrlToSlug:
    ...
    def test_ssh_form(self) -> None:
        assert _remote_url_to_slug("git@github.com:dbc-oduffy/.claude-prime.git") == "dbc-oduffy/.claude-prime"

    def test_https_form(self) -> None:
        assert _remote_url_to_slug("https://github.com/dbc-oduffy/project-makima") == "dbc-oduffy/project-makima"

    def test_https_form_with_git_suffix(self) -> None:
        assert _remote_url_to_slug("https://github.com/dbc-oduffy/project-makima.git") == "dbc-oduffy/project-makima"
...
class TestResolveRepoName:
    ...
    def test_resolves_slug_from_https_remote(self, tmp_path: Path) -> None:
        ...
        """
        fixture_url = "https://github.com/dbc-oduffy/.claude-prime"
    ...
    def test_does_not_fall_back_to_meta_repo_fallback(self, tmp_path: Path) -> None:
        ...
        Uses a directory literally named `.claude-prime` so the non-equality assertion is
        ...
        """
        fake_dir = tmp_path / ".claude-prime"
        fake_dir.mkdir()

        with patch("coordinator_core.ops.emit.context._run_git", return_value=None):
            result = resolve_repo_name(fake_dir)

        assert result == "local/.claude-prime", (
            "local-slug for a .claude-prime dir must be local/.claude-prime"
        )
        assert result != META_REPO_NAME_FALLBACK, (
...
class TestEmitContextResolve:
    ...
    def test_resolve_succeeds_with_valid_remote(self, tmp_path: Path) -> None:
        ...
        assert ctx.repo_name == "dbc-oduffy/project-makima"
...
class TestResolveOnRealRepoIsSpawnFreeForBranchAndSha:
    ...
    def test_no_git_subprocess_spawned_for_branch_or_sha(self) -> None:
        ...
        assert (real_repo_root / ".git").exists(), "expected the makima repo root"
...
class TestMetaRepoFallbackIsOracleOnly:
    ...
    def test_fallback_constant_holds_claude_prime_slug(self) -> None:
        """The constant's value matches the expected ~/.claude origin slug."""
        assert META_REPO_NAME_FALLBACK == "dbc-oduffy/.claude-prime"
