...
normalized on BOTH sides using a context-appropriate strategy:
  ...
  Section parity (assert_section_parity):
    ...
    - Derived fields (deliverable_status, shipped_sha) → None on BOTH sides.
      ...
      (Patrik F0 — a pure sentinel-on-both-sides approach hides that failure).
...
def _normalize_section(value):
    ...
    pass any non-null value (Patrik F0 fix).
...
# ``makima-strangler-2026-07-04``) that do not exist anywhere in ``fixtures/root``.
...
def assert_section_parity(name: str, ctx=None) -> None:
    ...
    Uses _section_normalize_and_sort (Patrik F0 fix): derived fields (deliverable_status,
...
# Spec backlink: docs/plans/2026-08-11-pull-surface-for-opticon-the-four-columns-and-the-archive.md § C1. [DEAD-CITATION: plan file never committed to this repo]
