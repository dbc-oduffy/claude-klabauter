...
priority-intent records dropped by project-opticon into the priority-ledger.

Purpose: project-opticon's Tactical route is dual-tier (Electron + hosted
...
exact failure the cross-repo contract forbids. Opticon instead DROPS a
...
authored in DoE-claude and vendored locally at
...
write direction is inverted, opticon writes INTO our tree).
...
``source: external-intent`` was, by construction, opticon-originated and
...
``target_kind`` field (opticon does not know our ledger's kind taxonomy);
...
# EXACTLY (DoE-claude) — same pattern as priority_set._TARGET_ID_PATTERN,
...
# _QUEUE_SCHEMA_PINS, re-vendored only via bin/makima-revendor-schema.py.
...
def _resolve_schema_path() -> Optional[Path]:
    ...
    unlike the pre-vendoring live-DoE-tree read, a missing vendored file is
