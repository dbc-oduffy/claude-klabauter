...
for makima's stdout-emitter shell-init resource-cap guard (DR-047 split).
...
Purpose (unchanged from the doc): resolve `project-makima`'s root
(`REPO_PROJECT_MAKIMA` env override, then `machine-local get
repos.project_makima`), and — only if `<makima_root>/bin/shell-init-guard.py`
...
`eval`s the guard's stdout at shell start. A machine without project-makima
checked out is a graceful no-op, not a failure — there is simply no guard to
source.

The resolved makima path is BAKED into the written block at install time,
...
Negative-spec:
    ...
    - Does NOT itself invoke/import `shell-init-guard.py` — only checks that
      ...
      rc snippet;
      the guard's own stdout-emitter logic is makima-resident and entirely
      out of scope for this seam (DR-047: makima owns the engine, DoE owns
      the rc-eval seam that sources it).
    - Does NOT gate on the guard script's execute bit, at install time or in
      the emitted rc block. makima's `bin/` is a pure-Python shop invoked
...
def resolve_makima_clone() -> str:
    """Tier 1: REPO_PROJECT_MAKIMA env. Tier 2: the direct-registry reader
    ...
    isolated every test from the operator's real `REPO_PROJECT_MAKIMA`
    ...
    tier-2 falling through (`test_graceful_skip_when_makima_not_resolvable`)
    cannot read the operator's REAL registry either."""
    env_override = os.environ.get("REPO_PROJECT_MAKIMA", "")
    if env_override:
        return env_override
    value = _machine_resolver.registry_get("repos.project_makima")
    return value or ""
...
def main(argv: List[str]) -> int:
    ...
    makima_clone = resolve_makima_clone()
    guard_src = os.path.join(makima_clone, "bin", "shell-init-guard.py") if makima_clone else ""
    ...
    block = (
        f"\n{SENTINEL}\n"
        "# Graceful no-op if makima absent or python3 missing: the -f check + eval's 2>/dev/null +\n"
        "# the emitter's own fail-open behavior combine to make this safe to source unconditionally.\n"
        "# -f, not -x: the guard is interpreter-invoked and ships 100644 like the rest of makima's bin/.\n"
