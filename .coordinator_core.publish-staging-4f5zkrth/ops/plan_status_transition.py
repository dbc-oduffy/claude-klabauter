...
``--override-reason`` (2026-08-10, cross-repo memo project-rag-em-close-out-
...
project-makima-em-plan-status-transition-wired-node-spawn-gone.md -- it was
declared deletable once this module's ``stamp-implemented`` had zero
remaining makima callers routed through it). There is therefore no live
oracle to hold byte-parity against for ``stamp-superseded``; this verb is
makima-native by construction, not a port. Flagged for the EM/PM, not
...
trampoline and makima's own ceremony callers meet at, so it owns the third outcome above.
...
Negative-spec:
    ...
    - RETIRED (2026-08-04, C6 Addendum Q4, PM-ratified): "no locking, single-writer,
      ...
      off this module's own main() -- both DoE's polyglot trampoline and makima's
...
def _parse_args(argv: List[str]) -> _Opts:
    ...
    2026-08-10-project-rag-em-close-out-stamps-implemented-without-reading-
...
def _refuse_if_live_foreign_holder(plan_path: Path, worktree_root: Path, closing_sid: Optional[str]) -> Optional[str]:
    ...
    memo `2026-08-10-project-rag-em-wsc-misdetection-wrote-to-a-live-peers-
    plan.md`).
...
def _ac_open_rows_warning(plan_path: str, plan_text: str) -> None:
    ...
    `2026-08-10-project-rag-em-close-out-stamps-implemented-without-
    reading-the-ac-table.md` from project-rag-em: this CLI's own flip gate
