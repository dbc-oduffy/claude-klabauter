"""
coordinator_core.ops.foreign_symbols — lazy-import seam over project-rag's
...
def _diagnostic_names_language(diagnostic: Dict[str, Any], language: Any) -> bool:
    ...
    pinned ``project-rag-symbol-extract`` wheel
...
def _is_name_invariant_drop(diagnostic: Dict[str, Any]) -> bool:
    ...
    validating choke point (disclosed by project-rag-em, memo
    ``cross-repo/inbox/2026-08-11-project-rag-em-symbol-name-invariant-now-drops-rows.md``):
    ...
    ``project-rag-symbol-extract`` wheel
...
def classify_foreign_symbol_coverage(result: Dict[str, Any], census: Dict[str, int]) -> List[Dict[str, Any]]:
    ...
    (review dc659900, P1):
      ...
      fields are guaranteed by the pinned ``project-rag-symbol-extract`` wheel
      (``60b109cc85b7f1fedc552c178214601040793111``, ``pyproject.toml``;
