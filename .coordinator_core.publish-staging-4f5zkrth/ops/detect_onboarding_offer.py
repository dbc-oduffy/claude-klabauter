...
Exit codes (main(), advisory-only, never blocks):
    0 -- always. This is a best-effort/advisory probe (mirrors the bash oracle's
         ...
         makima import/transport failure at the TRAMPOLINE layer degrades to a
...
def detect_onboarding_offer(repo_root: str, plugin_root: str) -> str:
    ...
    #
    # coordinator_currency.py lives in makima's own coordinator/lib/ (it
    # migrated out of the DoE plugin_root tree during the executable-surface
    # relocation), not under plugin_root -- resolve it off the makima root
    ...
    currency_lib = os.path.join(makima_root, "coordinator", "lib", "coordinator_currency.py") if makima_root else ""
