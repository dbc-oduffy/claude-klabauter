...
def test_live_no_probe():
    item_class, assertion, probe = parse_completeness_item(
        "live: claude mcp list shows holodeck-native-mcp connected"
    )
    assert item_class == "live"
    assert assertion == "claude mcp list shows holodeck-native-mcp connected"
    assert probe == ""


def test_restart_gated_with_probe():
    item_class, assertion, probe = parse_completeness_item(
        "restart-gated: holodeck MCP connects after restart "
        "[probe: claude mcp list | grep -q holodeck-native-mcp]"
    )
    assert item_class == "restart-gated"
    assert assertion == "holodeck MCP connects after restart"
    assert probe == "claude mcp list | grep -q holodeck-native-mcp"
...
def test_classification_for_hoist():
    ...
    c3, _, _ = parse_completeness_item(
        "restart-gated: holodeck MCP connects [probe: claude mcp list | grep holodeck]"
    )
