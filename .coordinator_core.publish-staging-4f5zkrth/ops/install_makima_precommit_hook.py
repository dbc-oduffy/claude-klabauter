"""
coordinator_core.ops.install_makima_precommit_hook — project-makima's own
pre-commit git-hook gate installer.

Purpose: installs (or appends onto an existing custom hook) project-makima's
`.git/hooks/pre-commit` gate chain. Idempotent, conditional: only installs
when the resolved target repo root IS project-makima itself. Consumer/sibling
...
this project-makima" from its OWN file location (`_self_repo_root()`), the
same self-relative principle `_bin_dir()` already applies to gate-script
lookup. This module ships INSIDE project-makima, so wherever it is actually
running from IS the makima repo root — there is no separate identity to look
...
_PROG = "install-makima-precommit-hook"
...
def _self_repo_root() -> str:
    ...
    "is the target project-makima". This module ships INSIDE project-makima,
    so wherever it is actually running from IS the makima repo — there is no
...
def _gate_block(gate: _Gate) -> List[str]:
    ...
    into `~/.claude` pointing at a gate in a DIFFERENT repo (makima's
    `coordinator/bin/`), where nothing relative could resolve. This installer
    writes a hook into makima pointing at a gate inside makima itself. The two
    ...
    lines += _cannot_proceed_branch(
        ...
        "re-run the coordinator installer (install-makima-precommit-hook) to rebake it",
    )
    lines += [
        "else",
        '  "$_py" "$_gate_script"',
...
_BODY_HEADER = (
    "#!/bin/sh\n"
    "# project-makima pre-commit gates — fire before drift can land.\n"
)
...
def _hook_body(gates: List[_Gate]) -> str:
    ...
    lines = [
        *_BODY_HEADER.rstrip("\n").split("\n"),
        "# Registry-driven (coordinator_core.ops.install_makima_precommit_hook);",
...
def _resolve_makima_target(target: str) -> Optional[str]:
    """Identity guard: resolve `target` to project-makima's own repo root, or
    None if `target` is not a git repo / not project-makima."""
    ...
    if _canon(toplevel) != _canon(_self_repo_root()):
        print(f"{_PROG}: not project-makima ({toplevel}) — skipping.", file=sys.stderr)
        return None
    return toplevel
...
def main(argv: List[str]) -> int:
    ...
    repo_root = _resolve_makima_target(target)
