...
declares ONE hard direct_dep of its own: project-makima (the engine).
"DAG root" here means "root of the plugin chain", not "zero dependencies" —
this walker probes that one dep (via the makima_seam_resolvable probe kind,
...
file_exists_any/python_import/command_succeeds/makima_seam_resolvable) are ported natively
...
C11 de-bash cutover). makima_seam_resolvable (added
...
registry-resolved dep (project-makima itself) cannot be sibling-directory
...
def _loaded_manifest_repo_id(manifest_path: Path | None) -> str:
    ...
    silently fallen back to `<makima-root>/coordinator`'s OWN manifest
    (`repo_id: project-makima`) still announced itself as walking
...
def _normalize_dep(dep: dict[str, Any]) -> dict[str, Any]:
    ...
    degraded to present-but-broken, INCLUDING the makima_seam_resolvable
...
def _sibling_search_root(repo_root: Path, manifest_path: Path) -> Path:
    ...
    answers a DIFFERENT question (WHICH repo this is: project-makima vs
...
def _resolve_dep_root(
    ...
    by construction (like ``makima_seam_resolvable``'s bypass), so the caller
...
def _engine_tree_root() -> Path:
    """The makima engine's own checkout root (this module's own tree),
...
def _sibling_fallback(
    ...
    - It must not BE the makima engine's own tree, nor a subdirectory of it,
      nor the walked repo's own ``repo_root`` (or a subdirectory of that) —
      the exact coincidental match (`<makima-root>/coordinator` resolving as
...
def command_succeeds_native(cmd: str, timeout: int) -> bool:
    ...
    is DoE-owned data, never makima-authored, so full shell-grammar parity
...
def dep_probe(
    ...
    # "makima_seam_resolvable": bypasses the sibling_dir_exists gate entirely.
    # Registry-resolved deps (project-makima, declared as coordinator-claude's
    ...
    # _resolve_makima_root ladder), not a path literally adjacent to
    # repo_root.parent. Chicken-egg note: this very module
    # (coordinator_core.ops.setup_chain_walker) is authored INSIDE
    # project-makima and only becomes importable AFTER setup.sh's trampoline
    # (_import_main -> _resolve_makima_root) has already resolved the engine root
    ...
    if probe_kind_early == "makima_seam_resolvable":
        try:
...
Informational (NOT --phase <name> values):
  dep-check:  coordinator-claude probes its one hard dep (project-makima); chain-walk terminates here"""
...
_COORDINATOR_ROOT_LADDER_REMEDIATION = (
    ...
    "  installer / self-heal, never by makima -- makima only reads this\n"
...
def _resolve_coordinator_root_ladder(
    ...
    makima engine (Defect B), and never a registered publish mirror
    ...
    `machine-local set engine.working_repos.doe_claude <path>` — makima only
    ...
    matched `<makima-root>/coordinator`, makima's OWN plugin-source mirror,
