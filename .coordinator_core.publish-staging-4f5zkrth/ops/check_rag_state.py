"""
coordinator_core.ops.check_rag_state — detect project-RAG freshness, print one token.
...
def check_rag_state() -> Tuple[str, int]:
    ...
    # 2. marker file written by the W1 hook (project-rag-detect.*) at session
...
def _doe_root_error(doe_root: str) -> Optional[str]:
    ...
    return (
        ...
        "python3 <project-makima>/scripts/setup.py"
    )
...
def _trust_error(plugin_root: str) -> str:
    return (
        ...
        "source; re-run python3 <project-makima>/scripts/setup.py (or set "
