...
makima state seam (C4/stop-the-rot) via `command -v coordinator_is_meta_repo`
guards — a check that is only ever true when the calling shell had already
sourced coordinator-is-meta-repo.sh/coordinator-makima-root.sh, which never
...
`_claude_home`/`_same_path`/`_makima_root` helpers are a deliberate duplicate
...
GENERATES = []  # writes <repo_root>/state/health-ledger.md for a caller-supplied REPO_ROOT (any target repo /repo-setup runs against), not scoped to makima's own tree
