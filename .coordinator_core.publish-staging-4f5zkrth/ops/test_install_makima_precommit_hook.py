"""Tests for coordinator_core.ops.install_makima_precommit_hook.
...
from coordinator_core.ops.install_makima_precommit_hook import (
    _GATE_REGISTRY,
    main,
)
import coordinator_core.ops.install_makima_precommit_hook as _mod
...
def test_non_makima_repo_root_is_skipped_cleanly(tmp_path, monkeypatch, capsys):
    ...
    assert "not project-makima" in err
...
def test_default_target_is_cwd(tmp_path, monkeypatch, capsys):
    ...
    rc = main([])
    assert rc == 0
    assert "not project-makima" in capsys.readouterr().err
...
def test_stale_baked_interpreter_blocks_loudly_with_exit_1(tmp_path, monkeypatch):
    ...
    # slash command.
    assert "install-makima-precommit-hook" in result.stderr
