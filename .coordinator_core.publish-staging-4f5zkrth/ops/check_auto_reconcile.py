...
test seam never needs a makima checkout to exercise the rendering contract; this
...
Spec backlink: DoE-claude:pln-doe-side-adoption-of-makima-au-284ced (C1) +
cross-repo/inbox/2026-07-13-project-makima-em-makima-auto-reconcile-wire-surfaces.md
...
Negative-spec:
    ...
    - Does NOT resolve the repo root from this module's own on-disk
      location -- that always resolved to makima's checkout regardless of
      the caller, mistargeting every invoker's reconcile (and, once
      dry_run is armed, write) at makima's own state/handoffs/ corpus. See
...
def _resolve_own_repo_root() -> Optional[Path]:
    ...
    always resolved to makima's OWN checkout (this module's on-disk
    ...
    project-makima's own state/handoffs/ corpus instead of its own. See
