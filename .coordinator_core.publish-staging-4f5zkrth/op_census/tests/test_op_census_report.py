...
def test_four_axis_report_raises_when_tax_uniformly_over_bar_across_every_op():
    """THE regression: `_four_axis_report`, not `timing.emit_dispositions`,
    is the real emitted-`cleared` boundary `op_census.report` clients read
    (`census()` calls this function directly, never `emit_dispositions`).
    Before the 2026-08-23 fix this shape -- every op OVER_BAR on
    invocation_tax -- silently produced `cleared: []` on every live
    request, because `FROZEN_INVOCATION_TAX_MS` was frozen at a
    bare-interpreter figure that always breaches the 50ms bar. It must now
    raise rather than emit a report that reads as "nothing qualifies"."""
    good = AxisResult(disposition=Disposition.UNDER_BAR, sample_count=1)
    over = AxisResult(disposition=Disposition.OVER_BAR, max_ms=343.8, sample_count=1)
    spawns = {"op.a": good, "op.b": good}
    handler_elapsed = {"op.a": good, "op.b": good}
    invocation_tax = {"op.a": over, "op.b": over}
    line_count = {"op.a": good, "op.b": good}

    with pytest.raises(ocr.UniformInvocationTaxError):
        ocr._four_axis_report(spawns, handler_elapsed, invocation_tax, line_count)


def test_frozen_invocation_tax_ms_is_under_its_own_bar():
    """Pins the 2026-08-23 correction: `FROZEN_INVOCATION_TAX_MS` is
    `census()`'s default `measured_tax_ms` on every real request, so if this
    constant is ever re-frozen back over the bar, every production
    `op_census.report` call silently returns to the exact defect this row
    fixed (module docstring's CORRECTED negative-spec block). This test is
    the tripwire a future re-freeze must reckon with, not
    `measure_invocation_tax_ms` itself, which this module never calls live.

    Plumbing-only, deliberately (review finding #2, slice C review of
    c7cb4a565): the reviewer noted this bare `<` assertion cannot distinguish
    a correctly-shaped re-freeze from a wrong-shape one that happens to land
    small. A shape-aware version was attempted (a live
    `measure_invocation_tax_ms` call from this test) and reverted: this
    file's own module docstring already warns that a literal
    `subprocess.run` call here would newly Rule-2/Rule-4-tier the WHOLE file
    onto the cadence suite, pulling every other fast, synthetic-fixture test
    in it off the per-commit path -- confirmed live, it tripped
    `coordinator_core/tests/test_no_new_spawning_tests.py`'s spawn ratchet
    and destabilized `test_census_cold_process_budget`'s in-file ordering.
    Per the brief that authorized this finding: a flaky/costly guard is
    worse than a filed gap. The gap is filed rather than shipped fragile."""
    from coordinator_core.op_census.timing import INVOCATION_TAX_BAR_MS

    assert ocr.FROZEN_INVOCATION_TAX_MS < INVOCATION_TAX_BAR_MS


def test_census_default_tax_does_not_uniformly_breach():
    """End-to-end: a real `census()` call (synthetic telemetry, real corpus
    walk, default `measured_tax_ms`) must not raise
    `UniformInvocationTaxError`. Before the 2026-08-23 fix, this exact call
    shape silently produced `cleared: []` on every invocation -- only an
    end-to-end call exercises the actual default-argument wiring the unit
    tests above bypass by passing fixtures directly."""
    report = ocr.census(telemetry_entries=[], persist_index=False)
    assert "cleared" in report["dispositions"]
...
def test_walk_corpus_counts_stat_errors_instead_of_silently_dropping_them(tmp_path: Path, monkeypatch):
    """staff-eng slice-A review Finding 4: the old `_corpus_paths` +
    `sum(p.stat().st_size for p in paths)` shape had no try/except at all and
    would raise loudly on a file vanishing mid-walk. `_walk_corpus` swallows
    the same race instead (a hard failure on a transient TOCTOU race would be
    worse on this box), but must not do so silently -- a caller passing
    `skip_counts` must see the drop, since it can only push module_count/
    bytes_scanned/line_count DOWN and could otherwise mask a real ratchet
    trip as an honestly smaller corpus."""
    root = tmp_path / "repo"
    root.mkdir()
    (root / "ok.py").write_text("x = 1\n", encoding="utf-8")
    (root / "flaky.py").write_text("y = 2\n", encoding="utf-8")

    real_scandir = ocr.os.scandir

    class _FlakyDirEntry:
        ...
        def stat(self, *args, **kwargs):
            if self._entry.name == "flaky.py":
                raise OSError("vanished mid-walk")
            return self._entry.stat(*args, **kwargs)

    def _flaky_scandir(path):
        return [_FlakyDirEntry(e) for e in real_scandir(path)]

    monkeypatch.setattr(ocr.os, "scandir", _flaky_scandir)

    skip_counts: dict = {}
    walked = sorted(ocr._walk_corpus(root, skip_counts=skip_counts), key=lambda pair: pair[0])

    assert [p.name for p, _size in walked] == ["ok.py"]
    assert skip_counts == {"stat_errors": 1}


def test_census_self_assessment_reports_entries_skipped_key():
    """`census()`'s `self_assessment.entries_skipped` must always be present
    with both counters, even on a clean run with nothing skipped -- a
    consumer checking for corpus shrinkage needs a stable key, not one that
    only appears when something goes wrong."""
    report = ocr.census(telemetry_entries=[], persist_index=False, strict=False)

    entries_skipped = report["self_assessment"]["entries_skipped"]
    assert entries_skipped == {"scandir_errors": 0, "stat_errors": 0}
