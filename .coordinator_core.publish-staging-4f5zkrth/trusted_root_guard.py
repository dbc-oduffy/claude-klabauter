...
anchors:
  ...
  3. the registry-resolved makima root (2026-07-22 — the settings-home
     machine-local registry key ``repos.project_makima``, the same anchor
     ...
     .makima-root`` pointer file retained as a fallback rung. Absence of the
     ``repos.project_makima`` key degrades cleanly to "this anchor
     contributes nothing" — it never raises and never widens trust on its
     own; makima's own bin scripts (and every consolidated caller in this
...
def _settings_home_dir_from_env(env: dict) -> str:
    ...
    precedence used by ``_doe_root``, ``_makima_root``, and
    ...
    be a 3x/4x-duplicated inline branch (``_doe_root``, ``_makima_root``, and
...
def _home_from_env(env: dict) -> str:
    ...
    skips *every* rung of ``_doe_root``/``_makima_root`` — including the canonical
...
def _registry_project_makima(settings_home_dir: str) -> Optional[str]:
    """Direct-tomllib read of the ``repos.project_makima`` registry key,
    ...
    for fname in ("registry.local.toml", "registry.toml"):
        flat = _flatten(_load_toml(reg_dir / fname))
        if "repos.project_makima" in flat:
            val = flat["repos.project_makima"]
            if isinstance(val, list):
                val = "\n".join(str(i) for i in val)
            s = str(val)
            if s:
...
def _makima_root(env: dict) -> str:
    """Read the registry-resolved makima root, registry-first with a durable
    pointer-file fallback — same shape as ``_doe_root`` above, minus the
    legacy ``${CLAUDE_HOME:-$HOME}/.claude/`` rung (makima has no such
    legacy sentinel; ``coordinator_core.engine_root.coordinator_engine_root()``
    is the in-process analog for callers that also want the ``MAKIMA_ROOT``
    ...
    key degrades to "" rather than raising or shelling out):
        1. registry ``repos.project_makima``               (canonical anchor)
        2. <settings-home>/machine-local/.makima-root       (durable file mirror)
    Returns "" if neither rung resolves — the caller (``is_trusted``) treats
    an empty makima root as "this anchor contributes nothing," never as an
    error.
    """
    content = ""
    ...
    if settings_home_dir:
        registry_value = _registry_project_makima(settings_home_dir)
        if registry_value:
            content = registry_value

    if not content and settings_home_dir:
        durable = os.path.join(settings_home_dir, "machine-local", ".makima-root")
        try:
...
def _doe_root_rungs(env: dict) -> list[tuple[str, str]]:
    ...
    (DoE-claude state/2026-07-28-striker-install-dogfood-friction-log.md F6).
    """
    home = _home_from_env(env)
    settings_home_dir = _settings_home_dir_from_env(env)
...
def _makima_root_rungs(env: dict) -> list[tuple[str, str]]:
    """Diagnostics-only sibling of ``_doe_root_rungs`` for ``_makima_root``
    (registry rung + durable-file rung only — see ``_makima_root`` docstring
    for why it has no legacy-file rung)."""
    settings_home_dir = _settings_home_dir_from_env(env)
    rungs: list[tuple[str, str]] = []

    if settings_home_dir:
        rungs.append(("registry repos.project_makima", _registry_project_makima(settings_home_dir) or "<absent>"))
    else:
        rungs.append(("registry repos.project_makima", "<skipped: settings-home dir resolved empty>"))
    ...
    if settings_home_dir:
        ...
        rungs.append(("<settings-home>/machine-local/.makima-root", "<skipped: settings-home dir resolved empty>"))
...
def _diagnose_untrusted(root: str, env: dict) -> str:
    ...
    lines.append(
        f"  makima_root resolved to: {makima_root!r}"
        + _flag(makima_root, "every rung below returned nothing")
    )
    for label, val in _makima_root_rungs(env):
        lines.append(f"      - {label}: {val!r}")

    if not home or not settings_home_dir or not doe_root or not makima_root:
        lines.append(
...
def is_trusted(root: str, *, env: dict | None = None) -> bool:
    ...
    makima_root = _norm(_makima_root(env))
    ...
    if os.name == "nt" and makima_root.endswith("/"):
        makima_root = makima_root[:-1]
    if makima_root and root_cmp.startswith(makima_root + "/"):
        trusted = True
    ...
    *, mode: str, root: str, site: str = "coordinator root", env: dict | None = None
    ...
    if mode == "fail-loud":
        ...
        print(
            ...
            "re-run python3 <project-makima>/scripts/setup.py -- an anchor below "
