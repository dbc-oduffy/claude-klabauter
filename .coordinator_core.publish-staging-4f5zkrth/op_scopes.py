...
_OP_KEY_SCOPE: Dict[str, str] = {
    ...
    # copy-error misreading this comment exists to head off (Review: zoli-cutover-review F17).
    ...
    # relative to a repo/worktree root. Spec: docs/plans/2026-07-10-percolation-engine-makima.md § Tasks C15.
    "percolate.run":                         "none",
    ...
    # Spec: docs/plans/2026-07-12-makima-cartography-substrate-strand-a.md § C2/C3/C4.
    ...
    # workflow-skeleton-stamper-makima-engine.md § C3.
    ...
    # supplied target_root, path-guarded. Spec: docs/plans/2026-07-11-makima-
    ...
    # makima-doe-full-test-runner.md § C5.
    ...
    # the fleet ops. Spec: docs/plans/2026-07-04-makima-commit-anchor-stamper.md § D2/AC2.
    ...
    # docs/plans/2026-07-13-makima-auto-reconcile-open-handoffs.md § C4
    ...
    # handoff_children.py. Spec: docs/plans/2026-07-05-makima-served-initiative-roadmap-read-model.md § C2/C5
    ...
    # Spec: docs/plans/2026-07-07-makima-fork-provenance-creation-path-tooling.md § C2
    ...
    # Spec: docs/plans/2026-07-07-makima-fork-provenance-creation-path-tooling.md § C4
    ...
    # Spec: state/handoffs/2026-07-21_184526_makima-check-addressee-verb.md
    ...
    # Spec: docs/plans/2026-07-23-makima-driven-ceremony-redesign.md § C16
    ...
    # Spec: docs/plans/2026-07-23-makima-driven-ceremony-redesign.md § C11
    ...
    # session.sweep_consumed_handoffs — same scope as session.boot_sweep, and for the
    # same reason: it now reuses coordinator_core.ops.session.boot_backstop's own
    # DR-084 substrate (C5, docs/plans/2026-08-22-the-boot-backstop-asks-git-
    # nothing.md), so it reads/writes state/handoffs/ and archive/ paths derived from
    # the main worktree root and git-commits each archival exactly as that op does.
    # Repo-touching ops must be listed explicitly — the unlisted default is "none",
    # which would be wrong here. Spec: docs/plans/2026-07-23-wsc-tail-slim-down.md § C21
    ...
    # the caller's own tree → scope "none" (makima's target-resolution convention: fleet-generic
    ...
    # (a /pickup may consume a handoff in a repo other than makima's own tree,
    ...
    # Spec: docs/plans/2026-07-07-makima-fork-provenance-creation-path-tooling.md § C3
    ...
    # targets makima's OWN checkout (Path(__file__) + git rev-parse --show-toplevel),
    # not the caller's repo_root — post-migration, handoffs live in makima regardless
    ...
    # read makima's own fixed vendored coordinator_core/frontmatter/schemas/ tree,
    ...
    # in makima's own clone instead of the caller's.
    ...
    # main_worktree_root(common_dir) or the count assertion silently reads makima's
    ...
    # docs/plans/*.md; an absent scope entry would silently scan makima's own
    ...
    # silently default to "none" and un-scope the op, scanning makima's own
    ...
    # entry here would silently scan makima's own tree instead of the
    # caller's.
    ...
    # registry pointer, same ladder as the advisory probe) and makima's own fixed
    ...
    # workdir archives against makima's own clone instead of the pipeline caller's.
    ...
    # worktrees like state/); omitting scope would restructure makima's own
    ...
    # classify makima's own handoffs instead of the caller's when invoked
    ...
    # settings-home/makima install state (settings-home bin/, the retired
    # ~/.claude/bin compat mirror, coordinator/bin/ via the makima-root
    ...
    # write_surface.emit_manifest — "none": the manifest describes MAKIMA'S OWN
