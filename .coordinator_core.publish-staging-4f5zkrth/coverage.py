...
#: (dlv-first-class-consumer-install-5.8-dogfood-2d336d, project-rag-ue-addon)
...
#: 2026-07-15-patrik-coverage-graph-walk-correctness.md) — conflating "absent
...
#: to a per-range `git rev-list` spawn for that range alone (Review: Patrik —
...
def _classify_out_of_window(
    ...
    in-memory membership — Review: Patrik, MAJOR-2/MINOR-3 fix's correctness
...
def _resolve_base_uncached(
    ...
    if _HEX_TOKEN.match(base):
        ...
        if len(matches) == 1:
            # Known divergence (Review: Patrik — MINOR-4, documented rather than
            ...
            # state/review-trail/findings/2026-07-15-patrik-coverage-graph-walk-correctness.md).
            return matches[0]
        if not matches:
...
def _resolve_endpoint(
    ...
    provably correct here (no foreign case possible mid-walk — Review: Patrik,
...
def _reach_chain(seed: Any, parent_map: Dict[str, List[str]], chain_set: Set[str]) -> Set[str]:
    ...
    the false-COVERED bug fixed 2026-07-15 (Review: Patrik — MAJOR-1/MAJOR-2).
    """
        ...
        for rec in records:
            ...
            if scope_kind is not None:
                ...
                if scope_kind not in _RECOGNIZED_SCOPE_KINDS:
                    ...
                    # cross-repo/inbox/2026-08-10-project-rag-ue-addon-em-
                    ...
                    # error (2026-08-15 project-rag-em memo). Accumulate
