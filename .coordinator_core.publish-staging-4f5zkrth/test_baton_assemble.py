...
_FAKE_OPERATOR_CONFIG = {
    ...
    "makima_bin": "/fake/settings-home/bin",
...
_REPO_MAKIMA_BIN = Path(__file__).resolve().parents[1] / "coordinator" / "bin"
...
def _stub_operator_config(monkeypatch):
    ...
    `_resolve_makima_bin` is pinned to the REAL in-repo `coordinator/bin`
...
class TestSuccessorDerivationSurvivesPredecessorArchival:
    ...
    def test_archived_collision_end_to_end_via_coordinator_doc_new_does_not_trip_guard(
        ...
        # Rung-1-pin MAKIMA_ROOT (cc_invoke._resolve_makima_root's cheapest,
        ...
        # repos.project_makima on its own -- it would otherwise fail loud
        # with "repos.project_makima is not set" regardless of the real
        # machine's actual registration.
        env["MAKIMA_ROOT"] = str(repo_root_here)
...
class TestCliSmoke:
    def test_brief_subcommand_smoke(self, tmp_path, capsys):
        ...
        # real git worktree root (this project-makima checkout itself is one)
...
# never re-derives its own settings_home/makima_root/doe_root.
# ---------------------------------------------------------------------------
...
class TestCompensateD1Scaffold:
    ...
    def test_an_unavailable_generator_declines_to_delete(self, tmp_path, monkeypatch):
        """AC-2: the other uncertainty axis -- `makima_bin` unresolvable (a
        ...
        monkeypatch.setattr(
            ba_apply, "_resolve_makima_bin", lambda: tmp_path / "does-not-exist"
        )
