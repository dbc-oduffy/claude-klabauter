...
def resolve_machine_local_bin(root: Path) -> Optional[str]:
    ...
    passes the example-doctrine-repo clone, which vendors nothing under `coordinator/bin`, so
    ...
    # `root` points at: when a caller passes the example-doctrine-repo clone (publish.py does),
    # neither <root>/bin nor <root>/coordinator/bin exists — example-doctrine-repo tracks
