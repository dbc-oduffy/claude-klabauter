...
def _maybe_emit_skew_advisory(ml_dir: Path, published: str) -> None:
    ...
    alongside the published-engine resolution just made. Silent, and never
    raises, on any other outcome (kill-switch set, already emitted, or
    ``repos.claude_klabauter`` unset/unresolved/nonexistent)."""
    global _skew_advisory_emitted
    if _skew_advisory_emitted:
        return
    # Review: code-reviewer — a bare truthy check on the raw string treats
    # CLAUDE_KLABAUTER_ROOT_SKEW_QUIET=0 or =false as suppressing too, since Python
    # treats those strings as truthy; an operator trying to re-enable the
    # advisory that way would silently stay suppressed. Only None, "", "0",
    # and "false" (case-insensitive) count as not-suppressing.
    quiet_raw = os.environ.get(CLAUDE_KLABAUTER_SKEW_ADVISORY_QUIET_VAR)
    if quiet_raw is not None and quiet_raw.strip().lower() not in ("", "0", "false"):
        return
    try:
    ...
    sys.stderr.write(
        "NOTE: resolved the published engine "
        f"'{published}' for this call, but repos.claude_klabauter also resolves "
        f"to a live working tree at '{live}' on this box — the CLI you "
        "invoked and the code you may be editing can differ. Set "
        f"{CLAUDE_KLABAUTER_SKEW_ADVISORY_QUIET_VAR}=1 to suppress this notice "
        "(any other non-empty value besides \"0\"/\"false\" also suppresses).\n"
    )
