...
coordinator_core/ops/register_coordinator_mirror.py per DR-047 (example-doctrine-repo owns
contract/generator, claude-klabauter owns engine). This file is now a thin example-doctrine-repo-side (contract)
trampoline over that claude-klabauter (engine) module — it resolves the example-doctrine-repo-local "coordinator
...
def _claude_home_argv(*args: str) -> list:
    ...
    if os.name == "nt":
        ...
        # the primary machine per example-doctrine-repo CLAUDE.md § Runtime conventions).
...
def _resolve_coordinator_live() -> str:
    """example-doctrine-repo-local "coordinator live path" resolution — unchanged from the bash oracle.
