...
"""read_doe_root_pointer.py — shared substrate for reading the example-doctrine-repo repo root pointer.

Shared substrate: read the example-doctrine-repo repo root from the durable pointer file under
...
Provides:
  coordinator_read_doe_root_pointer() -> str
    Returns the example-doctrine-repo repo root path (a single line, stripped) or "" if
...
CLI (for shell callers that cannot import Python in-process):
  read_doe_root_pointer.py --print
    Prints the resolved example-doctrine-repo root (or nothing) to stdout. Always exits 0.
"""
...
def coordinator_read_doe_root_pointer() -> str:
    """Read the example-doctrine-repo repo root from the durable pointer, legacy fallback.
