...
"""check-global-doctrine-mirror.py -- drift probe for the example-doctrine-repo in-repo
...
which is exactly what `global-doctrine/` at the example-doctrine-repo repo root is.
...
and a whole enforcement envelope now keys on it -- the example-doctrine-repo PostToolUse
...
Placement (load-bearing): `global-doctrine/` lives at the example-doctrine-repo REPO
...
resolved example-doctrine-repo repo root, this probe exits 0 with no output -- never a
...
install and on any example-doctrine-repo clone that predates this feature, not a health
...
Exit codes:
    ...
    1 -- DRIFT: at least one mirrored file differs from its ~/.claude
         ...
         OR the example-doctrine-repo repo root is unresolvable (see
...
Environment:
    ...
    DOE_ROOT / REPO_EXAMPLE_DOCTRINE_REPO -- overrides the example-doctrine-repo repo root that
...
Negative-spec (hard-won):
    ...
    - Does NOT derive the example-doctrine-repo repo root from this script's own
      __file__ location -- this script migrated to claude-klabauter while
      `global-doctrine/` stayed in example-doctrine-repo; self-location resolution
...
def _repo_root() -> str:
    """Resolve the example-doctrine-repo REPO ROOT that owns `global-doctrine/`.
    ...
    example-doctrine-repo) while `global-doctrine/` stayed put in example-doctrine-repo at the
    ...
    "where is the example-doctrine-repo repo," independent of where THIS script
    ...
    never-block hook, so an unresolvable example-doctrine-repo root must not silently
    ...
    try:
        ...
        sys.stderr.write(
            f"{PROG}: cannot resolve the example-doctrine-repo repo root ({exc}). Set "
...
def _sync(mirror_dir: str, claude_home: str) -> int:
    ...
    This is the same derivation the example-doctrine-repo PostToolUse hook
...
def main(argv: list[str]) -> int:
    ...
    if not os.path.isdir(mirror_dir):
        ...
        # the expected state on every OSS install and on any example-doctrine-repo clone
