...
docs/plans/2026-08-29-static-signal-is-a-candidate-generator.md). Example-retrieval-repo's
...
distinguishes:
  (a) `STATIC_NOT_MEASURABLE_DAEMON_UNREACHABLE` -- the daemon did not answer at all: connection
      refused, or the call exceeded example-retrieval-repo's documented 30s-per-call contract
      (`EXAMPLE_RETRIEVAL_REPO_CALL_TIMEOUT_S`). Raised by the transport as `ProjectRagTransportError` and
...
EXAMPLE_RETRIEVAL_REPO_MCP_URL = "http://127.0.0.1:8767/mcp"

#: example-retrieval-repo's own documented per-call contract (module docstring, cause (a)).
EXAMPLE_RETRIEVAL_REPO_CALL_TIMEOUT_S = 30
...
class ProjectRagTransportError(Exception):
    ...
    connection refused, or the call exceeded `EXAMPLE_RETRIEVAL_REPO_CALL_TIMEOUT_S`. Distinct from a
...
class _HttpStreamableTransport:
    """JSON-RPC-over-streamable-HTTP transport against `EXAMPLE_RETRIEVAL_REPO_MCP_URL`: `initialize` ->
    `notifications/initialized` -> `tools/call`, from a plain subprocess/process -- no MCP client
    harness. Mirrors example-retrieval-repo's own
    ...
    def __init__(self, url: str = EXAMPLE_RETRIEVAL_REPO_MCP_URL, timeout: float = EXAMPLE_RETRIEVAL_REPO_CALL_TIMEOUT_S) -> None:
        self._url = url
...
def fetch_duplicate_block_candidates(
    ...
    *,
    ...
    """Call example-retrieval-repo's `project_duplicate_blocks` over `paths` and classify the result per the
    ...
    try:
        ...
        return DuplicateBlocksCallResult(
            ...
            hint=f"example-retrieval-repo daemon unreachable: {exc}",
            response=None,
        )
...
def _hash_span_text(text: str) -> Optional[str]:
    ...
    algorithm (example-retrieval-repo, `xxhash.xxh3_64_hexdigest(text.encode("utf-8"))`) -- the same digest
...
#: the static basis does not have that luxury. One side is example-retrieval-repo's index, built at whatever
...
_STATIC_ATTRIBUTION_BASIS = (
    "per-path re-verified duplicate-block membership: example-retrieval-repo's index (built at its own last "
