...
describes a example-doctrine-repo-owned central-mode state drain: read the outbox's own
...
Two subcommands remain, one per mechanical step:
  ...
  assert-empty    Detector, not drainer — verifies the one-root invariant a drain silently
                          ...
                          two repos for six weeks — example-doctrine-repo's drain reads a single repo root, so
...
def assert_empty(this_repo_root: Path) -> dict:
    ...
    was silently split across two repos for six weeks because example-doctrine-repo's drain reads a single
