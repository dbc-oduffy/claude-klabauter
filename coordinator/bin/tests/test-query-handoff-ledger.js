 ...
 *   5. Records from archive/handoffs/ are included.
...
function writeHandoffFixture(slug, ledgers, options = {}) {
  let dir;
  if (options.archived) {
    dir = path.join(tmpRoot, 'archive', 'handoffs');
  } else {
    dir = path.join(tmpRoot, 'state', 'handoffs');
  }
...
before(() => {
  ...
  // Handoff A: 2 Session Ledger blocks — multi-ledger (the Staff Engineer F5)
...
describe('handoff-ledger — multi-ledger parse (the Staff Engineer F5)', () => {
...
describe('handoff-ledger — archived handoffs included', () => {
  ...
  it('archived record path contains archive/handoffs/', () => {
    const raw = runQuery(['--format', 'json']);
    const records = JSON.parse(raw);
    const archived = records.filter(r => r.path.includes('archive/handoffs/'));
