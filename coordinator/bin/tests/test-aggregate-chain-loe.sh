#!/usr/bin/env bash
# test-aggregate-chain-loe.sh — Smoke tests for bin/aggregate-chain-loe.py.
#
...
AGG="${SCRIPT_DIR}/../aggregate-chain-loe.py"
...
mkdir -p "${FAKE_REPO}/state/handoffs/archive/2026-05"
git -C "$FAKE_REPO" init -q 2>/dev/null || true

HANDOFFS="${FAKE_REPO}/state/handoffs"
ARCHIVE="${HANDOFFS}/archive/2026-05"

# On Git Bash (Windows), Node.js uses Windows-native paths (C:\...) rather than
# POSIX paths (/c/..., /tmp/...). cygpath -w converts a bash path to the form that
# Node.js's fs.existsSync() and path.resolve() can find. No-op on macOS/Linux.
_path_for_node() {
...
write_handoff() {
  ...
  # Convert the resolved path to Node-compatible form (Windows path on Git Bash).
  ...
  if [[ "$predecessor" != "null" && "$predecessor" != "none" && -n "$predecessor" ]]; then
    ...
    if [[ -f "${HANDOFFS}/${pred_basename}" ]]; then
      resolved_pred="$(_path_for_node "${HANDOFFS}/${pred_basename}")"
    elif [[ -f "${ARCHIVE}/${pred_basename}" ]]; then
      resolved_pred="$(_path_for_node "${ARCHIVE}/${pred_basename}")"
    elif [[ -f "$predecessor" ]]; then
      resolved_pred="$(_path_for_node "$predecessor")"  # already absolute
    fi
...
test_single_session_chain() {
  ...
  out=$(python "$AGG" --terminal-handoff "$h1" --format json 2>/dev/null)
...
test_three_session_chain() {
  ...
  out=$(python "$AGG" --terminal-handoff "$h3" --format json 2>/dev/null)
...
EOF
  ...
  out=$(python "$AGG" --terminal-handoff "$h2" --format json 2>/dev/null)
...
test_missing_predecessor_annotation() {
  ...
  write_handoff "missing-pred-terminal.md" "state/handoffs/nonexistent-handoff-ZZZZZ.md" "2026-05-11" \
...
test_cycle_detection() {
  ...
  # Convert paths to Node-compatible form (Windows path on Git Bash; POSIX elsewhere).
  local h_a_node h_b_node
  h_a_node="$(_path_for_node "${h_a}")"
  h_b_node="$(_path_for_node "${h_b}")"
...
EOFRONT
  ...
  out=$(python "$AGG" --terminal-handoff "$h_b" --format json 2>/dev/null)

  [[ "$out" == *'"chain_walk_terminated_early": "lineage-cycle"'* ]] || {
    echo "    should have chain_walk_terminated_early: lineage-cycle (primitive value): $out"; return 1
  }
...
test_archived_predecessor() {
  # h1 lives in archive; h2 (terminal) in state/handoffs/ and references h1 by absolute path.
...
test_yaml_frontmatter_output() {
  ...
  out=$(python "$AGG" --terminal-handoff "$h1" --format yaml-frontmatter 2>/dev/null)
...
test_concurrency_declarations_present() {
  local script="${SCRIPT_DIR}/../aggregate-chain-loe.py"
...
echo "=== aggregate-chain-loe.py smoke tests ==="
echo ""
