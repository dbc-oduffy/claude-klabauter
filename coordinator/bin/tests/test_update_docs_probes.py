...
Spec backlink: coordinator/commands/update-docs.md (example-doctrine-repo) — Pre-flight probe,
...
def test_retired_snippet_sync_sweep_verb_is_still_accepted(tmp_path):
    ...
    The caller lives in another repo (example-doctrine-repo `coordinator/commands/
    update-docs.md` Phase 11b), so dropping the verb outright would break
    `/update-docs` for every user until example-doctrine-repo lands a matching edit. This test
    ...
    assert proc.returncode == 0, (
        "the retired verb must no-op, not exit 2 — a example-doctrine-repo-side Phase 11b run "
        f"would break.\nstdout: {proc.stdout}\nstderr: {proc.stderr}"
    )
