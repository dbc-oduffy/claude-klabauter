...
found anywhere in the example-doctrine-repo doctrine corpus (skills/commands/agents/
...
arbitrary directories — PM-sharpened constraint, 2026-07-23:
  1. an OSS `coordinator-claude` consumer — NO example-doctrine-repo clone, NO claude-klabauter
     source checkout.
  2. a example-doctrine-repo developer — this repo is the live plugin source.
...
not exist for persona 1** (no example-doctrine-repo clone at all), so this whole test
...
`/Users/...`, no assumption example-doctrine-repo and claude-klabauter are siblings, no
...
Invocation-form coverage (empirically discovered against the live example-doctrine-repo
...
Spec backlink: M1 (claude-klabauter commit e90904a9, `_derive_agent_helper_target_map` +
  ...
  (`example-doctrine-repo docs/plans/2026-07-23-skills-carry-no-code-extirpation.md`).
"""
...
# example-doctrine-repo sibling clone and no machine-local registry entry) hits exactly
...
# ---------------------------------------------------------------------------
# example-doctrine-repo root resolution — skip (not fail) when unresolvable on this machine.
...
# raise) — persona 1 (no example-doctrine-repo clone at all) and persona 3 running in CI
...
def _resolve_doe_root() -> Path:
    ...
    if not (p / "coordinator").is_dir():
        pytest.skip(f"resolved example-doctrine-repo root {p} has no coordinator/ tree; skipping corpus scan")
    return p
...
# Settings-home bin-dir alias variable names in live use in the example-doctrine-repo corpus,
...
def test_every_referenced_cli_is_in_generator_installed_forwarder_set(_doe_root, _installed_dirs) -> None:
    """AC8: every `<settings-home>/bin/<cli>` reference in the example-doctrine-repo corpus
    ...
    assert not missing, (
        "example-doctrine-repo corpus references CLI name(s) a fresh installer run would NOT "
...
def _install_shims_to_scratch(doe_root: Path, tmp_path: Path) -> Path:
    ...
    if not ml_bin.is_dir():
        pytest.skip(f"example-doctrine-repo templates/bin not found at {ml_bin}; skipping corpus scan")
