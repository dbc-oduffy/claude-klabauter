...
7. Invoked as an ENTRYPOINT (the file executed directly, relying on its own
   shebang + exec bit -- not `python3 <path>`, which bypasses shebang
   resolution entirely and is exactly the invocation style that let a
   missing shebang / missing exec bit slip past this suite once already),
   the child's stdout and stderr both reach the parent's streams.
...
def test_entrypoint_executed_directly_streams_output(tmp_path):
    """Run the entrypoint the way a real caller does -- as an executable,
    relying on its own shebang + exec bit, NOT via `sys.executable
    <path>` (which every other test in this file uses, and which bypasses
    shebang resolution entirely -- the exact gap that let a missing
    shebang / missing exec bit reach a live repro undetected).
    ...
    # Windows cannot CreateProcess a shebang script directly (WinError 193,
    # "not a valid Win32 application") -- native Windows resolves a bare
    # entrypoint via its co-located `.cmd` shim (generated by
    # gen-launcher-shim.py), the same way a caller typing the bare name
    # in cmd.exe/PowerShell would. POSIX honours the shebang line directly.
    if sys.platform == "win32":
        ...
        argv = [str(_WRAPPER), "--", sys.executable, "-c", script]
