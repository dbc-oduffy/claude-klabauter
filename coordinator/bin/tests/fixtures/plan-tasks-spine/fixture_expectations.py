...
Two independent implementations of one locate rule exist:
  - DoE's `_locate_tasks_block` (coordinator/bin/coordinator-harvest-deferrals:317-372),
...
not this module's: DoE's locator collapses `ABSENT` and `MALFORMED` into a
single `None` return, so a caller comparing against DoE's `_locate_tasks_block`
