# Frozen golden fixtures — DEC-3(c) fallback (example-doctrine-repo JS oracle capture)
...
Example-doctrine-repo's `.js` files as *live differential oracles* — their Python ports are proven
...
proof (CI stays green-with-skips). DEC-3 gates example-doctrine-repo-side JS deletion on claude-klabauter confirming
that conversion; DEC-3(c) is the fallback that unblocks the wait — example-doctrine-repo captures the JS
oracles' output itself (snapshotting example-doctrine-repo's own JavaScript needs neither claude-klabauter's context
...
Every file below was produced by an **actual run of the real example-doctrine-repo `.js` oracle** against a
fixture tree reconstructed from the reference suite's own fixture-generation code (or, for
`test_verify_schema_registry_sync`, against example-doctrine-repo's real live `coordinator/schemas/` tree).
...
| `test_verify_schema_registry_sync.golden.json` | `coordinator/bin/query-records.js` | `node query-records.js --type <t> --format json --limit 1` (once per example-doctrine-repo schema carrying `applies_to:`) | `coordinator_core/ops/test_verify_schema_registry_sync.py::test_golden_oracle_parity_against_live_doe_repo` |
...
- **`test_verify_schema_registry_sync`**: re-derives the schema-name → `--type` mapping the
  ...
  `node coordinator/bin/query-records.js --type <t> --format json --limit 1` once per example-doctrine-repo
...
against example-doctrine-repo HEAD at commit range starting `b22f543b`.
