...
def test_no_retired_transport():
    ...
    token matrix vacated. lesson-promote's own `from cc_invoke import route as
    _cc_route` (see test_cc_route_import_present above) is the legitimate routing
    seam — it stays untouched; nothing here forbids it. This pin does not need
    widening for the pipe transport to stay caught if it were ever wrongly wired
    in here directly — the bare-string check already catches any of the four
    retired patterns appearing anywhere in this CLI's source, by name, regardless
    of shape. Spec backlink: docs/plans/2026-08-16-one-engine-for-the-whole-box.md § C19.
    """
    source = _source()
