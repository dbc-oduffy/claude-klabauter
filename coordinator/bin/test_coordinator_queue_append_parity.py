...
Parity cases by shape:
  ...
  coordinator-queue-append --schema improvement-queue: 5 cases
    ...
    by the existing test_coordinator_queue_append.py (Test 10 / Test 2 paths).
...
Run with: python3 -m pytest bin/test_coordinator_queue_append_parity.py
"""
...
def test_debt_backlog_missing_required_field() -> None:
    ...
    if _assert_reject(name, result):
        ...
        if "risk" not in combined.lower():
            raise AssertionError(f"{name}: " + (f"error output does not mention 'risk'. stderr: {result.stderr!r}"))
        else:
            pass
...
def test_debt_backlog_invalid_status_enum() -> None:
    ...
    if _assert_reject(name, result):
        ...
        if "open" not in combined:
            raise AssertionError(f"{name}: " + (f"error output does not name valid status values. stderr: {result.stderr!r}"))
        else:
            pass
...
def test_debt_backlog_invalid_severity_enum() -> None:
    ...
    if _assert_reject(name, result):
        ...
        if "P0" not in combined and "P1" not in combined:
            raise AssertionError(f"{name}: " + (f"error output does not name valid severity values. stderr: {result.stderr!r}"))
        else:
            pass
...
def test_bug_backlog_missing_required_field() -> None:
    ...
    if _assert_reject(name, result):
        ...
        if "severity" not in combined.lower():
            raise AssertionError(f"{name}: " + (f"error output does not mention 'severity'. stderr: {result.stderr!r}"))
        else:
            pass
...
def test_improvement_queue_missing_required_field() -> None:
    ...
    if _assert_reject(name, result):
        ...
        if "surface" not in combined.lower():
            raise AssertionError(f"{name}: " + (f"error output does not mention 'surface'. stderr: {result.stderr!r}"))
        else:
            pass
...
def test_improvement_queue_queue_scope_central_accepted() -> None:
    ...
    Tests for invalid --queue-scope are in test_coordinator_queue_append.py (Test 10);
...
def test_lesson_promote_valid() -> None:
    ...
    output to a temp dir, matching the pattern from test_coordinator_lesson_promote.py.
    """
...
def test_lesson_promote_missing_title() -> None:
    ...
    if _assert_reject(name, result):
        ...
        if "title" not in combined.lower():
            raise AssertionError(f"{name}: " + (f"error output does not mention 'title'. stderr: {result.stderr!r}"))
        else:
            pass
...
def test_lesson_promote_missing_change_kind() -> None:
    ...
    if _assert_reject(name, result):
        ...
        if "change-kind" not in combined.lower() and "change_kind" not in combined.lower():
            raise AssertionError(f"{name}: " + (f"error output does not mention 'change-kind'. stderr: {result.stderr!r}"))
        else:
            pass
