...
Two subcommands, each mirroring one bash fence from the example-doctrine-repo ceremony file
verbatim in behavior (both non-blocking — neither ever fails the ceremony):
    ...
    completion-reconcile
        ...
        `cs_live_session_ids` / the js_bridge_cli subprocess bridge the example-doctrine-repo
...
Spec backlink: example-doctrine-repo coordinator/commands/workday-complete.md § Step 1.5
    (Cruft Sweep Apply), § Step 2.6 (Completion-Entry Reconcile Sweep)
Port source: example-doctrine-repo coordinator/commands/workday-complete.md Step 1.5 +
