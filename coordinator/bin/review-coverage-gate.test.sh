#!/usr/bin/env bash
# review-coverage-gate.test.sh — TF1-TF16 for bin/review-coverage-gate.py.
#
...
SUBJECT="${SCRIPT_DIR}/review-coverage-gate.py"
...
_run_gate() {
  ...
  STDOUT_OUT=""
  STDERR_OUT=""
  STDOUT_OUT=$(cd "$repo" && python3 "$SUBJECT" "$@" 2>"$STDERR_TMP") || RC=$?
...
_add_commit "$TF6/repo" "src/other.py" > /dev/null  # NOT in scope
...
_add_commit "$TF10/repo" "src/main-thing.py" > /dev/null
...
_add_commit "$TF12/repo" "src/alpha.py" > /dev/null
