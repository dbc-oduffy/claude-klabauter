...
Example-retrieval-repo corpus artifact, or any oversized real blob.
...
Ported from example-retrieval-repo-ue-addon's ``bin/pre-commit-oversized-blob-guard.sh`` per the
...
Negative-spec:
  ...
  - Does NOT consult the network, example-retrieval-repo, or any registry. It must run offline in a
...
def main(argv: list[str] | None = None) -> int:
    ...
    if corpus_hits:
        print(
            "A example-retrieval-repo corpus artifact is staged. Fleet ruling: a corpus artifact is\n"
