@echo off
setlocal
REM Windows launcher for verify-ue-overrides.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "verify-ue-overrides.ps1" sibling of
REM     this file over this "verify-ue-overrides.cmd" file itself -- if both exist, `verify-ue-overrides`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "verify-ue-overrides.py"
...
echo [verify-ue-overrides] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [verify-ue-overrides] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0verify-ue-overrides.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0verify-ue-overrides.py" %*
exit /b %ERRORLEVEL%
