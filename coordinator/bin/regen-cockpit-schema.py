...
def main(argv: list[str] | None = None) -> int:
    ...
    if not mak_root or not os.path.isdir(mak_root):
        # foreign-identity: NOT-REACHABLE — operator-invoked schema-regen maintenance CLI
        # (Usage: `python coordinator/bin/regen-cockpit-schema.py`, deliberately typed);
        # a third-repo session never runs this ambiently, matching the audit's row 9/16
        # precedent for deliberately-invoked maintenance ops.
