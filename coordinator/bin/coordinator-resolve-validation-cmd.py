...
Output contract (both CLI and library ResolveResult):
  ...
  exit 126 — the configured value carries an un-interpretable backslash-escaped
             ...
             failure that is really a config defect (example-retrieval-repo, 2026-07-22 —
...
Consumers:
  - example-doctrine-repo coordinator/skills/validate/SKILL.md (single owner of FAST-tier
    resolver invocation, via the settings-home forwarder — this bin/ CLI)
  - example-doctrine-repo coordinator/commands/bug-blitz.md (single owner of FULL-tier
...
def _venv_interp(repo_root: str | None) -> str | None:
    ...
    environment mismatch (example-retrieval-repo saw 314 of them, 2026-07-22). A `.venv`
...
def _check_escape_residue(val: str, step: str, caller: str) -> None:
    ...
    then reports "build failure" cost example-retrieval-repo a day of unnoticed red
