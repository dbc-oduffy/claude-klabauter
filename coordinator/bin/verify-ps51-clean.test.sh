#!/usr/bin/env bash
# verify-ps51-clean.test.sh — Tests for bin/verify-ps51-clean.py
#
...
SUBJECT="${SCRIPT_DIR}/verify-ps51-clean.py"
...
OUT=$(python "$SUBJECT" "$CLEAN_PS1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  # Non-Windows: expect SKIP line and exit 0
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC2: no PS 5.1 host → SKIP line + exit 0 (OK-verdict assertion not exercised)"
  else
...
OUT=$(python "$SUBJECT" "$BAD_PS1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  # Non-Windows: SKIP behaviour
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC3: no PS 5.1 host → parse-error/FAIL-verdict assertion not exercised (subject SKIPs gracefully)"
  else
...
OUT=$(python "$SUBJECT" "$WARN_PS1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC4: no PS 5.1 host → WARN-from-bash-scan assertion not exercised (subject SKIPs gracefully)"
  else
...
OUT=$(python "$SUBJECT" "$DIR5" 2>"$ERR_F") || RC=$?

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC5: no PS 5.1 host → directory-arg parse-clean assertion not exercised (SKIP on directory arg)"
  else
...
OUT=$(python "$SUBJECT" "$CLEAN_PS1" "/nonexistent/path/that/does/not/exist.ps1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC6: no PS 5.1 host → missing-path WARN assertion not exercised (subject SKIPs gracefully)"
  else
...
OUT=$(python "$SUBJECT" "$DIR7" 2>"$ERR_F") || RC=$?

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC7: no PS 5.1 host → non-.ps1-ignored assertion not exercised (SKIP)"
  else
...
OUT=$(python "$SUBJECT" "$PS7_FLOORED_PS1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC8: no PS 5.1 host → EXPECTED-PS7 classification assertion not exercised (subject SKIPs gracefully)"
  else
...
OUT=$(python "$SUBJECT" "$DIR9" 2>"$ERR_F") || RC=$?
...
OUT=$(python "$SUBJECT" "$PS7_NO_MINOR_PS1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC10: PS51 absent → #requires -Version 7 no-minor EXPECTED-PS7 assertion not exercised (SKIP)"
  else
...
OUT=$(python "$SUBJECT" "$PS6_FLOORED_PS1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC11: PS51 absent → #requires -Version 6.0 FAIL-verdict assertion not exercised (SKIP)"
  else
...
OUT=$(python "$SUBJECT" "$BOM_FLOORED_PS1" 2>"$ERR_F") || RC=$?
ERR=$(cat "$ERR_F")

if [[ $HAS_PS51 -eq 0 ]]; then
  if [[ "$RC" -eq 0 ]] && echo "$OUT" | grep -q "^SKIP:"; then
    _skip "TC12: PS51 absent → BOM-floored EXPECTED-PS7 assertion not exercised (subject SKIPs gracefully)"
  else
