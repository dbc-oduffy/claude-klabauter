...
the example-doctrine-repo skill invokes it by name instead of carrying the heredoc + resolver
...
Subcommands:
    ...
    supersede <handoff-path> --continued-into <successor> [--exclude <path>]...
        ...
        example-doctrine-repo-side call site can be updated to pass the successor it already
...
Spec backlink: example-doctrine-repo coordinator/skills/handoff/SKILL.md
...
def cmd_supersede(handoff_path: str, continued_into: str | None, exclude: list[str]) -> int:
    ...
    # the silent-no-op bug (cross-repo example-doctrine-repo incident, 2026-07-27): the caller
