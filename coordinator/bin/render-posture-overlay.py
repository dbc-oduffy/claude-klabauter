...
DR-047 (example-doctrine-repo owns contract/generator, claude-klabauter owns engine). Performs an
...
design. Do not reintroduce a size gate here. The example-doctrine-repo-resident coordinator_root (anchor templates) is
resolved here and passed into the op explicitly, since the op
cannot re-derive a example-doctrine-repo-only path itself. Resolution honors a
...
to example-doctrine-repo in the 2026-07-22 executable-surface migration.
"""
...
# test_render_posture_overlay.py). This file is now a thin example-doctrine-repo-side
...
#
# coordinator_root is resolved HERE (example-doctrine-repo-side) and passed into the op's
# main() explicitly — the anchor templates (templates/postures/<anchor>.md)
# are example-doctrine-repo-resident, not claude-klabauter-resident, so the op cannot re-derive this root
# itself the way a claude-klabauter-native op would. The op no longer reads a
# size-guard SSOT at all; templates/ is now the only example-doctrine-repo-resident input.
#
...
# anchor for templates/ or hooks/ (those stayed in example-doctrine-repo, per DR-047).
# coordinator_root is now resolved via the shared
# coordinator_data_root.data_root() split-repo ladder (co-located rung 1 ->
# example-doctrine-repo-resident rung 2 via coordinator_registry.doe_root()), with a
...
def _import_main():
    ...
    example-doctrine-repo-resident, not derivable by the op) — `run_op_main`'s contract is
...
def _coordinator_root() -> str:
    """example-doctrine-repo-side coordinator/ root (parent of templates/ and hooks/).
    ...
    shared split-repo ladder (co-located rung 1 -> example-doctrine-repo-resident rung 2 via
