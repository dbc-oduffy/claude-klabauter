@echo off
setlocal
REM Windows launcher for coordinator-precommit-foreign-platform-check — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "coordinator-precommit-foreign-platform-check.ps1" sibling of
REM     this file over this "coordinator-precommit-foreign-platform-check.cmd" file itself -- if both exist, `coordinator-precommit-foreign-platform-check`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "coordinator-precommit-foreign-platform-check"
...
echo [coordinator-precommit-foreign-platform-check] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [coordinator-precommit-foreign-platform-check] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127

:run_baked
"%_py%" "%~dp0coordinator-precommit-foreign-platform-check" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0coordinator-precommit-foreign-platform-check" %*
exit /b %ERRORLEVEL%
