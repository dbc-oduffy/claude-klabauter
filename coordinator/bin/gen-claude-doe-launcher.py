...
def _default_template_dir() -> str:
    ...
    Resolved via `coordinator_data_root.data_root()`'s co-located/example-doctrine-repo-resident
    ...
    while `templates/` stayed in example-doctrine-repo (DR-047 contract/engine split), so
