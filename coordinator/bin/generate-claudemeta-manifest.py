#!/usr/bin/env python3
"""Generate the `--file-list` manifest for example-retrieval-repo's `claudemeta` band.
...
Contract (example-retrieval-repo-em, memo 2026-08-15-example-retrieval-repo-em-file-list-landed-manifest-contract):
...
consumer's skip contract holds — verified against example-retrieval-repo's `--file-list` parser,
...
def _no_console_creationflags() -> dict:
    ...
    module to keep this script runnable standalone (AC5: a plain command example-retrieval-repo can invoke
