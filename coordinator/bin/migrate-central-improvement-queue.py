...
def _normalise_from_repo(raw: str) -> str:
    ...
    Priority:
      ...
      3. Raw string as-is (preserves example-retrieval-repo, example-game-workbench-repo, etc.).
    """
    stripped = raw.strip()
    ...
    if stripped.startswith("~/.claude"):
        return "claude-central-em"
    # Return raw — peer repos (example-retrieval-repo, example-game-workbench-repo, etc.) keep their name.
    return stripped
