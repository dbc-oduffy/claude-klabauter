...
"""fan-out-integrator.py — example-doctrine-repo-side contract trampoline over the claude-klabauter
...
dispatch prompts, one per slice. Example-doctrine-repo owns the contract; claude-klabauter
...
# review-integrator prompts. Now a thin example-doctrine-repo-side (contract) trampoline over
# claude-klabauter coordinator_core.ops.fan_out_integrator (DR-047: example-doctrine-repo owns contract,
...
#   2 — invocation error (usage, environment, missing spec file, example-doctrine-repo-side snippet resolution)
...
# resolve via coordinator_data_root.data_root("snippets") (co-located rung 1, example-doctrine-repo-resident
# rung 2 — see that module's docstring), NOT a bare __file__-relative walk: this script now
# lives in claude-klabauter while snippets/ stayed in example-doctrine-repo (DR-047 split), so a naive
...
def _import_run_op_main():
    ...
    `data_root("snippets").parent` (co-located or example-doctrine-repo-resident per the split-repo
