...
for the example-doctrine-repo `/workweek-complete` ceremony (M3 chunk WWC-1 extirpation).

Ports FOUR genuine bash logic blocks out of example-doctrine-repo's
...
block are NOT ported (that plumbing is the D1/D2 wave's concern; the example-doctrine-repo
...
Negative-spec:
    - Does NOT resolve CLAUDE_KLABAUTER_ROOT via the machine-local registry / .doe-root
      ladder, and does NOT probe `repos.claude_klabauter` — the example-doctrine-repo ceremony
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Read-only advisory subcommands for the example-doctrine-repo /workweek-complete ceremony.",
    )
