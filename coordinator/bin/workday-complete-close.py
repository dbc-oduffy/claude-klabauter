...
dispatch ladders -- that was still living as inline bash in example-doctrine-repo's
commands/workday-complete.md. This CLI concentrates that residual logic into one
naked-Python, idempotent, self-resolving entrypoint so the example-doctrine-repo ceremony body can
...
Subcommands:
  stitch-sidecar [--today YYYY-MM-DD]
      ...
      `_stitch_rc != 0`, example-doctrine-repo workday-complete.md Step 4d).

  step9-dispatch [SCOPE_SUMMARY] [--only-mode] [--no-push] [--dry-run]
                 [--commit-span RANGE] [--for-date DATE]
      Step 9's dispatch gate. Under `--only-mode` (example-doctrine-repo `$_ONLY_MODE=1`), the
...
Spec backlink: commands/workday-complete.md (example-doctrine-repo) § Step 4d / Step 9 /
    Step 10.5 / Step 10.6 -- the bash this file replaces; the example-doctrine-repo repoint (D2) is a
    later wave and is out of scope for this port.
Prior bash form: see example-doctrine-repo git history for commands/workday-complete.md's
    inline Step 4d / Step 9 dispatch / Step 10.5 / Step 10.6 blocks (this port lifts
    that logic out; the example-doctrine-repo file itself is not edited here).
"""
