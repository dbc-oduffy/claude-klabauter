...
counting via `grep -rl | wc -l`) that example-doctrine-repo's `pipelines/deep-research/
...
engine, not in instruction-surface fences — see example-doctrine-repo's
...
history for (it backs example-doctrine-repo's project-orientation hook and update-docs
