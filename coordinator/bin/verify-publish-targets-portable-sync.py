...
Example-doctrine-repo-tracked `setup/publish-targets.portable` and the PERCOLATE_ROOT-resolved
live-install copy.
...
slice narrowed the example-doctrine-repo-tracked copy back to a curated 7-wiki seed and added
`coordinator/tests/test_publish_seed_wiki_allowlist.py` as a regression guard
— but that guard reads ONLY the example-doctrine-repo-tracked copy
...
now (chunk C1) prefers a registry-first example-doctrine-repo-root pointer rung ahead of the
shared install. On any machine with `repos.example_doctrine_repo` set — the machine
class that plan exists for — that made this check's "live" side and its
"example-doctrine-repo" side (`_doe_tracked_path()`, also resolved via the same registry key)
...
here), `main()` treats a live path that still equals the example-doctrine-repo path as
...
four `claude-klabauter*` rows moved from example-doctrine-repo's tracked copy into this
repo's own `setup/publish-targets.portable` (this script's own checkout).
Example-doctrine-repo keeps its copy until it runs its source-side residual audit (AC8/C6) —
until then both example-doctrine-repo's and this repo's tracked copies carry these four rows,
...
divergence). Once example-doctrine-repo strips its side, the example-doctrine-repo-tracked copy loses these four
...
excluded from the ordinary example-doctrine-repo-vs-live two-way comparison (see `_diff_rows`'s
...
instead. It runs against the two TRACKED copies (example-doctrine-repo's and this repo's own),
...
Negative-spec:
    ...
    - Does NOT resolve its live side through the full PERCOLATE_ROOT ladder
      (`coordinator_percolate_runtime_root()`) — that ladder's example-doctrine-repo-root-
      ...
      on any machine with `repos.example_doctrine_repo` set. `_resolve_live_targets_path()`
      resolves the shared-install rung (`_rung4_shared_install()`) directly.
    - Does NOT report a clean pass when the live path and the example-doctrine-repo path
    ...
    - Does NOT flag `KLABAUTER_TARGET_NAMES` as example-doctrine-repo-vs-live divergence — see
    ...
    - Does NOT skip the allowlist-rot check when the live copy is absent,
      unresolvable, or collapsed onto the example-doctrine-repo path. Rot detection needs no
      ...
      therefore says the example-doctrine-repo-vs-live comparison was not applicable AND exits
    ...
    - Does NOT hard-fail an allowlist row whose SOURCE ROOT does not resolve
      ...
      set cannot see example-doctrine-repo's source tree at all — neither is allowlist rot.
    - Does NOT apply `_resolve_source_sigil`'s non-strict
      `${meta_root}/plugins/<key>` fallback for an unset
...
# Rows relocated into this repo from example-doctrine-repo by
...
# for why these four names are excluded from the ordinary example-doctrine-repo-vs-live
...
# (AC5 — published bytes unchanged), so a literal field diff against example-doctrine-repo's
# still-fully-qualified-`plugin-source:`-sigil row would report a false
# divergence on every one of these rows until example-doctrine-repo strips its side (C6/AC8).
...
def _doe_tracked_path() -> Path:
    """Return the example-doctrine-repo-tracked publish-targets.portable path.

    Resolved via doe_root() (env DOE_ROOT/REPO_EXAMPLE_DOCTRINE_REPO -> machine-local
    repos.example_doctrine_repo -> raise), not this script's own __file__ location:
    this executable lives in claude-klabauter while setup/ stayed in
    example-doctrine-repo per DR-047 (see verify-templates-bin-sync.py's
    ...
    try:
        ...
        print(
            ...
            f"example-doctrine-repo repo root ({exc}). Set repos.example_doctrine_repo in the "
...
def _resolve_live_targets_path() -> Optional[Path]:
    ...
    ordering.md chunk C4): the full ladder's rung 3 (example-doctrine-repo-root pointer) now
    resolves via the same registry key (`repos.example_doctrine_repo`) this script's
    ...
    and `_doe_tracked_path()`'s "example-doctrine-repo" side resolve to the identical file —
    ...
    of whether a example-doctrine-repo clone pointer is configured.
...
def _row_diff_reason(doe_row: str, live_row: str) -> Optional[str]:
    ...
    if len(doe_fields) != len(live_fields):
        return (
            f"field count differs (example-doctrine-repo={len(doe_fields)} fields, "
            f"live={len(live_fields)} fields) — example-doctrine-repo row: {doe_row!r} "
    ...
    for idx, (doe_field, live_field) in enumerate(zip(doe_fields, live_fields)):
        if idx == 6:
            ...
            if doe_set != live_set:
                ...
                diffs.append(
                    f"allowlist (field 7) differs — only in example-doctrine-repo: {only_doe}; "
                    f"only in live: {only_live}"
                )
        elif doe_field != live_field:
            diffs.append(
                f"field {idx + 1} differs — example-doctrine-repo: {doe_field!r} live: {live_field!r}"
            )
...
def _diff_rows(
    ...
    *,
    ...
    THREE-COPY TOPOLOGY): those rows legitimately differ between example-doctrine-repo's and
    this repo's tracked copies (simplified source sigils) and legitimately go
    present-only-in-one-side once example-doctrine-repo strips its rows, neither of which is
    ...
    for name in all_names:
        ...
        if doe_row is None:
            findings.append(
                f"'{name}': present in live copy only (missing from example-doctrine-repo-tracked "
                f"copy) — live row: {live_row!r}"
            )
            continue
        if live_row is None:
            findings.append(
                f"'{name}': present in example-doctrine-repo-tracked copy only (missing from live "
                f"copy) — example-doctrine-repo row: {doe_row!r}"
            )
            continue
...
def _check_claude_klabauter_owned_rows_present(
    ...
    `_diff_rows` excludes from the example-doctrine-repo-vs-live comparison must still be
    ...
    """
    return [
        f"'{name}': excluded from the example-doctrine-repo-vs-live comparison as a "
...
def _resolve_row_source_root(source_subdir: str, checkout_root: Path) -> Optional[Path]:
    ...
    Negative-spec above: the non-strict `${meta_root}/plugins/<key>` fallback
    ...
    relative to the tree that tracks it, which is what makes example-doctrine-repo's copy and
...
def _check_allowlist_entries_resolve(
    ...
    `build_allowlisted_source` performs, so a finding here is a prediction of
    the exact `AllowlistError` a publish of that row would abort on, not a
    stricter parallel rule that could disagree with the engine.
    """
    findings: list[str] = []
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not doe_path.is_file():
        print(
            f"verify-publish-targets-portable-sync.py: ERROR — example-doctrine-repo-tracked "
    ...
    # example-doctrine-repo-vs-live comparison was not applicable.
    rot_findings = _check_allowlist_entries_resolve(
        doe_rows, doe_path.parent.parent, copy_label="example-doctrine-repo-tracked"
    )
    rot_findings += _check_allowlist_entries_resolve(
    ...
    if live_path is None:
        print(
            ...
            "has no setup/publish-targets.portable). Skipping the example-doctrine-repo-vs-live "
            "comparison cleanly.",
        )
        if rot_findings:
            _report_rot_findings(rot_findings)
    ...
    if live_path.resolve() == doe_path.resolve():
        ...
        print(
            ...
            f"copy resolves to the example-doctrine-repo-tracked copy itself ({doe_path}); "
            ...
            "the example-doctrine-repo-tracked path.",
            file=sys.stderr,
        )
    ...
    print(
        "verify-publish-targets-portable-sync.py: FAIL — example-doctrine-repo-tracked copy "
    ...
    print(
        ...
        "manually: decide which copy is correct (usually the example-doctrine-repo-tracked "
