...
#
# Finish-strangler port (DR-059, example-doctrine-repo owns contract, claude-klabauter owns engine): the
...
# test_verify_dist_publish_repo_sync.py). This file is now a thin example-doctrine-repo-side
...
def _plugin_root() -> str:
    ...
    example-doctrine-repo — self-location now resolves to <claude-klabauter>/coordinator, which
    ...
    is the correct authority for "where is the example-doctrine-repo repo,"
    ...
    the example-doctrine-repo tree — so the trampoline resolves and forwards it via the
    environment.
    ...
    try:
        ...
        print(
            "verify-dist-publish-repo-sync.py: cannot resolve the example-doctrine-repo repo root "
