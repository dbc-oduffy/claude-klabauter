...
Example-doctrine-repo `CLAUDE.local.md` § "A skill must LINK to an entrypoint, not carry a
...
Subcommands (one per ported fence):
    ...
    check-plugin-registered --plugin NAME --marketplace NAME
        ...
        example-retrieval-repo live 2026-07-28, reproducible fleet-wide). Two legitimate
        ...
        itself, resolved from example-doctrine-repo). Neither bare directory presence
...
Spec backlink: example-doctrine-repo coordinator/skills/setup/SKILL.md (Steps 1, 3, 4, 6 Probes 2/3)
"""
...
def cmd_check_plugin_registered(args: argparse.Namespace) -> int:
    ...
    example-retrieval-repo live 2026-07-28, reproducible fleet-wide.
    ...
    itself resolved from example-doctrine-repo's `coordinator/` tree) is correctly
    ...
    (example-doctrine-repo CLAUDE.md "Repo-specific gotchas"): that sentinel only
