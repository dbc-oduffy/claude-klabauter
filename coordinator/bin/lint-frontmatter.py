...
Example-doctrine-repo's contract surface, per DR-047 (example-doctrine-repo owns contract/generator,
Claude-klabauter owns engine). Consumed by example-doctrine-repo's 3 live callers via the resolved
forwarder. Wraps claude-klabauter's frontmatter validator + coordinator_core.dag
reachability primitives to restore the example-doctrine-repo-consumed
lint-frontmatter CLI after coordinator/bin/lint-frontmatter.js was deleted at
Claude-klabauter commit c79e66cd (declared "zero live callers" — false; example-doctrine-repo has 3 live
