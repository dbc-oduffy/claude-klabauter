...
"""wsc-tail.py — ceremony.wsc_tail native trampoline (example-doctrine-repo/claude-klabauter wsc_tail cutover).
...
This module owns transport + the example-doctrine-repo-side exit ladder only; every ceremony
...
Until 2026-07-23 this flag was HARD-REQUIRED on the example-doctrine-repo side specifically because
...
OUT of this op entirely and into a example-doctrine-repo-side WSC skill Step 2.6 re-expansion
...
Claude-klabauter landing this change and example-doctrine-repo landing its re-expansion (see the plan's
§ Landing Order and Skew Tolerance — "every claude-klabauter-side shed chunk must tolerate
the pre-example-doctrine-repo-landing state, accept-and-ignore params rather than hard-failing").
...
a behaviour change. Phase 2 (example-doctrine-repo, tracked separately, not this repo): the skill
...
Example-doctrine-repo hard failure. The memo's exit-2 semantics ("recoverable, not a breach")
...
Example-doctrine-repo-side exit ladder (this module's contract, distinct from the op's internal
...
Negative-spec (what this module does NOT do):
    - Does NOT require `--completion-title` (transitional, C4 Phase 1,
      ...
      Phase 1/2/3 landing-order rationale. Phase 2 (example-doctrine-repo) and Phase 3 (later,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "no longer mandatory. Phase 2 (example-doctrine-repo-side WSC skill re-expansion) moves "
...
def _print_failure_footer(result: dict, *, landed: bool) -> None:
    ...
    through `| tail -N` -- the shape the 2026-08-03 example-retrieval-repo-em memo
