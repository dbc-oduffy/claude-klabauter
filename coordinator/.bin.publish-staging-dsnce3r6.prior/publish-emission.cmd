@echo off
setlocal
REM Windows launcher for publish-emission.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "publish-emission.ps1" sibling of
REM     this file over this "publish-emission.cmd" file itself -- if both exist, `publish-emission`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "publish-emission.py"
...
echo [publish-emission] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [publish-emission] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0publish-emission.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0publish-emission.py" %*
exit /b %ERRORLEVEL%
