@echo off
setlocal
REM Windows launcher for probe-cwd-example-retrieval-repo-relevance.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "probe-cwd-example-retrieval-repo-relevance.ps1" sibling of
REM     this file over this "probe-cwd-example-retrieval-repo-relevance.cmd" file itself -- if both exist, `probe-cwd-example-retrieval-repo-relevance`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "probe-cwd-example-retrieval-repo-relevance.py"
...
echo [probe-cwd-example-retrieval-repo-relevance] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [probe-cwd-example-retrieval-repo-relevance] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0probe-cwd-example-retrieval-repo-relevance.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0probe-cwd-example-retrieval-repo-relevance.py" %*
exit /b %ERRORLEVEL%
