@echo off
setlocal
REM Windows launcher for claude-klabauter-commit-anchors.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "claude-klabauter-commit-anchors.ps1" sibling of
REM     this file over this "claude-klabauter-commit-anchors.cmd" file itself -- if both exist, `claude-klabauter-commit-anchors`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "claude-klabauter-commit-anchors.py"
...
echo [claude-klabauter-commit-anchors] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [claude-klabauter-commit-anchors] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0claude-klabauter-commit-anchors.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0claude-klabauter-commit-anchors.py" %*
exit /b %ERRORLEVEL%
