...
class TestVendorDriftRemediationDischarge:
    ...
    The previous remediation said "copy example-doctrine-repo's <name>.schema.json in verbatim". Following
    ...
    def test_probe_body_drift_remediation_matches_the_manifest_intent(
        ...
        fake.scan_vendored_schema_drift = lambda: {  # type: ignore[attr-defined]
            "status": "DRIFT",
            "summary": "3 of 13 vendored schemas behind example-doctrine-repo HEAD",
            "doe_repo_path": "/x",
            "checked": 13,
...
class TestSentinelShapeUnaffected:
    def test_seven_original_keys_plus_vendor_drift(self, tmp_path: Path) -> None:
        ...
        in example-doctrine-repo (see _write_doctor_sentinel's ADDITIVE-KEY POLICY)."""
