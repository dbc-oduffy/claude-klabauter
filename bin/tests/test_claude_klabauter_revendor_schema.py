...
(which compares against DoE HEAD) while breaking the gating tamper-check (which compares
...
Coverage:
  ...
  5. Decline records: a live decline blocks a matching re-vendor, `--ack-declined`
     ...
     regression reproducing the 2026-08-13 incident this feature responds to: DoE moves
...
def fake_clone(tmp_path: Path) -> Path:
    """A real local git repo shaped like DoE (coordinator/schemas/<name>.schema.json)."""
...
class TestRewritePinRegistry:
    ...
    def test_reason_is_recorded_as_a_comment(self, tmp_path: Path) -> None:
        ...
        src = _mod._rewrite_pin_registry(
            reg, [(pins["beta"], "f" * 40)], "HEAD", "DoE landed the widened enum"
        )
        assert "DoE landed the widened enum" in src
...
class TestDeclineGate:
    ...
    def test_live_decline_blocks_matching_revendor_incident_regression(
        ...
        declined_shape = _handoff_like_schema(False)  # DoE's shape-narrowing edit
    ...
    def test_decline_self_expires_once_version_moves(
        ...
        # DoE resolves it: same shape, but bumped past the declined version (8.0.0).
        ...
        rc = _mod.run(
            ...
            doe_clone_arg=str(fake_clone),
            ack_major=True,
            reason="DoE resolved the shape narrow by bumping to 8.0.0",
        )
