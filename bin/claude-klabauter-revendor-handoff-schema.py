...
Purpose: fail-closed, repeatable re-vendor of DoE's active-handoff schema pair
...
remediation told operators to `cp` DoE's file in by hand, which satisfies the
ADVISORY drift check (against DoE HEAD) while breaking the GATING tamper-check
...
entry for either file, so the "pin" is whatever DoE HEAD resolves to at run-time,
exactly as `check_schema_drift` compares against DoE HEAD at test-time. The general
...
Contract (unchanged, now enforced by the shared mechanism):
  - Fail-closed if the DoE clone is absent or `git show` fails (never silently no-ops).
  ...
  - DoE clone resolved via resolve_doe_clone() (machine-local registry), with a
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description=(
            "Re-vendor DoE HEAD's handoff.schema.json + handoff-archived.schema.json "
        ...
        epilog=(
            "Examples:\n"
            "  # Standard re-vendor from DoE HEAD:\n"
    ...
    p.add_argument(
        ...
        help=(
            ...
            "anything. Resolves the DoE clone and diffs bytes, but never touches disk."
        ),
    )
