...
keeps `bin/distill-ripe-filter.py` working for one release while example-doctrine-repo's PIPELINE.md
...
sys.stderr.write(
    "bin/distill-ripe-filter.py is deprecated — use "
    "coordinator/bin/distill-ripe-filter.py instead.\n"
)
_target = Path(__file__).resolve().parent.parent / "coordinator" / "bin" / "distill-ripe-filter.py"
_spec = importlib.util.spec_from_file_location("_distill_ripe_filter_relocated", _target)
