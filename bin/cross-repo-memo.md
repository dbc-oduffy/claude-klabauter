...
Example-doctrine-repo and now lives in THIS repo, at `coordinator/bin/cross-repo-memo`. It is
no longer a example-doctrine-repo-owned file; resolve it repo-relatively when you're already in this
...
Example-doctrine-repo-owned in the CLI, so the observable contract is simply: delivery is committed, not
left dirty.)
