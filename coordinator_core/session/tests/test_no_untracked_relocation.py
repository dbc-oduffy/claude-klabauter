...
_ALLOWED: Dict[Tuple[str, str, str, str, str, int], str] = {
    ...
    ("coordinator_core", "group_em/watch_heartbeat.py", "_write_atomic", "os.replace", "os.replace(tmp_path, path)", 1): "atomic tmp->final; `tempfile.mkstemp` sibling, temp source never claimed",
    ...
    # no session id resolved or the relocate raised, and the enclosing helper's
    # own docstring states why a migration step already committed to moving a
    # file must not fail on a claim-bookkeeping nicety. `git add rel_dst` plus
