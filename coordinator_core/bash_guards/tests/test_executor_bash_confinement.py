...
def test_coordinator_doc_new_allows(monkeypatch):
    _confine(monkeypatch)
    payload = _payload(
        "/x/claude-klabauter/coordinator/bin/coordinator-doc-new --type run-report"
    )
    assert guard.check(payload) is None
...
def test_machine_local_get_allows(monkeypatch):
    _confine(monkeypatch)
    payload = _payload("machine-local get repos.example_doctrine_repo")
    assert guard.check(payload) is None


def test_machine_local_has_allows(monkeypatch):
    _confine(monkeypatch)
    payload = _payload("machine-local has repos.example_doctrine_repo")
    assert guard.check(payload) is None
...
def test_machine_local_path_allows(monkeypatch):
    _confine(monkeypatch)
    payload = _payload("machine-local path repos.example_doctrine_repo")
    assert guard.check(payload) is None


def test_machine_local_dir_allows(monkeypatch):
    _confine(monkeypatch)
    payload = _payload("machine-local dir repos.example_doctrine_repo")
    assert guard.check(payload) is None
...
def test_machine_local_set_allowlist_allows_executor_but_destructive_guard_denies_both(
    ...
    cmd = "machine-local set repos.example_doctrine_repo /x/evil"
    _confine(monkeypatch)
    _wire_other_guard(monkeypatch, destructive_guard)
    _assert_allowed(guard.check(_payload(cmd)))
...
_AC4_REVIEWER_VERDICT_COMMANDS = (
    ...
    "machine-local path repos.example_doctrine_repo",
    "machine-local dir repos.example_doctrine_repo",
    "machine-local set repos.example_doctrine_repo /x/evil",
