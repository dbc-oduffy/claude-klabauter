...
def _reason(classification: str, path: str = _PATHS[0]) -> str:
    """A refusal string in `assert_paths_in_session_scope`'s own shape.

    Built from that module's format and its real constants rather than pasted,
    so a rewording there fails this file loudly instead of quietly
    un-matching the predicates under test -- the `F1` lesson
    `test_ownership_leg_denial_names_indeterminate_classification` records.
    """
    return (
...
def records(monkeypatch):
    """Capture the audit line instead of writing one, and silence the notice.

    Patched on `_write_bump_stand_down`, which is where
    `_ownership_leg_stand_down._record` imports both from -- so the test
    observes the real delegation rather than a re-implementation of it.
    """
...
def _permit(reason: str):
    """Drive `_git_commit_agent_pathspec_permitted` with a pathspec whose
    shape legs pass and whose ownership leg returns *reason*."""
...
def test_no_verdict_allows_and_records(records):
    """An aborted walk is absence of a verdict, not a negative verdict."""
...
def test_a_named_live_holder_still_denies(records):
    """The guard's real job. A live foreign-session claim denies whatever else
    the walk found -- the holder check wins over the stand-down."""
...
def test_a_mixed_pathspec_with_one_held_path_denies_whole(records):
    """One held path among indeterminate ones is a verdict about the pathspec,
    so the refusal is evidence-backed and nothing stands down."""
...
def test_a_determinate_no_claimant_still_denies(records):
    """`orphan`/`unclaimed` is a reading the index's own negative spec
    disclaims, and it is STILL a deny here: reversing it widens a hard-deny
    allow path against DR-246's strict default, which is a ruling to change
    rather than a predicate to patch. Pinned so the distinction is deliberate
    and a later reader does not fold the two absences together silently."""
...
def test_an_ordinary_allow_is_untouched(records):
    """No stand-down is consulted, and nothing is recorded, when the ownership
    leg allowed in the first place."""
...
def test_already_clean_classification_does_not_stand_down(records):
    """A verdict about the tree, not about ownership."""
...
def test_unresolvable_repo_root_names_its_own_leg():
    """The rootless-session denial used to select the static message, whose
    body re-spells the two sanctioned shapes and tells the reader to check
    scope -- neither reachable when no root resolved and the pathspec was
    never read."""
...
def test_static_message_does_not_imply_a_spent_allowance():
    """`"Used one already? Check scope, not argv."` was read as a
    one-commit-per-agent budget and sent a whole investigation after a counter
    this guard does not have. The text now says nothing is counted."""
...
def test_the_ownership_leg_latches_nothing_across_attempts():
    """No allowance is spent by an attempt, a refusal, or a success.

    The refusal message names two sanctioned shapes and an agent denied on
    the first dutifully tries the second, which made "the second attempt was
    doomed by the first" the natural reading of a same-run allow-then-deny
    pair. It is wrong, and the code has nowhere to keep such a counter: the
    verdict is a pure function of one payload plus a fresh claim-index
    rebuild. Pinned so a later reader does not have to re-derive that from an
    absence, and so any future attempt-keyed state fails here first.

    Drives the real sequence: a refused attempt, a second refused attempt by
    the same caller, then an allow -- and the allow must be an allow.
    """
