...
Three failure classes this pins, per example-doctrine-repo
...
3. **Precedence over simultaneous matches** (``TestPrecedence``) -- the
   ...
   FIND_EXEC_XARGS``) with the other two shapes present, in order, in
   ``.residue`` -- never an arbitrary pick and never a dropped match.
"""
...
class TestHeredocBodyIsNotShapeMaterial:
    ...
    merely describing one of these five shapes would then classify as one.
