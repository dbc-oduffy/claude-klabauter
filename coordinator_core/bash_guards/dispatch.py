...
dispatcher. Python port of example-doctrine-repo's ``coordinator/hooks/scripts/
...
first-non-empty-stdout-wins, in hooks.json REGISTRATION order:
 ...
 5b. check_test_suite_invocation           (hard, fail-closed) -- NO legacy
     bash predecessor; added 2026-07-23 for example-doctrine-repo's DR-088 test-breadth ladder.
 ...
 5c. check_raw_pid_liveness                (hard, fail-closed) -- NO legacy
     bash predecessor; added 2026-07-27 (example-doctrine-repo C14/RAW-PID-LIVENESS-GUARD) to
 ...
 5d. block_worktree_creation                (hard, fail-closed) -- NO legacy
     bash predecessor; added 2026-07-28 (example-doctrine-repo, fleet-wide structural
 ...
 5e. block_approval_sentinel_creation       (hard, fail-closed) -- NO legacy
     bash predecessor; added 2026-07-28 (example-doctrine-repo, doctrine-approval
 ...
 5f. block_worktree_sentinel_creation       (hard, fail-closed) -- NO legacy
     bash predecessor; added 2026-07-28 (example-doctrine-repo, closing a confirmed
 ...
 5g. block_stash_destruction                (hard, fail-closed) -- NO legacy
     bash predecessor; added 2026-07-30 (example-doctrine-repo) as the main-loop leg of
...
Port of: preuse-bash-dispatch.sh (example-doctrine-repo 2f8b8450, 2026-07-16)
...
"""Human-readable phrases for example-doctrine-repo's opaque ``resolution_class`` strings, used
ONLY to render the crash-deny envelope. The three keys are owned by example-doctrine-repo's
...
def _crash_deny(guard_name: str, exc: BaseException, resolution_class: Optional[str] = None) -> Dict[str, Any]:
    ...
    resolution class example-doctrine-repo's ``preuse-bash-dispatch.py`` computes via its own
    ...
    string this function does not recognize -- example-doctrine-repo's three values are treated
...
def evaluate_payload_json(
    ...
    *,
    ...
    ``policy_file`` (C5a/C6, 2026-07-27): the explicit path to example-doctrine-repo's
    ...
    resolution class computed by example-doctrine-repo's ``coordinator/hooks/scripts/preuse-
    ...
    sends. One of three example-doctrine-repo-owned opaque strings (``"resolved-engine"``,
    ``"live-working-tree"``, ``"unresolved"``); never imported from example-doctrine-repo's
...
def _build_guard_chain(
    ...
    guard_chain: List[GuardEntry] = [
        ...
        #
        # C4, docs/plans/2026-08-02-write-confinement-guards.md (example-doctrine-repo
        ...
        # 2026-07-27, example-doctrine-repo C14/RAW-PID-LIVENESS-GUARD). Not identity-gated
        # (fires on every caller, EM included -- see its module docstring);
        # has no parity ordering to preserve, so it sits at the very tail.
        # BX-16 (example-doctrine-repo docs/plans/2026-07-29-windows-viability-stop-the-spawn-
