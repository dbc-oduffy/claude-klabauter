"""coordinator_core.bash_guards.block_subagent_guard_grant -- PreToolUse
(Bash) hard-deny guard denying a dispatched subagent from acquiring an
EM guard-grant (`coordinator_core.session.em_guard_grant`'s `grant`
subcommand) over `python -m`/`python3 -m`, or over an inline `python
-c`/`python3 -c` import-and-call.

SPEC: `docs/plans/2026-08-13-em-exercisable-in-band-grant-route.md`, chunk
C3, read together with PM ruling 1 (2026-08-13) and the plan's `## Design`
section, which this docstring does not restate in full.

LOAD-BEARING, NOT DEFENCE-IN-DEPTH. Per PM ruling 1: `em_guard_grant.py`
(chunk C1) cannot self-discriminate its caller -- no hook payload reaches a
plain Python module call, so `resolve_effective_types`-style identity
resolution has nothing to read there. This guard, and its write-channel
sibling (`write_guards/block_subagent_guard_grant_write.py`, C4), ARE the
control. Nothing downstream of this guard stops a subagent that reaches the
module directly.

NEAR-EXACT PORT of `block_subagent_grant_acquisition.py` (the CLAUDE.md
write-grant's Bash-channel acquisition guard): same hard-deny class, same
`("Bash",)` matcher, same RAW-`agent_id`-presence discriminator (fail
CLOSED -- present-but-unresolvable is still not the EM), same `-m` and `-c`
token-shape classification reusing `_command_tokenizer` and
`block_subagent_destructive_action`'s tokenizer helpers. Gated CLI differs
(`em_guard_grant` vs `claude_md_grant`), and validated `granted_by` differs
(`"em"` vs `"pm"`, per PM ruling 1) -- see `em_guard_grant.py`'s own
docstring for why that value differs and why the sibling module's
`_VALID_GRANTED_BY` reasoning does not transfer.

IDENTITY-GATE POSTURE -- fail CLOSED, same posture as
`block_subagent_grant_acquisition.py` (itself modeled on
`block_subagent_stash_creation.py`'s and `block_subagent_commit.py`'s).
This guard has exactly one allowed caller (the EM, i.e. no `agent_id` on
the payload at all) and exactly one gated action (acquiring the guard
grant); there is no per-kind branching that a resolution failure could
legitimately need to fall through to. The discriminator is the RAW
presence of the harness-supplied `agent_id` field alone, not whether it
resolves further -- `agent_id` present but unresolvable is still,
unambiguously, "not the EM", so it denies.
...
against the known module path and function name. It is not, and cannot be
made, exhaustive. A subagent that writes a script file to disk and then
executes that file, or that reaches `em_guard_grant`'s write function
...
What backstops the residual: `block_subagent_guard_grant_write`
(`coordinator_core/write_guards/block_subagent_guard_grant_write.py`, plan
chunk C4, authored concurrently) denies the Write/Edit/MultiEdit/
NotebookEdit tool channel to BOTH grant artifacts (the durable record and
the DR-260 sentinel) directly, at their resolved locations -- independent
of whether this module's Bash-side token-shape recognition catches the
invocation that produced the write. The two legs cover different channels
(Bash tool-call token shape here; Write-tool-call path there) and neither
subsumes the other -- a write performed *inside* a spawned Python process
reached by an unrecognized Bash shape never surfaces as a Write-tool call
for C4 to see either.

RECOGNIZED SHAPES (classification only):

  1. `python`/`python3` + `-m` + module `coordinator_core.session.
     em_guard_grant` + a `grant` subcommand token immediately following the
     module path. `read` and `check` are explicitly NOT gated -- this
     guard's audience is acquisition, not every invocation of the CLI.
  2. `python`/`python3` + `-c` + a following string argument that both (a)
     contains the substring `coordinator_core.session.em_guard_grant` and
     (b) references the grant-writing function name
     `write_em_guard_grant` -- e.g. `python3 -c "from coordinator_core.
     session.em_guard_grant import write_em_guard_grant as w; w(...)"`. The
     identical substring check against a `-c` payload that instead
     references only `read_em_guard_grant`/`check_em_guard_grant` is NOT
...
docstrings for the bypass history each closes. The classification logic
here is a near-duplicate of `block_subagent_grant_acquisition.py`'s own
`-m`/`-c` classifiers, parameterized only by the constants below (gated
module path, gated function name); this is one of two near-copies, kept
separate rather than force-extracted into a shared helper (extraction is
the point to force only once a third near-copy appears).

OVERRIDE -- none at this layer, same as the sibling module.
"""
...
#: `block_subagent_stash_creation`, `block_subagent_grant_acquisition`) it
#: is registered near.
PRIORITY = 40

#: The dotted module path this guard gates acquisition of, matching
#: `em_guard_grant.py`'s own `python3 -m` invocation contract.
_GRANT_MODULE = "coordinator_core.session.em_guard_grant"

#: Only `grant` acquires the authorization this guard exists to gate --
#: `read`/`check` are informational and explicitly NOT gated.
...
#: to the sibling guard's own constant of the same name.
_PYTHON_BASENAME_RE = re.compile(r"^python[0-9.]*$")

#: Bundled `-m<module>` form, e.g. `python3 -mcoordinator_core.session.
#: em_guard_grant` -- CPython's own arg parser accepts the module path
...
def _extract_dash_m_module_and_rest(
    ...
    e.g. `["grant", "bump-foreign-repo-write", "<reason>"]`) -- or `None`
    if no `-m` flag is present. Mirrors
    `block_subagent_grant_acquisition._extract_dash_m_module_and_rest`'s
    own bundled/standalone scan shape.
    """
...
def _classify_dash_m_invocation(working: List[str]) -> Optional[str]:
    ...
    guard-grant-acquisition shape. Returns a deny_kind label, or `None` if
    this segment is not a gated `-m` invocation of `em_guard_grant grant`.
    """
...
def _classify_dash_c_invocation(working: List[str]) -> Optional[str]:
    """Classify a single already-segmented, subshell/env-stripped token
    list `working` for the `-c` inline-import guard-grant shape. Returns a
    deny_kind label, or `None` if this segment's `-c` payload (if any) does
    not reference both the grant module and the grant-writing function
    name as a whole identifier (not merely as a substring of some other
    identifier -- see `_WRITE_FUNC_NAME_RE`).
    """
...
def _evaluate(cmd: str) -> Optional[str]:
    ...
    against both the `-m` and `-c` guard-grant-acquisition shapes. Returns
    the first deny_kind label found, or `None` if nothing in `cmd` matches
...
def _deny_reason(cmd: str, deny_kind: str) -> str:
    """Design-as-offer deny text for a recognised guard-grant-acquisition
...
def check(payload: dict) -> Optional[dict]:
    """Evaluate the guard-grant-ACQUISITION gate against a PreToolUse
    payload.

    Identity-gated to subagents only -- fail CLOSED on a present-but-
    unresolvable `agent_id`, same posture as
    `block_subagent_grant_acquisition.check` (see module docstring
    "IDENTITY-GATE POSTURE"). Gates on the raw presence of `agent_id`
    alone -- never `resolve_effective_types`' OR-resolved triple: Claude
    Code documents `agent_type` as also populated for a session launched
    with `--agent`, so an EM on such a session would resolve as NOT-EM
    under that broader triple and have a legitimate grant denied.
    `agent_id` is documented subagent-only.
    """
