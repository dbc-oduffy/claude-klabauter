...
def _deny_reason(cmd: str, deny_kind: str) -> str:
    ...
    return (
        ...
        "  Command:  %s\n\n"
        "There is no undo. A dropped stash leaves NO reflog entry -- the "
        "content survives only as a dangling commit until the next gc.\n\n"
        "What you probably want instead:\n"
        "  - Need the content back? Read it without touching the stack:\n"
        "        git show stash@{N}:<path> > <path>     (per path)\n"
        "        git stash show -p stash@{N}            (see the whole diff)\n"
        "  - Already recovered it and want the entry gone? You don't need to "
        "do anything. Leave it. An orphaned stash entry costs a line of "
        "`git stash list` output; a wrongly-dropped peer session's entry "
        "costs their work.\n"
        "  - Restoring your OWN stash into your tree? `git stash pop "
        "stash@{N}` is allowed -- it drops only what it has just applied in "
        "front of you, so you are not guessing about whose entry it was. "
        "Name the index explicitly rather than relying on position 0.\n\n"
        "Sweeping an accumulated stack is a deliberate cleanup pass with a "
        "human reading the list, not a step inside other work."
    ) % (deny_kind, cmd_safe)
