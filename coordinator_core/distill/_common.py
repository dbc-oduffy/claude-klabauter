...
2 coupled scripts (harvest-debt, memo-triage) all import this module. It carries:
  ...
  1. The canonical distillation-log PARSER for the example-doctrine-repo canonical format (used by C4/C7's
  ...
  3. SIDECAR_SUFFIXES — the single named, full-suffix-anchored constant enumerating the
     process-scaffolding sidecar suffixes sidecar-sweep matches against. C8 (example-doctrine-repo's contract)
...
"""Valid disposition values for a canonical-log row (per the example-doctrine-repo canonical format)."""
...
Example-doctrine-repo owns this vocabulary as a cross-repo contract; claude-klabauter mirrors it here in lockstep,
same ownership split `SIDECAR_SUFFIXES`'s own comment already documents (example-doctrine-repo authors and
...
authored duplicate. C8 (example-doctrine-repo's contract) is the eventual cross-repo single owner
of this enumeration; keep in lockstep with example-doctrine-repo's list.
