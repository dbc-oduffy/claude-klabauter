...
check (2026-07-25 defect)" section: `cross-repo/archive/2026-07-25-example-doctrine-repo-
...
class TestLinkageMatching:
    ...
    def test_linked_via_tail_stem_elided_citation(self, tmp_path, monkeypatch):
        ...
        for it."""
        # Deliberately NOT `self._setup` — `example-doctrine-repo-em` resolves through
        ...
        monkeypatch.setenv("COORDINATOR_SETTINGS_HOME", str(settings_home))
        inbound_basename = "2026-07-25-example-doctrine-repo-em-test-red-record-contract-consult.md"
        _seed_sender_reply(
            sender_root,
            "cross-repo/inbox",
            "answers-q1-q2-q3.md",
        ...
        closure = pa.compute_reply_closure(
            {"kind": "consult", "from": "example-doctrine-repo-em", "created": "2026-07-25"},
            inbound_basename,
            repo,
        )
        assert closure["verdict"] == "evidenced"
...
class TestMinLinkStemLengthFloor:
    ...
    def test_degenerate_single_char_tail_stem_does_not_spuriously_link(self, tmp_path, monkeypatch):
        ...
        _register_sender(settings_home, "example_doctrine_repo", sender_root)
        monkeypatch.setenv("COORDINATOR_SETTINGS_HOME", str(settings_home))
        inbound_basename = "2026-07-25-example-doctrine-repo-em-m.md"
        for i in range(2):
            _seed_sender_reply(
                sender_root,
                "cross-repo/inbox",
