...
def _make_chain_range_resolver(
    ...
    Negative-spec:
        - The returned sets are intersected with `chain_set`, exactly as the
          ...
          even if the round also froze commits outside this gate's range.
          Intersecting BOTH sides of `classify_pending_records`' closure
          superset check with a common set is monotonic for a true closure but
          can manufacture a FALSE one: a pending record whose full range is
          `{D, X}` (D in `chain_set`, X outside it, unreached by any closer)
          stays correctly open under an unscoped check, but once both sides
          drop X it degrades to `{D} ⊆ {D}` — closed. This can only ever swallow
          a genuine `loop-open-mid-round` into a different leg's note (never
          manufacture a phantom OPEN round: the record's own
          `shas ∩ uncovered_set` gate is scope-invariant, since
          `uncovered_set ⊆ chain_set`). `_diagnose_open_review_loop`'s notes are
          fail-visible, never verdict-feeding, so the blast radius is a
          misleading note token, never a wrong `verdict`/`uncovered_shas`. Flat
          mode carries this identical scoping and the identical property — it
          is a deliberate, pre-existing, mirrored tradeoff, not new to DAG
          mode. Downstream: the four `_diagnose_open_review_loop` tokens are
          verdict-equivalent between scoped and unscoped resolution, but the
          specific note TOKEN chosen is not guaranteed identical.
