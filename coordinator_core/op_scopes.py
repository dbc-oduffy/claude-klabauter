...
_OP_KEY_SCOPE: Dict[str, str] = {
    ...
    # sizing.discharge_surfaced — same scope class as sizing.ship/sizing.decline
    # above: the handler reads/writes a main-worktree-rooted state/sizings/ file and
    # stats the resolving artifact under docs/decisions/, docs/plans/ or state/, all
    # derived via main_worktree_root(common_dir). Without this entry dispatch resolves
    # repo_root=None and the handler raises rather than silently deriving against the
    # wrong worktree.
    # Spec: coordinator_core/ops/sizing_discharge_surfaced.py docstring
