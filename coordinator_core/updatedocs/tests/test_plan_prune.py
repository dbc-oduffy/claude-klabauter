...
def test_no_status_key_lands_in_indeterminate_never_prunable(tmp_path):
    ...
    assert result.indeterminate[0].status is None
...
def test_terminal_status_old_and_unreferenced_is_prunable(tmp_path):
    ...
    _write_plan(
        tmp_path,
        "2026-01-01-terminal-unreferenced.md",
        [f'plan_id: "pln-terminal-unref-bbbbbb"', f"status: {status}"],
        age_days=OLD_MTIME_DAYS,
    )

    result = compute_plan_prune_candidates(tmp_path)

    assert len(result.prunable) == 1
    candidate = result.prunable[0]
    assert candidate.is_terminal_status is True
    assert candidate.referenced is False
    assert candidate.age_days >= DEFAULT_AGE_FLOOR_DAYS
...
def test_non_terminal_status_is_retained_not_indeterminate(tmp_path):
    ...
    assert len(result.retained) == 1
    assert result.retained[0].status == "draft"
    assert result.retained[0].is_terminal_status is False
...
def test_terminal_status_but_referenced_is_retained(tmp_path):
    ...
    assert result.prunable == []
    assert len(result.retained) == 1
    assert result.retained[0].referenced is True
    assert result.retained[0].reference_hits
...
def test_referenced_via_origin_plan_id_in_tasks(tmp_path):
    ...
    _write_plan(
        tmp_path,
        "2026-01-01-terminal-id-referenced.md",
...
def test_absent_plans_dir_raises_rather_than_returning_a_clean_empty_result(tmp_path):
    ...
    from coordinator_core.updatedocs.plan_prune import PlanPruneTargetMissing

    with pytest.raises(PlanPruneTargetMissing) as excinfo:
