...
Port of: gen-settings-hooks.sh (example-doctrine-repo a2078a9b, 2026-07-22).
...
gen-settings-hooks.test.sh (example-doctrine-repo c3322493, 2026-07-22) tests (a)-(g) 1:1 by
...
# against (2026-07-28 incident: `X:/example-doctrine-repo/...` baked into a macOS
...
# coordinator/bin/fixtures/gen-settings-hooks/), example-doctrine-repo-resident sibling
# checkout second (pre-migration / alternate layouts). This mirrors the
# co-located -> example-doctrine-repo-resident two-rung shape used elsewhere for the split
...
if (_COLOCATED_FIXTURES / "hooks.json").is_file():
    ...
    raise RuntimeError(
        ...
        f"example-doctrine-repo-resident tried: {_DOE_FIXTURES}."
    )
...
# was silently overwritten with a Windows peer's baked `X:/example-doctrine-repo/...`
...
def test_assert_portable_command_fails_loud_on_drive_letter():
    with pytest.raises(GenSettingsHooksError, match=r"[Dd]rive-letter"):
        _assert_portable_command(
            "python3 X:/example-doctrine-repo/coordinator/hooks/scripts/x.py", event="SessionStart"
        )
...
def test_cross_surface_pin_detect_foreign_platform_paths_clean_for_windows_shape():
    ...
    windows_root = "C:/Users/Jane/example-doctrine-repo/coordinator"  # abs-path-ok: illustrative placeholder, not a real host
