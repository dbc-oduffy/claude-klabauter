...
template still exec'd the now-empty coordinator-claude-side tree and the old
...
def test_derive_agent_helper_target_map_publish_namespaced_alias_additive():
    ...
    (coordinator-claude) still invokes it by that bareword. This is a real-tree
...
def test_sweep_does_not_remove_other_family_files(tmp_path: Path):
    ...
    decoys = {
        ...
        "platform-localize.cmd": "@echo off\nrem coordinator-claude template, not ours\n",
...
# exclusive to agent-helper forwarders (coordinator-claude's platform-localize.cmd /
...
def test_sweep_does_not_remove_legacy_marker_files_from_other_families(tmp_path: Path):
    """platform-localize.cmd and resolve-coordinator-clone.cmd are real coordinator-claude
