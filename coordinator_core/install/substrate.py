...
def _sweep_orphaned_agent_helpers(
    ...
    *,
    ...
    Condition 2 must hold together with EITHER condition 0 or condition 1 —
    a manifest-identified native forwarder never needs a marker match (it
    has none to give), and a marker-identified text forwarder never needs
    manifest membership. An operator-hand-edited forwarder that
    ...
    for entry in sorted(dst_dir.iterdir()):
        ...
        if name in protected_names:
            continue
        if name not in native_forwarder_names:
            # Condition 0 (manifest match) did not identify this entry —
            # fall back to the text-marker branches, which is the only
            # remaining path for the three text-forwarder shapes. A
            # manifest-identified name skips this read entirely: attempting
            # `read_text()` on an opaque binary image is exactly the defect
            # this manifest exists to route around (see this function's own
            # docstring, condition 0).
            try:
