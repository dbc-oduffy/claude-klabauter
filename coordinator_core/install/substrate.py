...
# MARKER`/`_AGENT_CMD_FORWARDER_MARKER` are substrings of the exact bodies
# `_write_agent_forwarder`/`_write_agent_cmd_forwarder` emit below — defined
...
# of sync with what the generators actually write. Neither string appears
# in any OTHER family this module installs (ml_family/ch_family's
...
def _write_agent_forwarder(name: str, dst: Path, check_only: bool, *, target: str) -> None:
    ...
    that duplication would scale to ~127+ near-identical copies. The ladder
    ...
    hazard (a) is ever triggered. This function's template is installed as
    ~290 forwarders repo-wide, so that tax is not one file's
    cost -- it is ~290 cold-`bash.exe`-avoidance wins per install, on
    Windows, which CLAUDE.md treats as the primary platform."""
...
def _write_agent_cmd_forwarder(
    ...
    duplicate that ladder once per forwarder (290+ copies and counting).
    ...
    never the claude-klabauter-side ``.py``/``.sh`` targets themselves. 236 launchers
    x 2 install dirs failed with "can't open file" on Windows. See
...
def _sweep_orphaned_agent_helpers(
    ...
    1. Marker match. A ``.cmd`` file must carry ``_AGENT_CMD_FORWARDER_
       ...
       copy-a-source-.cmd-verbatim approach); a non-``.cmd`` file must carry
       ``_AGENT_FORWARDER_MARKER``. ``_AGENT_FORWARDER_MARKER``/
       ``_AGENT_CMD_FORWARDER_MARKER`` are exclusive to this module's own
       f-string bodies (interpolated from the same constants the writers
       use, so they cannot drift) and appear in NO other family this module
       installs. ``_LEGACY_CMD_MARKER`` is NOT exclusive — it was stamped by
       ``coordinator/bin/gen-launcher-shim.py`` on every CLI's source-side
       ``.cmd``, including the legitimate, still-installed-every-run example-doctrine-repo
       templates ``platform-localize.cmd`` and
       ``resolve-coordinator-clone.cmd`` (confirmed on a live install), so a
       legacy-marker match is a necessary but not sufficient condition.
    2. Absence from this run's complete write set. A name is eligible for
       ...
       NOR ``_static_bin_family_names()`` (every OTHER family's complete,
       statically-known name set — this is what keeps
       ``platform-localize.cmd``/``resolve-coordinator-clone.cmd`` safe
       despite carrying the legacy marker: both names are members of that
       set, so condition 2 never holds for them).
...
def _resolve_baked_python_bin() -> str:
    ...
    *error* (e.g. ``PythonPinInvalid`` -- a pinned interpreter that exists but
    ...
    into ~290 launchers that only Windows executes, and every subsequent macOS
    install re-poisoned the ones a Windows install had just written correctly.
...
WRITE_SURFACE = WriteSurfaceDeclaration(
    ...
    clauses=(
        ...
        # Clause 4 — the agent-helper forwarder pair generator: ONE
        # discovery mechanism (`_derive_agent_helper_target_map`, scanning
        # `coordinator/bin/`) feeding ONE fixed `.py`/`.cmd` forwarder
        # template pair per discovered CLI. SHAPED, not ~290 independent
        # static entries — the map's size varies with `coordinator/bin/`'s
        # contents and is not enumerable in source.
        ShapedClause(
            discovered_by="_derive_agent_helper_target_map",
            entry_template=WriteSurfaceEntry(
                kind="file-path",
                path="<agent-bin-dir>/<forwarder-name>[.py|.cmd]",
            ),
        ),
...
"""This writer's declared surface — 27 clauses (C3c/C3c2's original six,
...
the `_careful_write` disposable backup tree (clause 15) — plus this final
...
module's declaration), plus a final closing clause 27 (`_fnm_step`'s
brew/curl third-party `fnm` installer leg, declared via the stated-reason
escape hatch rather than left silently undeclared — no kind in the frozen
eight-kind vocabulary honestly expresses an unenumerable third-party
installer footprint) — see
``state/audits/2026-08-06-install-substrate-write-surface-completeness.md``
refs C/D/E/F/G/I/W/X/Y/Z/AB for the census this closes. `write_strategy`
(`_write_strategy_for`, force/careful/refuse) has no field of its own on
`WriteSurfaceEntry` — it is expressed via the free-text `reason` on clauses
5, 6, and 15 rather than left unstated, since those are the entries it
