...
schema-validation half natively:
  ...
  2. On a live (non ``--check-only``) run that completed with rc 0, performs
     ...
     shell out to ``.github/scripts/validate-json-schemas.py`` (a example-doctrine-repo
...
Spec backlink: coordinator/commands/install.md § Step 9 [example-doctrine-repo repo]
"""
...
def _default_validate_schemas_path() -> Optional[Path]:
    """Resolve `.github/scripts/validate-json-schemas.py`, example-doctrine-repo-repo-
    relative — mirrors the retired bash step's plain-relative-path lookup
    (which assumed cwd == the example-doctrine-repo repo root at install time). Tries
    the resolved example-doctrine-repo root first (works regardless of caller cwd), falling
