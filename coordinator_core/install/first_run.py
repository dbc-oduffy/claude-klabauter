"""
coordinator_core.install.first_run -- Port of: the example-doctrine-repo-owned fresh-machine
bootstrap entrypoint `coordinator/scripts/first-run.sh` (example-doctrine-repo c3322493,
...
(example-doctrine-repo side) to a `#!/usr/bin/env python3` trampoline over THIS repo's own
...
Spec backlink: docs/plans/2026-07-21-claude-klabauter-pure-python-shop-retire-all-bash.md § C12
"""
...
#        example-doctrine-repo polyglot trampoline that imports it. Documented here so the two
...
def _seed_machine_local_registry(confirm: bool, non_interactive: bool) -> None:
    ...
    The example-doctrine-repo-side ``discover-working-repos.sh`` this used to shell out to is
    ...
    surface relocation moved it out of the example-doctrine-repo ``CLAUDE_PLUGIN_ROOT``
...
def _run_post_toolchain_steps(plugin_root: Path, args: _Args) -> int:
    ...
    # example-doctrine-repo CLAUDE_PLUGIN_ROOT entirely and into claude-klabauter's OWN checkout
    ...
    try:
        claude_klabauter_root_for_preflight = Path(coordinator_claude_klabauter_root())
    except RuntimeError as exc:
    ...
    # coordinator_core.hooks.platform_localize (example-doctrine-repo-owned file kept
...
def _main_body(argv: Optional[List[str]] = None) -> int:
    ...
    # The trampoline resolves and passes this via env (see example-doctrine-repo-side file);
    ...
    if plugin_root_env:
        ...
        # to a example-doctrine-repo coordinator/ tree in the general case (they are separate
