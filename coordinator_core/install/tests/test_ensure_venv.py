...
class TestWriteSurfaceDeclaration:
    ...
    def test_surface_is_six_clauses(self) -> None:
        """venv tree, general-pin write/delete, whoami-pin write/delete
        (AC7 — the additive split), and the build-lock sidecar.

        SIX, not seven: the seventh was the general-pin interpreter's
        site-packages — the one surface outside anything this installer owned,
        which existed solely to keep the now-retired `coordinator_whoami`
        editable-installed there. The count dropping is the retirement landing,
        not a clause going missing."""
        assert len(ev.WRITE_SURFACE.clauses) == 6
