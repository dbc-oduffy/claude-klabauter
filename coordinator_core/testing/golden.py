...
Claude-klabauter's native port **live** against a example-doctrine-repo JS oracle at test time, skipping
cleanly whenever node or the oracle checkout is absent. That skip-on-missing shape is
a silent-green hazard: the moment example-doctrine-repo deletes the oracle `.js` it no longer needs,
