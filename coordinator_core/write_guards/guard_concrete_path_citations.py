...
scan_repo`, consumed by example-doctrine-repo's `test_no_absolute_path_literals.py`)
...
`coordinator/hooks/scripts/guard-prompt-surface-citations.py` (example-doctrine-repo)
-- this guard reimplements that shape natively in claude-klabauter rather than
depending on a example-doctrine-repo-owned module, because (unlike that guard's SEED_WIKIS
data dependency) nothing here needs example-doctrine-repo-owned data at hook time.
...
# fleet's own doctrine (example-doctrine-repo CLAUDE.md's `state/ vs tasks/` section,
