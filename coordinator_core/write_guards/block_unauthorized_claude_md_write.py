...
finding, example-doctrine-repo 2026-07-27-claude-md-altitude-triage.md § C4): this
guard DOES cover example-doctrine-repo's OWN repo-root ``coordinator/CLAUDE.md`` and
...
Negative-spec — do NOT read this guard as reopening DR-058 generally:
  ...
  - DR-047 (example-doctrine-repo/claude-klabauter boundary redraw, contract-vs-engine split) is why
    this guard is ENGINE-resident (``claude-klabauter
    coordinator_core/write_guards/``) rather than a example-doctrine-repo-side hook: the
...
Authorizing decisions: example-doctrine-repo docs/decisions/DR-104 (this override),
