...
| `init_anchor_injection_state.py` | `ceremony.init_anchor_injection_state` | Resolves the coordinator-claude/example-doctrine-repo root + today's date and initializes the empty anchor-injection accumulator |
...
| `bootstrap_repo.py` | `repo_setup.validate_target_root` | Git-as-revert bootstrap primitive for one target repo (full port; example-doctrine-repo `.sh` becomes a thin polyglot trampoline over this) |
...
| `doe_drift.py` | example-doctrine-repo-HEAD conformance fixture resolver + drift-check |
...
| `policy_loader.py` | example-doctrine-repo-owned `auto-reconcile-policy.yaml` reader (`load_policy`) — fail-closed (absent=silent, malformed=warned) against the C9 grammar pin |
...
`percolate_run.py`/`percolate_validate.py` in `ops/`. Byte-parity with the retired example-doctrine-repo
...
Python port of the example-doctrine-repo coordinator JS text-manipulation primitives.
...
(`handoff.reconcile_open`, turnable-on) and `auto-reconcile-policy.grammar.md` (the example-doctrine-repo-owned
policy YAML's grammar pin).

### `subagent_sandbox/` — resolver + policy-load + provision/report seam
Python engine-ification of example-doctrine-repo's `block-reviewer-write-outside-sidecar.sh` PreToolUse hook
(Port of: block-reviewer-write-outside-sidecar.sh, example-doctrine-repo 8b29fa14, 2026-07-12; DR-047
...
shared resolver + policy-load layer and the `report_sidecar` provision/report path. Reads example-doctrine-repo's
