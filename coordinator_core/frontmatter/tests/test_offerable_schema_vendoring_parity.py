...
Companion to example-doctrine-repo's `test_every_ops_schema_has_vendored_counterpart`
(coordinator/tests/test_vendored_schema_version_parity.py) — that test
sweeps example-doctrine-repo's schema source directory; this one sweeps the manifest's
...
Example-doctrine-repo checkout.
...
All twelve were vendored the same day from example-doctrine-repo's committed bytes at
...
that a type WITHOUT one is legitimately excluded — that is example-doctrine-repo's
...
def test_every_offerable_doc_type_has_vendored_schema(schema_name: Optional[str]) -> None:
    ...
    if schema_name is None:
        pytest.skip(
            "manifest unresolvable (no live example-doctrine-repo checkout on this machine) -- "
