...
for coordinator_core.frontmatter.schema_cli, ported from example-doctrine-repo's own
...
Claude-klabauter-local parity subtleties called out in the chunk spec (each its own
...
ports the example-doctrine-repo bats *expectations* into pytest assertions against the local
