...
class TestWriteSidePreservesTrailingComment:
    """2026-08-01 break-class fix (example-cockpit-repo repro): `replace_fm_field`/
    ...
    section (the cockpit repro itself, quote-awareness, glued-hash,
...
# nested-block key directly (the Staff Engineer F4) — the mechanism, not just the new
...
class TestCRLFPresentButEmptyKey:
    ...
    def test_cockpit_repro_preserves_the_qualifier_comment(self, eol):
        """The literal example-cockpit-repo repro: a status stamp must not destroy
