...
Why this exists: claude-klabauter vendors example-doctrine-repo's schemas under
...
NEGATIVE SPEC -- what this is NOT:
  ...
  - NOT a byte hash of the file. A byte hash trips on reindentation, key
    ...
    validator's behaviour -- and on a schema example-doctrine-repo and claude-klabauter hold
  ...
  - NOT a cross-repo comparison. Three gates already compare claude-klabauter's copy to
    example-doctrine-repo's (`schema_drift_watch.scan_vendored_schema_drift`,
    ...
    shape edit has not reached example-doctrine-repo either. The axis this module serves is a
...
(AC1, AC2; chunk C1). The annotation-stripping RULE is borrowed from example-doctrine-repo's
