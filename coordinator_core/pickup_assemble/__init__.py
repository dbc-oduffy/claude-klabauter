...
Consumes manifest (the Director of Engineering F3, AC16) — orchestrates, reimplements none:
    ...
    coordinator_core.ops.fleet.memo_check_addressee.compute_check_addressee_candidate
        ...
        (2026-07-26 subprocess-elision spinoff — `coordinator/bin/cross-repo-memo
...
def _is_spinoff_kind(kind: str | None) -> bool:
    ...
    `predecessor: none`, not a continuation, and pickup/SKILL.md's
    banner-prepend logic depends on the classifier recognizing all three,
    not just the bare `spinoff` literal.
...
def _ready_summary(
    ...
    separation: past-tense fact, then a distinct future-tense instruction).
    ...
    if blocked_by:
        narration = (
            f"Computed the brief: {len(directives)} directive(s) ready, "
        ...
        narration = f"Computed the brief: {len(directives)} directive(s) ready to run."
