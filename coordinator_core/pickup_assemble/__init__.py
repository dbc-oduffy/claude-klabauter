...
def resolve_repo_root(start: Optional[Path] = None) -> Optional[Path]:
    ...
    `--show-toplevel` re-anchors `cwd` to the worktree it is actually inside —
    via `_discover_git_dirs(cwd)` served in-process by `_dispatch_git_readmodel`,
    not a git spawn (`rev-parse` is not in `_RUN_GIT_SPAWN_VERBS`). Building a
    path by joining onto `cwd` directly — or "simplifying" this to return `cwd`
    when the git call looks skippable — reintroduces that defect silently, and
    nothing warns. A third shape reintroduces it just as silently: re-anchoring
...
# `compute_repo_identity_gate` and its plausibility-band helper were
# extracted to `coordinator_core.repo_identity_gate` (C1,
# `pln-a-ceremony-must-not-be-able-to-5e9421`) so the ~25 `coordinator/bin`
# CLIs reaching it via `coordinator.bin.lib.repo_identity.resolve_checked_repo_root`
# stop importing this 10,072-line module (~360ms) just to reach a mechanical,
# zero-spawn comparison. Re-exported here at parity — behaviour, verdict
# vocabulary, and memoization semantics unchanged — so existing importers of
# `coordinator_core.pickup_assemble.compute_repo_identity_gate` keep working.
from coordinator_core.repo_identity_gate import (  # noqa: E402
    compute_repo_identity_gate,
    _repo_identity_plausible_cwd,
...
def main(argv: list[str]) -> int:
    ...
    print(
        f"pickup-assemble: 'brief' now lives in coordinator_core.pickup_brief "
        "-- this module no longer computes it, and only handles apply/drop/"
        "stamp-check.",
        file=sys.stderr,
    )
    return _usage("pickup-assemble")
