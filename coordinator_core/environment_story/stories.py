...
function-local `from _environment_story_omission_ledger import
RULE_BEARING_DISPOSITIONS`, reached only from the source module's
`__main__` block) stays DoE-resident. `_derive_register_rule_ids`,
`_read_ratified_omissions` and the `if __name__ == "__main__":` regenerator
are DELETED here -- only the DERIVED constant moved, as data. This module
imports nothing named `omission_ledger` and carries no `__main__`
regeneration block (AC, docs/plans/2026-09-18-doe-holds-no-scripts.md
chunk W2-C2). Regeneration continues to happen against the DoE-resident
doctrine-rule-class register and lands here as an updated committed
constant, same as any other cross-repo data port.
...
DoE-resident regenerator (see PORTING NOTE above -- it does not live in
this module). That generator is a stdlib-only LINE SCAN over the register's
...
Regenerate via the DoE-resident regenerator (see PORTING NOTE above),
