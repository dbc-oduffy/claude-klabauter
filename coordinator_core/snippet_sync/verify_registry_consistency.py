...
(example-doctrine-repo 93887f6f, 2026-07-17, bash, 721 LoC), which cross-checked
...
scripts are retired everywhere — deleted from example-doctrine-repo's `coordinator/bin` at
...
Consumed by example-doctrine-repo-side `coordinator/bin/verify-snippet-registry-consistency`
polyglot trampoline.

Spec backlinks:
  - example-doctrine-repo docs/plans/2026-06-15-snippet-sync-consumer-registry.md § Dispatch Ledger C4, C8
  - example-doctrine-repo docs/decisions/2026-06-15-snippet-registry-shape.md § Schema amendments — the Staff Engineer C2
  - cross-repo/archive/2026-07-22-claude-central-em-snippet-registry-consistency-fix-locus.md
"""
...
def run(plugin_root: Path) -> ConsistencyOutcome:
    """Run all checks against `plugin_root` (the example-doctrine-repo coordinator plugin root
...
def main(argv: list[str]) -> int:
    ...
    path string) — injected by the example-doctrine-repo trampoline, which computes it exactly
