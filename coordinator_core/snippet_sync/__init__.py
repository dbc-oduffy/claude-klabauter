...
Port of coordinator-claude-side `coordinator/bin/snippet-registry` (bash CLI, 477 LoC,
...
Modules:
  registry.py — reads `snippets/registry.toml`, resolves per-snippet
                ...
                TOML parsing via `tomllib`/`tomli`. Consumed by coordinator-claude-side
  ...
  verify.py   — the shared verify/--fix/--list engine. Per-snippet
                ...
                per-script code branches. Consumed by coordinator-claude-side
