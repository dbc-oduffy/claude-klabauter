"""
Tests for coordinator_core.claude_klabauter_root — CLAUDE_KLABAUTER_ROOT resolver primitive.
...
from coordinator_core import claude_klabauter_root as mr


def test_rung1_env_var_short_circuits(monkeypatch):
    monkeypatch.setenv("CLAUDE_KLABAUTER_ROOT", "/tmp/from-env")
    assert mr.coordinator_claude_klabauter_root() == "/tmp/from-env"
...
def test_rung1_5_pointer_file_used_when_env_unset(monkeypatch, tmp_path):
    ...
    assert mr.coordinator_claude_klabauter_root() == "/tmp/from-pointer"
...
def test_rung1_5_falls_through_when_pointer_absent(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError) as exc:
        mr.coordinator_claude_klabauter_root()
...
def test_rung2_machine_local_cli_used_when_pointer_absent(monkeypatch, tmp_path):
    ...
    assert mr.coordinator_claude_klabauter_root() == "/tmp/from-registry"
...
def test_env_var_wins_over_pointer_file(monkeypatch, tmp_path):
    ...
    assert mr.coordinator_claude_klabauter_root() == "/tmp/from-env-wins"
