"""
test_emit_grind_vocab_pin — design-time gate pinning the queue-grind
vocabulary emission surface: `contract.grind_vocab.emit_vocab` is DoE's
regeneration path for `queue-grind-profile.schema.json`'s imported
vocabulary (§ Design § Vocabulary contract, docs/plans/2026-09-21-bug-blitz-
emitter-engine-leg.md's C1 row). This mirrors
`coordinator_core/contract/tests/test_emit_schema_pin.py`'s own three-part
recipe: symbol path, `out_dir` precedence, and bytes against committed
goldens that are never regenerated by the code under test.

A red run here is a heads-up to DoE-claude, never a golden refresh — a
silent rename or shape change here would desync DoE's profile schema
without either side's test suite noticing.
"""
...
def test_engine_concurrency_ceiling_is_pinned_at_or_below_sixteen():
    # eng-director F4, kept pinned per overengineering-reviewer #5.
...
def test_emitted_bytes_are_pinned_for_grind_vocab_json(tmp_path):
    ...
    json.loads(emitted_bytes)  # still valid JSON
