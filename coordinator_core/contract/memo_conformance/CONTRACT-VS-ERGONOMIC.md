...
byte-compare — claude-klabauter's native ops deliberately diverge from the DoE CLI's bytes on
...
| Site 2 — schema `applies_to` | same | Frontmatter schema-validity is the contract; claude-klabauter's emitted memo must satisfy DoE's schema regardless of who wrote it. |
...
| `test_premise_check_advisory_*` (6 tests) | `cross-repo-memo.test.py` | Exercises a DoE-side PreToolUse advisory hook (`memo-pre-dispatch-guard`) reacting to the CLI's own dispatch path in DoE's repo. Hooks are discovery-resolved surface claude-klabauter does not own (see claude-klabauter CLAUDE.md § Project Overview — plugin/hook surfaces stay in coordinator-claude/DoE). |
...
2026-07-21. This is a bug in DoE's OWN fixture harness, not a claude-klabauter regression — claude-klabauter
