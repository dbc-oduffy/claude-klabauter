"""
cockpit_schema — pydantic port of example-doctrine-repo `coordinator/cockpit-contract` (Zod source).
...
# cockpit_schema.emit_schema` (example-doctrine-repo's `regen-cockpit-schema.py:208`
# invocation shape) — runpy finds the submodule already imported under its
# real name and emits a `RuntimeWarning` on example-doctrine-repo's release-critical
