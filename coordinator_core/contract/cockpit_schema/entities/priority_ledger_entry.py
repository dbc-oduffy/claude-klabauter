...
projected into the cockpit contract. Pydantic port of example-doctrine-repo
...
which stays OUT of `ENTITY_SCHEMAS` by design because it has no example-doctrine-repo-hosted
contract counterpart and is validated shape-based instead
(`validate.py:440-467`). This entity DOES need a example-doctrine-repo-hosted contract
counterpart: `state/cross-repo-commitments/2026-07-26-doe-owns-durable-
priority-ledger-for-cockpit.yaml` commits example-doctrine-repo to publishing the ledger shape
to cockpit, so `ENTITY_SCHEMAS` membership (and the example-doctrine-repo-side JSON-schema
