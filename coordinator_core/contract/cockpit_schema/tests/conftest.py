...
Every coordinator-claude-clone-backed test in this package (fixtures/*.json, schema/*.json,
conformance/**, provenance-parity's sibling artifact-shape-contract schema,
verify-superseded-retirement's CLAUDE.md/wiki greps) resolves the coordinator-claude local
...
Coordinator-claude's fixtures/schema/test-data — it is a READ, at test-time, of the coordinator-claude
...
exists — coordinator-claude commit 7cca4d4c ("delete redundant cockpit-contract TS mirror +
...
Parity oracle: coordinator-claude coordinator/cockpit-contract/test/*.ts (vitest, retired 7cca4d4c — historical parity
...
# ---------------------------------------------------------------------------
# coordinator-claude clone resolution — module-level (not fixture-scoped) so parametrized
...
skip_no_doe = pytest.mark.skipif(
    not DOE_AVAILABLE, reason="coordinator-claude clone not available on this machine (repos.example_doctrine_repo unset)"
)
...
skip_no_fixtures = pytest.mark.skipif(
    ...
    reason=(
        "coordinator-claude cockpit-contract fixtures/ not available on this machine "
        "(coordinator-claude clone absent, or fixtures/ retired — 2026-07-16 C3 7cca4d4c)"
    ),
)
skip_no_schema = pytest.mark.skipif(
    not SCHEMA_AVAILABLE,
    reason="coordinator-claude cockpit-contract schema/ not available on this machine (coordinator-claude clone absent or schema/ missing)",
)
skip_no_ts_mirror = pytest.mark.skipif(
    not TS_MIRROR_AVAILABLE,
    reason=(
        "coordinator-claude cockpit-contract TS mirror (src/) not available on this machine "
        "(coordinator-claude clone absent, or TS mirror retired — 2026-07-16 C3 7cca4d4c)"
    ),
)


def load_fixture(name: str) -> dict[str, Any]:
    """Load `fixtures/<name>.json` from the coordinator-claude clone. Caller must be
...
def load_schema(name: str) -> dict[str, Any]:
    """Load `schema/<name>.schema.json` from the coordinator-claude clone (committed,
...
def doe_clone() -> Path:
    if not DOE_AVAILABLE:
        pytest.skip("coordinator-claude clone not available on this machine")
...
def fixtures_dir(doe_clone: Path) -> Path:
    if not FIXTURES_AVAILABLE:
        pytest.skip(
            "coordinator-claude cockpit-contract fixtures/ not available (retired — 2026-07-16 C3 7cca4d4c)"
        )
...
def schema_dir(doe_clone: Path) -> Path:
    if not SCHEMA_AVAILABLE:
        pytest.skip("coordinator-claude cockpit-contract schema/ not available")
...
# data)` / `zod_safe_parse_ok(entity, data)` uniformly, mirroring the coordinator-claude
