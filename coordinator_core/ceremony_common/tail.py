...
`emit-cadence` with no args, both unconditional (`depends_on=None`,
`already_satisfied=False`). AC9's byte-identity check confirmed the two
tails are identical in every load-bearing field once workweek's separate
`hard_block` bookkeeping pass is accounted for (see negative-spec below)
— this module factors the shared pair so neither assembler hand-maintains
its own copy.
