...
Negative-spec:
    ...
    - ``write_review_trail`` is NOT shed by C6, and MUST NOT be shed as a follow-on "cleanup"
      ...
      PREDECESSOR reader: example-doctrine-repo's chain-end ``review-coverage-gate.py`` (SKILL.md:28) requires
...
Spec backlink: docs/plans/2026-07-16-wsc-pure-python-tail-rebuild.md § C6
Spec backlink: docs/plans/2026-07-23-wsc-tail-slim-down.md § C2
Spec backlink: docs/plans/2026-07-23-wsc-tail-slim-down.md § C6
Behavior reference (read for behavior, not structure, per the plan's Anti-scope):
    ...
    _cs_claim_held_by_me (example-doctrine-repo e34f2484, 2026-07-22).
"""
...
def refresh_roadmap_callout(worktree_root: Path, consumed_handoff_paths: List[str]) -> TailResult:
    ...
    mirroring the example-doctrine-repo pickup skill's allowlist guard.
