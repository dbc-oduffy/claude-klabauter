...
from coordinator_core.ops.ceremony.push import (
    ...
    _is_indeterminate_push_result,
    _is_push_reject,
    _pushed_range_diagnostic,
    _resolve_pushed_range,
    derive_push_status,
...
def explicit_stage(
    ...
    if ignore_indeterminate:
        ...
        if held:
            ...
            # `staged_paths` was already composed from `existing` above, so
            # it must be filtered here too. Reporting a path as staged when
            # it was deliberately held back is worse than the original
            # over-staging bug: a caller's rollback bookkeeping would try to
            # unstage a path this call never wrote.
            staged_paths = [p for p in staged_paths if p not in ignore_indeterminate]
            for p in held:
    ...
    try:
        ...
        add_result = _StageOk()
    except index_write.IndexWriteLockBusy as exc:
        # A peer holds `.git/index.lock`. This is contention on a shared
        # worktree, not a defect in this call -- `git add` refused the same
        # way rather than stealing the lock. Named separately so a caller
        # (and a reader of the failure text) can tell "retry in a moment"
        # apart from "this is broken", which the bare `except Exception`
        # below cannot express.
        add_result = _StageFailed(f"index lock held by a peer: {exc}")
...
#: Substrings `_agree_branch_cas_refusal` and `_commit_via_head_spine`'s
#: AC11(b) index re-check (git_native.py) each lead/embed in their own
#: `stderr` -- the ONLY way `commit()` can distinguish either refusal from
#: `commit_scoped()`'s other failure shapes without `git_native.py` itself
#: carrying a `reason` field (out of this chunk's file scope; see
#: docs/plans/2026-08-14-the-tool-stages-what-it-commits.md chunk C3's own
#: `writes:` list). Imported directly from `git_native` rather than
#: re-typed here (C1, claude-klabauter-75) -- independently-typed prose is
#: exactly how this marker match diverged from `_commit_via_head_spine`'s
#: wording in the first place; a shared constant can't diverge that way.
_INDEX_HEAD_CAS_MARKER = git_native.INDEX_HEAD_CAS_MARKER
_INDEX_STAT_CAS_MARKER = git_native.INDEX_STAT_CAS_MARKER
...
def _classify_commit_scoped_failure_reason(result: "git_native.GitResult") -> str:
    ...
    CAS refusal (`_agree_branch_cas_refusal`), `_commit_via_head_spine`'s
    own AC11(b) index-`stat_identity` CAS refusal (C1, claude-klabauter-75),
    or every other commit failure (a `pre-commit` hook BLOCK, the
    private-index branch's own `update-ref` compare-and-swap, a genuine git
    error, or `_commit_via_head_spine`'s UNRELATED ref-CAS "HEAD moved"
    failure). Never called for the ordinary already-committed no-op --
    that reclassification happens one layer up, in `scoped_git_commit.
    _classify_uncommitted`, which alone has the `git status` probe the
    distinction needs.
    """
    stderr = result.stderr or ""
    if _INDEX_HEAD_CAS_MARKER in stderr or _INDEX_STAT_CAS_MARKER in stderr:
