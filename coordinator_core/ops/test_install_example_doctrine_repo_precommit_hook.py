"""Tests for coordinator_core.ops.install_example_doctrine_repo_precommit_hook.
...
`_resolve_doe_root()` / `_resolve_example_doctrine_repo_target()` — an unresolved
`repos.example_doctrine_repo` (AC3) must be a clean advisory skip, never a traceback,
never a block.
...
from coordinator_core.ops.install_example_doctrine_repo_precommit_hook import (
    _GATE_REGISTRY,
    main,
)
import coordinator_core.ops.install_example_doctrine_repo_precommit_hook as _mod
...
def test_non_example_doctrine_repo_repo_root_is_skipped_cleanly(tmp_path, monkeypatch, capsys):
    other = tmp_path / "some-other-repo"
    other.mkdir()
    _git_init(other)
    # Identity anchor resolves somewhere else entirely — `other` is not it.
    monkeypatch.setattr(_mod, "_resolve_doe_root", lambda: str(tmp_path / "not-this-one"))
...
def test_unresolved_doe_root_is_a_clean_advisory_skip_not_a_block(tmp_path, monkeypatch, capsys):
    ...
    `repos.example_doctrine_repo` and never raises. This installer must treat that as a
