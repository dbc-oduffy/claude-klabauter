...
Exit codes (parity-critical — both callers branch on these):
  ...
  1 — one or more checks failed (diagnostic printed to stdout), OR the skill
      file / chunk manifest was not found, OR the example-doctrine-repo repo root could
...
verifies live in the example-doctrine-repo repo — the bash oracle never needed this
resolution step because it ran FROM inside that repo (SCRIPT_DIR-relative).
This port resolves the example-doctrine-repo root via
...
def run(
    ...
    """Core driver: parse args, resolve the example-doctrine-repo repo root, run static
    ...
    if not repo_root_str:
        return (
            [
                "ERROR: could not resolve the example-doctrine-repo repo root (needed to locate "
