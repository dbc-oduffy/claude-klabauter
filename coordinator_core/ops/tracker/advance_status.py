...
Ambiguity note (Bar: "implement the conservative reading and state the ambiguity"):
  - No tracker README on disk anywhere in DoE-claude or claude-klabauter was found
...
(self-commit) — READ BEFORE treating this as settled precedent:
  ...
  PM-ratified, authorized mutating carve-out per `DoE-claude
...
Spec backlink: DoE-claude coordinator/docs/wiki/delegate-execution.md § Phase 1.5, Re-dispatch budget
Spec backlink: DoE-claude commit f83fbb52 (names this op; documents the prior-art gap)
Spec backlink: DoE-claude docs/decisions/DR-094-tracker-advance-status-write-target-carveout.md
