...
Exit codes (parity-critical — both callers branch on these):
  ...
  3 — example-doctrine-repo trampoline transport failure (CLAUDE_KLABAUTER_ROOT resolution / import
      failure) — NEVER returned by this module itself; allocated by
      coordinator/bin/draft-plan-aging.sh (example-doctrine-repo) as a dedicated code so
...
def _normalize_prefix(path: str) -> str:
    ...
    resolve against this tree — the example-doctrine-repo-vs-published prefix trap.
    """
...
#
# Ownership predicate (spec of record: example-doctrine-repo
...
# built on C1's resolve_plan_owner. Spec of record: example-doctrine-repo
# coordinator/docs/wiki/coordinator-tripwires.md § PLAN-ORPHAN-OWNERSHIP,
...
# cross-repo/inbox/2026-08-03-example-doctrine-repo-em-two-rulings-plan-orphan-population-and-dr088-antiscope.md
# § 1, example-doctrine-repo coordinator/docs/wiki/coordinator-tripwires.md §
...
#
# `legacy_unjoinable` bucket (example-doctrine-repo ruling 95bf1cc4d, AC1a rescoping): a
...
# 2026-08-06 (example-doctrine-repo ruling 80b0b29fb) because the schema documents that value
