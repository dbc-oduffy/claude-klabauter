...
Purpose: Two-part blocked-work detector, unchanged from the bash oracle:
  1. Handoffs with `status: blocked` or `status: paused` — via
     `coordinator_core.ops.ceremony.records_query.query_records`, claude-klabauter's native records
...
Negative-spec:
    - Does NOT parse frontmatter directly for handoffs — delegates to
      ...
      where="status in (blocked,paused)")`, claude-klabauter's own native records seam (repointed
