...
def _resolve_doe_root() -> str:
    ...
    on an unresolved `repos.example_doctrine_repo` — never raises (see
...
def _hook_body(gates: List[_Gate]) -> str:
    ...
    lines = [
        *_BODY_HEADER.rstrip("\n").split("\n"),
        "# Registry-driven (coordinator_core.ops.install_example_doctrine_repo_precommit_hook);",
...
def main(argv: List[str]) -> int:
    ...
    repo_root = _resolve_example_doctrine_repo_target(target)
