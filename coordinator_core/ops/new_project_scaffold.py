...
Port of: new-project-scaffold.sh (example-doctrine-repo 290997c7, 2026-07-22)
Spec backlink: docs/plans/2026-06-22-new-project-bootstrap-skill.md § C3

Example-doctrine-repo-root resolution: the templates/ tree (coordinator/skills/new-project/templates/) still
lives in the example-doctrine-repo clone, so a example-doctrine-repo-root resolution remains needed for that lookup.
...
relative to this repo's own root — no example-doctrine-repo-root hop needed to find it. Only if the
co-located sibling is somehow absent does `_find_render_tree` fall back to the legacy
Example-doctrine-repo-root lookup. That example-doctrine-repo-root resolution order (env override, then `machine-local`
...
Exit-code contract (matches the bash oracle's own documented table):
    ...
    1  Pre-flight failure (occupied dir, bad args, missing example-doctrine-repo-side siblings), a
       ...
       (machine-local, to locate the example-doctrine-repo clone) shares exit 1 with pre-flight failure,
...
def _co_located_render_tree() -> Optional[str]:
    ...
    executable-surface migration (example-doctrine-repo commit b644d5a9) -- it is now
    claude-klabauter's OWN sibling executable, not example-doctrine-repo-resident content, so it is
    resolved relative to this repo unconditionally, ahead of any example-doctrine-repo-root
    lookup (env override or registry alike). The example-doctrine-repo-root fallback below is
...
def _find_render_tree(doe_root: str) -> Optional[str]:
    """Locate render-template-tree.py: co-located first, then the example-doctrine-repo root."""
...
def _register_repo(project_name: str, target: str) -> int:
    ...
    get repos.<slug>`. This is what lets the example-doctrine-repo new-project skill drop its
...
def main(argv: List[str]) -> int:
    ...
    # -----------------------------------------------------------------
    # Self-register repos.<slug> in machine-local (kills the example-doctrine-repo-side
