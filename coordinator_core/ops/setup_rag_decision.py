...
GENERATES = []  # appends the un-indexed tripwire block to <root>/CLAUDE.md where root is a caller-supplied --root/cwd target repo (any repo /repo-setup targets), not a fixed path inside claude-klabauter's own tree
