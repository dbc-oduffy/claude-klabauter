...
class TestNewTypeGlobCoverage:
    ...
    2026-07-22 — 3 live DoE runtime consumers, same false-empty shape),
...
class TestTypeToGlobDerivedGate:
    ...
    ``build_type_to_glob`` (schema-derived from DoE's ``coordinator/schemas/
...
class TestSiblingExclusionDerivedFromWiredSet:
    ...
    only from ``_TYPE_TO_GLOB``) is sound ONLY as long as no UNWIRED DoE
