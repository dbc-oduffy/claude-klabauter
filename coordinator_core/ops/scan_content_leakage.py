...
Purpose: replaces the percolate Step 2c fence (example-doctrine-repo skills/percolate/SKILL.md
...
``setup/.percolate-identity`` → ``PERSONAL_REVIEW_PATTERNS`` leg) stay in example-doctrine-repo
...
Port source: example-doctrine-repo coordinator/skills/percolate/SKILL.md § Step 2c (bash fence)

Negative-spec (hard-won):
  - Does NOT scan for operator-identity tokens (PERSONAL_REVIEW_PATTERNS) —
    that detector stays in example-doctrine-repo publish.py by settlement; folding it in here
