...
Purpose: claude-klabauter-native port of the example-doctrine-repo-owned `coordinator/whoami/run-tests.sh`
...
jsonschema dependency) nested inside the example-doctrine-repo meta-repo — bare `pytest` under
...
owns the pure provisioning/launch logic; the example-doctrine-repo trampoline owns resolving
its own directory and passing it in.

Behavior mirrors the bash oracle line-for-line:
  1. Resolve `base_dir` (the example-doctrine-repo trampoline passes its own directory — the
...
Port of: run-tests.sh (example-doctrine-repo 6fb5fb37, 2026-07-22)
...
Negative-spec:
  ...
  - Does NOT register as a JSON-RPC op (no `register_op`) — this is a
    ...
    example-doctrine-repo trampoline imports `main` in-process, no IPC round-trip.
"""
...
def main(argv: list[str], base_dir: str | None = None) -> int:
    ...
    `base_dir` defaults to os.getcwd() for standalone testability; the example-doctrine-repo
    ...
    # pytest. The example-doctrine-repo trampoline imports `main` in-process, so this inherits
