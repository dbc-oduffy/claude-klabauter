...
Example-doctrine-repo 2026-07-13, cross-repo/archive/2026-07-13-claude-central-em-doe-auto-reconcile-ratifications.md).
...
invariant; auto-mutation of work-state warrants opt-in until example-doctrine-repo ratifies flip-to-false
via the C6 co-memo).
...
Negative-spec:
  ...
  - Does NOT auto-transition a PROSE gate_dependency verdict — surfaces only,
    regardless of the C3 verdict value, per example-doctrine-repo alignment reply #3 (EM judgment
...
integration pass, not oversights:
  - Schema declaration: `handoff.schema.json`'s grammar SSOT is
    `coordinator/schemas/handoff.schema.json` in example-doctrine-repo, one-way vendored
    ...
    the vendored copy in place without a matching example-doctrine-repo-side SSOT edit +
    re-vendor makes the vendored copy diverge from example-doctrine-repo HEAD and fails that
...
`coordinator/auto-reconcile-policy.yaml`'s `dry_run` to `false` in example-doctrine-repo.
...
load-bearing so that THIS precondition is enforceable by a plain example-doctrine-repo-side
...
def _build_dry_run_report(
    ...
    (`cross-repo/inbox/2026-07-28-example-doctrine-repo-em-handoff-terminal-starvation-
    answers.md`).
...
def _resolve_dry_run(policy: Dict[str, Any], params: Dict[str, Any]) -> "tuple[bool, Optional[Dict[str, Any]]]":
    ...
    Arming is therefore only ever an explicit, present, valid, example-doctrine-repo-authored
    ...
    `params[_DRY_RUN_OVERRIDE_REASON_PARAM]` string:
      - reason present and non-empty -> override APPLIED, logged at WARNING
        (this is an exceptional escape from a example-doctrine-repo-declared posture — it
    ...
    _LOG.warning(
        ...
        "deferring to policy (example-doctrine-repo owns rules, not a bypassable precedence "
        "convention)",
...
def _is_open(meta: Dict[str, Any]) -> bool:
    ...
    # example-doctrine-repo lvv-04/C3 forward-compat — lockstep-update when consumed->claimed lands:
    # if example-doctrine-repo's lifecycle-vocab roadmap (lvv-04/C3) renames consumed->claimed or adds
...
async def _reconcile_ancestor_chain(
    ...
    DR-096 (example-doctrine-repo 2026-07-26 ruling) makes that distinction visible on disk,
    ...
    for ancestor_abs in ancestor_paths:
        ...
        try:
            ...
            gate = evidence["gate"]
            # DR-096 (example-doctrine-repo 2026-07-26 ruling): gate (a) stamps the SUCCESSOR's
...
async def _handler(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    Params:
        ...
        dry_run_override_reason (str, optional) — required, non-empty, ONLY when
                ...
                `dry_run` value; a named, logged escape from example-doctrine-repo's declared
    ...
    for handoff in open_handoffs:
        ...
        if deployment_state == _AWAITING_GATE_STATE:
            ...
            if verdict == "surface":
                ...
                # verdicts (surfaces only, per example-doctrine-repo alignment reply #3).
