...
Port of: verify-schema-registry-sync.sh (example-doctrine-repo b5a4192c, 2026-07-20),
snapshotted 2026-07-16. Positive corpus = the example-doctrine-repo repo's live
schemas/ dir. Negative corpus = missing schemas dir.

`test_golden_oracle_parity_against_live_doe_repo` (the file's one
live-example-doctrine-repo-repo test) was frozen to a committed golden via
...
# example-doctrine-repo sibling's live schemas/ dir) silently covers less and less of
...
def test_resolve_plugin_root_delegates_to_data_root(tmp_path, monkeypatch):
    ...
    schemas/ moved to a example-doctrine-repo-resident split-repo layout."""
...
def _find_doe_root() -> Optional[Path]:
    """Resolve the example-doctrine-repo sibling repo's coordinator/ dir via the pointer-file
...
def test_golden_oracle_parity_against_live_doe_repo():
    ...
    MISSING entries against the live example-doctrine-repo schemas/ corpus -- the 3
    ...
    from a live `vsrs.run()` against the example-doctrine-repo sibling checkout and
    ...
    example-doctrine-repo checkout needed at test time. `vsrs.run()` itself is now fully
    native (no node subprocess at all, post de-node port), so recapture no
    longer needs `node` on PATH either -- only the example-doctrine-repo sibling
    ...
    (requires the example-doctrine-repo sibling checkout to be resolvable via the
    .doe-root pointer file -- see coordinator_core.doe_root_pointer -- not
    needed for an ordinary run).

    Negative-spec: does NOT `pytest.skip` when the live example-doctrine-repo repo is
    ...
    if is_capturing():
        ...
        if doe_root is None:
            raise RuntimeError(
                "CAPTURE_GOLDENS=1 recapture requires the example-doctrine-repo sibling "
