...
Negative-spec:
  ...
  - Does NOT require, validate, or enumerate the sentinel's full key set. This
    ...
    module ever needs to read `vendor_drift` (a future example-doctrine-repo-side ask), do so via
  ...
  - Does NOT re-derive CLAUDE_KLABAUTER_ROOT via the shell resolution ladder — this
    module IS running from inside the resolved claude-klabauter root (the example-doctrine-repo-side
  ...
  - A sentinel-unreadable CLAUDE_KLABAUTER_ROOT (resolution itself failing) is not this
    module's concern — that failure mode is caught by the example-doctrine-repo-side trampoline
