...
test (mirrors the example-doctrine-repo stub's own direct-import call shape, not the async IPC
handler).

Coverage (reconciliation lens only — the pre-existing clobber-lens tests
live in the example-doctrine-repo-side pytest port, coordinator/tests/test_guard_settings_
...
unchanged by this addition and already covered by the example-doctrine-repo-side hook tests.
"""
