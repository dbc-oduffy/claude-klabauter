...
C4b substrate (this module, layered onto C4a's mechanism):
  - Heir determination is the `continued_into` field on the candidate's OWN
    frontmatter — the forward succession edge `archive_stamp`'s supersede
    mode writes (with `deployment_state: continued`) when a successor is
    created, and re-reads to verify before it proceeds. No corpus, no
    reverse-edge index, no `dag_index`: the edge is recorded once by the
    writer that knows both ends, and this op reads it back off bytes it
    already holds.
  - It replaced `archive_handoffs._classify_heir_children` over
    `archival.reverse_membership`, which answered the same question by
    building a reverse-edge index across the whole live corpus on every
    boot — a corpus-wide scan to rediscover a fact the system already
    records. That is also why `_is_terminal` was never called: its Branch-B
    and heir arms spawn `git cat-file -e` via `_shipped_in_resolvable`,
    which AC1 forbids. Neither is reachable from this module now.
...
Negative-spec:
  ...
  - Imports NOTHING from `archive_handoffs` — not `_is_terminal`, whose
    Branch-B and heir arms spawn `git cat-file -e` via
    `_shipped_in_resolvable` against AC1's zero-git-QUERY-spawn contract,
    and no longer `_classify_heir_children` either. Heir (e) suppression
    stamps `shipped` unconditionally (mirrors the retired composite's FIX-1
    revision, which made `_resolve_heir_deployment_state` single-valued)
    rather than re-deriving a resolvable-shipped_in verdict, so no
    resolvability check is needed here at all.
  - Does NOT scan the corpus to find a candidate's successor. The edge is
    stamped at write time; see `_apply_dr084_disposition`'s own negative
    spec for why reintroducing a scan to cover pre-stamp records is the
    wrong repair.
...
# Per-boot archival bite (PM ruling 2026-08-23). The backstop fires once per
# session on a box running dozens of them, so a bounded bite still drains the
# queue — it just refuses to let one boot's cost scale with however far the
# corpus has drifted. NOT a truncation: the remainder is counted into the
# envelope as `deferred_to_next_boot` and taken by the next boot, oldest
# first. Sized from measurement, not taste — relocation runs ~2ms per file,
# so 25 is ~50ms of the ~250ms end-to-end budget.
_MAX_CANDIDATES_PER_BOOT: int = 25
...
def _append_warn_marker(
    ...
    """
    _append_warn_markers(worktree, [(handoff_filename, sid, disposition_note, verb)])


def _append_warn_markers(
    worktree: Path,
    rows: Sequence[Tuple[str, str, str, str]],
) -> None:
    """The batch form of `_append_warn_marker`: ONE read of
    tasks/orphan-sweep-notes.md and ONE append handle for the whole
    disposition set, whatever its size.

    `rows` are `(handoff_filename, sid, disposition_note, verb)` tuples, in
    the order they should appear in the file.

    Same DR-084 (d) semantics as the single-row form, dedup key included:
    a row whose `(verb, handoff_filename, sid)` triple already appears in the
    file's CURRENT on-disk contents is skipped, and `disposition_note` is not
    part of the key. In-batch duplicates are dropped the same way, which is
    what the per-row form got for free by re-reading the file between every
    append — the equivalence holds because nothing else writes this file
    while the sweep is running, so a mid-batch re-read could only ever
    return the batch's own rows.

    Why a batch form exists at all: the per-row shape reads the whole
    (growing) marker file and opens it twice PER MARKER. At the 25-candidate
    per-boot cap that is 25 full reads and 50 opens of one file, measured at
    ~171ms of a 491ms profiled run — the single largest term in the
    backstop's own work, and the same repeated-read shape the corpus scan
    was already cured of. Fails silently on any I/O error, per the single-row
    contract: this runs on the session-boot path and must never wedge a boot.
    """
    if not rows:
        return
    ...
    try:
        existing_text = marker_path.read_text(encoding="utf-8") if existed else ""
    except OSError:
    ...
    for handoff_filename, sid, disposition_note, verb in rows:
        dedup_fragment = f"| {verb} {handoff_filename} (consumed_by={sid}"
        if dedup_fragment in seen_in_batch:
            continue
        if existing_text is not None and dedup_fragment in existing_text:
            continue
        seen_in_batch.add(dedup_fragment)
        pending.append(
...
def enumerate_handoffs(worktree: Path) -> List[Path]:
    ...
    """
    return [path for path, _mtime in enumerate_handoffs_with_mtime(worktree)]


def enumerate_handoffs_with_mtime(worktree: Path) -> List[Tuple[Path, float]]:
    """Every live state/handoffs/*.md file with its mtime, sorted by path.

    One `os.scandir` pass, taking mtime off the directory entry the OS already
    handed back — `DirEntry.stat()` is served from the scandir buffer on both
    Windows and Linux, so this costs no extra syscall per file and, crucially,
    no OPEN. Opening is the expensive part (~0.43ms/file cold on this box,
    against ~0.02ms for a buffered stat), and `_read_set` below uses these
    mtimes to decide which files need opening at all.
    """
    ...
    with os.scandir(live_root) as entries:
        for entry in entries:
            ...
            try:
                ...
                # Vanished between listing and stat (a peer archived it
                # mid-scan). Not ours to report — it is simply not here.
                continue
    out.sort(key=lambda item: item[0])
    return out


_SCAN_CACHE_NAME = "coordinator-boot-backstop-scan.json"
"""Incremental-scan cache, in the git COMMON DIR — never in state/.

It is a cache, not truth: a missing or unparseable one costs a full scan and
can never produce a wrong answer. It lives outside the worktree because this
op commits `state/` with `git add -A`, and a file that changes on every boot
would manufacture a commit on every boot."""


def _load_scan_cache(common_dir: Path) -> Tuple[Optional[float], List[str]]:
    """Return (scan_floor_mtime, carried_relpaths) from the cache.

    `(None, [])` means "no usable cache" — the caller falls back to reading
    the whole corpus, which is exactly the behaviour that predates this cache.
    """
    try:
        raw = (common_dir / _SCAN_CACHE_NAME).read_text(encoding="utf-8")
        blob = json.loads(raw)
        floor = float(blob["scanned_at"])
        carried = [str(p) for p in blob.get("carried", [])]
    except (OSError, ValueError, TypeError, KeyError):
        return None, []
    return floor, carried


def _write_scan_cache(common_dir: Path, scanned_at: float, carried: List[str]) -> None:
    """Best-effort: a cache that fails to write costs the next boot a full
    scan, which is correct, so this never raises into the boot path."""
    try:
        (common_dir / _SCAN_CACHE_NAME).write_text(
            json.dumps({"scanned_at": scanned_at, "carried": sorted(carried)}),
            encoding="utf-8", newline="\n",
        )
    except (OSError, TypeError) as exc:
        _LOG.debug("boot_backstop: could not write scan cache: %s", exc)
...
def _read_set(
    ...
    """Which handoffs this boot actually has to open.

    SOUNDNESS. A handoff's terminal-ness is a function of its own frontmatter,
    so it can only change by the file being WRITTEN. A file whose mtime
    predates the last completed scan was therefore already examined, under the
    same predicate, and found non-terminal — re-reading it cannot change the
    answer. That is the whole argument; there is no sampling and no heuristic
    in it.

    `carried` closes the one hole in that argument: a file found TERMINAL but
    not archived — deferred by the per-boot cap, or held in the (b)
    adjudication queue — is old on disk yet still owed. Those are named
    explicitly and always re-read, so the deferred queue drains instead of
    aging out of view.

    No floor (first boot, cleared cache, corrupt cache) reads everything.
    """
    if floor is None:
        return [path for path, _mtime in entries]
    # Matched on BASENAME, deliberately, and not on a resolved path. Every
    # entry comes from `enumerate_handoffs_with_mtime`, which lists exactly
    # one directory, so a basename is already unique across the set — while
    # `Path.resolve()` is a `_getfinalpathname` syscall per file on Windows.
    # Resolving here cost more than the opens this whole cache exists to
    # avoid, and measured SLOWER than no cache at all.
    carried_names = {rel.rsplit("/", 1)[-1] for rel in carried}
    return [
        path for path, mtime in entries
        if mtime >= floor or path.name in carried_names
    ]


_FRONTMATTER_HEAD_BYTES = 8192
"""Bytes read per handoff by `scan_corpus`. Sized against the live corpus:
272 handoffs, deepest closing `---` delimiter at byte 6500, median 940. A file
whose delimiter falls beyond this is re-read in full rather than misparsed —
the cap is a fast path, never a truncation the callers can observe."""


def scan_corpus(handoffs: List[Path]) -> dict:
    """Read every handoff's FRONTMATTER ONCE and keep it.

    The three phases downstream — candidate selection, the DR-084
    disposition, and the heir reverse-edge index — each independently walked
    this same corpus, reading the same bytes three times over. Measured at
    266 handoffs: 31ms + 47ms + 94ms. Reading once and passing the result
    down is the whole optimisation; nothing here is cached across calls, so
    there is no staleness window to reason about — the read happens inside
    the same op invocation that consumes it.

    Returns {Path: text}, where `text` is the file's leading
    `_FRONTMATTER_HEAD_BYTES` — enough to carry the whole frontmatter block
    and some body, not the whole file. A path whose closing `---` is not
    inside that head is read in full instead, so no consumer can be handed a
    frontmatter it cannot parse. A path that cannot be read is simply absent,
    and every consumer falls back to its own read for a miss, so this is a
    fast path and never a source of truth.

    NEGATIVE SPEC — a value from this dict is NOT the file's contents and
    must never be written back to disk or handed to `rebuild()`. Every
    consumer here reads frontmatter only (`split_frontmatter(...).fm_text`,
    `_parse_frontmatter`); `_stamp_deployment_state_shipped`, the one place
    that rewrites a handoff, deliberately takes its own full read. The whole
    corpus is 2.9MB at 272 handoffs against ~640KB of frontmatter, and the
    body was being decoded for every one of them to answer a question only
    ...
    for path in handoffs:
        try:
            with open(path, "rb") as fh:
                ...
                # No closing delimiter in the head: either the frontmatter is
                # unusually long or there is none at all. Pay the full read
                # rather than hand a consumer a block it would misparse.
                if head.startswith(b"---") and b"\n---" not in head[3:]:
                    head += fh.read()
            texts[path] = head.decode("utf-8", errors="replace")
        except OSError:
            continue
    return texts
...
def select_candidates(
    handoffs: List[Path], texts: Optional[dict] = None
    ...
    for path in handoffs:
        text = None if texts is None else texts.get(path)
        if text is None:
            try:
...
async def _apply_dr084_disposition(
    ...
    candidate set, reading each candidate's OWN frontmatter and nothing else.
    ...
    For each candidate, in order:
      ...
      2. Heir determination: `continued_into` on the candidate's own
         frontmatter, the forward succession edge `archive_stamp`'s supersede
         mode stamps (with `deployment_state: continued`) at the moment a
         successor is created, and re-reads to verify before it proceeds.
           - stamped -> (e) heir (b)-suppression: deployment_state is stamped
           ...
           - unstamped -> (b): a non-heir candidate whose frontmatter
             literally reads `deployment_state: in_flight` is neither
             archived nor flipped — it is pulled out, surfaced in
    ...
    WHY A FIELD AND NOT A SCAN. This used to answer "does this candidate have
    a succession heir?" with `archive_handoffs._classify_heir_children` over a
    reverse-edge index built across the whole live corpus — 272 frontmatter
    reads and a corpus-wide index, per boot, on a box running dozens of
    sessions, to rediscover an edge ONE writer already recorded. The successor
    knows its predecessor at creation; `archive_stamp` writes that edge onto
    the parent and verifies it landed. Reading it back is a dict lookup on
    bytes this op already holds.

    It is also STRICTER than the scan it replaces. `reverse_membership`
    counted only LIVE referencing children, so a parent whose sole successor
    had itself been archived read as childless — an admitted completeness gap
    in that function's own negative-spec, and the opposite of DR-224, which
    says an already-archived successor is still proof the baton was passed. A
    stamp does not decay when the successor moves.

    NEGATIVE SPEC — no reverse-edge index, no `dag_index`, no corpus argument,
    and no `archive_handoffs` import. A candidate predating the stamp reads as
    non-heir and takes the ordinary archival path, which is exactly what the
    old classifier did when it masked an archived heir. Do not reintroduce a
    scan to recover those: the requirement is that a succession is RECORDED
    when it happens, and that is the write side's job, not this one's.
    ...
    Returns (to_archive, awaiting_adjudication, retained):
      ...
      retained               — [{"path": Path, "reason": str}, ...] — a
                                candidate whose own frontmatter could not be
                                read (fail-closed retain). There is no longer
                                a scan that can be incomplete, so that class
                                of retain is gone with it.
    ...
    for handoff_path in candidates:
        text = None if texts is None else texts.get(handoff_path)
        if text is None:
            try:
                text = handoff_path.read_text(encoding="utf-8", errors="replace")
            except OSError as exc:
        ...
        # The forward succession edge, off the candidate's own frontmatter.
        # `archive_stamp`'s supersede mode writes it together with
        # `deployment_state: continued` and verifies it landed, so its
        # presence IS the heir proof the reverse-edge scan used to go
        # looking for.
        continued_into = _strip_quotes(read_fm_field(fm_text, "continued_into"))
        if continued_into:
            _stamp_deployment_state_shipped(handoff_path)
            to_archive.append({
                "path": handoff_path,
                "heir": True,
                "detail": Path(continued_into).name,
                "sid": sid,
            })
            continue
...
async def _handler(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    # Incremental scan. The floor is stamped BEFORE the walk, never after: a
    # handoff written while this run is in flight then has an mtime at or
    # above the floor and is re-read next boot. Stamping afterwards would let
    # a concurrent write fall into the gap and never be looked at again.
    scan_started_at = time.time()
    scan_floor, carried = _load_scan_cache(common_dir)

    entries = enumerate_handoffs_with_mtime(worktree)
    to_read = _read_set(entries, scan_floor, carried)
    corpus_texts = scan_corpus(to_read)
    candidates, unreadable = select_candidates(to_read, texts=corpus_texts)

    # PM ruling 2026-08-23: the backstop takes a BOUNDED bite per boot rather
    # than draining the whole backlog in one run. It fires once per session on
    # a box running dozens of them, so the queue drains across boots either
    # way; what a cap buys is a per-boot cost that does not scale with however
    # far the corpus has drifted. Deferred candidates are not dropped and not
    # silently truncated — the count is reported in the envelope, and the next
    # boot takes the next bite. Oldest-first by mtime so the queue cannot
    # starve at the tail.
    deferred_to_next_boot = 0
    deferred_paths: List[Path] = []
    if len(candidates) > _MAX_CANDIDATES_PER_BOOT:
        # Oldest-first by the mtimes the scandir pass already returned, so the
        # tail of the queue cannot starve and no candidate is re-stat'd.
        mtimes = {path: mtime for path, mtime in entries}
        candidates = sorted(candidates, key=lambda p: mtimes.get(p, 0.0))
        deferred_paths = candidates[_MAX_CANDIDATES_PER_BOOT:]
        deferred_to_next_boot = len(deferred_paths)
        candidates = candidates[:_MAX_CANDIDATES_PER_BOOT]

    if not candidates:
        # Nothing terminal on disk: the backstop's whole job is already done by
        # `pickup-assemble apply`'s incremental archival, and this run must not
        # pay the DR-084 substrate's import or corpus walk to discover that.
        # Every file read this run was examined and nothing is owed forward,
        # so the floor advances and the next boot reads only what changed.
        _write_scan_cache(common_dir, scan_started_at, [])
        return {
            "exit_code": 0,
            # Same warning shape the full path emits below — an early return
            # must be indistinguishable to a consumer, or it becomes a second
            # envelope contract nobody knows about.
            "warnings": [
                {
                    "scope": "handoffs",
                    "reason": f"unreadable ({item['path']}): {item['reason']}",
                }
            ...
            "commit": {"committed": False, "sha": None, "reason": "no candidates"},
        }

    to_archive, awaiting_adjudication, retained = await _apply_dr084_disposition(
        worktree, candidates, texts=corpus_texts
    )
    ...
    # all).
    # Accumulated and written in one pass: the per-marker form re-reads the
    # whole marker file and opens it twice for every row, which at the
    # 25-candidate cap was the largest single term in this handler.
    ...
    for item in to_archive:
        ...
        marker_rows.append((item["path"].name, item["sid"], note, "archived"))
    ...
    _append_warn_markers(worktree, marker_rows)
    ...
    # Advance the scan floor, carrying forward everything found terminal but
    # not archived: the cap's deferred tail, the (b) adjudication holds, the
    # fail-closed retains, and any move that failed. Those files are old on
    # disk and would otherwise fall below the floor and never be looked at
    # again — the deferred queue would age out instead of draining. Anything
    # NOT carried was either archived (it is gone from state/handoffs/) or
    # read this run and found non-terminal.
    carried_forward = (
        [p.relative_to(worktree).as_posix() for p in deferred_paths]
        + [item["path"] for item in skipped]
        + [item["path"] for item in failed]
    )
    _write_scan_cache(common_dir, scan_started_at, carried_forward)
    ...
    return {
        ...
        "handoffs": {
            ...
            "failed": failed,
            "skipped": skipped,
            "deferred_to_next_boot": deferred_to_next_boot,
        },
