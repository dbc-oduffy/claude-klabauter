...
Scalars (C4):
  ...
  critical_path — unweighted longest-chain of stub_ids by node count (NOT a
                  ...
                  — F7 the Staff Engineer review naming precision). Empty list when total==0.
                  Cycle guard: cyclic nodes are quarantined with a logged warning
                  and excluded from the traversal — no infinite loop.

Frontmatter parser (F0 — the Staff Engineer review): uses coordinator_core.dag._parse_frontmatter
...
shipped_sha precision (F1 — the Staff Engineer review): raw stub frontmatter carries shipped_in
...
Node identity (F6 — the Staff Engineer review): each emitted node and edge carries roadmap_id
...
Negative-spec:
  ...
  - Does NOT serve state/initiatives/ as a fleet-wide/cross-initiative query surface —
    state/ IS claude-klabauter's own disk-truth custody (per
...
def _compute_roll_up(nodes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Zero-node guard (F3 — the Staff Engineer review): when total == 0, pct_shipped is None
...
def _compute_critical_path(
    ...
    reasonable proxy for dependency depth (F7 — the Staff Engineer review naming precision).
...
def assemble_roadmap_dag(roadmap_id: str, worktree_root: Path) -> Dict[str, Any]:
    ...
    Dangling-edge handling (F4 — the Staff Engineer review):
    ...
    Zero-node guard (F3 — the Staff Engineer review):
