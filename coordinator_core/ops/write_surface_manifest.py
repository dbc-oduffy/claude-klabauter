...
produce — the single artifact a sibling repo (DoE) reads in their lockstep test, walked
...
`effect="delete"` is emitted with `"effect": "delete"`, never filtered — DoE ruled
...
Negative-spec — this module does NOT:
    ...
    - validate that a declared surface matches what a writer's source actually does at
      runtime (a lockstep/drift check DoE owns on their side);
...
def discover_declarations(
    ...
    already-frozen, externally-consumed manifest emission shape (DoE's
