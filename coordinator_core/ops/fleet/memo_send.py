...
all-hooks-off mechanism per the Staff Engineer's approach-review REQUIRES_CHANGES
...
Negative-spec:
  ...
  - Does NOT hardcode a single receiver (e.g. Example-retrieval-repo only) — the allowed-set
  ...
  - DOES commit into the receiver's tree with ALL hooks neutralized via
    ...
    2026-07-21; mechanism amended 2026-07-21 per the Staff Engineer REQUIRES_CHANGES —
  ...
  - Does NOT create a branch in the receiver repo (removed 2026-07-21 per
    the Staff Engineer REQUIRES_CHANGES — a headless engine switching a receiver's
...
1->N fan-out (DEC-3, C7 — 2026-07-24 memo-ownership-and-redesign plan):
    ...
    "did all N act?" visibility (never a claude-klabauter-local index, AC8).
...
# Review: the Staff Engineer — decoupled-literal finding; status/delivery_mode were previously
...
async def _commit_delivered_memo(
    ...
    Mechanism (amended 2026-07-21 per the Staff Engineer's approach-review REQUIRES_CHANGES;
    ...
    Branch-creation REMOVED (2026-07-21 per the Staff Engineer REQUIRES_CHANGES): a
...
async def _memo_send(
    ...
    mechanism amended 2026-07-21 (DR-214 amendment) per the Staff Engineer's
    ...
    # Review: the Staff Engineer — stale comment reworded; C3 is committed, repo_root=None is
    ...
    # Review: the Staff Engineer — stale comment reworded; guard fires on every dispatched
