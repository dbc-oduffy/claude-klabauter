...
Example-doctrine-repo CLI `cross-repo-memo --check-addressee` handler that `/pickup`'s M-addr
...
Spec backlink:
    ...
    Parity source: example-doctrine-repo CLI `cross-repo-memo --check-addressee` handler
    (example-doctrine-repo/coordinator/bin/cross-repo-memo:3249-3289).
...
Negative-spec:
  ...
  - Does NOT hardcode any redirect-alias literal — the redirect set is read
    declaratively via `read_redirect_aliases()`. Example-doctrine-repo promoted
...
#: Exit codes mirroring the example-doctrine-repo CLI's `--check-addressee` branch
...
def format_addressee_message(
    ...
    against the example-doctrine-repo CLI's `--check-addressee` stdout (`cross-repo-memo:
