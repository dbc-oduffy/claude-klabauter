...
class TestClassifyReceiver:
    ...
    def test_unknown_receiver_rejected_with_nearest_match(self, tmp_path, monkeypatch, caplog):
        ...
        Uses a wildly-unrelated `to` rather than 'claude-klabauter-em' -> the latter now
        ...
        """
        receiver_repo = tmp_path / "claude-klabauter-repo"
        receiver_repo.mkdir()
        claude_home = _make_claude_home(
            tmp_path, receiver_repos={"claude_klabauter": receiver_repo},
        )
        monkeypatch.setenv("CLAUDE_HOME", str(claude_home))
    ...
    def test_unique_did_you_mean_auto_accepted(self, tmp_path, monkeypatch):
        ...
        candidate ('claude-klabauter-em' -> 'claude-klabauter-em') -> drafts normally,
        ...
        result = _run(_memo_draft(
            _base_params(
                dry_run=False, topic="alias-test", to="claude-klabauter-em",
                classify_receiver=True,
            ),
            repo_root=common_dir,
        ))
    ...
    def test_classification_matches_memo_resolver_directly(self, tmp_path, monkeypatch):
        ...
        inbox_dir, receiver_repo_path, _ = resolve_receiver_inbox("example-retrieval-repo-em")
        assert inbox_dir is not None
        assert _classify_receiver_for_draft("example-retrieval-repo-em", dry_run=False) is None
