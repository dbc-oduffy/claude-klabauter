...
the right place?" check that the example-doctrine-repo bash CLI never had, which is exactly how
...
Two modes, selected by whether `to` is supplied:
  - Enumeration mode (`to` absent): returns the FULL structured registry data
    ...
    `candidates` rather than as new envelope keys):
      ...
      - `kind: "canonical_home_alias"` — example-doctrine-repo-canonical redirect aliases
        (`identity.redirectAliases`, promoted by example-doctrine-repo 2026-07-21) — `[]` only
    ...
    This is the data `_format_receiver_listing()` in the example-doctrine-repo CLI's
    ...
    section, a example-doctrine-repo-canonical-home-aliases section); moving it into this op
  ...
  - Resolution mode (`to` supplied): resolve `to` through the ONE shared
    ...
    every other caller (reported by example-doctrine-repo/claude-central-em: `cross-repo-memo
    --dry-run` previewed `claude-klabauter-engine`-namespaced filenames for example-doctrine-repo-origin
...
Spec backlink:
    ...
    Parity source: example-doctrine-repo coordinator/bin/cross-repo-memo receiver-listing section
    (this op is an ergonomic superset — it additionally proves no-write via a
    real dry-run resolution path, which the example-doctrine-repo CLI's bare `--title` probe did
    not have).
...
Negative-spec:
  ...
  - Does NOT conflate `publish.mirrors.*` entries with `repos.*` receivers —
    ...
    mirroring the example-doctrine-repo CLI's own hard separation (mirrors are outward OSS
...
def _enumerate_publish_mirrors(mirrors_by_key: Optional[dict] = None) -> list:
    ...
    `read_publish_mirror_owners()`'s own owner-required filter (the example-doctrine-repo CLI's
    ...
    verdicts for the identical string (defect: example-doctrine-repo's `--list-receivers`
    ...
    until example-doctrine-repo promotes `identity.redirectAliases` (see that function's own
...
def _enumerate_canonical_home_aliases() -> list:
    ...
    example-doctrine-repo-manifest-declarative equivalent of the example-doctrine-repo CLI's hardcoded
    ...
    common case: example-doctrine-repo promoted this field into
    ...
    return [
        {
            ...
            "is_receiver": False,
            "note": (
                "example-doctrine-repo-canonical home/redirect alias — not directly addressable "
...
def _enumerate_candidates() -> list:
    ...
    envelope keys:
      ...
      - `kind: "canonical_home_alias"` — example-doctrine-repo-canonical redirect aliases
        (`identity.redirectAliases`, promoted by example-doctrine-repo 2026-07-21); an empty
