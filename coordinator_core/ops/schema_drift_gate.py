...
Op-key / contract:
    schema.drift_gate
    params:   {} (no params; scan resolves the example-doctrine-repo clone and vendored dir itself,
...
Gating semantics — ok is False ONLY on a confirmed STATUS_DRIFT verdict:
    ...
    INDETERMINATE -> ok=True,  message notes the check could not run (a release
                     ...
                     unreadable example-doctrine-repo clone on the merging machine).
    UNRESOLVED    -> ok=True,  message notes no example-doctrine-repo clone was resolved on this
...
Idempotency: read-only comparison against example-doctrine-repo HEAD + the vendored tree on disk;
...
def evaluate() -> dict:
    ...
    resolves the example-doctrine-repo clone / vendored dir itself, same as the advisory probe.
    """
    ...
    if status == STATUS_DRIFT:
        ...
        return {
            ...
            "drifted": drifted,
            "message": (
                f"{len(drifted)} vendored schema(s) diverge from example-doctrine-repo HEAD: {named}. "
    ...
    if status == "INDETERMINATE":
        ...
        message = str(report.get("summary") or "no example-doctrine-repo clone resolved; drift not determinable")
