...
§ C6b/AC12/AC13, example-doctrine-repo):
    ...
    3. AC13, both halves: (a) a synthetic fixture reproducing the b644d5a9
       shape is flagged; (b) a historical replay against example-doctrine-repo's real
       ...
       (coordinator/lib/install-substrate.py, verified absent from example-doctrine-repo
...
class TestDocRelativeResolution:
    ...
    the repo root. Measured against example-doctrine-repo's real `coordinator/README.md`
...
class TestAC13HistoricalReplay:
    ...
    def test_true_positive_flagged_install_maximalist_when_no_sibling_root_resolvable(self):
        ...
        is "absent everywhere" at b644d5a9 — absent from example-doctrine-repo AND absent
        ...
        claude-klabauter-side file before removing the example-doctrine-repo-side one.
        ...
        # from the example-doctrine-repo tree.
    ...
    def test_negative_cross_repo_citation_not_flagged_install_substrate(self):
        ...
        # Confirmed absent from example-doctrine-repo's tree at this commit...
        assert not _git_ls_tree_exists(
