...
Covers:
  ...
  - AC4 conformance: known_types() is set-EQUAL (not merely a subset) to
    ...
    repo-root-relative — this module migrated in-repo via DoE commit
    ...
    it still resolves the live DoE clone via ``resolve_doe_clone()`` and
...
if not _REGISTRY_PATH.is_file():
    pytest.fail(
        ...
        "lived in-repo since DoE commit b644d5a9",
        pytrace=False,
    )
