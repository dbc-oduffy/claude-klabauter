...
skip-if-example-doctrine-repo-absent conformance LANE registered in ``LANES`` (C6, AC5; extended
...
module-level ``skipif(example-doctrine-repo clone unavailable)`` — on a genuinely
consumer/CI-standalone install with no sibling example-doctrine-repo clone, the guarantees those
...
Example-doctrine-repo-clone-absent is the *normal* consumer shape, not an edge case — a future
...
the example-doctrine-repo clone" (CI, or a dev machine that opts in), a missing/unresolvable
...
Registered lanes:
  - ``docgen-byte-identity``: ``test_c6_conformance.py`` and
    ``test_type_enum.py``'s ``TestAC4LiveConformance`` (~29 tests) — example-doctrine-repo-HEAD
  ...
  - ``contract-blocks-header-style``:
    ...
    extraction reproduces example-doctrine-repo's registered snippet content byte-for-byte.
...
def _resolve_docgen_doe_clone() -> None:
    """Raise iff the example-doctrine-repo-HEAD byte-identity conformance lane's clone is unresolvable."""
    resolve_doe_clone()


def _resolve_contract_blocks_doe_root() -> None:
    """Raise iff the contract_blocks/header_style byte-identity lane's example-doctrine-repo root
    ...
    if not present:
        raise DoeResolveError(
            f"sibling example-doctrine-repo checkout not resolvable (resolved root={root!r})"
        )


#: Each entry: ``(lane_id, human description used in the failure message, resolver)``.
#: ``resolver`` raises (any exception) iff the lane's example-doctrine-repo dependency is
...
def test_doe_clone_conformance_lane_ran_or_environment_is_documented_optional(
    ...
    lane but the example-doctrine-repo clone is unresolvable — the exact silent-skip class a
    ...
    install to carry a sibling example-doctrine-repo clone it has no other reason to have.
    """
    if not _environment_requires_doe_clone():
        pytest.skip(
            ...
            "should-have-example-doctrine-repo-clone (see module docstring)"
        )
    try:
        resolver()
        ...
        pytest.fail(
            ...
            f"should-have-example-doctrine-repo-clone (CI or CLAUDE_KLABAUTER_REQUIRE_DOE_CONFORMANCE "
            f"truthy): {exc}"
        )
