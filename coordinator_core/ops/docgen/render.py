...
Oracle: /coordinator/bin/coordinator-doc-new (example-doctrine-repo clone) — every ``_scaffold_*`` fn's
...
def _yaml_quote(value: Any) -> str:
    ...
    this module must not depend on the example-doctrine-repo clone at render time; C6's harness is
    the only surface permitted to resolve into the example-doctrine-repo tree.
    """
    s = str(value)
    escaped = (
        s.replace("\\", "\\\\")
