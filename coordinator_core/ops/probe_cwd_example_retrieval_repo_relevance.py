...
Port of: probe-cwd-example-retrieval-repo-relevance.sh (example-doctrine-repo b5a4192c, 2026-07-20), 342 lines
Port backlink: docs/plans/2026-07-15-bash-to-naked-python-engine-migration.md
Spec backlink (behavior): docs/plans/2026-05-20-portable-code-substrate.md § Chunk 3
Example-doctrine-repo-side test oracle: coordinator/tests/test_probe_cwd_example_retrieval_repo_relevance.sh
...
Example-doctrine-repo-side bats-style test suite runs unchanged against this port.
...
with the bash oracle, INCLUDING its known quirk):
    - The registry.toml/registry.local.toml lookup resolves under
      ...
      `${COORDINATOR_TEST_HOME}/.claude/machine-local` (where the example-doctrine-repo test's
    ...
    - The whoami project_kind probe MUST go through a subprocess call to a
      ...
      else python) — NOT a direct in-process import — because the example-doctrine-repo test's
