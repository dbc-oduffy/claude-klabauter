...
Purpose: Port of two completion/plan ceremony writers:
  completion.reconcile_commits ← reconcile-completion-commits.sh (example-doctrine-repo 432e3285, 2026-07-22)
                                  (--append mode write logic)
  plan.append_session          ← append-plan-session.sh (example-doctrine-repo 2737ca3d, 2026-07-19)
...
Byte-parity targets (``--append`` mode write logic only):
  completion.reconcile_commits:
    ``reconcile-completion-commits.sh`` (example-doctrine-repo 432e3285, 2026-07-22, awk write pass)
  plan.append_session:
    ``append-plan-session.sh`` (example-doctrine-repo 2737ca3d, 2026-07-19, mapfile/printf write pass)
...
def reconcile_completion_commits(
    ...
    dry_run (read-only mode, added to unblock example-doctrine-repo's detect-only facade branch —
...
async def _reconcile_commits_handler(
    ...
    Optional params (REOPENED git-resolution path — see module negative-spec):
        ...
        dry_run (bool, optional) — DEFAULTS TO FALSE. This DIVERGES deliberately from
            ...
            already has example-doctrine-repo-side callers (``--append``) that rely on writes happening
    ...
    Returns:
        ...
        output contract (see example-doctrine-repo's reconcile-completion-commits.sh detect-only branch,
...
# ---------------------------------------------------------------------------
# Byte-parity oracle: [example-doctrine-repo] coordinator/skills/merging-to-main/SKILL.md
...
#: (empirically: claude-central-em, example-doctrine-repo-em, example-cockpit-repo-em,
...
#: fleet repo root it knows (``"repos.example_doctrine_repo" = '/Users/…/example-doctrine-repo'``).
...
def _sibling_homed_session_ids(
    ...
    in example-doctrine-repo that commits into claude-klabauter's tree therefore leaves those
    artifacts in example-doctrine-repo's tree, not here. Since completion entries are single-repo
