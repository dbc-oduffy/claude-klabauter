...
tests (``test_emit_parity``) and the strang-02 example-doctrine-repo-HEAD conformance drift-check
...
Oracle: example-doctrine-repo emission-conformance-contract.md § AC5-PROVENANCE (three distinct sentinels).
strang-02 cross-repo conformance comparison normalizes to these same values on the example-doctrine-repo
side; using a single ``<NORMALIZED>`` for all field types would cause every
timestamp/SHA field to compare unequal when example-doctrine-repo normalizes to epoch-zero / zero-SHA.
...
# (Port of: emit-cockpit-snapshot.sh, example-doctrine-repo 07eedcfb, 2026-07-19); included here so
...
def _normalize(value):
    ...
    Applies to any dict/list value in a cockpit emission or example-doctrine-repo conformance fixture.
