"""C5: a guard test keeps the human axis off the strict wire until cockpit vendors it.
...
purely for cockpit's future consumption; C4 wires the resulting bundle into a NEW, additive
...
is the guard that keeps that true after this commit AND after every future one, until cockpit
vendors a contract that names the human axis explicitly.

Barrier phase this guards (P1 not started as of 2026-08-19): P1 is cockpit's `.strict()`
schema mode, which rejects any UNVENDORED key wholesale — a `human_*`/`contributor_slug` key
reaching the wire before cockpit vendors it does not merely go unread, it makes `.strict()`
...
Cockpit's vendored contract names `human_*`/`contributor_slug` as accepted keys AND an explicit
...
def test_no_section_emits_an_ungated_human_shaped_key():
    ...
    assert not offenders, (
        ...
        f"human_axis_vendored/_human_axis_on check, unauthorized before cockpit "
...
def test_no_section_sources_owner_from_person_resolver():
    ...
    assert not offenders, (
        ...
        f"source unauthorized for `owner` before cockpit vendors it: {offenders}"
    )
...
def test_no_owner_value_matches_a_resolved_human_alias():
    ...
    assert not offenders, (
        ...
        f"`owner` axis, unauthorized before cockpit vendors it: {offenders}"
    )
