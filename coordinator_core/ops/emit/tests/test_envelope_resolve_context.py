"""Tests for envelope.resolve_context(repo_root=None) and _empty_skeleton (C2).

Verifies the per-repo-emission-cutover (2026-07-07) invariants on the envelope module:
...
class TestResolveContextWithRepoRoot:
    ...
    def test_central_state_root_not_call_shell_seam(self, tmp_path: Path) -> None:
        ...
        with patch(
            ...
            "coordinator_core.ops.emit.envelope._resolve_central_state_root"
        ) as mock_seam:
...
class TestNoTopLevelCoordinatorRootPathScalar:
    ...
    def test_build_envelope_has_no_top_level_coordinator_root_path(self, tmp_path: Path) -> None:
        """build() must NOT set a top-level coordinator_root_path key in the assembled envelope."""
        from coordinator_core.ops.emit.context import EmitContext
        from coordinator_core.ops.emit.envelope import build

        # Build a minimal EmitContext with a known repo_root.
        ctx = EmitContext(
            tmp_path,          # repo_root
            tmp_path,          # coordinator_root
            tmp_path / "state",  # central_state_root
            "test-branch",     # git_branch
            "a" * 40,          # git_sha
        ...
        with patch("coordinator_core.ops.emit.validate.assert_pin_integrity"), \
             patch("coordinator_core.ops.emit.validate.assert_version_consistency", return_value="2.4.0"):
            envelope = build(ctx, sections={})

        assert "coordinator_root_path" not in envelope, (
            "top-level coordinator_root_path scalar was removed 2026-07-21 (zero readers; "
            "forbidden by SnapshotEnvelope additionalProperties:false) and must not reappear"
        )
        # emitted_by_machine — the scalar's former sibling — is still present.
        assert envelope["emitted_by_machine"] == "test-host"
