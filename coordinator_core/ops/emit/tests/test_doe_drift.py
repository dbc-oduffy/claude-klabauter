"""strang-02 drift-check tests — example-doctrine-repo-HEAD conformance fixture + version-band gate.
...
Covers every acceptance criterion from strang-02:
  ...
  AC_Q-a-sha:
    ...
    - Ref absent on example-doctrine-repo origin → graceful WARN, no exception (live constraint).
  ...
  AC_Q-b:
    - Fixture read from ``coordinator/cockpit-contract/conformance/emission-conformance.json``
      in the example-doctrine-repo local clone at ``repos.example_doctrine_repo``; NOT co-vendored in claude-klabauter's _vendor/.
  ...
  AC_DRIFT_CHECK:
    - Drift-check fails loud when pinned version < example-doctrine-repo min_supported (synthetic lag test).
  ...
  AC_REF_ABSENT:
    - When example-doctrine-repo origin has no ``cockpit-contract-release`` ref, check_freshness issues
...
Note: tests that require a live example-doctrine-repo clone use ``resolve_doe_clone()`` and skip
...
# ---------------------------------------------------------------------------
# Helpers — skip guard for live example-doctrine-repo clone
# ---------------------------------------------------------------------------

def _live_doe_clone() -> Optional[Path]:
    """Return the live example-doctrine-repo clone path or None if unavailable on this machine."""
    try:
...
# ---------------------------------------------------------------------------
# AC_Q-b: fixture read from example-doctrine-repo clone, not co-vendored
# ---------------------------------------------------------------------------

class TestFixtureResolution:
    """AC_Q-b: fixture lives in the example-doctrine-repo clone, not co-vendored in claude-klabauter's _vendor/."""

    @pytest.mark.skipif(not _DOE_AVAILABLE, reason="example-doctrine-repo clone not available on this machine")
    ...
    def test_fixture_not_in_vendor_tree(self) -> None:
        ...
        assert not co_vendored.exists(), (
            ...
            "it must be read from the example-doctrine-repo clone at check-time (DR-210 anti-drift invariant)."
        )
...
class TestVersionBand:
    ...
    def test_contract_version_bump_alone_does_not_fail(self) -> None:
        ...
        This is the reader-first invariant. Example-doctrine-repo bumps contract_version first and raises
        ...
        """
        # Simulate: example-doctrine-repo bumps contract_version to 2.6.0 but leaves min_supported at 2.5.0.
    ...
    def test_min_supported_past_pin_fails_loud(self) -> None:
        ...
        After the re-vendor window closes example-doctrine-repo raises min_supported.  A still-at-2.5.0
        claude-klabauter pin must fail loud — the grace window has closed.
        """
        with pytest.raises(DriftError, match="below the example-doctrine-repo min_supported_contract_version"):
    ...
    def test_missing_min_supported_warns_not_raises(self) -> None:
        ...
        example-doctrine-repo commitment: always publish the field.  Absent means example-doctrine-repo has not yet;
...
class TestFreshnessRef:
    ...
    def test_ref_absent_graceful_no_exception(self, tmp_path: Path) -> None:
        """AC_REF_ABSENT: ref absent on example-doctrine-repo origin → WARN, no DriftError.

        Live constraint: example-doctrine-repo has NOT published refs/tags/cockpit-contract-release at
        strang-02 pickup.  This is the expected path in CI and on live machines until
        example-doctrine-repo publishes the ref.  Must NEVER hard-fail on ref absence.
        """
    ...
    def test_sha_mismatch_raises_drift_error(self, tmp_path: Path) -> None:
        ...
        the current example-doctrine-repo origin returns a different (newer) SHA.  Explicitly exercises
    ...
    def test_pin_absent_sentinel_skips_freshness(self, tmp_path: Path) -> None:
        ...
        strang-02 window before example-doctrine-repo publishes the ref.
    ...
    def test_pin_absent_but_ref_now_on_origin_emits_drift_warning(
        ...
        State (B): example-doctrine-repo published the cockpit-contract-release ref but
        ...
        Rationale for WARN vs ERROR: a hard DriftError the instant example-doctrine-repo publishes would
    ...
    def test_probe_freshness_ref_live_ref_absent_returns_none(self) -> None:
        """Live test: example-doctrine-repo origin returns None (ref not yet published) → no exception.
...
class TestProvenanceNormalizerReuse:
    ...
    correctly handles the example-doctrine-repo fixture's AC5-PROVENANCE paths so both sides of the
    ...
    def test_all_seven_ac5_provenance_fields_scrubbed(self) -> None:
        ...
        Oracle: example-doctrine-repo emission-conformance-contract.md § AC5-PROVENANCE.
        """
    ...
    def test_doe_fixture_normalizes_consistently(self) -> None:
        """example-doctrine-repo fixture normalizes to a consistent state (no runtime-varying residue).

        The example-doctrine-repo fixture already uses sentinel values for volatile fields (it was
...
class TestRunDriftCheck:
    ...
    def test_drift_check_passes_with_current_pin(self) -> None:
        """Current vendored pin (2.5.0) passes the version-band gate against example-doctrine-repo fixture."""
...
class TestConformanceViaStrang01Path:
    ...
    Proves the SEAM conforms:
      ...
      - The output schema_version satisfies the example-doctrine-repo version-band gate.
      - Normalize-then-compare uses the shared _normalize helper on both sides.
      - Full C2 entity coverage: all entity types from the example-doctrine-repo fixture are structurally
    ...
    def test_strang01_output_passes_version_band_synthetic(self, tmp_path: Path) -> None:
        """Synthetic-example-doctrine-repo variant: strang-01 path conforms against a fixture in tmp_path.

        CI regression net — runs WITHOUT a live example-doctrine-repo clone (gated on requires_vendor_pin
        ...
        Does NOT prove conformance against example-doctrine-repo's actual fixture, but DOES prove:
    ...
    def test_strang01_output_passes_version_band(self, tmp_path: Path) -> None:
        """Strang-01 strangled path (envelope.emit()) output satisfies example-doctrine-repo version band.

        Loads example-doctrine-repo fixture → extracts min_supported → runs envelope.emit() (strang-01
        ...
        # Load example-doctrine-repo fixture for version-band reference.
        doe_clone = resolve_doe_clone()
        fixture = read_doe_fixture(doe_clone)
    ...
    def test_strang01_output_has_c2_entity_types(self, tmp_path: Path) -> None:
        """Strang-01 output covers the same C2 entity types as the example-doctrine-repo fixture (structural).

        The example-doctrine-repo fixture is a round-trip over full C2 entity coverage.  The strang-01
        ...
        # C2 entity type keys from the example-doctrine-repo fixture (top-level list-valued keys, excluding metadata).
        _SCALAR_KEYS = {
        ...
        assert not missing, (
            f"Strang-01 output is missing C2 entity type keys present in example-doctrine-repo fixture: {missing}.  "
            "The strang-01 facade must produce all entity types the example-doctrine-repo fixture covers."
        )

    @pytest.mark.usefixtures("requires_vendor_pin")
    @pytest.mark.skipif(not _DOE_AVAILABLE, reason="example-doctrine-repo clone not available on this machine")
    def test_both_sides_normalize_identically(self, tmp_path: Path) -> None:
        """Normalize-then-compare: _normalize applied to strang-01 and example-doctrine-repo fixture is idempotent.
        ...
        assert not doe_violations, (
            f"example-doctrine-repo fixture still has live provenance values after normalization: {doe_violations}"
        )
