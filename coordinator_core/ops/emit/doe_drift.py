"""
coordinator_core.ops.emit.doe_drift — example-doctrine-repo-HEAD conformance fixture resolver + drift-check.

Purpose: resolve the example-doctrine-repo-HEAD emission-conformance fixture at check-time and run a
fail-loud drift-check when claude-klabauter's vendored schema-version lags the example-doctrine-repo-HEAD
...
Resolution contract (example-doctrine-repo emission-conformance-contract.md § CD-1/CD-2):
  - **Body read — no origin.** Resolve example-doctrine-repo clone via machine-local ``repos.example_doctrine_repo``
  ...
  - **Freshness signal — dedicated ref.** Probe
    ...
    the ref is absent** — example-doctrine-repo has not published it yet; treat as WARN/skip, not a
...
ancestry helper uses the example-doctrine-repo local clone only; it issues NO network call.
...
Oracle: /Users/example-operator/X/example-doctrine-repo/coordinator/docs/wiki/emission-conformance-contract.md
"""
...
# yet published (live constraint at strang-02 pickup — example-doctrine-repo lands it in parallel).
PIN_SHA_FILE = _VENDOR_CONTRACT / ".doe-ref-pin"

# Path inside the example-doctrine-repo clone where the conformance fixture lives (CD-1).
_FIXTURE_REL = Path("coordinator/cockpit-contract/conformance/emission-conformance.json")

# Dedicated freshness ref example-doctrine-repo advances on every intentional contract change (CD-2 / AC8).
...
class DriftWarning(UserWarning):
    ...
    Currently used when the pin file contains the ABSENT sentinel but example-doctrine-repo has already
...
class AheadOfReleaseWarning(UserWarning):
    ...
    new contract version before example-doctrine-repo has advanced the ``cockpit-contract-release`` tag
    (per DR-203 reader-first invariant).  This is NOT a drift error; the SHA difference
    will self-heal when example-doctrine-repo advances the tag.
...
class DoeResolveError(RuntimeError):
    """Raised when the example-doctrine-repo clone cannot be located via the machine-local registry."""
...
def resolve_doe_clone() -> Path:
    """Resolve the example-doctrine-repo local clone root from the machine-local registry.
    ...
    caveat; example-doctrine-repo emission-conformance-contract.md § CD-3 / § Resolution Contract).
    ...
    raise DoeResolveError(
        "Cannot locate example-doctrine-repo clone: 'repos.example_doctrine_repo' unset or path absent in "
        f"{_REGISTRY_LOCAL} and {_REGISTRY_BASE}.  "
        "Set it with: machine-local set repos.example_doctrine_repo /path/to/example-doctrine-repo"
    )
...
def read_doe_fixture(doe_clone: Optional[Path] = None) -> dict:
    """Read and parse the example-doctrine-repo-HEAD conformance fixture body from the local clone.

    No network: this is a direct file read of the clone's HEAD on disk (CD-1).
    Resolves the example-doctrine-repo clone via ``resolve_doe_clone()`` when not supplied.

    Returns the parsed fixture dict containing ``contract_version``,
    ``min_supported_contract_version``, and all C2 entity arrays.

    Raises DoeResolveError when the example-doctrine-repo clone or fixture file cannot be found.

    Negative-spec: does NOT co-vendor the fixture — reading from the example-doctrine-repo clone
    ...
    if not fixture_path.exists():
        raise DoeResolveError(
            f"example-doctrine-repo conformance fixture not found at {fixture_path}.  "
            "example-doctrine-repo must commit it at coordinator/cockpit-contract/conformance/"
    ...
    try:
        ...
        raise DoeResolveError(
            f"example-doctrine-repo conformance fixture is not valid JSON at {fixture_path}: {exc}"
        ) from exc
...
def check_version_band(pinned_version: str, fixture: dict) -> None:
    ...
    (example-doctrine-repo emission-conformance-contract.md § CD-2 / Gate semantics).
    """
    min_supported_str = fixture.get("min_supported_contract_version")
    if not min_supported_str:
        # Missing field — cannot check. Warn and pass (example-doctrine-repo commitment: field must be present).
        warnings.warn(
            "example-doctrine-repo fixture missing 'min_supported_contract_version' — cannot run version-band "
            "check; treating as PASS.  example-doctrine-repo must publish this field per CD-2.",
            stacklevel=2,
        )
        return
    ...
    if pinned_t < min_t:
        raise DriftError(
            f"claude-klabauter vendored schema-version {pinned_version} is below the example-doctrine-repo "
...
def probe_freshness_ref(doe_clone: Path) -> Optional[str]:
    """Probe example-doctrine-repo origin for the cockpit-contract-release ref SHA via git ls-remote.

    Returns the **commit** SHA when the ref exists on origin, or None when absent.
    A missing ref means example-doctrine-repo has not published it yet (ref-absent graceful path).
    ...
    downstream as "example-doctrine-repo has not published it yet", a confident false claim about
    ...
    if unusable is not None:
        warnings.warn(
            f"example-doctrine-repo clone at {doe_clone} could not be read as a git repository "
            f"({unusable}) — skipping freshness check; this is not a finding "
            "about what example-doctrine-repo has or has not published.",
            stacklevel=3,
        )
        return None
    ...
    try:
        ...
        raise DriftError(
            "git executable not found — cannot probe example-doctrine-repo freshness ref.  "
            "Install git to enable the freshness check."
        ) from exc

    if origin.returncode != 0:
        warnings.warn(
            f"Cannot determine example-doctrine-repo origin URL for freshness probe: {origin.stderr.strip()}; "
    ...
    if not origin_url:
        warnings.warn(
            "example-doctrine-repo clone has no git remote 'origin' — skipping freshness check.",
            stacklevel=3,
        )
        return None
    ...
    try:
        ...
        warnings.warn(
            f"git ls-remote failed for example-doctrine-repo origin {origin_url!r}: {exc}; "
            "skipping freshness check.",
            stacklevel=3,
        )
        return None
    ...
    if ls_remote.returncode != 0:
        warnings.warn(
            f"git ls-remote returned non-zero for example-doctrine-repo origin: {ls_remote.stderr.strip()}; "
            "skipping freshness check.",
            stacklevel=3,
        )
        return None
    ...
    if not output:
        # Ref absent on origin — example-doctrine-repo hasn't published it yet (live constraint).
        return None
    ...
    # a maintained example-doctrine-repo clone); if the object is absent, we fall back to raw_sha
...
def _tag_is_ancestor_of_pin(
    ...
    """Check whether tag_sha is a git-ancestor of pin_sha in the example-doctrine-repo clone.
    ...
    No network call; operates entirely on the example-doctrine-repo local clone's object database.
    ...
    "the question never reached example-doctrine-repo's object database".
    """
...
def check_freshness(doe_clone: Optional[Path] = None) -> None:
    ...
    Resolution (CD-2):
      1. Read the pin SHA from ``_vendor/cockpit-contract/.doe-ref-pin``.
         ...
         - Absent or ABSENT sentinel → still probe origin to distinguish two states:
             (A) Ref absent on origin too → WARN/skip (example-doctrine-repo hasn't published yet).
             (B) Ref present on origin but pin not updated → emit DriftWarning (action
                 required: re-vendor to arm the gate).
      2. Probe example-doctrine-repo origin: ``git ls-remote <origin> refs/tags/cockpit-contract-release``.
         - Ref absent on origin → WARN and skip (example-doctrine-repo hasn't published yet — graceful).
      ...
      3. Ancestry check via ``_tag_is_ancestor_of_pin`` (local, no network):
         - True (tag is ancestor of pin, pin AHEAD): emit ``AheadOfReleaseWarning`` and
           ...
           when example-doctrine-repo advances the tag.
    ...
    example-doctrine-repo publishes would break claude-klabauter before re-vendor can run; a loud, distinct,
    ...
    if pin_sha is None:
        ...
        # If the example-doctrine-repo clone cannot be resolved (DoeResolveError), degrade gracefully.
        try:
            ...
            warnings.warn(
                ...
                f"({PIN_SHA_FILE}) and example-doctrine-repo clone unavailable for origin probe — "
                "freshness check skipped.  "
                "Run python3 bin/claude-klabauter-revendor-cockpit-contract.py to populate the pin when example-doctrine-repo publishes the ref.",
                stacklevel=2,
            )
            return
        ...
        if origin_sha is not None:
            # State (B): example-doctrine-repo published the ref, but re-vendor hasn't been run.
            # Emit a LOUD, category-distinct DriftWarning — not DriftError, because a hard
            # failure the instant example-doctrine-repo publishes would break claude-klabauter before
            # bin/claude-klabauter-revendor-cockpit-contract.py runs.
            warnings.warn(
                "[ACTION REQUIRED] example-doctrine-repo has published the cockpit-contract-release ref "
            ...
            # State (A): ref also absent on origin — example-doctrine-repo hasn't published yet.
            warnings.warn(
                "No cockpit-contract-release SHA recorded in vendor pin "
                f"({PIN_SHA_FILE}) — freshness check skipped.  "
                "Pin was likely cut before example-doctrine-repo published the ref (expected during initial "
    ...
    if origin_sha is None:
        # Ref absent on example-doctrine-repo origin — graceful skip (live constraint).
        warnings.warn(
            f"example-doctrine-repo origin has no '{_CONTRACT_RELEASE_REF}' ref — "
            "example-doctrine-repo has not published the dedicated freshness ref yet.  "
    ...
    if origin_sha != pin_sha:
        ...
        if is_ahead is True:
            warnings.warn(
                ...
                "has vendored the new contract version before example-doctrine-repo has advanced the release "
                "tag.  The discrepancy will self-heal when example-doctrine-repo runs the tag-advance step.  "
            ...
            return
        raise DriftError(
            f"example-doctrine-repo cockpit-contract-release ref SHA mismatch: "
            f"vendored-pin SHA={pin_sha!r}, example-doctrine-repo origin SHA={origin_sha!r}.  "
...
def run_drift_check(
    ...
    """Run the full example-doctrine-repo-HEAD drift-check and return the parsed conformance fixture.

    Steps (strang-02 Spec items 1/2/3):
      1. Resolve example-doctrine-repo clone via machine-local registry (body read — no origin).
      2. Read the example-doctrine-repo-HEAD conformance fixture body.
