...
Cockpit's store has a ``provenance_fk NOT NULL`` constraint (source memo line 60), so the
``provenance`` object MUST be fully populated (non-null) even in the D9 path — emitting
null provenance would strand cockpit's FK.
...
def collect(ctx: EmitContext) -> dict:
    ...
    The block-level ``provenance`` is ALWAYS non-null — cockpit's store has a
