...
Source: cross-repo/inbox/2026-08-06-example-doctrine-repo-em-updatedocs-gates-structured-
verdicts.md (kind: proposal, from example-doctrine-repo-em; ADOPTED). The measured cost this
...
Every external CLI this op shells out to (the example-doctrine-repo-owned `bin/verify-*`,
...
this file never edits a example-doctrine-repo-owned path, it only invokes already-shipped
...
def _run(
    ...
    *,
    ...
    prune-*.py); every other CLI (the extensionless example-doctrine-repo bin/ scripts)
...
def _gate_plugin_wiki(_repo_root: Path, settings_home: Path, overrides: dict) -> GateResult:
    ...
    # drift (a example-doctrine-repo-owned CLI this repo does not control). The memo's
...
async def _updatedocs_gates(params: dict, repo_root: Optional[Path] = None) -> dict:
    ...
    Params (all optional):
        ...
        settings_home  (str) — override for $COORDINATOR_SETTINGS_HOME
                         resolution (external example-doctrine-repo CLI location).
