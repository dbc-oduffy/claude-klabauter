...
Invocation contract (for the example-doctrine-repo consumer — see the driving
handoff's Part 3 and its own anti-scope: "every coordinator hook lives in
Example-doctrine-repo, not here"):
...
Negative-spec:
    ...
    - Does NOT emit surfacing/nudge prose itself — the `description`/
      ...
      not agent-facing copy; a consuming surface (built in example-doctrine-repo, not
