...
Requires the sibling example-doctrine-repo checkout (where `coordinator/snippets/` and
...
pytestmark = pytest.mark.skipif(
    ...
    reason="sibling example-doctrine-repo checkout not resolvable on this machine "
    "(see coordinator_core.testing.doe_root.resolve_doe_root)",
)

#: (block_name, consumer .md path repo-relative to the example-doctrine-repo root) --
...
#: rows here) were retired 2026-07-25: example-doctrine-repo completed a paste-to-inject
...
#: to `sentinel-embedded` in the same example-doctrine-repo commit) -- see
...
#: migration (a pre-existing example-doctrine-repo content drift, out of this
#: module's/repo's scope to fix).
#:
#: Reconciled 2026-08-02 (stale-test cleanup, triage-F): example-doctrine-repo commit c76ba3e36
...
#: not resolvable from a example-doctrine-repo-relative consumer_relpath the way this
...
# ANY live paste consumer in example-doctrine-repo -- every snippet declaring either
