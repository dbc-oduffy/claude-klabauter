...
Negative-spec:
  ...
  - Does NOT vendor or copy the policy YAML — reads it fresh from the injected
    path every invocation (single source of truth with example-doctrine-repo's policy file).
...
#: Env var the example-doctrine-repo shim injects the primary policy path through.
...
def load_policy(policy_path: Optional[str] = None) -> Policy:
    ...
    Resolution order:
      ...
      2. ``SUBAGENT_SANDBOX_POLICY`` env var (the shim's primary injection —
         only example-doctrine-repo knows its plugin root; this is the expected common case).
    ...
    # confined/exempt/sanctioned_dirs fields, pending example-doctrine-repo's lockstep YAML
...
def _resolve_git_root_uncached(cwd: Optional[str] = None) -> Optional[str]:
    ...
    that helper isn't loaded. `_cs_git_root` is example-doctrine-repo-bash-specific with no
