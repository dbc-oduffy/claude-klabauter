...
@pytest.fixture
def as_example_doctrine_repo(monkeypatch: pytest.MonkeyPatch, sibling_repo: Path) -> Path:
    """Route `repo: example_doctrine_repo` at `sibling_repo` via the same monkeypatch
...
def test_stale_title_status_mismatch_surfaces_actionable_without_auto_flip(
    ledger_dir: Path, as_example_doctrine_repo: Path
) -> None:
    sha = _make_commit(as_example_doctrine_repo, "second")
...
def test_file_evidence_resolves_true_when_present(ledger_dir: Path, as_example_doctrine_repo: Path) -> None:
    target = as_example_doctrine_repo / "coordinator" / "docs" / "wiki"
...
def test_file_evidence_strips_known_leading_repo_name_segment(
    ...
    target = as_example_doctrine_repo / "coordinator" / "hooks" / "scripts"
    target.mkdir(parents=True)
...
def test_symbol_evidence_resolves_via_module_file_existence(
    ...
    target = as_example_doctrine_repo / "coordinator_core" / "reconcile"
    target.mkdir(parents=True)
