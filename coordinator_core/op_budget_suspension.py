...
SUSPENDED_OPS: Dict[str, Dict[str, object]] = {
    ...
    "deliverable.cascade_terminal": {
        ...
        "disposition": (
            ...
            "_run_cascade. BASELINE (state/audits/2026-08-30-the-cascade-16ms-figure-"
            "measured-a-no-op.md; docs/research/spike-verdicts/2026-08-30-cascade-"
            "one-corpus-read-collapse.md), 1 advanced candidate: the prior "
            "'16.3ms/call process, 0 spawns' figure measured a zero-candidate "
            "no-op — 0 spawns is only reachable when nothing advances, because the "
            "spawns are emitted by the per-candidate advance itself. Re-measured at "
            "the real call shape: 218.8ms p50 (max 328.1ms, n=8) on a synthetic "
            "275-file corpus, and 343-578ms on the real, live 303-file handoff "
            "corpus (baseline falsifier run; the live corpus grows across a "
            "session as peers land handoffs -- the post-fix falsifier runs below "
            "self-report corpus_size=305) -- both over the 200ms bar. FIX LANDED "
            "2026-08-30 "
            "(docs/plans/2026-08-30-the-terminal-cascade-reads-the-corpus-once.md, "
            "C1/C3): the corpus-wide read `_predicate_refusal`'s leg (b) repeated is "
            "collapsed to the one read `_collect_live_candidates_for_kind` already "
            "does, threaded via `corpus_metas`. POST-FIX, 1 advanced candidate, "
            "measured by the plan's falsifier "
            "(docs/plans/2026-08-30-the-terminal-cascade-reads-the-corpus-once."
            "falsifier.py) on the live handoff corpus (corpus_size=305 on every "
            "run), 5/5 runs GREEN: 62.50-93.75ms (per-run collapsed values 93.75, "
            "62.50, 78.12, 62.50, 62.50ms), under the 200ms bar, with the collapsed and baseline arms "
            "advancing identical candidates (arms_agree=True every run). The two "
            "git spawns are unchanged -- archive_stamp.stamp_shipped_in's "
            "scope-derived leg (git log + git status, each scoped to that "
            "candidate's own scope paths), unbatched, ~10ms of the total -- and "
            "were never the bottleneck: the corpus re-read was."
        ),
        "spinoff": None,
    },
