...
Purpose: unify the two independent, drifting literals this module replaces:
    - coordinator-claude `coordinator/hooks/scripts/check-claude-md-size.py`: 39900 hard / 39000 soft
...
constant consumed by claude-klabauter's own Check 7 cannot live in a coordinator-claude hook script).
...
leave under it (coordinator-claude's hook: 100-byte margin; claude-klabauter's Check 7: 2000-byte
...
that module's docstring for why no exact tokenizer dependency is pinned):
    ...
    coordinator/CLAUDE.md   (coordinator-claude, dev-repo sentinel)   39,896 B  -> ~9,974 tokens
...
repo. Coordinator-claude's own manifest content (its `GOVERNED_AUTHORING_SURFACES`
tuple, `coordinator/hooks/scripts/_claude_md_ledger.py`) is coordinator-claude-owned
...
convention coordinator-claude's `_claude_md_ledger.py` (C7a) already defined and reads for
its own PreToolUse admission gate. This module does NOT import that coordinator-claude
module -- `check_claude_md_size.py`'s own docstring already rules out this
engine resolving "the coordinator-claude repo" and reading its working data, and that
...
the two enforcement points (coordinator-claude's PreToolUse hook, this engine's commit-time
...
#: Unified byte thresholds — see "Derivation" above. Both coordinator-claude's hook and
...
#: SOURCE repo (coordinator-claude) rather than an OSS/percolated install or an
#: unrelated sibling repo. See coordinator-claude `.coordinator-dev-repo` and
...
def is_governed_claude_md(
    ...
    (coordinator-claude's own repo-root CLAUDE.md, claude-klabauter's, any sibling
    ...
    Governed (True), unconditionally:
        ...
        - `<repo-root>/coordinator/CLAUDE.md` where `<repo-root>` carries the
          ...
          coordinator-claude coordinator plugin-doctrine source file specifically,
    ...
    manifest entry naming it:
        - `<repo-root>/CLAUDE.md` (any repo's own project-scoped file,
          coordinator-claude's included — that file lives at repo root, not under
          `coordinator/`).
...
def is_claude_md_class(path: Union[str, Path]) -> bool:
    ...
    every file a coordinator-claude session can encounter that carries CLAUDE.md-shaped
    ...
    enumerated in coordinator-claude `coordinator/docs/wiki/claude-md-surfaces.md` —
    ...
    § C2; class definition: coordinator-claude coordinator/docs/wiki/claude-md-surfaces.md.
    """
    resolved = Path(path).resolve()
...
def is_ledger_admission_scoped(path: Union[str, Path]) -> bool:
    ...
    (`_claude_md_ledger.py`, coordinator-claude): a PreToolUse hook and a CI-path
...
def governed_surface_paths(
    ...
    caller running from a non-coordinator-claude repo (or omitting `repo_root`) gets
...
# coordinator-claude's `coordinator/hooks/scripts/_claude_md_ledger.py` -- see the
...
#: convention -- mirrors coordinator-claude's `_claude_md_ledger._LEDGER_PATH_OVERRIDES`
...
class RatchetWatermarkError(Exception):
    ...
    ratchet it claims to enforce. Mirrors coordinator-claude's `_claude_md_ledger.LedgerError`
...
def surface_slug(surface: str) -> str:
    ...
    `_LEDGER_PATH_OVERRIDES` entry. Mirrors coordinator-claude's `_claude_md_ledger.
...
def resolve_ledger_path(repo_root: Union[str, Path], surface: str) -> Path:
    ...
    convention coordinator-claude's `_claude_md_ledger.resolve_ledger_path` already uses, so
