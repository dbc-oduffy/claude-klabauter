"""
coordinator_core.workstream_complete.test_apply_op_dispatch — AC4/AC7
coverage for C7's admission control on `workstream_complete.apply`.

Purpose: `workstream_complete.apply` is one of the three completion-family
...
def test_every_consumes_manifest_member_is_allowlisted() -> None:
    ...
    for cli_name in wsc_apply._CLI_DISPATCH:
        assert cli_name in allowed, (
            f"{cli_name!r} is in workstream_complete's CONSUMES_MANIFEST-derived "
            f"_CLI_DISPATCH but absent from ASSEMBLER_DISPATCHABLE['workstream_complete']"
        )
...
def test_resolve_cli_unrecognized_name_raises_before_dispatch() -> None:
    with pytest.raises(UnrecognizedDirective):
        wsc_apply._resolve_cli("not-a-real-cli-name")
