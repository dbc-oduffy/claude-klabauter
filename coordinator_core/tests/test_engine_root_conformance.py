"""
test_engine_root_conformance.py — consumes example-doctrine-repo's engine-root
...
against every declarative case in example-doctrine-repo's fixture, and asserts both agree
with each other and with the fixture's ``expect``.

Spec backlink:
    docs/plans/2026-08-07-two-tier-engine-root-adopt-dr132.md (chunk C8)
    example-doctrine-repo coordinator/engine-root-contract/conformance/
    ...
    example-doctrine-repo coordinator/hooks/scripts/_engine_root.py
...
Example-doctrine-repo checkout, resolved via ``machine-local get
...
key then ``.claude-klabauter-root`` sentinel only); example-doctrine-repo's ``_resolve_live_working_tree``
...
_SKIP_REASON = (
    ...
    "register the example-doctrine-repo checkout via 'machine-local set "
    "engine.working_repos.example_doctrine_repo <path>' to run the example-doctrine-repo conformance oracle. "
...
def _load_fixture(doe_root: Path) -> dict:
    ...
    if not fixture_path.is_file():
        raise AssertionError(
            f"example-doctrine-repo checkout is registered at {doe_root} but the conformance "
...
def _sibling_engine_checkout_absent(doe_root: Path) -> bool:
    """Recompute example-doctrine-repo's rung-3 sibling-walk expression against example-doctrine-repo's own
    ...
    of example-doctrine-repo's ``_engine_root.py`` by path, purely to evaluate
    ``Path(__file__).resolve().parents[3].parent / _CLAUDE_KLABAUTER_SIBLING_DIR_NAME``
    exactly as example-doctrine-repo's own rung 3 would."""
...
_NO_ENV_RUNGS_REASON_TEMPLATE = (
    ...
    "'.claude-klabauter-root' sentinel only. Example-doctrine-repo's _resolve_live_working_tree "
...
_XFAIL_ENV_RUNG_REASON: dict[str, str] = {
    ...
    "entrypoints disagree with EACH OTHER, not just with example-doctrine-repo's expectation.",
}
...
def _invoke(fn, error_type) -> Any:
    ...
    rather than propagating — example-doctrine-repo's own ladder never raises (fail-open,
        ...
        if preconditions.get("sibling_engine_checkout_absent"):
            if not _sibling_engine_checkout_absent(_DOE_ROOT):
                pytest.skip(
                    ...
                    "'claude-klabauter' sibling checkout next to the example-doctrine-repo clone, so "
                    "example-doctrine-repo's rung-3 __file__-relative sibling walk always answers "
