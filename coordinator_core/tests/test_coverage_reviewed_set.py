...
# § C4) — coverage's read side honours the same record-keyed reviewer
# attestation C1/C2 persist and C3 credits, so the two coverage surfaces do
# not disagree (AC6). `_FOREIGN_STRIPPED_SCOPES` itself is untouched (AC7):
# a foreign commit with NO attestation is stripped exactly as before, for
# every scope in that set.
# ---------------------------------------------------------------------------

_ATTESTED_SESSION = "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee"


def _write_attested_record(
    path: Path,
    sha_range: str,
    scope: str,
    session_id: str,
    attested_shas: List[str],
) -> None:
    """Write a review-trail record carrying C1's `reviewer_attestation` key
    directly (the JSON shape `_parse_reviewer_attestation` reads) — bypassing
    `write_review_trail_entry`/`_guard_foreign_session_range` entirely, since
    those are C1/C2's own write-path tests. This module tests only the READ
    side (C4): what `coverage.py` does when handed a record that already
    carries the key.
    """
    record = {
        "sha_range": sha_range,
        ...
        "reviewer_attestation": {
            ...
            "sidecar": "state/subagent-share/x/reviewer-attest.md",
            "shas": attested_shas,
        },
    }
    path.write_text(json.dumps(record), encoding="utf-8")
...
def test_record_attested_shas_reads_via_parse_reviewer_attestation() -> None:
    """`_record_attested_shas` returns the persisted `shas` list as a
    frozenset when present, and an empty set for every absent/malformed
    shape — fail-closed, never a blanket "attests to everything" fallback.
    """
    from coordinator_core.coverage import _record_attested_shas

    assert _record_attested_shas({}) == frozenset()
    assert _record_attested_shas({"reviewer_attestation": None}) == frozenset()
    assert _record_attested_shas(
        {"reviewer_attestation": {"shas": ["deadbeef", "cafef00d"]}}
    ) == frozenset({"deadbeef", "cafef00d"})
    # Malformed shas value (not a list) — empty, not an exception.
    assert _record_attested_shas(
        {"reviewer_attestation": {"shas": "deadbeef"}}
    ) == frozenset()
    # Non-string / empty-string entries in the list are dropped, not raised.
    assert _record_attested_shas(
        {"reviewer_attestation": {"shas": ["deadbeef", "", None, 123]}}
    ) == frozenset({"deadbeef"})


def test_narrow_foreign_session_scope_admits_attested_sha(tmp_path: Path) -> None:
    """A commit that is foreign (per its own Session-Id trailer) AND named in
    `attested_shas` is pulled back OUT of `_narrow_foreign_session_scope`'s
    returned foreign-strip set — the record-keyed admission this chunk adds.
    """
    from coordinator_core.coverage import _narrow_foreign_session_scope
    ...
    own_session = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"
    predecessor_session = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"

    p1 = _commit_for_session(repo, "P1: predecessor commit", predecessor_session)
    p2 = _commit_for_session(repo, "P2: a SECOND, unattested predecessor commit", predecessor_session)

    sha_range = f"{p1}^..{p2}"
    cache: dict = {}

    # No attestation (default) — both foreign commits are stripped, exactly
    # as before this chunk (AC5-equivalent no-op for the un-narrowed path).
    foreign_unattested = _narrow_foreign_session_scope(sha_range, own_session, str(repo), cache)
    assert foreign_unattested == {p1, p2}

    # p1 attested — pulled OUT of the returned foreign set; p2 stays foreign.
    foreign_attested = _narrow_foreign_session_scope(
        sha_range, own_session, str(repo), dict(cache), frozenset({p1}),
    )
    assert p1 not in foreign_attested
    assert p2 in foreign_attested


def test_build_reviewed_set_chain_scope_credits_attested_predecessor_commit(
    tmp_path: Path,
) -> None:
    """End-to-end (fan-out strategy): a scope="chain" record whose persisted
    `reviewer_attestation.shas` names an otherwise-foreign predecessor commit
    credits that commit — the write-side admission (C2) made creditable on
    the read side (C4), not by ancestry or chain membership (Anti-scope).
    """
    from coordinator_core.coverage import build_reviewed_set
    ...
    predecessor_session = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"
    p1 = _commit_for_session(repo, "P1: predecessor's reviewed commit", predecessor_session)
    p2 = _commit_for_session(repo, "P2: predecessor's SECOND commit, own session's own end", predecessor_session)

    chain_set = {p1, p2}

    record_path = tmp_path / "chain_record.json"
    _write_attested_record(
        record_path, f"{p1}^..{p2}", "chain", _ATTESTED_SESSION, [p1],
    )
    ...
    assert p1 in reviewed, (
        "an attested predecessor commit named in reviewer_attestation.shas "
        "must be credited — this is the read-side half of AC6"
    )
    assert p2 not in reviewed, (
        "p2 was never named in the attestation — per-SHA admission (AC2's "
        "governing rule, mirrored on the read side): the attested {p1} set "
        "does not widen to admit an unattested sibling commit"
    )


def test_build_reviewed_set_attestation_is_per_sha_not_full_range(tmp_path: Path) -> None:
    """AC2, restated on the read side: an attestation naming SHA A does not
    admit sibling foreign SHA B merely because both fall inside the same
    sha_range — each must be named individually.
    """
    from coordinator_core.coverage import build_reviewed_set
    ...
    predecessor_session = "cccccccc-cccc-4ccc-8ccc-cccccccccccc"
    a = _commit_for_session(repo, "A: attested predecessor commit", predecessor_session)
    b = _commit_for_session(repo, "B: UNattested predecessor commit", predecessor_session)
    c = _commit_for_session(repo, "C: predecessor's range end", predecessor_session)

    chain_set = {a, b, c}

    record_path = tmp_path / "chain_record.json"
    _write_attested_record(
        record_path, f"{a}^..{c}", "chain", _ATTESTED_SESSION, [a],
    )
    ...
    assert b not in reviewed, "attested {A} must not widen to admit sibling B"
    assert c not in reviewed, (
        "c carries predecessor_session's trailer too and is unattested — "
        "must not be credited either"
    )


def test_build_reviewed_set_no_attestation_key_behaves_exactly_as_before(
    tmp_path: Path,
) -> None:
    """A record with no `reviewer_attestation` key at all strips a foreign
    predecessor commit exactly as pre-C4 (AC5-equivalent: absence changes
    nothing for the overwhelming majority of records, which never carry the
    key).
    """
    from coordinator_core.coverage import build_reviewed_set
    ...
    predecessor_session = "dddddddd-dddd-4ddd-8ddd-dddddddddddd"
    p1 = _commit_for_session(repo, "P1: unreviewed predecessor commit", predecessor_session)
    p2 = _commit_for_session(repo, "P2: predecessor's range end", predecessor_session)
    chain_set = {p1, p2}
    ...
    assert reviewed == set(), (
        "no attestation key at all must strip every foreign commit exactly "
        "as before this chunk — absence is a no-op"
    )


def test_build_reviewed_set_graphwalk_credits_attested_predecessor_commit(
    tmp_path: Path,
) -> None:
    """Strategy A (single-graph-walk, engaged when graph_range + intersect_shas
    are both supplied) must admit the identical attested commit the fan-out
    strategy (Strategy B) admits — both code paths route through
    `_narrow_foreign_session_scope`'s new `attested_shas` parameter via the
    `attested_by_range_session` side-table this chunk threads through
    `_reviewed_via_graph_walk`.
    """
    from coordinator_core.coverage import build_reviewed_set
    ...
    predecessor_session = "ffffffff-0000-4fff-8fff-ffffffffffff"
    c0 = _make_commit(repo, "C0: window base")
    p1 = _commit_for_session(repo, "P1: attested predecessor commit", predecessor_session)
    p2 = _commit_for_session(repo, "P2: predecessor's range end", predecessor_session)
    ...
    walk = build_reviewed_set(
        [str(record_path)], on_record_error="skip", intersect_shas=chain_set,
        repo_root=str(repo), graph_range=graph_range,
    )
    fanout = build_reviewed_set(
        [str(record_path)], on_record_error="skip", intersect_shas=chain_set,
        repo_root=str(repo), graph_range=None,
    )

    assert walk == fanout == {p1}, (
        f"graph-walk and fan-out must agree on the attested set.\n"
        f"  walk={sorted(walk)} fanout={sorted(fanout)}"
    )


def test_build_reviewed_set_plan_attestation_does_not_leak_into_sibling_diff_record(
    tmp_path: Path,
) -> None:
    """BLOCKING finding (coordinatorcode-reviewer-e2cc5b19, slice B): two
    records share one (sha_range, session_id) but differ in `scope_kind`
    (kind) — a `scope_kind: "plan"` record attests foreign predecessor P,
    a sibling `scope_kind: "diff"` record citing the SAME range/session
    carries no attestation of its own. `attested_by_range_session` used to
    be keyed on (sha_range, session_id) alone, so the plan record's
    attestation leaked into narrowing the diff record too — crediting P
    against the CODE obligation using an attestation that only ever named
    it for the PLANNING obligation. The accumulator key is now
    (sha_range, scope, session_id, kind), matching `distinct_ranges`' own
    dedup key (the same tuple the surrounding comment there documents this
    exact plan/diff-same-range collision for, AC6).

    Both Strategy A (graph-walk) and Strategy B (fan-out) must agree P is
    NOT credited via the diff-kind record — and C3's per-record reader
    (`_record_membership_shas`, immune to this class of leak by
    construction, since it is always called with `attested_shas(record)`
    for THAT record only) must land on the identical verdict.
    """
    from coordinator_core.coverage import (
        build_reviewed_set,
        _narrow_foreign_session_scope,
        _record_attested_shas,
    )
    ...
    predecessor_session = "22222222-2222-4222-8222-222222222222"
    p = _commit_for_session(repo, "P: predecessor's commit", predecessor_session)

    sha_range = f"{p}^..{p}"
    chain_set = {p}

    plan_record_path = tmp_path / "plan_record.json"
    _write_attested_record(plan_record_path, sha_range, "chain", own_session, [p])
    plan_record = json.loads(plan_record_path.read_text(encoding="utf-8"))
    plan_record["scope_kind"] = "plan"
    plan_record_path.write_text(json.dumps(plan_record), encoding="utf-8")
    ...
    diff_record_path = tmp_path / "diff_record.json"
    diff_record_path.write_text(json.dumps(diff_record), encoding="utf-8")

    fanout = build_reviewed_set(
        [str(plan_record_path), str(diff_record_path)],
        on_record_error="skip",
        intersect_shas=chain_set,
        repo_root=str(repo),
        graph_range=None,
    ...
    assert p not in fanout and p not in walk, (
        f"the plan record's own attestation of P must not leak into "
        f"crediting the sibling diff-kind record at the same "
        f"(sha_range, session_id) — P was never named for the CODE "
        f"obligation, only the planning one.\n"
        f"  fanout={sorted(fanout)} walk={sorted(walk)}"
    )

    # AC6 agreement, extended to the previously-uncovered kind-collision
    # case: C3's per-record reader must reach the identical verdict for
    # the diff-kind (code) record.
    cache: dict = {}

    def _narrow_foreign_shas(rng: str, session_id):
        return _narrow_foreign_session_scope(rng, session_id, str(repo), cache)

    membership = _record_membership_shas(
        diff_record,
        resolve_range_shas=lambda rng: {p},
        chain_dag_sha_set=chain_set,
        chain_code_sha_set=chain_set,
        narrow_foreign_shas=_narrow_foreign_shas,
        attested_shas=_record_attested_shas,
    )
    assert not membership, (
        "C3's per-record reader must also decline to credit P for the "
        "diff-kind record (it was already immune to this leak — "
        "attested_shas is called with THIS record, never a shared "
        "session-keyed accumulator); coverage.py's fixed behaviour must "
        f"agree with it, not merely stop diverging — got {membership!r}"
    )


def test_foreign_stripped_scopes_unchanged_by_c4() -> None:
    """AC7: `_FOREIGN_STRIPPED_SCOPES` itself is not widened, narrowed, or
    otherwise touched by C4 — admission is decided per-record via the
    attested-shas side table, never by adding a new scope value to this set.
    """
