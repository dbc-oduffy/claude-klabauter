...
Spec backlink: docs/plans/2026-08-07-spawn-storm-culprit-taxonomy-and-detectors.md,
chunk D9 / AC11.

Targets one entrypoint per hook family, each a cold interpreter spawned by a
Example-doctrine-repo `coordinator/hooks/scripts/*.py` PLUMBING wrapper on every matching
tool call:
...
wrapper is a real available fix but is example-doctrine-repo-tree territory (this claude-klabauter
