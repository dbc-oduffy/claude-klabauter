...
Ports the 7 hand-picked cases from example-doctrine-repo-side test-normalize-snippet.sh (bash
oracle, retired on cutover) plus a differential fuzz pass against the
original bash `normalize()` function when the example-doctrine-repo sibling repo is
discoverable on disk.

Port of: normalize-snippet.sh (example-doctrine-repo 67202df6, 2026-07-16)
Test oracle: test-normalize-snippet.sh (example-doctrine-repo 67202df6, 2026-07-16)
Spec backlink: example-doctrine-repo scratch/subagent-sandbox/bash-to-python-engine-migration/recipe-normalize-snippet.md § 6
"""
...
# example-doctrine-repo sibling repo (holding coordinator/lib/normalize-snippet.sh) is not
...
@pytest.mark.skipif(
    ...
    reason="bash or example-doctrine-repo sibling repo (coordinator/lib/normalize-snippet.sh) not available",
)
