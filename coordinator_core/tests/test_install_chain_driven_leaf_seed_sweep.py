...
uses) out of example-doctrine-repo's own
...
Claude-klabauter does not vendor that template (it is coordinator-claude/example-doctrine-repo
content, not claude-klabauter's — see claude-klabauter's own CLAUDE.md § What this repo
is), so this test resolves the example-doctrine-repo repo root the same registry-first,
...
def test_driven_leaf_seed_sweep_pickup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    resolving the example-doctrine-repo sibling root is a READ-ONLY oracle lookup (same
    class of use the marker exists for). That opt-out is held open ONLY long
    enough to perform the read-only example-doctrine-repo-root lookup and to resolve the real
