...
_DISPOSITIONS: Dict[str, Tuple[str, str]] = {
    ...
    'coordinator_core/hooks/postuse_advisory_dispatch.py': ('in-repo-non-state', 'raw-write site(s), no state/-component signal: _check_group_em_watch_arm_sync, _check_group_em_watch_arm_sync._touch_checked_sentinel, _check_runtime_tripwire_sync, _check_workflow_monitor_arm_sync, _save_advisory_state'),
    ...
    'coordinator_core/ops/append_integrator_dispositions.py': ('claims-explicitly', 'claim token in append_dispositions'),
    ...
    'coordinator_core/ops/emit/recorder.py': ('to-fix', "raw-write site(s): record; runtime observed=no (n=0), sample=n/a [static-only: flagged by census row 2, not exercised in C2's one run — gap stays visible, per C3 body]"),
...
_TO_FIX_CEILING: FrozenSet[str] = frozenset({
    ...
    'coordinator_core/ops/emit/recorder.py',
