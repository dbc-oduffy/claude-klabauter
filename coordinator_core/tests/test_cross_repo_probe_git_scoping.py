...
def decoy(tmp_path: Path) -> Path:
    ...
    It deliberately carries the SAME paths the probes read out of the example-doctrine-repo clone,
...
def doe(tmp_path: Path) -> Path:
    """A real repo shaped like a example-doctrine-repo clone."""
    root = tmp_path / "example-doctrine-repo-fake"
    _seed_repo(
        root,
        {
...
def vendored(tmp_path: Path, doe: Path) -> Path:
    """A vendored schema copy byte-identical to the example-doctrine-repo clone's HEAD."""
...
def test_check_schema_drift_is_not_retargeted_by_an_inherited_git_dir(
    ...
    """The vendored copy MATCHES example-doctrine-repo HEAD, so this must pass silently.
...
def test_check_schema_drift_reports_an_unreadable_clone_as_not_a_drift_finding(
    ...
    """A example-doctrine-repo path that is not a git repository is a could-not-check, and the
...
def test_probe_freshness_ref_reads_the_doe_clones_own_origin(monkeypatch, doe, decoy):
    ...
    "example-doctrine-repo has not published it yet", a confident false claim about a remote this
...
def test_probe_freshness_ref_skips_an_unreadable_clone_without_claiming_absence(tmp_path):
    ...
    assert "not a finding about what example-doctrine-repo has or has not published" in text
...
def test_tag_ancestry_is_not_retargeted_by_an_inherited_git_dir(monkeypatch, doe, decoy):
    ...
    assert doe_drift._tag_is_ancestor_of_pin(doe, doe_head, doe_head) is True, (
        "a commit that exists only in the example-doctrine-repo clone became unresolvable — the "
