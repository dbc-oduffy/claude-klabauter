...
def collect(cadence: str, *, run_id: Optional[str] = None) -> ReaderResult:
    ...
    the CALLING project's own `state/bug-backlog/` (per-repo, or claude-klabauter's
    central state only when the calling git root IS the meta-repo), never
    `claude-klabauter`'s own repo root. When the state root can't be
    resolved (no git root, or claude-klabauter unresolvable), returns an empty
