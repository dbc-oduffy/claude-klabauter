...
Arrival note (W4-C6, `docs/plans/2026-09-18-doe-holds-no-scripts.md`): this
module is a plain importable support library, not a `hooks.<name>` op — it
carries no payload-dict-in/response-out contract (see `_payload.py`,
`_envelope.py` for the same non-op-handler shape in this package). It is
NOT listed in `coordinator_core/hooks/__init__.py`'s `_EAGER_HOOK_MODULES`.
The DR-141 predicate cluster that actually reads publish-locality policy
(`_prompt_surface_locality.py`, `guard-oss-payload-locality.py`, and the
...
a DoE-repo-relative `Path(__file__)` walk, because that YAML is a
harness-discovery surface this module no longer sits three directories
under (see `_resolve_mcp_topology_path` below). `_engine_sibling_record`'s
`import _oss_payload` is UNCHANGED and deliberately still dynamic/fail-open:
it is a runtime `sys.path` import, not a `Path(__file__)`-relative one, so
it resolves correctly when a DoE-resident caller (which still has its own
`coordinator/hooks/scripts/` on `sys.path`) imports this module, and
degrades to `{}` here in claude-klabauter's own process, where no `_oss_payload`
module exists — exactly the fail-open contract the original docstring
already specified.
...
def _resolve_mcp_topology_path() -> Optional[Path]:
    """Locate `mcp-topology.yaml` under the coordinator-claude plugin's
    CONTENT root — a doctrine/harness-discovery surface this module (now
    engine-plane, in claude-klabauter) does not ship, mirroring
    ...
    Local imports: this module is a plain support library other packages
    import broadly (including DoE-resident consumers over the wire), so it
    keeps a bare top-level import surface and only pulls the settings-home
    / data-root helpers on the cold path that actually needs them.

    Returns `None` on no match at any rung; the caller (`mcp_tool_prefixes`)
    fails open to an empty frozenset.
    """
    ...
    for candidate_root in (plugin_base / "coordinator", plugin_base):
        candidate = candidate_root / "mcp-topology.yaml"
...
def _engine_sibling_record() -> dict:
    ...
    `import _oss_payload` is deliberately still a bare, dynamic (runtime
    `sys.path`) import, not a `Path(__file__)`-relative one — see this
    module's own docstring's arrival note. It resolves when a DoE-resident
    caller (which still has its own `coordinator/hooks/scripts/` on
    `sys.path`) imports this module, and fails open to `{}` when this
    module runs in claude-klabauter's own process, where no `_oss_payload` module
    exists."""
    try:
...
#: file. The rationale ships alongside the literal instead of drifting into
#: a separate changelog no one re-reads.
...
# ---------------------------------------------------------------------------
# MCP tool-name prefixes — derived from mcp-topology.yaml
# ---------------------------------------------------------------------------
...
def mcp_tool_prefixes() -> frozenset:
    """Every `mcp__<server>__` prefix derivable from `mcp-topology.yaml`'s
    declared `configKey` list. Empty on any read/parse/resolve failure
    (missing PyYAML, missing/malformed file, unresolvable plugin content
    root) — fail-open, same contract as the citation detector's
    ratified-data loaders: a load failure narrows this ONE carve-out rather
    than bricking the whole detector."""
    try:
        import yaml
    except Exception:
        return frozenset()
    topology_path = _resolve_mcp_topology_path()
