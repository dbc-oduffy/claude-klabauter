...
-------------------------------------------------
`guard_runner_contract.py` earned its twelve clauses solving ONE specific
...
deny surfaces; every advisory concatenates). The four PostToolUse
write-path guards this contract enrols speak a DIFFERENT native protocol
entirely -- the Stop-family shape (`message_envelope.CHANNEL_STOP`, or the
identical hand-rolled stderr-write-then-`return 2` idiom): each guard
EITHER writes its own prose directly to stderr and returns `2`, OR stays
silent and returns `0`. There is no deny/advisory class distinction here,
no `permissionDecisionReason` vs `additionalContext` split -- just N
independent "did I have something to say" signals that must combine into
ONE process's ONE stderr write and ONE exit code.
`guard_runner_contract.py`'s own docstring scopes this out explicitly:
"CHANNEL_STOP is a Stop-family shape out of scope here." Widening that
contract's vocabulary to also describe this shape would produce one
contract describing two protocols imprecisely, forcing every future guard
author to work out which half applies to them. Two small honest contracts
beat one general dishonest one -- this module is the second one,
deliberately kept independent (its own enrolment list, its own aggregation
rule, its own conformance test) rather than layered onto the first.

The ONE thing reused from `guard_runner_contract.py`, and why that is not
...
1. ENTRYPOINT SHAPE. A guard exposes `main() -> int`, returning `0` (silent)
   ...
   flow -- same reasoning as `guard_runner_contract.py` clause 1: the
...
2. NO `os._exit`, NO `atexit` HANDLER REGISTRATION, NO CWD/`sys.path`
   ...
   `guard_runner_contract.py` clauses 2-5, 7 (general Python-hygiene rules
...
4. AGGREGATION IS CONCATENATE-ALL, NEVER FIRST-FIRES-WINS. Across one
   ...
   `guard_runner_contract.py` clause 10's class-aware DENY-vs-advisory
   split -- there is no "class" here, every fired guard's text is equally
   an advisory the reader must see. First-fires-wins would silently drop
   every subsequent guard's advisory on a payload where two or more fire
   together.
...
5. EXCEPTION ISOLATION. Each guard's `main()` call runs inside its own
   ...
   same reasoning as `guard_runner_contract.py` clause 11.

6. LAZY IMPORT IS TWO-STAGE, via `GuardScopeDescriptor` (imported from
   `guard_runner_contract.py` -- see the module docstring's "ONE thing
   ...
   `guard_runner_contract.py` clause 12.

Conformance test: a grep-conformance sweep mirroring
`guard_runner_contract.py`'s own, sourced from THIS module's own
