"""coordinator_core.hooks.phantom_staged_deletion — pure predicate for the
phantom-staged-deletion guard: is a staged deletion in this commit about to
...
is not reachable from `pytest` without spawning, and this repo's spawn
budget is already governed by `coordinator_core/tests/
test_no_unbatched_per_item_git_spawn.py`. This module is PURE (no `git`
subprocess, no filesystem read of its own) so it needs no such exemption.

THE HAZARD. On a tree a dozen sessions share, `.git/index` is shared
mutable state. A session loads the index at T0; another tool commits at T1
and HEAD advances; the first session writes its T0-derived index back at
T2. The index is now BEHIND HEAD, and `git status` reports one path as both
`D ` (staged deletion, HEAD-vs-index) and `??` (untracked, index-vs-
worktree) while the file is present in HEAD and on disk. The next bare
`git commit` carries that deletion, erasing a live artifact inside a commit
about something else. A leaked staged ADD is visible in review as an
unexpected file; a leaked staged DELETE removes evidence.

THREE VANTAGE POINTS, NOT TWO. This is the design constraint. Index-versus-
worktree alone reads as "a file was deleted and an untracked file
appeared." HEAD-versus-index alone reads as "a committed file is staged for
deletion." Both are ordinary. Only adding disk-versus-HEAD shows that the
file the index says is gone is sitting on disk with HEAD's own content.
`classify()` therefore takes all three as separate inputs and refuses to
conclude from any two.

WHAT THIS DELIBERATELY DOES NOT DO. It does not distinguish the
stale-index phantom from a deliberate `git rm --cached` -- measured, they
are byte-identical in `git status` (`D ` plus `??`, file on disk). Both
mean a path in HEAD is about to leave HEAD while its content sits on disk,
so both are worth stopping; the caller offers an override rather than
...
def parse_name_status_z(raw: str) -> "list[tuple[str, str]]":
    ...
    A rename or copy record spends THREE fields -- status, source,
    destination -- so its destination must be consumed or every subsequent
    row shifts by one and the whole parse walks off. Renames are returned
    under their DESTINATION path, since that is the path the commit ends up
    carrying.
    """
