...
_MSG_WITH_PLUGIN = (
    "DELEGATION REQUIRED: You are about to do web research as Opus. "
    "The EM orchestrates — researchers execute. Use the dedicated research infrastructure, "
    "not ad-hoc agent dispatch:\n\n"
    "- Internet research (web sources) → /coordinator:research --mode=web <topic>\n"
    ...
    "Only proceed with direct web calls if: (1) the user pasted you a specific URL and asked you to "
    "read it — one fetch, no research, or (2) you are verifying a single fact mid-conversation where "
    "dispatching an agent is pure overhead.\n\n"
    "Do NOT spin up a generic Agent(prompt='go search for...') — that discards tested guardrails "
    "(phase separation, quality gates, Haiku grounding). Opus tokens are for judgment, not for "
    "reading web pages."
)
...
_MSG_WITHOUT_PLUGIN = (
    ...
    "The EM orchestrates — researchers execute. Use the dedicated research infrastructure, "
    "not ad-hoc agent dispatch:\n\n"
    "- Any research (web/repo/structured) → install the deep-research plugin, then /coordinator:research --mode={web,repo,structured}\n"
    "- Quick codebase exploration → Agent with subagent_type='Explore'\n"
    "- Enriching specs with codebase facts → Agent with subagent_type='coordinator:enricher'\n\n"
    "Only proceed with direct web calls if: (1) the user pasted you a specific URL and asked you to "
    "read it — one fetch, no research, or (2) you are verifying a single fact mid-conversation where "
