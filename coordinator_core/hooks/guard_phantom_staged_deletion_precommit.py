...
`coordinator_core.hooks.phantom_staged_deletion` — same pattern the W4-C5
sibling used for `doctrine_changelog_prose`. `main()` below is otherwise
unchanged from the source. `command/native-door` (per the W4-C1 verdict)
does not apply to this row's entry: a git pre-commit hook is invoked
directly by `git`, never dialled through `hook-run`, so it carries no
`register_op` and no `hooks.<name>` op contract — see the module-level
`# guard-not-a-hook-entrypoint` marker above.

WHY THIS LAYER AND NOT THE BASH GUARD. The obvious cheaper answer is to
extend the existing scope guard, which already notices part of this and
reports it as an ownership question ("staged but not in this session's
touch list"). That guard's decision logic lives in the engine plane —
this repo, now — but its wiring is a different chunk's scope; extending it
is out of this row's own footprint. This layer is complete on its own and
needs no such grant.
...
A pathspec commit makes git build a temporary index holding only those
paths, so the armed path is invisible here and this hook stays silent. It
fires on exactly the case that loses data: a commit that would actually
carry the deletion. Firing merely because the tree has an armed path would
make it noise on a tree where that state is common, and noise gets
disabled.

ALL LOGIC LIVES IN `phantom_staged_deletion.py`, which is pure and unit
tested. This file is the untested wrapper: run git, hand the rows over,
print, exit. Do not move predicate logic in here -- a git hook is not
reachable from `pytest` without spawning, and this repo's own spawn budget
(`coordinator_core/tests/test_no_unbatched_per_item_git_spawn.py`) governs
that.

INHERITING `GIT_INDEX_FILE` IS LOAD-BEARING, NOT INCIDENTAL. Git sets it
(and `GIT_DIR`/`GIT_WORK_TREE`) when it fires a pre-commit hook, pointing
at the index for THIS commit -- which for a pathspec commit is a temporary
one holding only those paths. That inheritance is exactly why the scoped-
commit case above stays quiet. Do not scrub the git environment or pass an
explicit `--git-dir` here to make the subprocess "cleaner": that repoints
these reads at the repository index instead, and the guard would start
firing on armed paths the commit does not touch -- the noise mode that
gets a guard disabled. The opposite hazard, a hook spawning git in a
*different* cwd and staging into the real index through the inherited
variable, is a documented concurrent-EM hazard around `GIT_INDEX_FILE`;
nothing here changes directory.

NEVER TAKES `.git/index.lock`. Every git call here is a read
(`diff --cached`, `cat-file`) and passes `--no-optional-locks` so none of
them opportunistically refreshes the index. A detector that serialises
