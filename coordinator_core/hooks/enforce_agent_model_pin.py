...
for how the two compose into the one example-doctrine-repo-registered `PreToolUse(Agent)`
seam.
...
treating every dispatch as unpinned. In the composed, example-doctrine-repo-registered path
...
Two entrypoints, matching the sibling module's own convention:
    ...
    main()          -- stdin-JSON / stdout-JSON / exit-0 standalone script,
        for direct `hooks.json` registration (example-doctrine-repo-side; out of scope here,
