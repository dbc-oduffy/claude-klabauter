...
Path derivation is this op's job, not the caller's — example-doctrine-repo (the caller) must not open
...
def _derive_subagent_transcript_path(transcript_path: str, agent_id: str) -> str:
    ...
    (example-doctrine-repo) must not derive or open this path themselves.
    """
