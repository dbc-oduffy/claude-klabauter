"""
coordinator_core.warm.http_hook_forwarder -- the fixed-port front door Claude Code's `type: "http"`
...
_ENGINE_ROOT_RESOLVED_AT: str = _utc_now()
...
def engine_root_snapshot() -> Dict[str, Optional[str]]:
    """What this process resolves as the engine, for the dial file.

    Fixed at import rather than cached-and-invalidated -- see `_ENGINE_ROOT`'s own note for why
    the registry-lookup failure mode this used to report (`engine_root` silently wrong for the
    life of the process) cannot occur once the root is the module's own tree. `discovery_path` is
    still the one field worth memoising here: see `_note_discovery_path`.
    """
    with _engine_root_lock:
        return {
            "engine_root": str(_ENGINE_ROOT),
            "resolution_class": "module-tree",
...
def _resolved_engine_root() -> Optional[str]:
    """The engine root this module resolves against -- always `_ENGINE_ROOT`, never `None`.
    ...
    Still `Optional[str]` in signature, matching every caller's `if not engine_root:` guard below
    -- kept for call-site symmetry with `_resolve_backend`'s other resolution steps, even though
    this one never actually answers `None`.
    """
    return str(_ENGINE_ROOT)
...
def _note_discovery_path(supervisor, engine_root: str) -> None:
    ...
    Memoised because it is a pure function of the root, and the root here (`_ENGINE_ROOT`) is
    fixed for the process's whole life -- unlike the registry-resolved root this module used to
    run against, nothing invalidates this memo, so it is computed at most once. Best-effort and
    silent: an engine too old to expose `discovery_path` must not take the hot path down over a
    log field.
    """
...
def make_server(port: int = FIXED_PORT, host: Optional[str] = None) -> _ExclusiveServer:
    ...
    `host` defaults to `http_listener.bind_host()`, and falls back to the `127.0.0.1` literal if
    that import ever fails (the forwarder itself does not need the engine to bind; only
    per-request forwarding does) -- so this module still binds correctly, on the correct address,
    even against a broken `coordinator_core` install. A bind failure (port already exclusively
    held, e.g. by a prior winner of the binder election, or by a foreign process) raises `OSError`
    to the caller -- per `DR-http-hook-forwarder-fixed-port.md` Decision 4, that is a
    supervisor-level "not my port" signal, not a condition this module papers over by retrying a
    different port or degrading silently.
    """
    if host is None:
        try:
            from coordinator_core.warm import http_listener as _http_listener
...
def main(argv: Optional[list] = None) -> int:
    """CLI entry point: `python -m coordinator_core.warm.http_hook_forwarder [port]`.
