...
def seed_trust_anchor_keys(report: Report) -> None:
    ...
    THIS IS THE ORDERING SETUP.PY DEPENDS ON, not a convenience.
    ...
    unwired, `.doe-root` unwritten, and `cloud_setup.py` still exiting 0 — a
    ...
    def _write(key: str, value: str, *, fatal: bool) -> None:
        ...
        # `set` exiting 0 is not proof the value this process just wrote is the
        # value a SESSION will resolve for this key — the write and the read a
        # session performs (`machine-local get`) are two different code paths,
        # and a settings-home mismatch or an autodiscovery rung shadowing the
        # fresh write would otherwise land here as a silent pass while the
        # session reads something else entirely. Read the key back through the
        # exact same CLI a session uses, and treat a mismatch as the write
        # having failed — this is the failure this container hit: the report
        # recorded one path while the registry a session reads held another.
        ...
        read_back = verify.stdout.strip() if verify.returncode == 0 else None
        if read_back != value:
            report.machine_local_keys[key] = (
                f"failed: wrote {value!r} but a session's own read-back "
                f"resolved {read_back!r} (get exit {verify.returncode}) -- "
                "the write and the session's read diverged"
            )
            if fatal:
                failures.append(f"{key} (read-back mismatch)")
            return
...
def install_hooks_fleet(report: Report) -> None:
    ...
    SECOND vehicle, not only the fleet one, and BOTH before the STAT: the
    fleet walk's own worktree/mirror classification (`_is_coordinator_
    worktree`) deliberately excludes a repo with no `CLAUDE.md`/cross-repo
    marker as a push-mirror it must not touch — correct on a workstation,
    where klabauter is purely a publish destination separate from a real
    `claude_klabauter` checkout. In THIS container `repos.claude_klabauter` is
    pointed at this very klabauter clone (there is no other engine root to
    name), which carries neither marker, so the fleet silently never
    attempts a hook there — 100% of cloud boots, not a flake; the downstream
    report is `hooks.preuse_bash_dispatch` finding no `_origin_worktree` for
    a commit made in it. This function therefore also calls
    `coordinator-ensure-prepare-commit-msg-hook.py` directly, per
    `TRUST_ANCHOR_KEYS` destination, with that repo as `cwd` — bypassing the
    fleet's classification question for the two repos THIS script already
    knows must carry the hook, rather than teaching the shared classifier a
    cloud-specific carve-out it cannot detect on its own. The STAT-based
    assertion below is unchanged and remains the actual verdict: this is a
    second, best-effort install PATH, not a second trust boundary.

    Dependency-free by design (module docstring): both vehicle scripts are
    invoked as subprocesses; `git_hook_install` machinery is never imported
    here.
    """
    ...
    # ROOT CAUSE this closes: the fleet walk's own `_is_coordinator_worktree`
    # classifier (coordinator/bin/lib/git_hook_install.py) tells a genuine
    # push-mirror (no CLAUDE.md / cross-repo marker) apart from a working
    # tree it should install into — correct on a WORKSTATION, where
    # `repos.claude_klabauter` names an authoring checkout and the klabauter
    # clone is purely a publish destination. In THIS container there is no
    # separate claude-klabauter checkout: `seed_trust_anchor_keys` points
    # `repos.claude_klabauter` at this very klabauter clone (the only engine
    # root that exists here), which carries neither marker and so the fleet
    # classifies it `mirror` and silently never attempts a hook there —
    # deterministically, every cloud boot, not a flake. Downstream:
    # `hooks.preuse_bash_dispatch` sees no `_origin_worktree` routing key for
    # commits made in this checkout and the guard chain cannot evaluate.
    # Fixed here, not in the classifier: the classifier's heuristic is
    # correct for the fleet-wide case (14 other repos on a real workstation),
    # and this script is the one place that already knows — by construction,
    # via `TRUST_ANCHOR_KEYS` — exactly which two destinations MUST carry the
    # hook regardless of that heuristic. Calling the single-repo vehicle
    # directly, per destination, bypasses the classification question
    # entirely rather than teaching the classifier a cloud-specific carve-out
    # it has no way to detect on its own.
    single_repo_vehicle = (
        engine_root / "coordinator" / "bin" / "coordinator-ensure-prepare-commit-msg-hook.py"
    )
    if single_repo_vehicle.is_file():
        for clone_name in TRUST_ANCHOR_KEYS:
            ...
            try:
                single_result = subprocess.run(
                    ["python3", str(single_repo_vehicle)],
                    cwd=str(repo_root),
                    capture_output=True,
                    text=True,
                    timeout=MACHINE_LOCAL_TIMEOUT_S,
            ...
            except Exception as exc:  # noqa: BLE001 - the STAT check below is
                # the real verdict; this direct top-up is a best-effort
                # second install path, never the sole gate.
                print(
                    f"[cloud_setup] direct hook install into {clone_name} "
                    f"({repo_root}) failed to run: {exc}",
                    file=sys.stderr,
                )
                continue
            if single_result.stdout:
                print(single_result.stdout, end="")
            if single_result.stderr:
                print(single_result.stderr, end="", file=sys.stderr)
    else:
        print(
            f"[cloud_setup] single-repo hook vehicle not found at "
            f"{single_repo_vehicle} — relying on the fleet walk alone",
            file=sys.stderr,
        )
...
#: Claude Code gives a headersHelper 10 s, fixed, and runs it before every
#: connect (code.claude.com/docs/en/mcp § dynamic headers). The wait stays well
#: inside that so interpreter start and the final print never get cut off; a
#: killed helper would cost the connect it exists to rescue.
CONNECT_HELPER_WAIT_S = 7.0


def _connect_helper_path() -> Path:
    """Where the headersHelper lives: beside `.claude.json`, so the file the
    entry names survives the snapshot along with the entry itself."""
    return _claude_home() / f"{RETRIEVAL_REPO_SLUG}-connect-helper.py"
...
def _connect_helper_source(url: str, start_argv: list[str] | None) -> str:
    ...
    Traps the generated script guards:
      - stdout carries exactly ``{}``. The daemon child gets no inherited
        stdout, or Claude Code would wait on the pipe for the daemon's lifetime.
      - the child is a new session, so it outlives the helper.
      - the port probe comes first: every reconnect re-runs the helper, and a
        live daemon must cost one connect, not a spawn.
    """
    ...
    return (
        "#!/usr/bin/env python3\n"
        f'"""headersHelper for mcpServers.{RETRIEVAL_REPO_SLUG}, generated by\n'
        "scripts/cloud_setup.py :: arm_retrieval_connect_helper. Do not hand-edit.\n"
        "\n"
        "Claude Code runs this before its one connect to the daemon. Start the\n"
        "daemon if the port is closed, wait a bounded time for it, print {}.\n"
        '"""\n'
        "import os, socket, subprocess, sys, time\n"
        "\n"
        f"HOST, PORT = {host!r}, {port!r}\n"
        f"START_ARGV = {start_argv!r}\n"
        f"WAIT_S = {CONNECT_HELPER_WAIT_S!r}\n"
        ...
        "            time.sleep(0.1)\n"
        "    except OSError as e:\n"
        "        sys.stderr.write(f'daemon start failed: {e}\\n')\n"
        "\n"
        "sys.stdout.write('{}')\n"
    )


def _write_connect_helper(start_argv: list[str] | None, url: str) -> Path:
    path = _connect_helper_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(".py.tmp")
    tmp.write_text(_connect_helper_source(url, start_argv), encoding="utf-8", newline="\n")
    tmp.chmod(0o755)
    tmp.replace(path)
    return path
...
def register_retrieval_mcp_entry(report: Report) -> None:
    ...
    `mcpServers` is read once at startup.

    Trap: Claude Code connects to an http entry at launch and gives up after a
    ...
    gets a correct registration, with no second pass over the config.
...
def arm_retrieval_connect_helper(report: Report) -> None:
    """Rewrite the headersHelper with the argv that starts the daemon.

    The argv is example-retrieval-repo's own idempotent ensure path
    (``ensure_example_retrieval_repo_server.py``: O_EXCL supervisor lock, reap, crash-loop
    breaker), run against the project the install used, so a helper racing
    ...
    if install is None or install.get("exit_code") != 0:
        raise RuntimeError(
            f"{RETRIEVAL_REPO_SLUG} cloud install did not succeed; the helper stays probe-only"
        )
    ensure = rag_root / f"{RETRIEVAL_MODULE_PREFIX}_scripts" / f"ensure_{RETRIEVAL_MODULE_PREFIX}_server.py"
    if not ensure.is_file():
        raise FileNotFoundError(f"{ensure} not found — the helper stays probe-only")
    url, _ = _expected_daemon_url(rag_root)
    start_argv = [
        sys.executable,
        str(ensure),
        "--project-root",
