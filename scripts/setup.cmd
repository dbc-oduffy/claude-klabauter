@echo off
setlocal
REM Windows launcher for setup.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "setup.ps1" sibling of
REM     this file over this "setup.cmd" file itself -- if both exist, `setup`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "setup.py"
...
echo [setup] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [setup] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
