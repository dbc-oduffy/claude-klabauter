...
def main(argv: list[str]) -> int:
    ...
    if args.register_only:
        ...
        # permission, a guarded interpreter — and a failed install exits before
        # `register_claude_klabauter_root` is ever called, so the registration the operator asked
        # for does not happen. Derive the probe list instead (pure, reads pyproject) and
        # let `verify_coordinator_core_importable` below report an engine that genuinely
        # is not installed. That verification IS the flag's second half; it is not a
        # weaker substitute for the install, and it names its own remediation.
